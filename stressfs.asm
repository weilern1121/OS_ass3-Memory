...
printint(int fd, int xx, int base, int sgn)
{
 ...
 47f:	f7 f6                	div    %esi
 481:	0f b6 92 48 09 00 00 	movzbl 0x948(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5f8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 5fb:	b8 3f 09 00 00       	mov    $0x93f,%eax
...
free(void *ap)
{
 ...
 681:	a1 88 0c 00 00       	mov    0xc88,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 6d7:	89 08                	mov    %ecx,(%eax)
  freep = p;
 6d9:	a3 88 0c 00 00       	mov    %eax,0xc88
}
 6de:	5b                   	pop    %ebx
 6df:	5e                   	pop    %esi
 6e0:	5f                   	pop    %edi
 ...
 6fa:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 6fd:	a3 88 0c 00 00       	mov    %eax,0xc88
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
morecore(uint nu)
{
 ...
 714:	83 ec 04             	sub    $0x4,%esp
  ...
  if(nu < 4096)
 717:	3d ff 0f 00 00       	cmp    $0xfff,%eax
 71c:	76 3a                	jbe    758 <morecore+0x48>
 71e:	89 c3                	mov    %eax,%ebx
 720:	8d 04 c5 00 00 00 00 	lea    0x0(,%eax,8),%eax
    nu = 4096;
  p = sbrk(nu * sizeof(Header));
 727:	83 ec 0c             	sub    $0xc,%esp
 72a:	50                   	push   %eax
 72b:	e8 da fc ff ff       	call   40a <sbrk>
  if(p == (char*)-1)
 730:	83 c4 10             	add    $0x10,%esp
 733:	83 f8 ff             	cmp    $0xffffffff,%eax
 736:	74 30                	je     768 <morecore+0x58>
    return 0;
  hp = (Header*)p;
  hp->s.size = nu;
 ...
 741:	50                   	push   %eax
 742:	e8 39 ff ff ff       	call   680 <free>
  return freep;
 747:	a1 88 0c 00 00       	mov    0xc88,%eax
 ...
 758:	b8 00 80 00 00       	mov    $0x8000,%eax
{
  char *p;
  Header *hp;
  ...
  if(nu < 4096)
    nu = 4096;
 75d:	bb 00 10 00 00       	mov    $0x1000,%ebx
 762:	eb c3                	jmp    727 <morecore+0x17>
 764:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  p = sbrk(nu * sizeof(Header));
  if(p == (char*)-1)
    return 0;
 768:	31 c0                	xor    %eax,%eax
 76a:	eb e3                	jmp    74f <morecore+0x3f>
...
malloc(uint nbytes)
{
 ...
 77a:	8d 58 07             	lea    0x7(%eax),%ebx
    if((prevp = freep) == 0){
 77d:	a1 88 0c 00 00       	mov    0xc88,%eax
malloc(uint nbytes)
{
    Header *p, *prevp;
    uint nunits;
    ...
    nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 782:	c1 eb 03             	shr    $0x3,%ebx
 785:	83 c3 01             	add    $0x1,%ebx
    if((prevp = freep) == 0){
 788:	85 c0                	test   %eax,%eax
 78a:	74 52                	je     7de <malloc+0x6e>
 ...
 797:	76 1f                	jbe    7b8 <malloc+0x48>
        ...
        if(p == freep)
 799:	39 15 88 0c 00 00    	cmp    %edx,0xc88
 79f:	89 d0                	mov    %edx,%eax
 7a1:	75 ed                	jne    790 <malloc+0x20>
            if((p = morecore(nunits)) == 0)
 7a3:	89 d8                	mov    %ebx,%eax
 7a5:	e8 66 ff ff ff       	call   710 <morecore>
 7aa:	85 c0                	test   %eax,%eax
 7ac:	75 e2                	jne    790 <malloc+0x20>
                return 0;
 7ae:	31 c0                	xor    %eax,%eax
 7b0:	eb 1d                	jmp    7cf <malloc+0x5f>
 7b2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    ...
    for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
        if(p->s.size >= nunits){
            if(p->s.size == nunits)
 7b8:	39 cb                	cmp    %ecx,%ebx
 7ba:	74 1c                	je     7d8 <malloc+0x68>
                prevp->s.ptr = p->s.ptr;
            else {
                p->s.size -= nunits;
 7bc:	29 d9                	sub    %ebx,%ecx
 7be:	89 4a 04             	mov    %ecx,0x4(%edx)
                p += p->s.size;
 7c1:	8d 14 ca             	lea    (%edx,%ecx,8),%edx
                p->s.size = nunits;
 7c4:	89 5a 04             	mov    %ebx,0x4(%edx)
            }
            freep = prevp;
 7c7:	a3 88 0c 00 00       	mov    %eax,0xc88
            return (void*)(p + 1);
 7cc:	8d 42 08             	lea    0x8(%edx),%eax
        }
        if(p == freep)
            if((p = morecore(nunits)) == 0)
                return 0;
 ...
 7dc:	eb e9                	jmp    7c7 <malloc+0x57>
    ...
    if((prevp = freep) == 0){
        base.s.ptr = freep = prevp = &base;
 7de:	c7 05 88 0c 00 00 8c 	movl   $0xc8c,0xc88
 7e5:	0c 00 00 
 7e8:	c7 05 8c 0c 00 00 8c 	movl   $0xc8c,0xc8c
 7ef:	0c 00 00 
        base.s.size = 0;
 7f2:	ba 8c 0c 00 00       	mov    $0xc8c,%edx
 7f7:	c7 05 90 0c 00 00 00 	movl   $0x0,0xc90
 7fe:	00 00 00 
 801:	eb 96                	jmp    799 <malloc+0x29>
...
pmalloc()
{
 ...
 814:	83 ec 04             	sub    $0x4,%esp
    ...
    if((prevp = freep) == 0){
 817:	a1 88 0c 00 00       	mov    0xc88,%eax
 81c:	85 c0                	test   %eax,%eax
 81e:	74 56                	je     876 <pmalloc+0x66>
        base.s.ptr = freep = prevp = &base;
        base.s.size = 0;
    }
    for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 ...
 829:	74 25                	je     850 <pmalloc+0x40>
        ...
        if(p == freep)
 82b:	39 15 88 0c 00 00    	cmp    %edx,0xc88
 831:	89 d0                	mov    %edx,%eax
 833:	75 eb                	jne    820 <pmalloc+0x10>
            if((p = morecore(nunits)) == 0)
 835:	b8 01 02 00 00       	mov    $0x201,%eax
 83a:	e8 d1 fe ff ff       	call   710 <morecore>
 83f:	85 c0                	test   %eax,%eax
 841:	75 dd                	jne    820 <pmalloc+0x10>
                return 0;
 843:	31 db                	xor    %ebx,%ebx
 845:	eb 28                	jmp    86f <pmalloc+0x5f>
 ...
 85d:	89 08                	mov    %ecx,(%eax)
            freep = prevp;
            (p+1)->x = 1;
 85f:	c7 42 08 01 00 00 00 	movl   $0x1,0x8(%edx)
            turnOnPM( (void *)( p + 1 ) );
 866:	53                   	push   %ebx
 867:	e8 b6 fb ff ff       	call   422 <turnOnPM>
            return (void*)(p + 1);
 ...
 875:	c3                   	ret    
    ...
    if((prevp = freep) == 0){
        base.s.ptr = freep = prevp = &base;
 876:	c7 05 88 0c 00 00 8c 	movl   $0xc8c,0xc88
 87d:	0c 00 00 
 880:	c7 05 8c 0c 00 00 8c 	movl   $0xc8c,0xc8c
 887:	0c 00 00 
        base.s.size = 0;
 88a:	ba 8c 0c 00 00       	mov    $0xc8c,%edx
 88f:	c7 05 90 0c 00 00 00 	movl   $0x0,0xc90
 896:	00 00 00 
 899:	eb 90                	jmp    82b <pmalloc+0x1b>
 89b:	90                   	nop
 89c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

000008a0 <protect_page>:
}
...
protect_page(void* ap)
{
 ...
 8a8:	74 16                	je     8c0 <protect_page+0x20>
    {
        if( (uint)ap % 4096 != 0 )
 8aa:	a9 ff 0f 00 00       	test   $0xfff,%eax
 8af:	75 0f                	jne    8c0 <protect_page+0x20>
        p = ap;
        return turnOffW( p );
    }
    else
 ...
 8b1:	5d                   	pop    %ebp
        ...
        p = ap;
        return turnOffW( p );
 8b2:	e9 7b fb ff ff       	jmp    432 <turnOffW>
...
pfree(void* ap) {
 ...
 8d7:	8b 5d 08             	mov    0x8(%ebp),%ebx
    Header *p;
    if (ap) {
 8da:	85 db                	test   %ebx,%ebx
 8dc:	74 32                	je     910 <pfree+0x40>
        if ((uint) ap % 4096 != 0)
 8de:	f7 c3 ff 0f 00 00    	test   $0xfff,%ebx
 8e4:	75 2a                	jne    910 <pfree+0x40>
            return -1;
 ...
 8e9:	53                   	push   %ebx
 8ea:	e8 3b fb ff ff       	call   42a <checkOnPM>
 8ef:	83 c4 10             	add    $0x10,%esp
 8f2:	85 c0                	test   %eax,%eax
 8f4:	74 1a                	je     910 <pfree+0x40>
            free(ap);
 8f6:	83 ec 0c             	sub    $0xc,%esp
 8f9:	53                   	push   %ebx
 8fa:	e8 81 fd ff ff       	call   680 <free>
            return 1;
 8ff:	83 c4 10             	add    $0x10,%esp
 902:	b8 01 00 00 00       	mov    $0x1,%eax
        }
    }
    return -1;
}
