
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4                   	.byte 0xe4

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 90 10 00       	mov    $0x109000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc c0 b5 10 80       	mov    $0x8010b5c0,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 90 32 10 80       	mov    $0x80103290,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax
80100034:	66 90                	xchg   %ax,%ax
80100036:	66 90                	xchg   %ax,%ax
80100038:	66 90                	xchg   %ax,%ax
8010003a:	66 90                	xchg   %ax,%ax
8010003c:	66 90                	xchg   %ax,%ax
8010003e:	66 90                	xchg   %ax,%ax

80100040 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100040:	55                   	push   %ebp
80100041:	89 e5                	mov    %esp,%ebp
80100043:	53                   	push   %ebx

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100044:	bb f4 b5 10 80       	mov    $0x8010b5f4,%ebx
{
80100049:	83 ec 0c             	sub    $0xc,%esp
  initlock(&bcache.lock, "bcache");
8010004c:	68 40 74 10 80       	push   $0x80107440
80100051:	68 c0 b5 10 80       	push   $0x8010b5c0
80100056:	e8 d5 46 00 00       	call   80104730 <initlock>
  bcache.head.prev = &bcache.head;
8010005b:	c7 05 0c fd 10 80 bc 	movl   $0x8010fcbc,0x8010fd0c
80100062:	fc 10 80 
  bcache.head.next = &bcache.head;
80100065:	c7 05 10 fd 10 80 bc 	movl   $0x8010fcbc,0x8010fd10
8010006c:	fc 10 80 
8010006f:	83 c4 10             	add    $0x10,%esp
80100072:	ba bc fc 10 80       	mov    $0x8010fcbc,%edx
80100077:	eb 09                	jmp    80100082 <binit+0x42>
80100079:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100080:	89 c3                	mov    %eax,%ebx
    b->next = bcache.head.next;
    b->prev = &bcache.head;
    initsleeplock(&b->lock, "buffer");
80100082:	8d 43 0c             	lea    0xc(%ebx),%eax
80100085:	83 ec 08             	sub    $0x8,%esp
    b->next = bcache.head.next;
80100088:	89 53 54             	mov    %edx,0x54(%ebx)
    b->prev = &bcache.head;
8010008b:	c7 43 50 bc fc 10 80 	movl   $0x8010fcbc,0x50(%ebx)
    initsleeplock(&b->lock, "buffer");
80100092:	68 47 74 10 80       	push   $0x80107447
80100097:	50                   	push   %eax
80100098:	e8 83 45 00 00       	call   80104620 <initsleeplock>
    bcache.head.next->prev = b;
8010009d:	a1 10 fd 10 80       	mov    0x8010fd10,%eax
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
801000a2:	83 c4 10             	add    $0x10,%esp
801000a5:	89 da                	mov    %ebx,%edx
    bcache.head.next->prev = b;
801000a7:	89 58 50             	mov    %ebx,0x50(%eax)
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
801000aa:	8d 83 5c 02 00 00    	lea    0x25c(%ebx),%eax
    bcache.head.next = b;
801000b0:	89 1d 10 fd 10 80    	mov    %ebx,0x8010fd10
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
801000b6:	3d bc fc 10 80       	cmp    $0x8010fcbc,%eax
801000bb:	72 c3                	jb     80100080 <binit+0x40>
  }
}
801000bd:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801000c0:	c9                   	leave  
801000c1:	c3                   	ret    
801000c2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801000c9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801000d0 <bread>:
}

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
801000d0:	55                   	push   %ebp
801000d1:	89 e5                	mov    %esp,%ebp
801000d3:	57                   	push   %edi
801000d4:	56                   	push   %esi
801000d5:	53                   	push   %ebx
801000d6:	83 ec 18             	sub    $0x18,%esp
801000d9:	8b 75 08             	mov    0x8(%ebp),%esi
801000dc:	8b 7d 0c             	mov    0xc(%ebp),%edi
  acquire(&bcache.lock);
801000df:	68 c0 b5 10 80       	push   $0x8010b5c0
801000e4:	e8 37 47 00 00       	call   80104820 <acquire>
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000e9:	8b 1d 10 fd 10 80    	mov    0x8010fd10,%ebx
801000ef:	83 c4 10             	add    $0x10,%esp
801000f2:	81 fb bc fc 10 80    	cmp    $0x8010fcbc,%ebx
801000f8:	75 11                	jne    8010010b <bread+0x3b>
801000fa:	eb 24                	jmp    80100120 <bread+0x50>
801000fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100100:	8b 5b 54             	mov    0x54(%ebx),%ebx
80100103:	81 fb bc fc 10 80    	cmp    $0x8010fcbc,%ebx
80100109:	74 15                	je     80100120 <bread+0x50>
    if(b->dev == dev && b->blockno == blockno){
8010010b:	3b 73 04             	cmp    0x4(%ebx),%esi
8010010e:	75 f0                	jne    80100100 <bread+0x30>
80100110:	3b 7b 08             	cmp    0x8(%ebx),%edi
80100113:	75 eb                	jne    80100100 <bread+0x30>
      b->refcnt++;
80100115:	83 43 4c 01          	addl   $0x1,0x4c(%ebx)
80100119:	eb 3f                	jmp    8010015a <bread+0x8a>
8010011b:	90                   	nop
8010011c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100120:	8b 1d 0c fd 10 80    	mov    0x8010fd0c,%ebx
80100126:	81 fb bc fc 10 80    	cmp    $0x8010fcbc,%ebx
8010012c:	75 0d                	jne    8010013b <bread+0x6b>
8010012e:	eb 60                	jmp    80100190 <bread+0xc0>
80100130:	8b 5b 50             	mov    0x50(%ebx),%ebx
80100133:	81 fb bc fc 10 80    	cmp    $0x8010fcbc,%ebx
80100139:	74 55                	je     80100190 <bread+0xc0>
    if(b->refcnt == 0 && (b->flags & B_DIRTY) == 0) {
8010013b:	8b 43 4c             	mov    0x4c(%ebx),%eax
8010013e:	85 c0                	test   %eax,%eax
80100140:	75 ee                	jne    80100130 <bread+0x60>
80100142:	f6 03 04             	testb  $0x4,(%ebx)
80100145:	75 e9                	jne    80100130 <bread+0x60>
      b->dev = dev;
80100147:	89 73 04             	mov    %esi,0x4(%ebx)
      b->blockno = blockno;
8010014a:	89 7b 08             	mov    %edi,0x8(%ebx)
      b->flags = 0;
8010014d:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
      b->refcnt = 1;
80100153:	c7 43 4c 01 00 00 00 	movl   $0x1,0x4c(%ebx)
      release(&bcache.lock);
8010015a:	83 ec 0c             	sub    $0xc,%esp
8010015d:	68 c0 b5 10 80       	push   $0x8010b5c0
80100162:	e8 d9 47 00 00       	call   80104940 <release>
      acquiresleep(&b->lock);
80100167:	8d 43 0c             	lea    0xc(%ebx),%eax
8010016a:	89 04 24             	mov    %eax,(%esp)
8010016d:	e8 ee 44 00 00       	call   80104660 <acquiresleep>
80100172:	83 c4 10             	add    $0x10,%esp
  struct buf *b;

  b = bget(dev, blockno);
  if((b->flags & B_VALID) == 0) {
80100175:	f6 03 02             	testb  $0x2,(%ebx)
80100178:	75 0c                	jne    80100186 <bread+0xb6>
    iderw(b);
8010017a:	83 ec 0c             	sub    $0xc,%esp
8010017d:	53                   	push   %ebx
8010017e:	e8 8d 23 00 00       	call   80102510 <iderw>
80100183:	83 c4 10             	add    $0x10,%esp
  }
  return b;
}
80100186:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100189:	89 d8                	mov    %ebx,%eax
8010018b:	5b                   	pop    %ebx
8010018c:	5e                   	pop    %esi
8010018d:	5f                   	pop    %edi
8010018e:	5d                   	pop    %ebp
8010018f:	c3                   	ret    
  panic("bget: no buffers");
80100190:	83 ec 0c             	sub    $0xc,%esp
80100193:	68 4e 74 10 80       	push   $0x8010744e
80100198:	e8 f3 01 00 00       	call   80100390 <panic>
8010019d:	8d 76 00             	lea    0x0(%esi),%esi

801001a0 <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
801001a0:	55                   	push   %ebp
801001a1:	89 e5                	mov    %esp,%ebp
801001a3:	53                   	push   %ebx
801001a4:	83 ec 10             	sub    $0x10,%esp
801001a7:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(!holdingsleep(&b->lock))
801001aa:	8d 43 0c             	lea    0xc(%ebx),%eax
801001ad:	50                   	push   %eax
801001ae:	e8 4d 45 00 00       	call   80104700 <holdingsleep>
801001b3:	83 c4 10             	add    $0x10,%esp
801001b6:	85 c0                	test   %eax,%eax
801001b8:	74 0f                	je     801001c9 <bwrite+0x29>
    panic("bwrite");
  b->flags |= B_DIRTY;
801001ba:	83 0b 04             	orl    $0x4,(%ebx)
  iderw(b);
801001bd:	89 5d 08             	mov    %ebx,0x8(%ebp)
}
801001c0:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801001c3:	c9                   	leave  
  iderw(b);
801001c4:	e9 47 23 00 00       	jmp    80102510 <iderw>
    panic("bwrite");
801001c9:	83 ec 0c             	sub    $0xc,%esp
801001cc:	68 5f 74 10 80       	push   $0x8010745f
801001d1:	e8 ba 01 00 00       	call   80100390 <panic>
801001d6:	8d 76 00             	lea    0x0(%esi),%esi
801001d9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801001e0 <brelse>:

// Release a locked buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
801001e0:	55                   	push   %ebp
801001e1:	89 e5                	mov    %esp,%ebp
801001e3:	56                   	push   %esi
801001e4:	53                   	push   %ebx
801001e5:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(!holdingsleep(&b->lock))
801001e8:	83 ec 0c             	sub    $0xc,%esp
801001eb:	8d 73 0c             	lea    0xc(%ebx),%esi
801001ee:	56                   	push   %esi
801001ef:	e8 0c 45 00 00       	call   80104700 <holdingsleep>
801001f4:	83 c4 10             	add    $0x10,%esp
801001f7:	85 c0                	test   %eax,%eax
801001f9:	74 66                	je     80100261 <brelse+0x81>
    panic("brelse");

  releasesleep(&b->lock);
801001fb:	83 ec 0c             	sub    $0xc,%esp
801001fe:	56                   	push   %esi
801001ff:	e8 bc 44 00 00       	call   801046c0 <releasesleep>

  acquire(&bcache.lock);
80100204:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
8010020b:	e8 10 46 00 00       	call   80104820 <acquire>
  b->refcnt--;
80100210:	8b 43 4c             	mov    0x4c(%ebx),%eax
  if (b->refcnt == 0) {
80100213:	83 c4 10             	add    $0x10,%esp
  b->refcnt--;
80100216:	83 e8 01             	sub    $0x1,%eax
  if (b->refcnt == 0) {
80100219:	85 c0                	test   %eax,%eax
  b->refcnt--;
8010021b:	89 43 4c             	mov    %eax,0x4c(%ebx)
  if (b->refcnt == 0) {
8010021e:	75 2f                	jne    8010024f <brelse+0x6f>
    // no one is waiting for it.
    b->next->prev = b->prev;
80100220:	8b 43 54             	mov    0x54(%ebx),%eax
80100223:	8b 53 50             	mov    0x50(%ebx),%edx
80100226:	89 50 50             	mov    %edx,0x50(%eax)
    b->prev->next = b->next;
80100229:	8b 43 50             	mov    0x50(%ebx),%eax
8010022c:	8b 53 54             	mov    0x54(%ebx),%edx
8010022f:	89 50 54             	mov    %edx,0x54(%eax)
    b->next = bcache.head.next;
80100232:	a1 10 fd 10 80       	mov    0x8010fd10,%eax
    b->prev = &bcache.head;
80100237:	c7 43 50 bc fc 10 80 	movl   $0x8010fcbc,0x50(%ebx)
    b->next = bcache.head.next;
8010023e:	89 43 54             	mov    %eax,0x54(%ebx)
    bcache.head.next->prev = b;
80100241:	a1 10 fd 10 80       	mov    0x8010fd10,%eax
80100246:	89 58 50             	mov    %ebx,0x50(%eax)
    bcache.head.next = b;
80100249:	89 1d 10 fd 10 80    	mov    %ebx,0x8010fd10
  }
  
  release(&bcache.lock);
8010024f:	c7 45 08 c0 b5 10 80 	movl   $0x8010b5c0,0x8(%ebp)
}
80100256:	8d 65 f8             	lea    -0x8(%ebp),%esp
80100259:	5b                   	pop    %ebx
8010025a:	5e                   	pop    %esi
8010025b:	5d                   	pop    %ebp
  release(&bcache.lock);
8010025c:	e9 df 46 00 00       	jmp    80104940 <release>
    panic("brelse");
80100261:	83 ec 0c             	sub    $0xc,%esp
80100264:	68 66 74 10 80       	push   $0x80107466
80100269:	e8 22 01 00 00       	call   80100390 <panic>
8010026e:	66 90                	xchg   %ax,%ax

80100270 <consoleread>:
  }
}

int
consoleread(struct inode *ip, char *dst, int n)
{
80100270:	55                   	push   %ebp
80100271:	89 e5                	mov    %esp,%ebp
80100273:	57                   	push   %edi
80100274:	56                   	push   %esi
80100275:	53                   	push   %ebx
80100276:	83 ec 28             	sub    $0x28,%esp
80100279:	8b 7d 08             	mov    0x8(%ebp),%edi
8010027c:	8b 75 0c             	mov    0xc(%ebp),%esi
  uint target;
  int c;

  iunlock(ip);
8010027f:	57                   	push   %edi
80100280:	e8 3b 15 00 00       	call   801017c0 <iunlock>
  target = n;
  acquire(&cons.lock);
80100285:	c7 04 24 20 a5 10 80 	movl   $0x8010a520,(%esp)
8010028c:	e8 8f 45 00 00       	call   80104820 <acquire>
  while(n > 0){
80100291:	8b 5d 10             	mov    0x10(%ebp),%ebx
80100294:	83 c4 10             	add    $0x10,%esp
80100297:	31 c0                	xor    %eax,%eax
80100299:	85 db                	test   %ebx,%ebx
8010029b:	0f 8e a1 00 00 00    	jle    80100342 <consoleread+0xd2>
    while(input.r == input.w){
801002a1:	8b 15 a0 ff 10 80    	mov    0x8010ffa0,%edx
801002a7:	39 15 a4 ff 10 80    	cmp    %edx,0x8010ffa4
801002ad:	74 2c                	je     801002db <consoleread+0x6b>
801002af:	eb 5f                	jmp    80100310 <consoleread+0xa0>
801002b1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      if(myproc()->killed){
        release(&cons.lock);
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &cons.lock);
801002b8:	83 ec 08             	sub    $0x8,%esp
801002bb:	68 20 a5 10 80       	push   $0x8010a520
801002c0:	68 a0 ff 10 80       	push   $0x8010ffa0
801002c5:	e8 a6 3f 00 00       	call   80104270 <sleep>
    while(input.r == input.w){
801002ca:	8b 15 a0 ff 10 80    	mov    0x8010ffa0,%edx
801002d0:	83 c4 10             	add    $0x10,%esp
801002d3:	3b 15 a4 ff 10 80    	cmp    0x8010ffa4,%edx
801002d9:	75 35                	jne    80100310 <consoleread+0xa0>
      if(myproc()->killed){
801002db:	e8 20 39 00 00       	call   80103c00 <myproc>
801002e0:	8b 40 24             	mov    0x24(%eax),%eax
801002e3:	85 c0                	test   %eax,%eax
801002e5:	74 d1                	je     801002b8 <consoleread+0x48>
        release(&cons.lock);
801002e7:	83 ec 0c             	sub    $0xc,%esp
801002ea:	68 20 a5 10 80       	push   $0x8010a520
801002ef:	e8 4c 46 00 00       	call   80104940 <release>
        ilock(ip);
801002f4:	89 3c 24             	mov    %edi,(%esp)
801002f7:	e8 e4 13 00 00       	call   801016e0 <ilock>
        return -1;
801002fc:	83 c4 10             	add    $0x10,%esp
  }
  release(&cons.lock);
  ilock(ip);

  return target - n;
}
801002ff:	8d 65 f4             	lea    -0xc(%ebp),%esp
        return -1;
80100302:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100307:	5b                   	pop    %ebx
80100308:	5e                   	pop    %esi
80100309:	5f                   	pop    %edi
8010030a:	5d                   	pop    %ebp
8010030b:	c3                   	ret    
8010030c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    c = input.buf[input.r++ % INPUT_BUF];
80100310:	8d 42 01             	lea    0x1(%edx),%eax
80100313:	a3 a0 ff 10 80       	mov    %eax,0x8010ffa0
80100318:	89 d0                	mov    %edx,%eax
8010031a:	83 e0 7f             	and    $0x7f,%eax
8010031d:	0f be 80 20 ff 10 80 	movsbl -0x7fef00e0(%eax),%eax
    if(c == C('D')){  // EOF
80100324:	83 f8 04             	cmp    $0x4,%eax
80100327:	74 3f                	je     80100368 <consoleread+0xf8>
    *dst++ = c;
80100329:	83 c6 01             	add    $0x1,%esi
    --n;
8010032c:	83 eb 01             	sub    $0x1,%ebx
    if(c == '\n')
8010032f:	83 f8 0a             	cmp    $0xa,%eax
    *dst++ = c;
80100332:	88 46 ff             	mov    %al,-0x1(%esi)
    if(c == '\n')
80100335:	74 43                	je     8010037a <consoleread+0x10a>
  while(n > 0){
80100337:	85 db                	test   %ebx,%ebx
80100339:	0f 85 62 ff ff ff    	jne    801002a1 <consoleread+0x31>
8010033f:	8b 45 10             	mov    0x10(%ebp),%eax
  release(&cons.lock);
80100342:	83 ec 0c             	sub    $0xc,%esp
80100345:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80100348:	68 20 a5 10 80       	push   $0x8010a520
8010034d:	e8 ee 45 00 00       	call   80104940 <release>
  ilock(ip);
80100352:	89 3c 24             	mov    %edi,(%esp)
80100355:	e8 86 13 00 00       	call   801016e0 <ilock>
  return target - n;
8010035a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010035d:	83 c4 10             	add    $0x10,%esp
}
80100360:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100363:	5b                   	pop    %ebx
80100364:	5e                   	pop    %esi
80100365:	5f                   	pop    %edi
80100366:	5d                   	pop    %ebp
80100367:	c3                   	ret    
80100368:	8b 45 10             	mov    0x10(%ebp),%eax
8010036b:	29 d8                	sub    %ebx,%eax
      if(n < target){
8010036d:	3b 5d 10             	cmp    0x10(%ebp),%ebx
80100370:	73 d0                	jae    80100342 <consoleread+0xd2>
        input.r--;
80100372:	89 15 a0 ff 10 80    	mov    %edx,0x8010ffa0
80100378:	eb c8                	jmp    80100342 <consoleread+0xd2>
8010037a:	8b 45 10             	mov    0x10(%ebp),%eax
8010037d:	29 d8                	sub    %ebx,%eax
8010037f:	eb c1                	jmp    80100342 <consoleread+0xd2>
80100381:	eb 0d                	jmp    80100390 <panic>
80100383:	90                   	nop
80100384:	90                   	nop
80100385:	90                   	nop
80100386:	90                   	nop
80100387:	90                   	nop
80100388:	90                   	nop
80100389:	90                   	nop
8010038a:	90                   	nop
8010038b:	90                   	nop
8010038c:	90                   	nop
8010038d:	90                   	nop
8010038e:	90                   	nop
8010038f:	90                   	nop

80100390 <panic>:
{
80100390:	55                   	push   %ebp
80100391:	89 e5                	mov    %esp,%ebp
80100393:	56                   	push   %esi
80100394:	53                   	push   %ebx
80100395:	83 ec 30             	sub    $0x30,%esp
}

static inline void
cli(void)
{
  asm volatile("cli");
80100398:	fa                   	cli    
  cons.locking = 0;
80100399:	c7 05 54 a5 10 80 00 	movl   $0x0,0x8010a554
801003a0:	00 00 00 
  getcallerpcs(&s, pcs);
801003a3:	8d 5d d0             	lea    -0x30(%ebp),%ebx
801003a6:	8d 75 f8             	lea    -0x8(%ebp),%esi
  cprintf("lapicid %d: panic: ", lapicid());
801003a9:	e8 72 27 00 00       	call   80102b20 <lapicid>
801003ae:	83 ec 08             	sub    $0x8,%esp
801003b1:	50                   	push   %eax
801003b2:	68 6d 74 10 80       	push   $0x8010746d
801003b7:	e8 a4 02 00 00       	call   80100660 <cprintf>
  cprintf(s);
801003bc:	58                   	pop    %eax
801003bd:	ff 75 08             	pushl  0x8(%ebp)
801003c0:	e8 9b 02 00 00       	call   80100660 <cprintf>
  cprintf("\n");
801003c5:	c7 04 24 17 7f 10 80 	movl   $0x80107f17,(%esp)
801003cc:	e8 8f 02 00 00       	call   80100660 <cprintf>
  getcallerpcs(&s, pcs);
801003d1:	5a                   	pop    %edx
801003d2:	8d 45 08             	lea    0x8(%ebp),%eax
801003d5:	59                   	pop    %ecx
801003d6:	53                   	push   %ebx
801003d7:	50                   	push   %eax
801003d8:	e8 73 43 00 00       	call   80104750 <getcallerpcs>
801003dd:	83 c4 10             	add    $0x10,%esp
    cprintf(" %p", pcs[i]);
801003e0:	83 ec 08             	sub    $0x8,%esp
801003e3:	ff 33                	pushl  (%ebx)
801003e5:	83 c3 04             	add    $0x4,%ebx
801003e8:	68 81 74 10 80       	push   $0x80107481
801003ed:	e8 6e 02 00 00       	call   80100660 <cprintf>
  for(i=0; i<10; i++)
801003f2:	83 c4 10             	add    $0x10,%esp
801003f5:	39 f3                	cmp    %esi,%ebx
801003f7:	75 e7                	jne    801003e0 <panic+0x50>
  panicked = 1; // freeze other CPU
801003f9:	c7 05 58 a5 10 80 01 	movl   $0x1,0x8010a558
80100400:	00 00 00 
80100403:	eb fe                	jmp    80100403 <panic+0x73>
80100405:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100409:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80100410 <consputc>:
  if(panicked){
80100410:	8b 0d 58 a5 10 80    	mov    0x8010a558,%ecx
80100416:	85 c9                	test   %ecx,%ecx
80100418:	74 06                	je     80100420 <consputc+0x10>
8010041a:	fa                   	cli    
8010041b:	eb fe                	jmp    8010041b <consputc+0xb>
8010041d:	8d 76 00             	lea    0x0(%esi),%esi
{
80100420:	55                   	push   %ebp
80100421:	89 e5                	mov    %esp,%ebp
80100423:	57                   	push   %edi
80100424:	56                   	push   %esi
80100425:	53                   	push   %ebx
80100426:	89 c6                	mov    %eax,%esi
80100428:	83 ec 0c             	sub    $0xc,%esp
  if(c == BACKSPACE){
8010042b:	3d 00 01 00 00       	cmp    $0x100,%eax
80100430:	0f 84 b1 00 00 00    	je     801004e7 <consputc+0xd7>
    uartputc(c);
80100436:	83 ec 0c             	sub    $0xc,%esp
80100439:	50                   	push   %eax
8010043a:	e8 21 5c 00 00       	call   80106060 <uartputc>
8010043f:	83 c4 10             	add    $0x10,%esp
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100442:	bb d4 03 00 00       	mov    $0x3d4,%ebx
80100447:	b8 0e 00 00 00       	mov    $0xe,%eax
8010044c:	89 da                	mov    %ebx,%edx
8010044e:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010044f:	b9 d5 03 00 00       	mov    $0x3d5,%ecx
80100454:	89 ca                	mov    %ecx,%edx
80100456:	ec                   	in     (%dx),%al
  pos = inb(CRTPORT+1) << 8;
80100457:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010045a:	89 da                	mov    %ebx,%edx
8010045c:	c1 e0 08             	shl    $0x8,%eax
8010045f:	89 c7                	mov    %eax,%edi
80100461:	b8 0f 00 00 00       	mov    $0xf,%eax
80100466:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80100467:	89 ca                	mov    %ecx,%edx
80100469:	ec                   	in     (%dx),%al
8010046a:	0f b6 d8             	movzbl %al,%ebx
  pos |= inb(CRTPORT+1);
8010046d:	09 fb                	or     %edi,%ebx
  if(c == '\n')
8010046f:	83 fe 0a             	cmp    $0xa,%esi
80100472:	0f 84 f3 00 00 00    	je     8010056b <consputc+0x15b>
  else if(c == BACKSPACE){
80100478:	81 fe 00 01 00 00    	cmp    $0x100,%esi
8010047e:	0f 84 d7 00 00 00    	je     8010055b <consputc+0x14b>
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
80100484:	89 f0                	mov    %esi,%eax
80100486:	0f b6 c0             	movzbl %al,%eax
80100489:	80 cc 07             	or     $0x7,%ah
8010048c:	66 89 84 1b 00 80 0b 	mov    %ax,-0x7ff48000(%ebx,%ebx,1)
80100493:	80 
80100494:	83 c3 01             	add    $0x1,%ebx
  if(pos < 0 || pos > 25*80)
80100497:	81 fb d0 07 00 00    	cmp    $0x7d0,%ebx
8010049d:	0f 8f ab 00 00 00    	jg     8010054e <consputc+0x13e>
  if((pos/80) >= 24){  // Scroll up.
801004a3:	81 fb 7f 07 00 00    	cmp    $0x77f,%ebx
801004a9:	7f 66                	jg     80100511 <consputc+0x101>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801004ab:	be d4 03 00 00       	mov    $0x3d4,%esi
801004b0:	b8 0e 00 00 00       	mov    $0xe,%eax
801004b5:	89 f2                	mov    %esi,%edx
801004b7:	ee                   	out    %al,(%dx)
801004b8:	b9 d5 03 00 00       	mov    $0x3d5,%ecx
  outb(CRTPORT+1, pos>>8);
801004bd:	89 d8                	mov    %ebx,%eax
801004bf:	c1 f8 08             	sar    $0x8,%eax
801004c2:	89 ca                	mov    %ecx,%edx
801004c4:	ee                   	out    %al,(%dx)
801004c5:	b8 0f 00 00 00       	mov    $0xf,%eax
801004ca:	89 f2                	mov    %esi,%edx
801004cc:	ee                   	out    %al,(%dx)
801004cd:	89 d8                	mov    %ebx,%eax
801004cf:	89 ca                	mov    %ecx,%edx
801004d1:	ee                   	out    %al,(%dx)
  crt[pos] = ' ' | 0x0700;
801004d2:	b8 20 07 00 00       	mov    $0x720,%eax
801004d7:	66 89 84 1b 00 80 0b 	mov    %ax,-0x7ff48000(%ebx,%ebx,1)
801004de:	80 
}
801004df:	8d 65 f4             	lea    -0xc(%ebp),%esp
801004e2:	5b                   	pop    %ebx
801004e3:	5e                   	pop    %esi
801004e4:	5f                   	pop    %edi
801004e5:	5d                   	pop    %ebp
801004e6:	c3                   	ret    
    uartputc('\b'); uartputc(' '); uartputc('\b');
801004e7:	83 ec 0c             	sub    $0xc,%esp
801004ea:	6a 08                	push   $0x8
801004ec:	e8 6f 5b 00 00       	call   80106060 <uartputc>
801004f1:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
801004f8:	e8 63 5b 00 00       	call   80106060 <uartputc>
801004fd:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80100504:	e8 57 5b 00 00       	call   80106060 <uartputc>
80100509:	83 c4 10             	add    $0x10,%esp
8010050c:	e9 31 ff ff ff       	jmp    80100442 <consputc+0x32>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100511:	52                   	push   %edx
80100512:	68 60 0e 00 00       	push   $0xe60
    pos -= 80;
80100517:	83 eb 50             	sub    $0x50,%ebx
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
8010051a:	68 a0 80 0b 80       	push   $0x800b80a0
8010051f:	68 00 80 0b 80       	push   $0x800b8000
80100524:	e8 27 45 00 00       	call   80104a50 <memmove>
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
80100529:	b8 80 07 00 00       	mov    $0x780,%eax
8010052e:	83 c4 0c             	add    $0xc,%esp
80100531:	29 d8                	sub    %ebx,%eax
80100533:	01 c0                	add    %eax,%eax
80100535:	50                   	push   %eax
80100536:	8d 04 1b             	lea    (%ebx,%ebx,1),%eax
80100539:	6a 00                	push   $0x0
8010053b:	2d 00 80 f4 7f       	sub    $0x7ff48000,%eax
80100540:	50                   	push   %eax
80100541:	e8 5a 44 00 00       	call   801049a0 <memset>
80100546:	83 c4 10             	add    $0x10,%esp
80100549:	e9 5d ff ff ff       	jmp    801004ab <consputc+0x9b>
    panic("pos under/overflow");
8010054e:	83 ec 0c             	sub    $0xc,%esp
80100551:	68 85 74 10 80       	push   $0x80107485
80100556:	e8 35 fe ff ff       	call   80100390 <panic>
    if(pos > 0) --pos;
8010055b:	85 db                	test   %ebx,%ebx
8010055d:	0f 84 48 ff ff ff    	je     801004ab <consputc+0x9b>
80100563:	83 eb 01             	sub    $0x1,%ebx
80100566:	e9 2c ff ff ff       	jmp    80100497 <consputc+0x87>
    pos += 80 - pos%80;
8010056b:	89 d8                	mov    %ebx,%eax
8010056d:	b9 50 00 00 00       	mov    $0x50,%ecx
80100572:	99                   	cltd   
80100573:	f7 f9                	idiv   %ecx
80100575:	29 d1                	sub    %edx,%ecx
80100577:	01 cb                	add    %ecx,%ebx
80100579:	e9 19 ff ff ff       	jmp    80100497 <consputc+0x87>
8010057e:	66 90                	xchg   %ax,%ax

80100580 <printint>:
{
80100580:	55                   	push   %ebp
80100581:	89 e5                	mov    %esp,%ebp
80100583:	57                   	push   %edi
80100584:	56                   	push   %esi
80100585:	53                   	push   %ebx
80100586:	89 d3                	mov    %edx,%ebx
80100588:	83 ec 2c             	sub    $0x2c,%esp
  if(sign && (sign = xx < 0))
8010058b:	85 c9                	test   %ecx,%ecx
{
8010058d:	89 4d d4             	mov    %ecx,-0x2c(%ebp)
  if(sign && (sign = xx < 0))
80100590:	74 04                	je     80100596 <printint+0x16>
80100592:	85 c0                	test   %eax,%eax
80100594:	78 5a                	js     801005f0 <printint+0x70>
    x = xx;
80100596:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)
  i = 0;
8010059d:	31 c9                	xor    %ecx,%ecx
8010059f:	8d 75 d7             	lea    -0x29(%ebp),%esi
801005a2:	eb 06                	jmp    801005aa <printint+0x2a>
801005a4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    buf[i++] = digits[x % base];
801005a8:	89 f9                	mov    %edi,%ecx
801005aa:	31 d2                	xor    %edx,%edx
801005ac:	8d 79 01             	lea    0x1(%ecx),%edi
801005af:	f7 f3                	div    %ebx
801005b1:	0f b6 92 b0 74 10 80 	movzbl -0x7fef8b50(%edx),%edx
  }while((x /= base) != 0);
801005b8:	85 c0                	test   %eax,%eax
    buf[i++] = digits[x % base];
801005ba:	88 14 3e             	mov    %dl,(%esi,%edi,1)
  }while((x /= base) != 0);
801005bd:	75 e9                	jne    801005a8 <printint+0x28>
  if(sign)
801005bf:	8b 45 d4             	mov    -0x2c(%ebp),%eax
801005c2:	85 c0                	test   %eax,%eax
801005c4:	74 08                	je     801005ce <printint+0x4e>
    buf[i++] = '-';
801005c6:	c6 44 3d d8 2d       	movb   $0x2d,-0x28(%ebp,%edi,1)
801005cb:	8d 79 02             	lea    0x2(%ecx),%edi
801005ce:	8d 5c 3d d7          	lea    -0x29(%ebp,%edi,1),%ebx
801005d2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    consputc(buf[i]);
801005d8:	0f be 03             	movsbl (%ebx),%eax
801005db:	83 eb 01             	sub    $0x1,%ebx
801005de:	e8 2d fe ff ff       	call   80100410 <consputc>
  while(--i >= 0)
801005e3:	39 f3                	cmp    %esi,%ebx
801005e5:	75 f1                	jne    801005d8 <printint+0x58>
}
801005e7:	83 c4 2c             	add    $0x2c,%esp
801005ea:	5b                   	pop    %ebx
801005eb:	5e                   	pop    %esi
801005ec:	5f                   	pop    %edi
801005ed:	5d                   	pop    %ebp
801005ee:	c3                   	ret    
801005ef:	90                   	nop
    x = -xx;
801005f0:	f7 d8                	neg    %eax
801005f2:	eb a9                	jmp    8010059d <printint+0x1d>
801005f4:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801005fa:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80100600 <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100600:	55                   	push   %ebp
80100601:	89 e5                	mov    %esp,%ebp
80100603:	57                   	push   %edi
80100604:	56                   	push   %esi
80100605:	53                   	push   %ebx
80100606:	83 ec 18             	sub    $0x18,%esp
80100609:	8b 75 10             	mov    0x10(%ebp),%esi
  int i;

  iunlock(ip);
8010060c:	ff 75 08             	pushl  0x8(%ebp)
8010060f:	e8 ac 11 00 00       	call   801017c0 <iunlock>
  acquire(&cons.lock);
80100614:	c7 04 24 20 a5 10 80 	movl   $0x8010a520,(%esp)
8010061b:	e8 00 42 00 00       	call   80104820 <acquire>
  for(i = 0; i < n; i++)
80100620:	83 c4 10             	add    $0x10,%esp
80100623:	85 f6                	test   %esi,%esi
80100625:	7e 18                	jle    8010063f <consolewrite+0x3f>
80100627:	8b 7d 0c             	mov    0xc(%ebp),%edi
8010062a:	8d 1c 37             	lea    (%edi,%esi,1),%ebx
8010062d:	8d 76 00             	lea    0x0(%esi),%esi
    consputc(buf[i] & 0xff);
80100630:	0f b6 07             	movzbl (%edi),%eax
80100633:	83 c7 01             	add    $0x1,%edi
80100636:	e8 d5 fd ff ff       	call   80100410 <consputc>
  for(i = 0; i < n; i++)
8010063b:	39 fb                	cmp    %edi,%ebx
8010063d:	75 f1                	jne    80100630 <consolewrite+0x30>
  release(&cons.lock);
8010063f:	83 ec 0c             	sub    $0xc,%esp
80100642:	68 20 a5 10 80       	push   $0x8010a520
80100647:	e8 f4 42 00 00       	call   80104940 <release>
  ilock(ip);
8010064c:	58                   	pop    %eax
8010064d:	ff 75 08             	pushl  0x8(%ebp)
80100650:	e8 8b 10 00 00       	call   801016e0 <ilock>

  return n;
}
80100655:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100658:	89 f0                	mov    %esi,%eax
8010065a:	5b                   	pop    %ebx
8010065b:	5e                   	pop    %esi
8010065c:	5f                   	pop    %edi
8010065d:	5d                   	pop    %ebp
8010065e:	c3                   	ret    
8010065f:	90                   	nop

80100660 <cprintf>:
{
80100660:	55                   	push   %ebp
80100661:	89 e5                	mov    %esp,%ebp
80100663:	57                   	push   %edi
80100664:	56                   	push   %esi
80100665:	53                   	push   %ebx
80100666:	83 ec 1c             	sub    $0x1c,%esp
  locking = cons.locking;
80100669:	a1 54 a5 10 80       	mov    0x8010a554,%eax
  if(locking)
8010066e:	85 c0                	test   %eax,%eax
  locking = cons.locking;
80100670:	89 45 dc             	mov    %eax,-0x24(%ebp)
  if(locking)
80100673:	0f 85 6f 01 00 00    	jne    801007e8 <cprintf+0x188>
  if (fmt == 0)
80100679:	8b 45 08             	mov    0x8(%ebp),%eax
8010067c:	85 c0                	test   %eax,%eax
8010067e:	89 c7                	mov    %eax,%edi
80100680:	0f 84 77 01 00 00    	je     801007fd <cprintf+0x19d>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100686:	0f b6 00             	movzbl (%eax),%eax
  argp = (uint*)(void*)(&fmt + 1);
80100689:	8d 4d 0c             	lea    0xc(%ebp),%ecx
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
8010068c:	31 db                	xor    %ebx,%ebx
  argp = (uint*)(void*)(&fmt + 1);
8010068e:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100691:	85 c0                	test   %eax,%eax
80100693:	75 56                	jne    801006eb <cprintf+0x8b>
80100695:	eb 79                	jmp    80100710 <cprintf+0xb0>
80100697:	89 f6                	mov    %esi,%esi
80100699:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    c = fmt[++i] & 0xff;
801006a0:	0f b6 16             	movzbl (%esi),%edx
    if(c == 0)
801006a3:	85 d2                	test   %edx,%edx
801006a5:	74 69                	je     80100710 <cprintf+0xb0>
801006a7:	83 c3 02             	add    $0x2,%ebx
    switch(c){
801006aa:	83 fa 70             	cmp    $0x70,%edx
801006ad:	8d 34 1f             	lea    (%edi,%ebx,1),%esi
801006b0:	0f 84 84 00 00 00    	je     8010073a <cprintf+0xda>
801006b6:	7f 78                	jg     80100730 <cprintf+0xd0>
801006b8:	83 fa 25             	cmp    $0x25,%edx
801006bb:	0f 84 ff 00 00 00    	je     801007c0 <cprintf+0x160>
801006c1:	83 fa 64             	cmp    $0x64,%edx
801006c4:	0f 85 8e 00 00 00    	jne    80100758 <cprintf+0xf8>
      printint(*argp++, 10, 1);
801006ca:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801006cd:	ba 0a 00 00 00       	mov    $0xa,%edx
801006d2:	8d 48 04             	lea    0x4(%eax),%ecx
801006d5:	8b 00                	mov    (%eax),%eax
801006d7:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
801006da:	b9 01 00 00 00       	mov    $0x1,%ecx
801006df:	e8 9c fe ff ff       	call   80100580 <printint>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801006e4:	0f b6 06             	movzbl (%esi),%eax
801006e7:	85 c0                	test   %eax,%eax
801006e9:	74 25                	je     80100710 <cprintf+0xb0>
801006eb:	8d 53 01             	lea    0x1(%ebx),%edx
    if(c != '%'){
801006ee:	83 f8 25             	cmp    $0x25,%eax
801006f1:	8d 34 17             	lea    (%edi,%edx,1),%esi
801006f4:	74 aa                	je     801006a0 <cprintf+0x40>
801006f6:	89 55 e0             	mov    %edx,-0x20(%ebp)
      consputc(c);
801006f9:	e8 12 fd ff ff       	call   80100410 <consputc>
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801006fe:	0f b6 06             	movzbl (%esi),%eax
      continue;
80100701:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100704:	89 d3                	mov    %edx,%ebx
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100706:	85 c0                	test   %eax,%eax
80100708:	75 e1                	jne    801006eb <cprintf+0x8b>
8010070a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  if(locking)
80100710:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100713:	85 c0                	test   %eax,%eax
80100715:	74 10                	je     80100727 <cprintf+0xc7>
    release(&cons.lock);
80100717:	83 ec 0c             	sub    $0xc,%esp
8010071a:	68 20 a5 10 80       	push   $0x8010a520
8010071f:	e8 1c 42 00 00       	call   80104940 <release>
80100724:	83 c4 10             	add    $0x10,%esp
}
80100727:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010072a:	5b                   	pop    %ebx
8010072b:	5e                   	pop    %esi
8010072c:	5f                   	pop    %edi
8010072d:	5d                   	pop    %ebp
8010072e:	c3                   	ret    
8010072f:	90                   	nop
    switch(c){
80100730:	83 fa 73             	cmp    $0x73,%edx
80100733:	74 43                	je     80100778 <cprintf+0x118>
80100735:	83 fa 78             	cmp    $0x78,%edx
80100738:	75 1e                	jne    80100758 <cprintf+0xf8>
      printint(*argp++, 16, 0);
8010073a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010073d:	ba 10 00 00 00       	mov    $0x10,%edx
80100742:	8d 48 04             	lea    0x4(%eax),%ecx
80100745:	8b 00                	mov    (%eax),%eax
80100747:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
8010074a:	31 c9                	xor    %ecx,%ecx
8010074c:	e8 2f fe ff ff       	call   80100580 <printint>
      break;
80100751:	eb 91                	jmp    801006e4 <cprintf+0x84>
80100753:	90                   	nop
80100754:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      consputc('%');
80100758:	b8 25 00 00 00       	mov    $0x25,%eax
8010075d:	89 55 e0             	mov    %edx,-0x20(%ebp)
80100760:	e8 ab fc ff ff       	call   80100410 <consputc>
      consputc(c);
80100765:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100768:	89 d0                	mov    %edx,%eax
8010076a:	e8 a1 fc ff ff       	call   80100410 <consputc>
      break;
8010076f:	e9 70 ff ff ff       	jmp    801006e4 <cprintf+0x84>
80100774:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      if((s = (char*)*argp++) == 0)
80100778:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010077b:	8b 10                	mov    (%eax),%edx
8010077d:	8d 48 04             	lea    0x4(%eax),%ecx
80100780:	89 4d e0             	mov    %ecx,-0x20(%ebp)
80100783:	85 d2                	test   %edx,%edx
80100785:	74 49                	je     801007d0 <cprintf+0x170>
      for(; *s; s++)
80100787:	0f be 02             	movsbl (%edx),%eax
      if((s = (char*)*argp++) == 0)
8010078a:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
      for(; *s; s++)
8010078d:	84 c0                	test   %al,%al
8010078f:	0f 84 4f ff ff ff    	je     801006e4 <cprintf+0x84>
80100795:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
80100798:	89 d3                	mov    %edx,%ebx
8010079a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801007a0:	83 c3 01             	add    $0x1,%ebx
        consputc(*s);
801007a3:	e8 68 fc ff ff       	call   80100410 <consputc>
      for(; *s; s++)
801007a8:	0f be 03             	movsbl (%ebx),%eax
801007ab:	84 c0                	test   %al,%al
801007ad:	75 f1                	jne    801007a0 <cprintf+0x140>
      if((s = (char*)*argp++) == 0)
801007af:	8b 45 e0             	mov    -0x20(%ebp),%eax
801007b2:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
801007b5:	89 45 e4             	mov    %eax,-0x1c(%ebp)
801007b8:	e9 27 ff ff ff       	jmp    801006e4 <cprintf+0x84>
801007bd:	8d 76 00             	lea    0x0(%esi),%esi
      consputc('%');
801007c0:	b8 25 00 00 00       	mov    $0x25,%eax
801007c5:	e8 46 fc ff ff       	call   80100410 <consputc>
      break;
801007ca:	e9 15 ff ff ff       	jmp    801006e4 <cprintf+0x84>
801007cf:	90                   	nop
        s = "(null)";
801007d0:	ba 98 74 10 80       	mov    $0x80107498,%edx
      for(; *s; s++)
801007d5:	89 5d e4             	mov    %ebx,-0x1c(%ebp)
801007d8:	b8 28 00 00 00       	mov    $0x28,%eax
801007dd:	89 d3                	mov    %edx,%ebx
801007df:	eb bf                	jmp    801007a0 <cprintf+0x140>
801007e1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    acquire(&cons.lock);
801007e8:	83 ec 0c             	sub    $0xc,%esp
801007eb:	68 20 a5 10 80       	push   $0x8010a520
801007f0:	e8 2b 40 00 00       	call   80104820 <acquire>
801007f5:	83 c4 10             	add    $0x10,%esp
801007f8:	e9 7c fe ff ff       	jmp    80100679 <cprintf+0x19>
    panic("null fmt");
801007fd:	83 ec 0c             	sub    $0xc,%esp
80100800:	68 9f 74 10 80       	push   $0x8010749f
80100805:	e8 86 fb ff ff       	call   80100390 <panic>
8010080a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80100810 <consoleintr>:
{
80100810:	55                   	push   %ebp
80100811:	89 e5                	mov    %esp,%ebp
80100813:	57                   	push   %edi
80100814:	56                   	push   %esi
80100815:	53                   	push   %ebx
  int c, doprocdump = 0;
80100816:	31 f6                	xor    %esi,%esi
{
80100818:	83 ec 18             	sub    $0x18,%esp
8010081b:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&cons.lock);
8010081e:	68 20 a5 10 80       	push   $0x8010a520
80100823:	e8 f8 3f 00 00       	call   80104820 <acquire>
  while((c = getc()) >= 0){
80100828:	83 c4 10             	add    $0x10,%esp
8010082b:	90                   	nop
8010082c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100830:	ff d3                	call   *%ebx
80100832:	85 c0                	test   %eax,%eax
80100834:	89 c7                	mov    %eax,%edi
80100836:	78 48                	js     80100880 <consoleintr+0x70>
    switch(c){
80100838:	83 ff 10             	cmp    $0x10,%edi
8010083b:	0f 84 e7 00 00 00    	je     80100928 <consoleintr+0x118>
80100841:	7e 5d                	jle    801008a0 <consoleintr+0x90>
80100843:	83 ff 15             	cmp    $0x15,%edi
80100846:	0f 84 ec 00 00 00    	je     80100938 <consoleintr+0x128>
8010084c:	83 ff 7f             	cmp    $0x7f,%edi
8010084f:	75 54                	jne    801008a5 <consoleintr+0x95>
      if(input.e != input.w){
80100851:	a1 a8 ff 10 80       	mov    0x8010ffa8,%eax
80100856:	3b 05 a4 ff 10 80    	cmp    0x8010ffa4,%eax
8010085c:	74 d2                	je     80100830 <consoleintr+0x20>
        input.e--;
8010085e:	83 e8 01             	sub    $0x1,%eax
80100861:	a3 a8 ff 10 80       	mov    %eax,0x8010ffa8
        consputc(BACKSPACE);
80100866:	b8 00 01 00 00       	mov    $0x100,%eax
8010086b:	e8 a0 fb ff ff       	call   80100410 <consputc>
  while((c = getc()) >= 0){
80100870:	ff d3                	call   *%ebx
80100872:	85 c0                	test   %eax,%eax
80100874:	89 c7                	mov    %eax,%edi
80100876:	79 c0                	jns    80100838 <consoleintr+0x28>
80100878:	90                   	nop
80100879:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  release(&cons.lock);
80100880:	83 ec 0c             	sub    $0xc,%esp
80100883:	68 20 a5 10 80       	push   $0x8010a520
80100888:	e8 b3 40 00 00       	call   80104940 <release>
  if(doprocdump) {
8010088d:	83 c4 10             	add    $0x10,%esp
80100890:	85 f6                	test   %esi,%esi
80100892:	0f 85 f8 00 00 00    	jne    80100990 <consoleintr+0x180>
}
80100898:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010089b:	5b                   	pop    %ebx
8010089c:	5e                   	pop    %esi
8010089d:	5f                   	pop    %edi
8010089e:	5d                   	pop    %ebp
8010089f:	c3                   	ret    
    switch(c){
801008a0:	83 ff 08             	cmp    $0x8,%edi
801008a3:	74 ac                	je     80100851 <consoleintr+0x41>
      if(c != 0 && input.e-input.r < INPUT_BUF){
801008a5:	85 ff                	test   %edi,%edi
801008a7:	74 87                	je     80100830 <consoleintr+0x20>
801008a9:	a1 a8 ff 10 80       	mov    0x8010ffa8,%eax
801008ae:	89 c2                	mov    %eax,%edx
801008b0:	2b 15 a0 ff 10 80    	sub    0x8010ffa0,%edx
801008b6:	83 fa 7f             	cmp    $0x7f,%edx
801008b9:	0f 87 71 ff ff ff    	ja     80100830 <consoleintr+0x20>
801008bf:	8d 50 01             	lea    0x1(%eax),%edx
801008c2:	83 e0 7f             	and    $0x7f,%eax
        c = (c == '\r') ? '\n' : c;
801008c5:	83 ff 0d             	cmp    $0xd,%edi
        input.buf[input.e++ % INPUT_BUF] = c;
801008c8:	89 15 a8 ff 10 80    	mov    %edx,0x8010ffa8
        c = (c == '\r') ? '\n' : c;
801008ce:	0f 84 cc 00 00 00    	je     801009a0 <consoleintr+0x190>
        input.buf[input.e++ % INPUT_BUF] = c;
801008d4:	89 f9                	mov    %edi,%ecx
801008d6:	88 88 20 ff 10 80    	mov    %cl,-0x7fef00e0(%eax)
        consputc(c);
801008dc:	89 f8                	mov    %edi,%eax
801008de:	e8 2d fb ff ff       	call   80100410 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801008e3:	83 ff 0a             	cmp    $0xa,%edi
801008e6:	0f 84 c5 00 00 00    	je     801009b1 <consoleintr+0x1a1>
801008ec:	83 ff 04             	cmp    $0x4,%edi
801008ef:	0f 84 bc 00 00 00    	je     801009b1 <consoleintr+0x1a1>
801008f5:	a1 a0 ff 10 80       	mov    0x8010ffa0,%eax
801008fa:	83 e8 80             	sub    $0xffffff80,%eax
801008fd:	39 05 a8 ff 10 80    	cmp    %eax,0x8010ffa8
80100903:	0f 85 27 ff ff ff    	jne    80100830 <consoleintr+0x20>
          wakeup(&input.r);
80100909:	83 ec 0c             	sub    $0xc,%esp
          input.w = input.e;
8010090c:	a3 a4 ff 10 80       	mov    %eax,0x8010ffa4
          wakeup(&input.r);
80100911:	68 a0 ff 10 80       	push   $0x8010ffa0
80100916:	e8 35 3b 00 00       	call   80104450 <wakeup>
8010091b:	83 c4 10             	add    $0x10,%esp
8010091e:	e9 0d ff ff ff       	jmp    80100830 <consoleintr+0x20>
80100923:	90                   	nop
80100924:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      doprocdump = 1;
80100928:	be 01 00 00 00       	mov    $0x1,%esi
8010092d:	e9 fe fe ff ff       	jmp    80100830 <consoleintr+0x20>
80100932:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      while(input.e != input.w &&
80100938:	a1 a8 ff 10 80       	mov    0x8010ffa8,%eax
8010093d:	39 05 a4 ff 10 80    	cmp    %eax,0x8010ffa4
80100943:	75 2b                	jne    80100970 <consoleintr+0x160>
80100945:	e9 e6 fe ff ff       	jmp    80100830 <consoleintr+0x20>
8010094a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
        input.e--;
80100950:	a3 a8 ff 10 80       	mov    %eax,0x8010ffa8
        consputc(BACKSPACE);
80100955:	b8 00 01 00 00       	mov    $0x100,%eax
8010095a:	e8 b1 fa ff ff       	call   80100410 <consputc>
      while(input.e != input.w &&
8010095f:	a1 a8 ff 10 80       	mov    0x8010ffa8,%eax
80100964:	3b 05 a4 ff 10 80    	cmp    0x8010ffa4,%eax
8010096a:	0f 84 c0 fe ff ff    	je     80100830 <consoleintr+0x20>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
80100970:	83 e8 01             	sub    $0x1,%eax
80100973:	89 c2                	mov    %eax,%edx
80100975:	83 e2 7f             	and    $0x7f,%edx
      while(input.e != input.w &&
80100978:	80 ba 20 ff 10 80 0a 	cmpb   $0xa,-0x7fef00e0(%edx)
8010097f:	75 cf                	jne    80100950 <consoleintr+0x140>
80100981:	e9 aa fe ff ff       	jmp    80100830 <consoleintr+0x20>
80100986:	8d 76 00             	lea    0x0(%esi),%esi
80100989:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
}
80100990:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100993:	5b                   	pop    %ebx
80100994:	5e                   	pop    %esi
80100995:	5f                   	pop    %edi
80100996:	5d                   	pop    %ebp
    procdump();  // now call procdump() wo. cons.lock held
80100997:	e9 c4 3b 00 00       	jmp    80104560 <procdump>
8010099c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        input.buf[input.e++ % INPUT_BUF] = c;
801009a0:	c6 80 20 ff 10 80 0a 	movb   $0xa,-0x7fef00e0(%eax)
        consputc(c);
801009a7:	b8 0a 00 00 00       	mov    $0xa,%eax
801009ac:	e8 5f fa ff ff       	call   80100410 <consputc>
801009b1:	a1 a8 ff 10 80       	mov    0x8010ffa8,%eax
801009b6:	e9 4e ff ff ff       	jmp    80100909 <consoleintr+0xf9>
801009bb:	90                   	nop
801009bc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801009c0 <consoleinit>:

void
consoleinit(void)
{
801009c0:	55                   	push   %ebp
801009c1:	89 e5                	mov    %esp,%ebp
801009c3:	83 ec 10             	sub    $0x10,%esp
  initlock(&cons.lock, "console");
801009c6:	68 a8 74 10 80       	push   $0x801074a8
801009cb:	68 20 a5 10 80       	push   $0x8010a520
801009d0:	e8 5b 3d 00 00       	call   80104730 <initlock>

  devsw[CONSOLE].write = consolewrite;
  devsw[CONSOLE].read = consoleread;
  cons.locking = 1;

  ioapicenable(IRQ_KBD, 0);
801009d5:	58                   	pop    %eax
801009d6:	5a                   	pop    %edx
801009d7:	6a 00                	push   $0x0
801009d9:	6a 01                	push   $0x1
  devsw[CONSOLE].write = consolewrite;
801009db:	c7 05 6c 09 11 80 00 	movl   $0x80100600,0x8011096c
801009e2:	06 10 80 
  devsw[CONSOLE].read = consoleread;
801009e5:	c7 05 68 09 11 80 70 	movl   $0x80100270,0x80110968
801009ec:	02 10 80 
  cons.locking = 1;
801009ef:	c7 05 54 a5 10 80 01 	movl   $0x1,0x8010a554
801009f6:	00 00 00 
  ioapicenable(IRQ_KBD, 0);
801009f9:	e8 c2 1c 00 00       	call   801026c0 <ioapicenable>
}
801009fe:	83 c4 10             	add    $0x10,%esp
80100a01:	c9                   	leave  
80100a02:	c3                   	ret    
80100a03:	66 90                	xchg   %ax,%ax
80100a05:	66 90                	xchg   %ax,%ax
80100a07:	66 90                	xchg   %ax,%ax
80100a09:	66 90                	xchg   %ax,%ax
80100a0b:	66 90                	xchg   %ax,%ax
80100a0d:	66 90                	xchg   %ax,%ax
80100a0f:	90                   	nop

80100a10 <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100a10:	55                   	push   %ebp
80100a11:	89 e5                	mov    %esp,%ebp
80100a13:	57                   	push   %edi
80100a14:	56                   	push   %esi
80100a15:	53                   	push   %ebx
80100a16:	81 ec 18 01 00 00    	sub    $0x118,%esp
    if (DEBUGMODE==1)
        cprintf("EXEC-");
80100a1c:	68 c1 74 10 80       	push   $0x801074c1
80100a21:	e8 3a fc ff ff       	call   80100660 <cprintf>
  uint argc, sz, sp, ustack[3+MAXARG+1];
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;
  struct proc *curproc = myproc();
80100a26:	e8 d5 31 00 00       	call   80103c00 <myproc>
80100a2b:	89 85 ec fe ff ff    	mov    %eax,-0x114(%ebp)

  begin_op();
80100a31:	e8 5a 25 00 00       	call   80102f90 <begin_op>

  if((ip = namei(path)) == 0){
80100a36:	5a                   	pop    %edx
80100a37:	ff 75 08             	pushl  0x8(%ebp)
80100a3a:	e8 01 15 00 00       	call   80101f40 <namei>
80100a3f:	83 c4 10             	add    $0x10,%esp
80100a42:	85 c0                	test   %eax,%eax
80100a44:	0f 84 3a 03 00 00    	je     80100d84 <exec+0x374>
    end_op();
    cprintf("exec: fail\n");
    return -1;
  }
  ilock(ip);
80100a4a:	83 ec 0c             	sub    $0xc,%esp
80100a4d:	89 c6                	mov    %eax,%esi
80100a4f:	50                   	push   %eax
80100a50:	e8 8b 0c 00 00       	call   801016e0 <ilock>
  pgdir = 0;

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) != sizeof(elf))
80100a55:	8d 85 24 ff ff ff    	lea    -0xdc(%ebp),%eax
80100a5b:	6a 34                	push   $0x34
80100a5d:	6a 00                	push   $0x0
80100a5f:	50                   	push   %eax
80100a60:	56                   	push   %esi
80100a61:	e8 5a 0f 00 00       	call   801019c0 <readi>
80100a66:	83 c4 20             	add    $0x20,%esp
80100a69:	83 f8 34             	cmp    $0x34,%eax
80100a6c:	0f 85 f8 00 00 00    	jne    80100b6a <exec+0x15a>
    goto bad;
  if(elf.magic != ELF_MAGIC)
80100a72:	81 bd 24 ff ff ff 7f 	cmpl   $0x464c457f,-0xdc(%ebp)
80100a79:	45 4c 46 
80100a7c:	0f 85 e8 00 00 00    	jne    80100b6a <exec+0x15a>
    goto bad;

  if((pgdir = setupkvm()) == 0)
80100a82:	e8 29 67 00 00       	call   801071b0 <setupkvm>
80100a87:	85 c0                	test   %eax,%eax
80100a89:	89 85 f4 fe ff ff    	mov    %eax,-0x10c(%ebp)
80100a8f:	0f 84 d5 00 00 00    	je     80100b6a <exec+0x15a>
    goto bad;

  // Load program into memory.
  sz = 0;
80100a95:	31 ff                	xor    %edi,%edi
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100a97:	66 83 bd 50 ff ff ff 	cmpw   $0x0,-0xb0(%ebp)
80100a9e:	00 
80100a9f:	8b 85 40 ff ff ff    	mov    -0xc0(%ebp),%eax
80100aa5:	89 85 f0 fe ff ff    	mov    %eax,-0x110(%ebp)
80100aab:	0f 84 a5 02 00 00    	je     80100d56 <exec+0x346>
80100ab1:	31 db                	xor    %ebx,%ebx
80100ab3:	eb 7d                	jmp    80100b32 <exec+0x122>
80100ab5:	8d 76 00             	lea    0x0(%esi),%esi
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
80100ab8:	83 bd 04 ff ff ff 01 	cmpl   $0x1,-0xfc(%ebp)
80100abf:	75 63                	jne    80100b24 <exec+0x114>
      continue;
    if(ph.memsz < ph.filesz)
80100ac1:	8b 85 18 ff ff ff    	mov    -0xe8(%ebp),%eax
80100ac7:	3b 85 14 ff ff ff    	cmp    -0xec(%ebp),%eax
80100acd:	0f 82 86 00 00 00    	jb     80100b59 <exec+0x149>
80100ad3:	03 85 0c ff ff ff    	add    -0xf4(%ebp),%eax
80100ad9:	72 7e                	jb     80100b59 <exec+0x149>
      goto bad;
    if(ph.vaddr + ph.memsz < ph.vaddr)
      goto bad;
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100adb:	83 ec 04             	sub    $0x4,%esp
80100ade:	50                   	push   %eax
80100adf:	57                   	push   %edi
80100ae0:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
80100ae6:	e8 e5 64 00 00       	call   80106fd0 <allocuvm>
80100aeb:	83 c4 10             	add    $0x10,%esp
80100aee:	85 c0                	test   %eax,%eax
80100af0:	89 c7                	mov    %eax,%edi
80100af2:	74 65                	je     80100b59 <exec+0x149>
      goto bad;
    if(ph.vaddr % PGSIZE != 0)
80100af4:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100afa:	a9 ff 0f 00 00       	test   $0xfff,%eax
80100aff:	75 58                	jne    80100b59 <exec+0x149>
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100b01:	83 ec 0c             	sub    $0xc,%esp
80100b04:	ff b5 14 ff ff ff    	pushl  -0xec(%ebp)
80100b0a:	ff b5 08 ff ff ff    	pushl  -0xf8(%ebp)
80100b10:	56                   	push   %esi
80100b11:	50                   	push   %eax
80100b12:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
80100b18:	e8 f3 63 00 00       	call   80106f10 <loaduvm>
80100b1d:	83 c4 20             	add    $0x20,%esp
80100b20:	85 c0                	test   %eax,%eax
80100b22:	78 35                	js     80100b59 <exec+0x149>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b24:	0f b7 85 50 ff ff ff 	movzwl -0xb0(%ebp),%eax
80100b2b:	83 c3 01             	add    $0x1,%ebx
80100b2e:	39 d8                	cmp    %ebx,%eax
80100b30:	7e 6e                	jle    80100ba0 <exec+0x190>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100b32:	89 d8                	mov    %ebx,%eax
80100b34:	6a 20                	push   $0x20
80100b36:	c1 e0 05             	shl    $0x5,%eax
80100b39:	03 85 f0 fe ff ff    	add    -0x110(%ebp),%eax
80100b3f:	50                   	push   %eax
80100b40:	8d 85 04 ff ff ff    	lea    -0xfc(%ebp),%eax
80100b46:	50                   	push   %eax
80100b47:	56                   	push   %esi
80100b48:	e8 73 0e 00 00       	call   801019c0 <readi>
80100b4d:	83 c4 10             	add    $0x10,%esp
80100b50:	83 f8 20             	cmp    $0x20,%eax
80100b53:	0f 84 5f ff ff ff    	je     80100ab8 <exec+0xa8>
        cprintf(">EXEC-DONE!\t");
  return 0;

 bad:
  if(pgdir)
    freevm(pgdir);
80100b59:	83 ec 0c             	sub    $0xc,%esp
80100b5c:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
80100b62:	e8 c9 65 00 00       	call   80107130 <freevm>
80100b67:	83 c4 10             	add    $0x10,%esp
  if(ip){
    iunlockput(ip);
80100b6a:	83 ec 0c             	sub    $0xc,%esp
80100b6d:	56                   	push   %esi
80100b6e:	e8 fd 0d 00 00       	call   80101970 <iunlockput>
    end_op();
80100b73:	e8 88 24 00 00       	call   80103000 <end_op>
80100b78:	83 c4 10             	add    $0x10,%esp
  }
    if (DEBUGMODE==1)
        cprintf(">EXEC-FAILED-GOTO_BAD!\t");
80100b7b:	83 ec 0c             	sub    $0xc,%esp
80100b7e:	68 e0 74 10 80       	push   $0x801074e0
80100b83:	e8 d8 fa ff ff       	call   80100660 <cprintf>
  return -1;
80100b88:	83 c4 10             	add    $0x10,%esp
80100b8b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100b90:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100b93:	5b                   	pop    %ebx
80100b94:	5e                   	pop    %esi
80100b95:	5f                   	pop    %edi
80100b96:	5d                   	pop    %ebp
80100b97:	c3                   	ret    
80100b98:	90                   	nop
80100b99:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100ba0:	81 c7 ff 0f 00 00    	add    $0xfff,%edi
80100ba6:	81 e7 00 f0 ff ff    	and    $0xfffff000,%edi
80100bac:	8d 9f 00 20 00 00    	lea    0x2000(%edi),%ebx
  iunlockput(ip);
80100bb2:	83 ec 0c             	sub    $0xc,%esp
80100bb5:	56                   	push   %esi
80100bb6:	e8 b5 0d 00 00       	call   80101970 <iunlockput>
  end_op();
80100bbb:	e8 40 24 00 00       	call   80103000 <end_op>
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100bc0:	8b b5 f4 fe ff ff    	mov    -0x10c(%ebp),%esi
80100bc6:	83 c4 0c             	add    $0xc,%esp
80100bc9:	53                   	push   %ebx
80100bca:	57                   	push   %edi
80100bcb:	56                   	push   %esi
80100bcc:	e8 ff 63 00 00       	call   80106fd0 <allocuvm>
80100bd1:	83 c4 10             	add    $0x10,%esp
80100bd4:	85 c0                	test   %eax,%eax
80100bd6:	89 c7                	mov    %eax,%edi
80100bd8:	0f 84 83 00 00 00    	je     80100c61 <exec+0x251>
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100bde:	8d 80 00 e0 ff ff    	lea    -0x2000(%eax),%eax
80100be4:	83 ec 08             	sub    $0x8,%esp
  for(argc = 0; argv[argc]; argc++) {
80100be7:	89 fb                	mov    %edi,%ebx
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100be9:	50                   	push   %eax
80100bea:	56                   	push   %esi
  for(argc = 0; argv[argc]; argc++) {
80100beb:	31 f6                	xor    %esi,%esi
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100bed:	e8 5e 66 00 00       	call   80107250 <clearpteu>
  for(argc = 0; argv[argc]; argc++) {
80100bf2:	8b 45 0c             	mov    0xc(%ebp),%eax
80100bf5:	83 c4 10             	add    $0x10,%esp
80100bf8:	8b 08                	mov    (%eax),%ecx
80100bfa:	85 c9                	test   %ecx,%ecx
80100bfc:	0f 84 5e 01 00 00    	je     80100d60 <exec+0x350>
80100c02:	89 bd f0 fe ff ff    	mov    %edi,-0x110(%ebp)
80100c08:	8b 7d 0c             	mov    0xc(%ebp),%edi
80100c0b:	eb 21                	jmp    80100c2e <exec+0x21e>
80100c0d:	8d 76 00             	lea    0x0(%esi),%esi
80100c10:	8d 46 01             	lea    0x1(%esi),%eax
    ustack[3+argc] = sp;
80100c13:	8d 95 58 ff ff ff    	lea    -0xa8(%ebp),%edx
80100c19:	89 9c b5 64 ff ff ff 	mov    %ebx,-0x9c(%ebp,%esi,4)
  for(argc = 0; argv[argc]; argc++) {
80100c20:	8b 0c 87             	mov    (%edi,%eax,4),%ecx
80100c23:	85 c9                	test   %ecx,%ecx
80100c25:	74 50                	je     80100c77 <exec+0x267>
    if(argc >= MAXARG)
80100c27:	83 f8 20             	cmp    $0x20,%eax
80100c2a:	74 35                	je     80100c61 <exec+0x251>
80100c2c:	89 c6                	mov    %eax,%esi
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100c2e:	83 ec 0c             	sub    $0xc,%esp
80100c31:	51                   	push   %ecx
80100c32:	e8 89 3f 00 00       	call   80104bc0 <strlen>
80100c37:	f7 d0                	not    %eax
80100c39:	01 c3                	add    %eax,%ebx
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100c3b:	58                   	pop    %eax
80100c3c:	ff 34 b7             	pushl  (%edi,%esi,4)
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100c3f:	83 e3 fc             	and    $0xfffffffc,%ebx
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100c42:	e8 79 3f 00 00       	call   80104bc0 <strlen>
80100c47:	83 c0 01             	add    $0x1,%eax
80100c4a:	50                   	push   %eax
80100c4b:	ff 34 b7             	pushl  (%edi,%esi,4)
80100c4e:	53                   	push   %ebx
80100c4f:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
80100c55:	e8 46 67 00 00       	call   801073a0 <copyout>
80100c5a:	83 c4 20             	add    $0x20,%esp
80100c5d:	85 c0                	test   %eax,%eax
80100c5f:	79 af                	jns    80100c10 <exec+0x200>
    freevm(pgdir);
80100c61:	83 ec 0c             	sub    $0xc,%esp
80100c64:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
80100c6a:	e8 c1 64 00 00       	call   80107130 <freevm>
80100c6f:	83 c4 10             	add    $0x10,%esp
80100c72:	e9 04 ff ff ff       	jmp    80100b7b <exec+0x16b>
80100c77:	8b bd f0 fe ff ff    	mov    -0x110(%ebp),%edi
80100c7d:	89 85 f0 fe ff ff    	mov    %eax,-0x110(%ebp)
80100c83:	8d 46 04             	lea    0x4(%esi),%eax
80100c86:	8d 34 b5 08 00 00 00 	lea    0x8(,%esi,4),%esi
80100c8d:	8d 4e 0c             	lea    0xc(%esi),%ecx
  ustack[3+argc] = 0;
80100c90:	c7 84 85 58 ff ff ff 	movl   $0x0,-0xa8(%ebp,%eax,4)
80100c97:	00 00 00 00 
  ustack[1] = argc;
80100c9b:	8b 85 f0 fe ff ff    	mov    -0x110(%ebp),%eax
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100ca1:	51                   	push   %ecx
80100ca2:	52                   	push   %edx
  ustack[0] = 0xffffffff;  // fake return PC
80100ca3:	c7 85 58 ff ff ff ff 	movl   $0xffffffff,-0xa8(%ebp)
80100caa:	ff ff ff 
  ustack[1] = argc;
80100cad:	89 85 5c ff ff ff    	mov    %eax,-0xa4(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100cb3:	89 d8                	mov    %ebx,%eax
  sp -= (3+argc+1) * 4;
80100cb5:	29 cb                	sub    %ecx,%ebx
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100cb7:	53                   	push   %ebx
80100cb8:	ff b5 f4 fe ff ff    	pushl  -0x10c(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100cbe:	29 f0                	sub    %esi,%eax
80100cc0:	89 85 60 ff ff ff    	mov    %eax,-0xa0(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100cc6:	e8 d5 66 00 00       	call   801073a0 <copyout>
80100ccb:	83 c4 10             	add    $0x10,%esp
80100cce:	85 c0                	test   %eax,%eax
80100cd0:	78 8f                	js     80100c61 <exec+0x251>
  for(last=s=path; *s; s++)
80100cd2:	8b 45 08             	mov    0x8(%ebp),%eax
80100cd5:	8b 55 08             	mov    0x8(%ebp),%edx
80100cd8:	0f b6 00             	movzbl (%eax),%eax
80100cdb:	84 c0                	test   %al,%al
80100cdd:	74 18                	je     80100cf7 <exec+0x2e7>
80100cdf:	89 d1                	mov    %edx,%ecx
80100ce1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80100ce8:	83 c1 01             	add    $0x1,%ecx
80100ceb:	3c 2f                	cmp    $0x2f,%al
80100ced:	0f b6 01             	movzbl (%ecx),%eax
80100cf0:	0f 44 d1             	cmove  %ecx,%edx
80100cf3:	84 c0                	test   %al,%al
80100cf5:	75 f1                	jne    80100ce8 <exec+0x2d8>
  safestrcpy(curproc->name, last, sizeof(curproc->name));
80100cf7:	8b b5 ec fe ff ff    	mov    -0x114(%ebp),%esi
80100cfd:	83 ec 04             	sub    $0x4,%esp
80100d00:	6a 10                	push   $0x10
80100d02:	52                   	push   %edx
80100d03:	8d 46 6c             	lea    0x6c(%esi),%eax
80100d06:	50                   	push   %eax
80100d07:	e8 74 3e 00 00       	call   80104b80 <safestrcpy>
  curproc->pgdir = pgdir;
80100d0c:	8b 8d f4 fe ff ff    	mov    -0x10c(%ebp),%ecx
  oldpgdir = curproc->pgdir;
80100d12:	89 f0                	mov    %esi,%eax
80100d14:	8b 76 04             	mov    0x4(%esi),%esi
  curproc->sz = sz;
80100d17:	89 38                	mov    %edi,(%eax)
  curproc->pgdir = pgdir;
80100d19:	89 48 04             	mov    %ecx,0x4(%eax)
  curproc->tf->eip = elf.entry;  // main
80100d1c:	89 c1                	mov    %eax,%ecx
80100d1e:	8b 95 3c ff ff ff    	mov    -0xc4(%ebp),%edx
80100d24:	8b 40 18             	mov    0x18(%eax),%eax
80100d27:	89 50 38             	mov    %edx,0x38(%eax)
  curproc->tf->esp = sp;
80100d2a:	8b 41 18             	mov    0x18(%ecx),%eax
80100d2d:	89 58 44             	mov    %ebx,0x44(%eax)
  switchuvm(curproc);
80100d30:	89 0c 24             	mov    %ecx,(%esp)
80100d33:	e8 48 60 00 00       	call   80106d80 <switchuvm>
  freevm(oldpgdir);
80100d38:	89 34 24             	mov    %esi,(%esp)
80100d3b:	e8 f0 63 00 00       	call   80107130 <freevm>
        cprintf(">EXEC-DONE!\t");
80100d40:	c7 04 24 d3 74 10 80 	movl   $0x801074d3,(%esp)
80100d47:	e8 14 f9 ff ff       	call   80100660 <cprintf>
  return 0;
80100d4c:	83 c4 10             	add    $0x10,%esp
80100d4f:	31 c0                	xor    %eax,%eax
80100d51:	e9 3a fe ff ff       	jmp    80100b90 <exec+0x180>
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100d56:	bb 00 20 00 00       	mov    $0x2000,%ebx
80100d5b:	e9 52 fe ff ff       	jmp    80100bb2 <exec+0x1a2>
  for(argc = 0; argv[argc]; argc++) {
80100d60:	b9 10 00 00 00       	mov    $0x10,%ecx
80100d65:	be 04 00 00 00       	mov    $0x4,%esi
80100d6a:	b8 03 00 00 00       	mov    $0x3,%eax
80100d6f:	c7 85 f0 fe ff ff 00 	movl   $0x0,-0x110(%ebp)
80100d76:	00 00 00 
80100d79:	8d 95 58 ff ff ff    	lea    -0xa8(%ebp),%edx
80100d7f:	e9 0c ff ff ff       	jmp    80100c90 <exec+0x280>
    end_op();
80100d84:	e8 77 22 00 00       	call   80103000 <end_op>
    cprintf("exec: fail\n");
80100d89:	83 ec 0c             	sub    $0xc,%esp
80100d8c:	68 c7 74 10 80       	push   $0x801074c7
80100d91:	e8 ca f8 ff ff       	call   80100660 <cprintf>
    return -1;
80100d96:	83 c4 10             	add    $0x10,%esp
80100d99:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100d9e:	e9 ed fd ff ff       	jmp    80100b90 <exec+0x180>
80100da3:	66 90                	xchg   %ax,%ax
80100da5:	66 90                	xchg   %ax,%ax
80100da7:	66 90                	xchg   %ax,%ax
80100da9:	66 90                	xchg   %ax,%ax
80100dab:	66 90                	xchg   %ax,%ax
80100dad:	66 90                	xchg   %ax,%ax
80100daf:	90                   	nop

80100db0 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100db0:	55                   	push   %ebp
80100db1:	89 e5                	mov    %esp,%ebp
80100db3:	83 ec 10             	sub    $0x10,%esp
  initlock(&ftable.lock, "ftable");
80100db6:	68 f8 74 10 80       	push   $0x801074f8
80100dbb:	68 c0 ff 10 80       	push   $0x8010ffc0
80100dc0:	e8 6b 39 00 00       	call   80104730 <initlock>
}
80100dc5:	83 c4 10             	add    $0x10,%esp
80100dc8:	c9                   	leave  
80100dc9:	c3                   	ret    
80100dca:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80100dd0 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100dd0:	55                   	push   %ebp
80100dd1:	89 e5                	mov    %esp,%ebp
80100dd3:	53                   	push   %ebx
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100dd4:	bb f4 ff 10 80       	mov    $0x8010fff4,%ebx
{
80100dd9:	83 ec 10             	sub    $0x10,%esp
  acquire(&ftable.lock);
80100ddc:	68 c0 ff 10 80       	push   $0x8010ffc0
80100de1:	e8 3a 3a 00 00       	call   80104820 <acquire>
80100de6:	83 c4 10             	add    $0x10,%esp
80100de9:	eb 10                	jmp    80100dfb <filealloc+0x2b>
80100deb:	90                   	nop
80100dec:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100df0:	83 c3 18             	add    $0x18,%ebx
80100df3:	81 fb 54 09 11 80    	cmp    $0x80110954,%ebx
80100df9:	73 25                	jae    80100e20 <filealloc+0x50>
    if(f->ref == 0){
80100dfb:	8b 43 04             	mov    0x4(%ebx),%eax
80100dfe:	85 c0                	test   %eax,%eax
80100e00:	75 ee                	jne    80100df0 <filealloc+0x20>
      f->ref = 1;
      release(&ftable.lock);
80100e02:	83 ec 0c             	sub    $0xc,%esp
      f->ref = 1;
80100e05:	c7 43 04 01 00 00 00 	movl   $0x1,0x4(%ebx)
      release(&ftable.lock);
80100e0c:	68 c0 ff 10 80       	push   $0x8010ffc0
80100e11:	e8 2a 3b 00 00       	call   80104940 <release>
      return f;
    }
  }
  release(&ftable.lock);
  return 0;
}
80100e16:	89 d8                	mov    %ebx,%eax
      return f;
80100e18:	83 c4 10             	add    $0x10,%esp
}
80100e1b:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80100e1e:	c9                   	leave  
80100e1f:	c3                   	ret    
  release(&ftable.lock);
80100e20:	83 ec 0c             	sub    $0xc,%esp
  return 0;
80100e23:	31 db                	xor    %ebx,%ebx
  release(&ftable.lock);
80100e25:	68 c0 ff 10 80       	push   $0x8010ffc0
80100e2a:	e8 11 3b 00 00       	call   80104940 <release>
}
80100e2f:	89 d8                	mov    %ebx,%eax
  return 0;
80100e31:	83 c4 10             	add    $0x10,%esp
}
80100e34:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80100e37:	c9                   	leave  
80100e38:	c3                   	ret    
80100e39:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80100e40 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100e40:	55                   	push   %ebp
80100e41:	89 e5                	mov    %esp,%ebp
80100e43:	53                   	push   %ebx
80100e44:	83 ec 10             	sub    $0x10,%esp
80100e47:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&ftable.lock);
80100e4a:	68 c0 ff 10 80       	push   $0x8010ffc0
80100e4f:	e8 cc 39 00 00       	call   80104820 <acquire>
  if(f->ref < 1)
80100e54:	8b 43 04             	mov    0x4(%ebx),%eax
80100e57:	83 c4 10             	add    $0x10,%esp
80100e5a:	85 c0                	test   %eax,%eax
80100e5c:	7e 1a                	jle    80100e78 <filedup+0x38>
    panic("filedup");
  f->ref++;
80100e5e:	83 c0 01             	add    $0x1,%eax
  release(&ftable.lock);
80100e61:	83 ec 0c             	sub    $0xc,%esp
  f->ref++;
80100e64:	89 43 04             	mov    %eax,0x4(%ebx)
  release(&ftable.lock);
80100e67:	68 c0 ff 10 80       	push   $0x8010ffc0
80100e6c:	e8 cf 3a 00 00       	call   80104940 <release>
  return f;
}
80100e71:	89 d8                	mov    %ebx,%eax
80100e73:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80100e76:	c9                   	leave  
80100e77:	c3                   	ret    
    panic("filedup");
80100e78:	83 ec 0c             	sub    $0xc,%esp
80100e7b:	68 ff 74 10 80       	push   $0x801074ff
80100e80:	e8 0b f5 ff ff       	call   80100390 <panic>
80100e85:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80100e89:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80100e90 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100e90:	55                   	push   %ebp
80100e91:	89 e5                	mov    %esp,%ebp
80100e93:	57                   	push   %edi
80100e94:	56                   	push   %esi
80100e95:	53                   	push   %ebx
80100e96:	83 ec 28             	sub    $0x28,%esp
80100e99:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct file ff;

  acquire(&ftable.lock);
80100e9c:	68 c0 ff 10 80       	push   $0x8010ffc0
80100ea1:	e8 7a 39 00 00       	call   80104820 <acquire>
  if(f->ref < 1)
80100ea6:	8b 43 04             	mov    0x4(%ebx),%eax
80100ea9:	83 c4 10             	add    $0x10,%esp
80100eac:	85 c0                	test   %eax,%eax
80100eae:	0f 8e 9b 00 00 00    	jle    80100f4f <fileclose+0xbf>
    panic("fileclose");
  if(--f->ref > 0){
80100eb4:	83 e8 01             	sub    $0x1,%eax
80100eb7:	85 c0                	test   %eax,%eax
80100eb9:	89 43 04             	mov    %eax,0x4(%ebx)
80100ebc:	74 1a                	je     80100ed8 <fileclose+0x48>
    release(&ftable.lock);
80100ebe:	c7 45 08 c0 ff 10 80 	movl   $0x8010ffc0,0x8(%ebp)
  else if(ff.type == FD_INODE){
    begin_op();
    iput(ff.ip);
    end_op();
  }
}
80100ec5:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100ec8:	5b                   	pop    %ebx
80100ec9:	5e                   	pop    %esi
80100eca:	5f                   	pop    %edi
80100ecb:	5d                   	pop    %ebp
    release(&ftable.lock);
80100ecc:	e9 6f 3a 00 00       	jmp    80104940 <release>
80100ed1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  ff = *f;
80100ed8:	0f b6 43 09          	movzbl 0x9(%ebx),%eax
80100edc:	8b 3b                	mov    (%ebx),%edi
  release(&ftable.lock);
80100ede:	83 ec 0c             	sub    $0xc,%esp
  ff = *f;
80100ee1:	8b 73 0c             	mov    0xc(%ebx),%esi
  f->type = FD_NONE;
80100ee4:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  ff = *f;
80100eea:	88 45 e7             	mov    %al,-0x19(%ebp)
80100eed:	8b 43 10             	mov    0x10(%ebx),%eax
  release(&ftable.lock);
80100ef0:	68 c0 ff 10 80       	push   $0x8010ffc0
  ff = *f;
80100ef5:	89 45 e0             	mov    %eax,-0x20(%ebp)
  release(&ftable.lock);
80100ef8:	e8 43 3a 00 00       	call   80104940 <release>
  if(ff.type == FD_PIPE)
80100efd:	83 c4 10             	add    $0x10,%esp
80100f00:	83 ff 01             	cmp    $0x1,%edi
80100f03:	74 13                	je     80100f18 <fileclose+0x88>
  else if(ff.type == FD_INODE){
80100f05:	83 ff 02             	cmp    $0x2,%edi
80100f08:	74 26                	je     80100f30 <fileclose+0xa0>
}
80100f0a:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100f0d:	5b                   	pop    %ebx
80100f0e:	5e                   	pop    %esi
80100f0f:	5f                   	pop    %edi
80100f10:	5d                   	pop    %ebp
80100f11:	c3                   	ret    
80100f12:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    pipeclose(ff.pipe, ff.writable);
80100f18:	0f be 5d e7          	movsbl -0x19(%ebp),%ebx
80100f1c:	83 ec 08             	sub    $0x8,%esp
80100f1f:	53                   	push   %ebx
80100f20:	56                   	push   %esi
80100f21:	e8 1a 28 00 00       	call   80103740 <pipeclose>
80100f26:	83 c4 10             	add    $0x10,%esp
80100f29:	eb df                	jmp    80100f0a <fileclose+0x7a>
80100f2b:	90                   	nop
80100f2c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    begin_op();
80100f30:	e8 5b 20 00 00       	call   80102f90 <begin_op>
    iput(ff.ip);
80100f35:	83 ec 0c             	sub    $0xc,%esp
80100f38:	ff 75 e0             	pushl  -0x20(%ebp)
80100f3b:	e8 d0 08 00 00       	call   80101810 <iput>
    end_op();
80100f40:	83 c4 10             	add    $0x10,%esp
}
80100f43:	8d 65 f4             	lea    -0xc(%ebp),%esp
80100f46:	5b                   	pop    %ebx
80100f47:	5e                   	pop    %esi
80100f48:	5f                   	pop    %edi
80100f49:	5d                   	pop    %ebp
    end_op();
80100f4a:	e9 b1 20 00 00       	jmp    80103000 <end_op>
    panic("fileclose");
80100f4f:	83 ec 0c             	sub    $0xc,%esp
80100f52:	68 07 75 10 80       	push   $0x80107507
80100f57:	e8 34 f4 ff ff       	call   80100390 <panic>
80100f5c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80100f60 <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
80100f60:	55                   	push   %ebp
80100f61:	89 e5                	mov    %esp,%ebp
80100f63:	53                   	push   %ebx
80100f64:	83 ec 04             	sub    $0x4,%esp
80100f67:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(f->type == FD_INODE){
80100f6a:	83 3b 02             	cmpl   $0x2,(%ebx)
80100f6d:	75 31                	jne    80100fa0 <filestat+0x40>
    ilock(f->ip);
80100f6f:	83 ec 0c             	sub    $0xc,%esp
80100f72:	ff 73 10             	pushl  0x10(%ebx)
80100f75:	e8 66 07 00 00       	call   801016e0 <ilock>
    stati(f->ip, st);
80100f7a:	58                   	pop    %eax
80100f7b:	5a                   	pop    %edx
80100f7c:	ff 75 0c             	pushl  0xc(%ebp)
80100f7f:	ff 73 10             	pushl  0x10(%ebx)
80100f82:	e8 09 0a 00 00       	call   80101990 <stati>
    iunlock(f->ip);
80100f87:	59                   	pop    %ecx
80100f88:	ff 73 10             	pushl  0x10(%ebx)
80100f8b:	e8 30 08 00 00       	call   801017c0 <iunlock>
    return 0;
80100f90:	83 c4 10             	add    $0x10,%esp
80100f93:	31 c0                	xor    %eax,%eax
  }
  return -1;
}
80100f95:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80100f98:	c9                   	leave  
80100f99:	c3                   	ret    
80100f9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  return -1;
80100fa0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100fa5:	eb ee                	jmp    80100f95 <filestat+0x35>
80100fa7:	89 f6                	mov    %esi,%esi
80100fa9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80100fb0 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
80100fb0:	55                   	push   %ebp
80100fb1:	89 e5                	mov    %esp,%ebp
80100fb3:	57                   	push   %edi
80100fb4:	56                   	push   %esi
80100fb5:	53                   	push   %ebx
80100fb6:	83 ec 0c             	sub    $0xc,%esp
80100fb9:	8b 5d 08             	mov    0x8(%ebp),%ebx
80100fbc:	8b 75 0c             	mov    0xc(%ebp),%esi
80100fbf:	8b 7d 10             	mov    0x10(%ebp),%edi
  int r;

  if(f->readable == 0)
80100fc2:	80 7b 08 00          	cmpb   $0x0,0x8(%ebx)
80100fc6:	74 60                	je     80101028 <fileread+0x78>
    return -1;
  if(f->type == FD_PIPE)
80100fc8:	8b 03                	mov    (%ebx),%eax
80100fca:	83 f8 01             	cmp    $0x1,%eax
80100fcd:	74 41                	je     80101010 <fileread+0x60>
    return piperead(f->pipe, addr, n);
  if(f->type == FD_INODE){
80100fcf:	83 f8 02             	cmp    $0x2,%eax
80100fd2:	75 5b                	jne    8010102f <fileread+0x7f>
    ilock(f->ip);
80100fd4:	83 ec 0c             	sub    $0xc,%esp
80100fd7:	ff 73 10             	pushl  0x10(%ebx)
80100fda:	e8 01 07 00 00       	call   801016e0 <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
80100fdf:	57                   	push   %edi
80100fe0:	ff 73 14             	pushl  0x14(%ebx)
80100fe3:	56                   	push   %esi
80100fe4:	ff 73 10             	pushl  0x10(%ebx)
80100fe7:	e8 d4 09 00 00       	call   801019c0 <readi>
80100fec:	83 c4 20             	add    $0x20,%esp
80100fef:	85 c0                	test   %eax,%eax
80100ff1:	89 c6                	mov    %eax,%esi
80100ff3:	7e 03                	jle    80100ff8 <fileread+0x48>
      f->off += r;
80100ff5:	01 43 14             	add    %eax,0x14(%ebx)
    iunlock(f->ip);
80100ff8:	83 ec 0c             	sub    $0xc,%esp
80100ffb:	ff 73 10             	pushl  0x10(%ebx)
80100ffe:	e8 bd 07 00 00       	call   801017c0 <iunlock>
    return r;
80101003:	83 c4 10             	add    $0x10,%esp
  }
  panic("fileread");
}
80101006:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101009:	89 f0                	mov    %esi,%eax
8010100b:	5b                   	pop    %ebx
8010100c:	5e                   	pop    %esi
8010100d:	5f                   	pop    %edi
8010100e:	5d                   	pop    %ebp
8010100f:	c3                   	ret    
    return piperead(f->pipe, addr, n);
80101010:	8b 43 0c             	mov    0xc(%ebx),%eax
80101013:	89 45 08             	mov    %eax,0x8(%ebp)
}
80101016:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101019:	5b                   	pop    %ebx
8010101a:	5e                   	pop    %esi
8010101b:	5f                   	pop    %edi
8010101c:	5d                   	pop    %ebp
    return piperead(f->pipe, addr, n);
8010101d:	e9 ce 28 00 00       	jmp    801038f0 <piperead>
80101022:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    return -1;
80101028:	be ff ff ff ff       	mov    $0xffffffff,%esi
8010102d:	eb d7                	jmp    80101006 <fileread+0x56>
  panic("fileread");
8010102f:	83 ec 0c             	sub    $0xc,%esp
80101032:	68 11 75 10 80       	push   $0x80107511
80101037:	e8 54 f3 ff ff       	call   80100390 <panic>
8010103c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101040 <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
80101040:	55                   	push   %ebp
80101041:	89 e5                	mov    %esp,%ebp
80101043:	57                   	push   %edi
80101044:	56                   	push   %esi
80101045:	53                   	push   %ebx
80101046:	83 ec 1c             	sub    $0x1c,%esp
80101049:	8b 75 08             	mov    0x8(%ebp),%esi
8010104c:	8b 45 0c             	mov    0xc(%ebp),%eax
  int r;

  if(f->writable == 0)
8010104f:	80 7e 09 00          	cmpb   $0x0,0x9(%esi)
{
80101053:	89 45 dc             	mov    %eax,-0x24(%ebp)
80101056:	8b 45 10             	mov    0x10(%ebp),%eax
80101059:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(f->writable == 0)
8010105c:	0f 84 aa 00 00 00    	je     8010110c <filewrite+0xcc>
    return -1;
  if(f->type == FD_PIPE)
80101062:	8b 06                	mov    (%esi),%eax
80101064:	83 f8 01             	cmp    $0x1,%eax
80101067:	0f 84 c3 00 00 00    	je     80101130 <filewrite+0xf0>
    return pipewrite(f->pipe, addr, n);
  if(f->type == FD_INODE){
8010106d:	83 f8 02             	cmp    $0x2,%eax
80101070:	0f 85 d9 00 00 00    	jne    8010114f <filewrite+0x10f>
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((MAXOPBLOCKS-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
80101076:	8b 45 e4             	mov    -0x1c(%ebp),%eax
    int i = 0;
80101079:	31 ff                	xor    %edi,%edi
    while(i < n){
8010107b:	85 c0                	test   %eax,%eax
8010107d:	7f 34                	jg     801010b3 <filewrite+0x73>
8010107f:	e9 9c 00 00 00       	jmp    80101120 <filewrite+0xe0>
80101084:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        n1 = max;

      begin_op();
      ilock(f->ip);
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
        f->off += r;
80101088:	01 46 14             	add    %eax,0x14(%esi)
      iunlock(f->ip);
8010108b:	83 ec 0c             	sub    $0xc,%esp
8010108e:	ff 76 10             	pushl  0x10(%esi)
        f->off += r;
80101091:	89 45 e0             	mov    %eax,-0x20(%ebp)
      iunlock(f->ip);
80101094:	e8 27 07 00 00       	call   801017c0 <iunlock>
      end_op();
80101099:	e8 62 1f 00 00       	call   80103000 <end_op>
8010109e:	8b 45 e0             	mov    -0x20(%ebp),%eax
801010a1:	83 c4 10             	add    $0x10,%esp

      if(r < 0)
        break;
      if(r != n1)
801010a4:	39 c3                	cmp    %eax,%ebx
801010a6:	0f 85 96 00 00 00    	jne    80101142 <filewrite+0x102>
        panic("short filewrite");
      i += r;
801010ac:	01 df                	add    %ebx,%edi
    while(i < n){
801010ae:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
801010b1:	7e 6d                	jle    80101120 <filewrite+0xe0>
      int n1 = n - i;
801010b3:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
801010b6:	b8 00 06 00 00       	mov    $0x600,%eax
801010bb:	29 fb                	sub    %edi,%ebx
801010bd:	81 fb 00 06 00 00    	cmp    $0x600,%ebx
801010c3:	0f 4f d8             	cmovg  %eax,%ebx
      begin_op();
801010c6:	e8 c5 1e 00 00       	call   80102f90 <begin_op>
      ilock(f->ip);
801010cb:	83 ec 0c             	sub    $0xc,%esp
801010ce:	ff 76 10             	pushl  0x10(%esi)
801010d1:	e8 0a 06 00 00       	call   801016e0 <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
801010d6:	8b 45 dc             	mov    -0x24(%ebp),%eax
801010d9:	53                   	push   %ebx
801010da:	ff 76 14             	pushl  0x14(%esi)
801010dd:	01 f8                	add    %edi,%eax
801010df:	50                   	push   %eax
801010e0:	ff 76 10             	pushl  0x10(%esi)
801010e3:	e8 d8 09 00 00       	call   80101ac0 <writei>
801010e8:	83 c4 20             	add    $0x20,%esp
801010eb:	85 c0                	test   %eax,%eax
801010ed:	7f 99                	jg     80101088 <filewrite+0x48>
      iunlock(f->ip);
801010ef:	83 ec 0c             	sub    $0xc,%esp
801010f2:	ff 76 10             	pushl  0x10(%esi)
801010f5:	89 45 e0             	mov    %eax,-0x20(%ebp)
801010f8:	e8 c3 06 00 00       	call   801017c0 <iunlock>
      end_op();
801010fd:	e8 fe 1e 00 00       	call   80103000 <end_op>
      if(r < 0)
80101102:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101105:	83 c4 10             	add    $0x10,%esp
80101108:	85 c0                	test   %eax,%eax
8010110a:	74 98                	je     801010a4 <filewrite+0x64>
    }
    return i == n ? n : -1;
  }
  panic("filewrite");
}
8010110c:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return -1;
8010110f:	bf ff ff ff ff       	mov    $0xffffffff,%edi
}
80101114:	89 f8                	mov    %edi,%eax
80101116:	5b                   	pop    %ebx
80101117:	5e                   	pop    %esi
80101118:	5f                   	pop    %edi
80101119:	5d                   	pop    %ebp
8010111a:	c3                   	ret    
8010111b:	90                   	nop
8010111c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    return i == n ? n : -1;
80101120:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
80101123:	75 e7                	jne    8010110c <filewrite+0xcc>
}
80101125:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101128:	89 f8                	mov    %edi,%eax
8010112a:	5b                   	pop    %ebx
8010112b:	5e                   	pop    %esi
8010112c:	5f                   	pop    %edi
8010112d:	5d                   	pop    %ebp
8010112e:	c3                   	ret    
8010112f:	90                   	nop
    return pipewrite(f->pipe, addr, n);
80101130:	8b 46 0c             	mov    0xc(%esi),%eax
80101133:	89 45 08             	mov    %eax,0x8(%ebp)
}
80101136:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101139:	5b                   	pop    %ebx
8010113a:	5e                   	pop    %esi
8010113b:	5f                   	pop    %edi
8010113c:	5d                   	pop    %ebp
    return pipewrite(f->pipe, addr, n);
8010113d:	e9 9e 26 00 00       	jmp    801037e0 <pipewrite>
        panic("short filewrite");
80101142:	83 ec 0c             	sub    $0xc,%esp
80101145:	68 1a 75 10 80       	push   $0x8010751a
8010114a:	e8 41 f2 ff ff       	call   80100390 <panic>
  panic("filewrite");
8010114f:	83 ec 0c             	sub    $0xc,%esp
80101152:	68 20 75 10 80       	push   $0x80107520
80101157:	e8 34 f2 ff ff       	call   80100390 <panic>
8010115c:	66 90                	xchg   %ax,%ax
8010115e:	66 90                	xchg   %ax,%ax

80101160 <balloc>:
// Blocks.

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
80101160:	55                   	push   %ebp
80101161:	89 e5                	mov    %esp,%ebp
80101163:	57                   	push   %edi
80101164:	56                   	push   %esi
80101165:	53                   	push   %ebx
80101166:	83 ec 1c             	sub    $0x1c,%esp
  int b, bi, m;
  struct buf *bp;

  bp = 0;
  for(b = 0; b < sb.size; b += BPB){
80101169:	8b 0d c0 09 11 80    	mov    0x801109c0,%ecx
{
8010116f:	89 45 d8             	mov    %eax,-0x28(%ebp)
  for(b = 0; b < sb.size; b += BPB){
80101172:	85 c9                	test   %ecx,%ecx
80101174:	0f 84 87 00 00 00    	je     80101201 <balloc+0xa1>
8010117a:	c7 45 dc 00 00 00 00 	movl   $0x0,-0x24(%ebp)
    bp = bread(dev, BBLOCK(b, sb));
80101181:	8b 75 dc             	mov    -0x24(%ebp),%esi
80101184:	83 ec 08             	sub    $0x8,%esp
80101187:	89 f0                	mov    %esi,%eax
80101189:	c1 f8 0c             	sar    $0xc,%eax
8010118c:	03 05 d8 09 11 80    	add    0x801109d8,%eax
80101192:	50                   	push   %eax
80101193:	ff 75 d8             	pushl  -0x28(%ebp)
80101196:	e8 35 ef ff ff       	call   801000d0 <bread>
8010119b:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
8010119e:	a1 c0 09 11 80       	mov    0x801109c0,%eax
801011a3:	83 c4 10             	add    $0x10,%esp
801011a6:	89 45 e0             	mov    %eax,-0x20(%ebp)
801011a9:	31 c0                	xor    %eax,%eax
801011ab:	eb 2f                	jmp    801011dc <balloc+0x7c>
801011ad:	8d 76 00             	lea    0x0(%esi),%esi
      m = 1 << (bi % 8);
801011b0:	89 c1                	mov    %eax,%ecx
      if((bp->data[bi/8] & m) == 0){  // Is block free?
801011b2:	8b 55 e4             	mov    -0x1c(%ebp),%edx
      m = 1 << (bi % 8);
801011b5:	bb 01 00 00 00       	mov    $0x1,%ebx
801011ba:	83 e1 07             	and    $0x7,%ecx
801011bd:	d3 e3                	shl    %cl,%ebx
      if((bp->data[bi/8] & m) == 0){  // Is block free?
801011bf:	89 c1                	mov    %eax,%ecx
801011c1:	c1 f9 03             	sar    $0x3,%ecx
801011c4:	0f b6 7c 0a 5c       	movzbl 0x5c(%edx,%ecx,1),%edi
801011c9:	85 df                	test   %ebx,%edi
801011cb:	89 fa                	mov    %edi,%edx
801011cd:	74 41                	je     80101210 <balloc+0xb0>
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801011cf:	83 c0 01             	add    $0x1,%eax
801011d2:	83 c6 01             	add    $0x1,%esi
801011d5:	3d 00 10 00 00       	cmp    $0x1000,%eax
801011da:	74 05                	je     801011e1 <balloc+0x81>
801011dc:	39 75 e0             	cmp    %esi,-0x20(%ebp)
801011df:	77 cf                	ja     801011b0 <balloc+0x50>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801011e1:	83 ec 0c             	sub    $0xc,%esp
801011e4:	ff 75 e4             	pushl  -0x1c(%ebp)
801011e7:	e8 f4 ef ff ff       	call   801001e0 <brelse>
  for(b = 0; b < sb.size; b += BPB){
801011ec:	81 45 dc 00 10 00 00 	addl   $0x1000,-0x24(%ebp)
801011f3:	83 c4 10             	add    $0x10,%esp
801011f6:	8b 45 dc             	mov    -0x24(%ebp),%eax
801011f9:	39 05 c0 09 11 80    	cmp    %eax,0x801109c0
801011ff:	77 80                	ja     80101181 <balloc+0x21>
  }
  panic("balloc: out of blocks");
80101201:	83 ec 0c             	sub    $0xc,%esp
80101204:	68 2a 75 10 80       	push   $0x8010752a
80101209:	e8 82 f1 ff ff       	call   80100390 <panic>
8010120e:	66 90                	xchg   %ax,%ax
        bp->data[bi/8] |= m;  // Mark block in use.
80101210:	8b 7d e4             	mov    -0x1c(%ebp),%edi
        log_write(bp);
80101213:	83 ec 0c             	sub    $0xc,%esp
        bp->data[bi/8] |= m;  // Mark block in use.
80101216:	09 da                	or     %ebx,%edx
80101218:	88 54 0f 5c          	mov    %dl,0x5c(%edi,%ecx,1)
        log_write(bp);
8010121c:	57                   	push   %edi
8010121d:	e8 3e 1f 00 00       	call   80103160 <log_write>
        brelse(bp);
80101222:	89 3c 24             	mov    %edi,(%esp)
80101225:	e8 b6 ef ff ff       	call   801001e0 <brelse>
  bp = bread(dev, bno);
8010122a:	58                   	pop    %eax
8010122b:	5a                   	pop    %edx
8010122c:	56                   	push   %esi
8010122d:	ff 75 d8             	pushl  -0x28(%ebp)
80101230:	e8 9b ee ff ff       	call   801000d0 <bread>
80101235:	89 c3                	mov    %eax,%ebx
  memset(bp->data, 0, BSIZE);
80101237:	8d 40 5c             	lea    0x5c(%eax),%eax
8010123a:	83 c4 0c             	add    $0xc,%esp
8010123d:	68 00 02 00 00       	push   $0x200
80101242:	6a 00                	push   $0x0
80101244:	50                   	push   %eax
80101245:	e8 56 37 00 00       	call   801049a0 <memset>
  log_write(bp);
8010124a:	89 1c 24             	mov    %ebx,(%esp)
8010124d:	e8 0e 1f 00 00       	call   80103160 <log_write>
  brelse(bp);
80101252:	89 1c 24             	mov    %ebx,(%esp)
80101255:	e8 86 ef ff ff       	call   801001e0 <brelse>
}
8010125a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010125d:	89 f0                	mov    %esi,%eax
8010125f:	5b                   	pop    %ebx
80101260:	5e                   	pop    %esi
80101261:	5f                   	pop    %edi
80101262:	5d                   	pop    %ebp
80101263:	c3                   	ret    
80101264:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
8010126a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80101270 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101270:	55                   	push   %ebp
80101271:	89 e5                	mov    %esp,%ebp
80101273:	57                   	push   %edi
80101274:	56                   	push   %esi
80101275:	53                   	push   %ebx
80101276:	89 c7                	mov    %eax,%edi
  struct inode *ip, *empty;

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
80101278:	31 f6                	xor    %esi,%esi
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
8010127a:	bb 14 0a 11 80       	mov    $0x80110a14,%ebx
{
8010127f:	83 ec 28             	sub    $0x28,%esp
80101282:	89 55 e4             	mov    %edx,-0x1c(%ebp)
  acquire(&icache.lock);
80101285:	68 e0 09 11 80       	push   $0x801109e0
8010128a:	e8 91 35 00 00       	call   80104820 <acquire>
8010128f:	83 c4 10             	add    $0x10,%esp
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
80101292:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80101295:	eb 17                	jmp    801012ae <iget+0x3e>
80101297:	89 f6                	mov    %esi,%esi
80101299:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
801012a0:	81 c3 90 00 00 00    	add    $0x90,%ebx
801012a6:	81 fb 34 26 11 80    	cmp    $0x80112634,%ebx
801012ac:	73 22                	jae    801012d0 <iget+0x60>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
801012ae:	8b 4b 08             	mov    0x8(%ebx),%ecx
801012b1:	85 c9                	test   %ecx,%ecx
801012b3:	7e 04                	jle    801012b9 <iget+0x49>
801012b5:	39 3b                	cmp    %edi,(%ebx)
801012b7:	74 4f                	je     80101308 <iget+0x98>
      ip->ref++;
      release(&icache.lock);
      return ip;
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
801012b9:	85 f6                	test   %esi,%esi
801012bb:	75 e3                	jne    801012a0 <iget+0x30>
801012bd:	85 c9                	test   %ecx,%ecx
801012bf:	0f 44 f3             	cmove  %ebx,%esi
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801012c2:	81 c3 90 00 00 00    	add    $0x90,%ebx
801012c8:	81 fb 34 26 11 80    	cmp    $0x80112634,%ebx
801012ce:	72 de                	jb     801012ae <iget+0x3e>
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801012d0:	85 f6                	test   %esi,%esi
801012d2:	74 5b                	je     8010132f <iget+0xbf>
  ip = empty;
  ip->dev = dev;
  ip->inum = inum;
  ip->ref = 1;
  ip->valid = 0;
  release(&icache.lock);
801012d4:	83 ec 0c             	sub    $0xc,%esp
  ip->dev = dev;
801012d7:	89 3e                	mov    %edi,(%esi)
  ip->inum = inum;
801012d9:	89 56 04             	mov    %edx,0x4(%esi)
  ip->ref = 1;
801012dc:	c7 46 08 01 00 00 00 	movl   $0x1,0x8(%esi)
  ip->valid = 0;
801012e3:	c7 46 4c 00 00 00 00 	movl   $0x0,0x4c(%esi)
  release(&icache.lock);
801012ea:	68 e0 09 11 80       	push   $0x801109e0
801012ef:	e8 4c 36 00 00       	call   80104940 <release>

  return ip;
801012f4:	83 c4 10             	add    $0x10,%esp
}
801012f7:	8d 65 f4             	lea    -0xc(%ebp),%esp
801012fa:	89 f0                	mov    %esi,%eax
801012fc:	5b                   	pop    %ebx
801012fd:	5e                   	pop    %esi
801012fe:	5f                   	pop    %edi
801012ff:	5d                   	pop    %ebp
80101300:	c3                   	ret    
80101301:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101308:	39 53 04             	cmp    %edx,0x4(%ebx)
8010130b:	75 ac                	jne    801012b9 <iget+0x49>
      release(&icache.lock);
8010130d:	83 ec 0c             	sub    $0xc,%esp
      ip->ref++;
80101310:	83 c1 01             	add    $0x1,%ecx
      return ip;
80101313:	89 de                	mov    %ebx,%esi
      release(&icache.lock);
80101315:	68 e0 09 11 80       	push   $0x801109e0
      ip->ref++;
8010131a:	89 4b 08             	mov    %ecx,0x8(%ebx)
      release(&icache.lock);
8010131d:	e8 1e 36 00 00       	call   80104940 <release>
      return ip;
80101322:	83 c4 10             	add    $0x10,%esp
}
80101325:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101328:	89 f0                	mov    %esi,%eax
8010132a:	5b                   	pop    %ebx
8010132b:	5e                   	pop    %esi
8010132c:	5f                   	pop    %edi
8010132d:	5d                   	pop    %ebp
8010132e:	c3                   	ret    
    panic("iget: no inodes");
8010132f:	83 ec 0c             	sub    $0xc,%esp
80101332:	68 40 75 10 80       	push   $0x80107540
80101337:	e8 54 f0 ff ff       	call   80100390 <panic>
8010133c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101340 <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101340:	55                   	push   %ebp
80101341:	89 e5                	mov    %esp,%ebp
80101343:	57                   	push   %edi
80101344:	56                   	push   %esi
80101345:	53                   	push   %ebx
80101346:	89 c6                	mov    %eax,%esi
80101348:	83 ec 1c             	sub    $0x1c,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
8010134b:	83 fa 0b             	cmp    $0xb,%edx
8010134e:	77 18                	ja     80101368 <bmap+0x28>
80101350:	8d 3c 90             	lea    (%eax,%edx,4),%edi
    if((addr = ip->addrs[bn]) == 0)
80101353:	8b 5f 5c             	mov    0x5c(%edi),%ebx
80101356:	85 db                	test   %ebx,%ebx
80101358:	74 76                	je     801013d0 <bmap+0x90>
    brelse(bp);
    return addr;
  }

  panic("bmap: out of range");
}
8010135a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010135d:	89 d8                	mov    %ebx,%eax
8010135f:	5b                   	pop    %ebx
80101360:	5e                   	pop    %esi
80101361:	5f                   	pop    %edi
80101362:	5d                   	pop    %ebp
80101363:	c3                   	ret    
80101364:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  bn -= NDIRECT;
80101368:	8d 5a f4             	lea    -0xc(%edx),%ebx
  if(bn < NINDIRECT){
8010136b:	83 fb 7f             	cmp    $0x7f,%ebx
8010136e:	0f 87 90 00 00 00    	ja     80101404 <bmap+0xc4>
    if((addr = ip->addrs[NDIRECT]) == 0)
80101374:	8b 90 8c 00 00 00    	mov    0x8c(%eax),%edx
8010137a:	8b 00                	mov    (%eax),%eax
8010137c:	85 d2                	test   %edx,%edx
8010137e:	74 70                	je     801013f0 <bmap+0xb0>
    bp = bread(ip->dev, addr);
80101380:	83 ec 08             	sub    $0x8,%esp
80101383:	52                   	push   %edx
80101384:	50                   	push   %eax
80101385:	e8 46 ed ff ff       	call   801000d0 <bread>
    if((addr = a[bn]) == 0){
8010138a:	8d 54 98 5c          	lea    0x5c(%eax,%ebx,4),%edx
8010138e:	83 c4 10             	add    $0x10,%esp
    bp = bread(ip->dev, addr);
80101391:	89 c7                	mov    %eax,%edi
    if((addr = a[bn]) == 0){
80101393:	8b 1a                	mov    (%edx),%ebx
80101395:	85 db                	test   %ebx,%ebx
80101397:	75 1d                	jne    801013b6 <bmap+0x76>
      a[bn] = addr = balloc(ip->dev);
80101399:	8b 06                	mov    (%esi),%eax
8010139b:	89 55 e4             	mov    %edx,-0x1c(%ebp)
8010139e:	e8 bd fd ff ff       	call   80101160 <balloc>
801013a3:	8b 55 e4             	mov    -0x1c(%ebp),%edx
      log_write(bp);
801013a6:	83 ec 0c             	sub    $0xc,%esp
      a[bn] = addr = balloc(ip->dev);
801013a9:	89 c3                	mov    %eax,%ebx
801013ab:	89 02                	mov    %eax,(%edx)
      log_write(bp);
801013ad:	57                   	push   %edi
801013ae:	e8 ad 1d 00 00       	call   80103160 <log_write>
801013b3:	83 c4 10             	add    $0x10,%esp
    brelse(bp);
801013b6:	83 ec 0c             	sub    $0xc,%esp
801013b9:	57                   	push   %edi
801013ba:	e8 21 ee ff ff       	call   801001e0 <brelse>
801013bf:	83 c4 10             	add    $0x10,%esp
}
801013c2:	8d 65 f4             	lea    -0xc(%ebp),%esp
801013c5:	89 d8                	mov    %ebx,%eax
801013c7:	5b                   	pop    %ebx
801013c8:	5e                   	pop    %esi
801013c9:	5f                   	pop    %edi
801013ca:	5d                   	pop    %ebp
801013cb:	c3                   	ret    
801013cc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      ip->addrs[bn] = addr = balloc(ip->dev);
801013d0:	8b 00                	mov    (%eax),%eax
801013d2:	e8 89 fd ff ff       	call   80101160 <balloc>
801013d7:	89 47 5c             	mov    %eax,0x5c(%edi)
}
801013da:	8d 65 f4             	lea    -0xc(%ebp),%esp
      ip->addrs[bn] = addr = balloc(ip->dev);
801013dd:	89 c3                	mov    %eax,%ebx
}
801013df:	89 d8                	mov    %ebx,%eax
801013e1:	5b                   	pop    %ebx
801013e2:	5e                   	pop    %esi
801013e3:	5f                   	pop    %edi
801013e4:	5d                   	pop    %ebp
801013e5:	c3                   	ret    
801013e6:	8d 76 00             	lea    0x0(%esi),%esi
801013e9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
801013f0:	e8 6b fd ff ff       	call   80101160 <balloc>
801013f5:	89 c2                	mov    %eax,%edx
801013f7:	89 86 8c 00 00 00    	mov    %eax,0x8c(%esi)
801013fd:	8b 06                	mov    (%esi),%eax
801013ff:	e9 7c ff ff ff       	jmp    80101380 <bmap+0x40>
  panic("bmap: out of range");
80101404:	83 ec 0c             	sub    $0xc,%esp
80101407:	68 50 75 10 80       	push   $0x80107550
8010140c:	e8 7f ef ff ff       	call   80100390 <panic>
80101411:	eb 0d                	jmp    80101420 <readsb>
80101413:	90                   	nop
80101414:	90                   	nop
80101415:	90                   	nop
80101416:	90                   	nop
80101417:	90                   	nop
80101418:	90                   	nop
80101419:	90                   	nop
8010141a:	90                   	nop
8010141b:	90                   	nop
8010141c:	90                   	nop
8010141d:	90                   	nop
8010141e:	90                   	nop
8010141f:	90                   	nop

80101420 <readsb>:
{
80101420:	55                   	push   %ebp
80101421:	89 e5                	mov    %esp,%ebp
80101423:	56                   	push   %esi
80101424:	53                   	push   %ebx
80101425:	8b 75 0c             	mov    0xc(%ebp),%esi
  bp = bread(dev, 1);
80101428:	83 ec 08             	sub    $0x8,%esp
8010142b:	6a 01                	push   $0x1
8010142d:	ff 75 08             	pushl  0x8(%ebp)
80101430:	e8 9b ec ff ff       	call   801000d0 <bread>
80101435:	89 c3                	mov    %eax,%ebx
  memmove(sb, bp->data, sizeof(*sb));
80101437:	8d 40 5c             	lea    0x5c(%eax),%eax
8010143a:	83 c4 0c             	add    $0xc,%esp
8010143d:	6a 1c                	push   $0x1c
8010143f:	50                   	push   %eax
80101440:	56                   	push   %esi
80101441:	e8 0a 36 00 00       	call   80104a50 <memmove>
  brelse(bp);
80101446:	89 5d 08             	mov    %ebx,0x8(%ebp)
80101449:	83 c4 10             	add    $0x10,%esp
}
8010144c:	8d 65 f8             	lea    -0x8(%ebp),%esp
8010144f:	5b                   	pop    %ebx
80101450:	5e                   	pop    %esi
80101451:	5d                   	pop    %ebp
  brelse(bp);
80101452:	e9 89 ed ff ff       	jmp    801001e0 <brelse>
80101457:	89 f6                	mov    %esi,%esi
80101459:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80101460 <bfree>:
{
80101460:	55                   	push   %ebp
80101461:	89 e5                	mov    %esp,%ebp
80101463:	56                   	push   %esi
80101464:	53                   	push   %ebx
80101465:	89 d3                	mov    %edx,%ebx
80101467:	89 c6                	mov    %eax,%esi
  readsb(dev, &sb);
80101469:	83 ec 08             	sub    $0x8,%esp
8010146c:	68 c0 09 11 80       	push   $0x801109c0
80101471:	50                   	push   %eax
80101472:	e8 a9 ff ff ff       	call   80101420 <readsb>
  bp = bread(dev, BBLOCK(b, sb));
80101477:	58                   	pop    %eax
80101478:	5a                   	pop    %edx
80101479:	89 da                	mov    %ebx,%edx
8010147b:	c1 ea 0c             	shr    $0xc,%edx
8010147e:	03 15 d8 09 11 80    	add    0x801109d8,%edx
80101484:	52                   	push   %edx
80101485:	56                   	push   %esi
80101486:	e8 45 ec ff ff       	call   801000d0 <bread>
  m = 1 << (bi % 8);
8010148b:	89 d9                	mov    %ebx,%ecx
  if((bp->data[bi/8] & m) == 0)
8010148d:	c1 fb 03             	sar    $0x3,%ebx
  m = 1 << (bi % 8);
80101490:	ba 01 00 00 00       	mov    $0x1,%edx
80101495:	83 e1 07             	and    $0x7,%ecx
  if((bp->data[bi/8] & m) == 0)
80101498:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
8010149e:	83 c4 10             	add    $0x10,%esp
  m = 1 << (bi % 8);
801014a1:	d3 e2                	shl    %cl,%edx
  if((bp->data[bi/8] & m) == 0)
801014a3:	0f b6 4c 18 5c       	movzbl 0x5c(%eax,%ebx,1),%ecx
801014a8:	85 d1                	test   %edx,%ecx
801014aa:	74 25                	je     801014d1 <bfree+0x71>
  bp->data[bi/8] &= ~m;
801014ac:	f7 d2                	not    %edx
801014ae:	89 c6                	mov    %eax,%esi
  log_write(bp);
801014b0:	83 ec 0c             	sub    $0xc,%esp
  bp->data[bi/8] &= ~m;
801014b3:	21 ca                	and    %ecx,%edx
801014b5:	88 54 1e 5c          	mov    %dl,0x5c(%esi,%ebx,1)
  log_write(bp);
801014b9:	56                   	push   %esi
801014ba:	e8 a1 1c 00 00       	call   80103160 <log_write>
  brelse(bp);
801014bf:	89 34 24             	mov    %esi,(%esp)
801014c2:	e8 19 ed ff ff       	call   801001e0 <brelse>
}
801014c7:	83 c4 10             	add    $0x10,%esp
801014ca:	8d 65 f8             	lea    -0x8(%ebp),%esp
801014cd:	5b                   	pop    %ebx
801014ce:	5e                   	pop    %esi
801014cf:	5d                   	pop    %ebp
801014d0:	c3                   	ret    
    panic("freeing free block");
801014d1:	83 ec 0c             	sub    $0xc,%esp
801014d4:	68 63 75 10 80       	push   $0x80107563
801014d9:	e8 b2 ee ff ff       	call   80100390 <panic>
801014de:	66 90                	xchg   %ax,%ax

801014e0 <iinit>:
{
801014e0:	55                   	push   %ebp
801014e1:	89 e5                	mov    %esp,%ebp
801014e3:	53                   	push   %ebx
801014e4:	bb 20 0a 11 80       	mov    $0x80110a20,%ebx
801014e9:	83 ec 0c             	sub    $0xc,%esp
  initlock(&icache.lock, "icache");
801014ec:	68 76 75 10 80       	push   $0x80107576
801014f1:	68 e0 09 11 80       	push   $0x801109e0
801014f6:	e8 35 32 00 00       	call   80104730 <initlock>
801014fb:	83 c4 10             	add    $0x10,%esp
801014fe:	66 90                	xchg   %ax,%ax
    initsleeplock(&icache.inode[i].lock, "inode");
80101500:	83 ec 08             	sub    $0x8,%esp
80101503:	68 7d 75 10 80       	push   $0x8010757d
80101508:	53                   	push   %ebx
80101509:	81 c3 90 00 00 00    	add    $0x90,%ebx
8010150f:	e8 0c 31 00 00       	call   80104620 <initsleeplock>
  for(i = 0; i < NINODE; i++) {
80101514:	83 c4 10             	add    $0x10,%esp
80101517:	81 fb 40 26 11 80    	cmp    $0x80112640,%ebx
8010151d:	75 e1                	jne    80101500 <iinit+0x20>
  readsb(dev, &sb);
8010151f:	83 ec 08             	sub    $0x8,%esp
80101522:	68 c0 09 11 80       	push   $0x801109c0
80101527:	ff 75 08             	pushl  0x8(%ebp)
8010152a:	e8 f1 fe ff ff       	call   80101420 <readsb>
  cprintf("sb: size %d nblocks %d ninodes %d nlog %d logstart %d\
8010152f:	ff 35 d8 09 11 80    	pushl  0x801109d8
80101535:	ff 35 d4 09 11 80    	pushl  0x801109d4
8010153b:	ff 35 d0 09 11 80    	pushl  0x801109d0
80101541:	ff 35 cc 09 11 80    	pushl  0x801109cc
80101547:	ff 35 c8 09 11 80    	pushl  0x801109c8
8010154d:	ff 35 c4 09 11 80    	pushl  0x801109c4
80101553:	ff 35 c0 09 11 80    	pushl  0x801109c0
80101559:	68 28 76 10 80       	push   $0x80107628
8010155e:	e8 fd f0 ff ff       	call   80100660 <cprintf>
}
80101563:	83 c4 30             	add    $0x30,%esp
80101566:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80101569:	c9                   	leave  
8010156a:	c3                   	ret    
8010156b:	90                   	nop
8010156c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101570 <ialloc>:
{
80101570:	55                   	push   %ebp
80101571:	89 e5                	mov    %esp,%ebp
80101573:	57                   	push   %edi
80101574:	56                   	push   %esi
80101575:	53                   	push   %ebx
80101576:	83 ec 1c             	sub    $0x1c,%esp
  for(inum = 1; inum < sb.ninodes; inum++){
80101579:	83 3d c8 09 11 80 01 	cmpl   $0x1,0x801109c8
{
80101580:	8b 45 0c             	mov    0xc(%ebp),%eax
80101583:	8b 75 08             	mov    0x8(%ebp),%esi
80101586:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  for(inum = 1; inum < sb.ninodes; inum++){
80101589:	0f 86 91 00 00 00    	jbe    80101620 <ialloc+0xb0>
8010158f:	bb 01 00 00 00       	mov    $0x1,%ebx
80101594:	eb 21                	jmp    801015b7 <ialloc+0x47>
80101596:	8d 76 00             	lea    0x0(%esi),%esi
80101599:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    brelse(bp);
801015a0:	83 ec 0c             	sub    $0xc,%esp
  for(inum = 1; inum < sb.ninodes; inum++){
801015a3:	83 c3 01             	add    $0x1,%ebx
    brelse(bp);
801015a6:	57                   	push   %edi
801015a7:	e8 34 ec ff ff       	call   801001e0 <brelse>
  for(inum = 1; inum < sb.ninodes; inum++){
801015ac:	83 c4 10             	add    $0x10,%esp
801015af:	39 1d c8 09 11 80    	cmp    %ebx,0x801109c8
801015b5:	76 69                	jbe    80101620 <ialloc+0xb0>
    bp = bread(dev, IBLOCK(inum, sb));
801015b7:	89 d8                	mov    %ebx,%eax
801015b9:	83 ec 08             	sub    $0x8,%esp
801015bc:	c1 e8 03             	shr    $0x3,%eax
801015bf:	03 05 d4 09 11 80    	add    0x801109d4,%eax
801015c5:	50                   	push   %eax
801015c6:	56                   	push   %esi
801015c7:	e8 04 eb ff ff       	call   801000d0 <bread>
801015cc:	89 c7                	mov    %eax,%edi
    dip = (struct dinode*)bp->data + inum%IPB;
801015ce:	89 d8                	mov    %ebx,%eax
    if(dip->type == 0){  // a free inode
801015d0:	83 c4 10             	add    $0x10,%esp
    dip = (struct dinode*)bp->data + inum%IPB;
801015d3:	83 e0 07             	and    $0x7,%eax
801015d6:	c1 e0 06             	shl    $0x6,%eax
801015d9:	8d 4c 07 5c          	lea    0x5c(%edi,%eax,1),%ecx
    if(dip->type == 0){  // a free inode
801015dd:	66 83 39 00          	cmpw   $0x0,(%ecx)
801015e1:	75 bd                	jne    801015a0 <ialloc+0x30>
      memset(dip, 0, sizeof(*dip));
801015e3:	83 ec 04             	sub    $0x4,%esp
801015e6:	89 4d e0             	mov    %ecx,-0x20(%ebp)
801015e9:	6a 40                	push   $0x40
801015eb:	6a 00                	push   $0x0
801015ed:	51                   	push   %ecx
801015ee:	e8 ad 33 00 00       	call   801049a0 <memset>
      dip->type = type;
801015f3:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
801015f7:	8b 4d e0             	mov    -0x20(%ebp),%ecx
801015fa:	66 89 01             	mov    %ax,(%ecx)
      log_write(bp);   // mark it allocated on the disk
801015fd:	89 3c 24             	mov    %edi,(%esp)
80101600:	e8 5b 1b 00 00       	call   80103160 <log_write>
      brelse(bp);
80101605:	89 3c 24             	mov    %edi,(%esp)
80101608:	e8 d3 eb ff ff       	call   801001e0 <brelse>
      return iget(dev, inum);
8010160d:	83 c4 10             	add    $0x10,%esp
}
80101610:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return iget(dev, inum);
80101613:	89 da                	mov    %ebx,%edx
80101615:	89 f0                	mov    %esi,%eax
}
80101617:	5b                   	pop    %ebx
80101618:	5e                   	pop    %esi
80101619:	5f                   	pop    %edi
8010161a:	5d                   	pop    %ebp
      return iget(dev, inum);
8010161b:	e9 50 fc ff ff       	jmp    80101270 <iget>
  panic("ialloc: no inodes");
80101620:	83 ec 0c             	sub    $0xc,%esp
80101623:	68 83 75 10 80       	push   $0x80107583
80101628:	e8 63 ed ff ff       	call   80100390 <panic>
8010162d:	8d 76 00             	lea    0x0(%esi),%esi

80101630 <iupdate>:
{
80101630:	55                   	push   %ebp
80101631:	89 e5                	mov    %esp,%ebp
80101633:	56                   	push   %esi
80101634:	53                   	push   %ebx
80101635:	8b 5d 08             	mov    0x8(%ebp),%ebx
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101638:	83 ec 08             	sub    $0x8,%esp
8010163b:	8b 43 04             	mov    0x4(%ebx),%eax
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010163e:	83 c3 5c             	add    $0x5c,%ebx
  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101641:	c1 e8 03             	shr    $0x3,%eax
80101644:	03 05 d4 09 11 80    	add    0x801109d4,%eax
8010164a:	50                   	push   %eax
8010164b:	ff 73 a4             	pushl  -0x5c(%ebx)
8010164e:	e8 7d ea ff ff       	call   801000d0 <bread>
80101653:	89 c6                	mov    %eax,%esi
  dip = (struct dinode*)bp->data + ip->inum%IPB;
80101655:	8b 43 a8             	mov    -0x58(%ebx),%eax
  dip->type = ip->type;
80101658:	0f b7 53 f4          	movzwl -0xc(%ebx),%edx
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010165c:	83 c4 0c             	add    $0xc,%esp
  dip = (struct dinode*)bp->data + ip->inum%IPB;
8010165f:	83 e0 07             	and    $0x7,%eax
80101662:	c1 e0 06             	shl    $0x6,%eax
80101665:	8d 44 06 5c          	lea    0x5c(%esi,%eax,1),%eax
  dip->type = ip->type;
80101669:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
8010166c:	0f b7 53 f6          	movzwl -0xa(%ebx),%edx
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
80101670:	83 c0 0c             	add    $0xc,%eax
  dip->major = ip->major;
80101673:	66 89 50 f6          	mov    %dx,-0xa(%eax)
  dip->minor = ip->minor;
80101677:	0f b7 53 f8          	movzwl -0x8(%ebx),%edx
8010167b:	66 89 50 f8          	mov    %dx,-0x8(%eax)
  dip->nlink = ip->nlink;
8010167f:	0f b7 53 fa          	movzwl -0x6(%ebx),%edx
80101683:	66 89 50 fa          	mov    %dx,-0x6(%eax)
  dip->size = ip->size;
80101687:	8b 53 fc             	mov    -0x4(%ebx),%edx
8010168a:	89 50 fc             	mov    %edx,-0x4(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010168d:	6a 34                	push   $0x34
8010168f:	53                   	push   %ebx
80101690:	50                   	push   %eax
80101691:	e8 ba 33 00 00       	call   80104a50 <memmove>
  log_write(bp);
80101696:	89 34 24             	mov    %esi,(%esp)
80101699:	e8 c2 1a 00 00       	call   80103160 <log_write>
  brelse(bp);
8010169e:	89 75 08             	mov    %esi,0x8(%ebp)
801016a1:	83 c4 10             	add    $0x10,%esp
}
801016a4:	8d 65 f8             	lea    -0x8(%ebp),%esp
801016a7:	5b                   	pop    %ebx
801016a8:	5e                   	pop    %esi
801016a9:	5d                   	pop    %ebp
  brelse(bp);
801016aa:	e9 31 eb ff ff       	jmp    801001e0 <brelse>
801016af:	90                   	nop

801016b0 <idup>:
{
801016b0:	55                   	push   %ebp
801016b1:	89 e5                	mov    %esp,%ebp
801016b3:	53                   	push   %ebx
801016b4:	83 ec 10             	sub    $0x10,%esp
801016b7:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&icache.lock);
801016ba:	68 e0 09 11 80       	push   $0x801109e0
801016bf:	e8 5c 31 00 00       	call   80104820 <acquire>
  ip->ref++;
801016c4:	83 43 08 01          	addl   $0x1,0x8(%ebx)
  release(&icache.lock);
801016c8:	c7 04 24 e0 09 11 80 	movl   $0x801109e0,(%esp)
801016cf:	e8 6c 32 00 00       	call   80104940 <release>
}
801016d4:	89 d8                	mov    %ebx,%eax
801016d6:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801016d9:	c9                   	leave  
801016da:	c3                   	ret    
801016db:	90                   	nop
801016dc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801016e0 <ilock>:
{
801016e0:	55                   	push   %ebp
801016e1:	89 e5                	mov    %esp,%ebp
801016e3:	56                   	push   %esi
801016e4:	53                   	push   %ebx
801016e5:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(ip == 0 || ip->ref < 1)
801016e8:	85 db                	test   %ebx,%ebx
801016ea:	0f 84 b7 00 00 00    	je     801017a7 <ilock+0xc7>
801016f0:	8b 53 08             	mov    0x8(%ebx),%edx
801016f3:	85 d2                	test   %edx,%edx
801016f5:	0f 8e ac 00 00 00    	jle    801017a7 <ilock+0xc7>
  acquiresleep(&ip->lock);
801016fb:	8d 43 0c             	lea    0xc(%ebx),%eax
801016fe:	83 ec 0c             	sub    $0xc,%esp
80101701:	50                   	push   %eax
80101702:	e8 59 2f 00 00       	call   80104660 <acquiresleep>
  if(ip->valid == 0){
80101707:	8b 43 4c             	mov    0x4c(%ebx),%eax
8010170a:	83 c4 10             	add    $0x10,%esp
8010170d:	85 c0                	test   %eax,%eax
8010170f:	74 0f                	je     80101720 <ilock+0x40>
}
80101711:	8d 65 f8             	lea    -0x8(%ebp),%esp
80101714:	5b                   	pop    %ebx
80101715:	5e                   	pop    %esi
80101716:	5d                   	pop    %ebp
80101717:	c3                   	ret    
80101718:	90                   	nop
80101719:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101720:	8b 43 04             	mov    0x4(%ebx),%eax
80101723:	83 ec 08             	sub    $0x8,%esp
80101726:	c1 e8 03             	shr    $0x3,%eax
80101729:	03 05 d4 09 11 80    	add    0x801109d4,%eax
8010172f:	50                   	push   %eax
80101730:	ff 33                	pushl  (%ebx)
80101732:	e8 99 e9 ff ff       	call   801000d0 <bread>
80101737:	89 c6                	mov    %eax,%esi
    dip = (struct dinode*)bp->data + ip->inum%IPB;
80101739:	8b 43 04             	mov    0x4(%ebx),%eax
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
8010173c:	83 c4 0c             	add    $0xc,%esp
    dip = (struct dinode*)bp->data + ip->inum%IPB;
8010173f:	83 e0 07             	and    $0x7,%eax
80101742:	c1 e0 06             	shl    $0x6,%eax
80101745:	8d 44 06 5c          	lea    0x5c(%esi,%eax,1),%eax
    ip->type = dip->type;
80101749:	0f b7 10             	movzwl (%eax),%edx
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
8010174c:	83 c0 0c             	add    $0xc,%eax
    ip->type = dip->type;
8010174f:	66 89 53 50          	mov    %dx,0x50(%ebx)
    ip->major = dip->major;
80101753:	0f b7 50 f6          	movzwl -0xa(%eax),%edx
80101757:	66 89 53 52          	mov    %dx,0x52(%ebx)
    ip->minor = dip->minor;
8010175b:	0f b7 50 f8          	movzwl -0x8(%eax),%edx
8010175f:	66 89 53 54          	mov    %dx,0x54(%ebx)
    ip->nlink = dip->nlink;
80101763:	0f b7 50 fa          	movzwl -0x6(%eax),%edx
80101767:	66 89 53 56          	mov    %dx,0x56(%ebx)
    ip->size = dip->size;
8010176b:	8b 50 fc             	mov    -0x4(%eax),%edx
8010176e:	89 53 58             	mov    %edx,0x58(%ebx)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101771:	6a 34                	push   $0x34
80101773:	50                   	push   %eax
80101774:	8d 43 5c             	lea    0x5c(%ebx),%eax
80101777:	50                   	push   %eax
80101778:	e8 d3 32 00 00       	call   80104a50 <memmove>
    brelse(bp);
8010177d:	89 34 24             	mov    %esi,(%esp)
80101780:	e8 5b ea ff ff       	call   801001e0 <brelse>
    if(ip->type == 0)
80101785:	83 c4 10             	add    $0x10,%esp
80101788:	66 83 7b 50 00       	cmpw   $0x0,0x50(%ebx)
    ip->valid = 1;
8010178d:	c7 43 4c 01 00 00 00 	movl   $0x1,0x4c(%ebx)
    if(ip->type == 0)
80101794:	0f 85 77 ff ff ff    	jne    80101711 <ilock+0x31>
      panic("ilock: no type");
8010179a:	83 ec 0c             	sub    $0xc,%esp
8010179d:	68 9b 75 10 80       	push   $0x8010759b
801017a2:	e8 e9 eb ff ff       	call   80100390 <panic>
    panic("ilock");
801017a7:	83 ec 0c             	sub    $0xc,%esp
801017aa:	68 95 75 10 80       	push   $0x80107595
801017af:	e8 dc eb ff ff       	call   80100390 <panic>
801017b4:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801017ba:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

801017c0 <iunlock>:
{
801017c0:	55                   	push   %ebp
801017c1:	89 e5                	mov    %esp,%ebp
801017c3:	56                   	push   %esi
801017c4:	53                   	push   %ebx
801017c5:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
801017c8:	85 db                	test   %ebx,%ebx
801017ca:	74 28                	je     801017f4 <iunlock+0x34>
801017cc:	8d 73 0c             	lea    0xc(%ebx),%esi
801017cf:	83 ec 0c             	sub    $0xc,%esp
801017d2:	56                   	push   %esi
801017d3:	e8 28 2f 00 00       	call   80104700 <holdingsleep>
801017d8:	83 c4 10             	add    $0x10,%esp
801017db:	85 c0                	test   %eax,%eax
801017dd:	74 15                	je     801017f4 <iunlock+0x34>
801017df:	8b 43 08             	mov    0x8(%ebx),%eax
801017e2:	85 c0                	test   %eax,%eax
801017e4:	7e 0e                	jle    801017f4 <iunlock+0x34>
  releasesleep(&ip->lock);
801017e6:	89 75 08             	mov    %esi,0x8(%ebp)
}
801017e9:	8d 65 f8             	lea    -0x8(%ebp),%esp
801017ec:	5b                   	pop    %ebx
801017ed:	5e                   	pop    %esi
801017ee:	5d                   	pop    %ebp
  releasesleep(&ip->lock);
801017ef:	e9 cc 2e 00 00       	jmp    801046c0 <releasesleep>
    panic("iunlock");
801017f4:	83 ec 0c             	sub    $0xc,%esp
801017f7:	68 aa 75 10 80       	push   $0x801075aa
801017fc:	e8 8f eb ff ff       	call   80100390 <panic>
80101801:	eb 0d                	jmp    80101810 <iput>
80101803:	90                   	nop
80101804:	90                   	nop
80101805:	90                   	nop
80101806:	90                   	nop
80101807:	90                   	nop
80101808:	90                   	nop
80101809:	90                   	nop
8010180a:	90                   	nop
8010180b:	90                   	nop
8010180c:	90                   	nop
8010180d:	90                   	nop
8010180e:	90                   	nop
8010180f:	90                   	nop

80101810 <iput>:
{
80101810:	55                   	push   %ebp
80101811:	89 e5                	mov    %esp,%ebp
80101813:	57                   	push   %edi
80101814:	56                   	push   %esi
80101815:	53                   	push   %ebx
80101816:	83 ec 28             	sub    $0x28,%esp
80101819:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquiresleep(&ip->lock);
8010181c:	8d 7b 0c             	lea    0xc(%ebx),%edi
8010181f:	57                   	push   %edi
80101820:	e8 3b 2e 00 00       	call   80104660 <acquiresleep>
  if(ip->valid && ip->nlink == 0){
80101825:	8b 53 4c             	mov    0x4c(%ebx),%edx
80101828:	83 c4 10             	add    $0x10,%esp
8010182b:	85 d2                	test   %edx,%edx
8010182d:	74 07                	je     80101836 <iput+0x26>
8010182f:	66 83 7b 56 00       	cmpw   $0x0,0x56(%ebx)
80101834:	74 32                	je     80101868 <iput+0x58>
  releasesleep(&ip->lock);
80101836:	83 ec 0c             	sub    $0xc,%esp
80101839:	57                   	push   %edi
8010183a:	e8 81 2e 00 00       	call   801046c0 <releasesleep>
  acquire(&icache.lock);
8010183f:	c7 04 24 e0 09 11 80 	movl   $0x801109e0,(%esp)
80101846:	e8 d5 2f 00 00       	call   80104820 <acquire>
  ip->ref--;
8010184b:	83 6b 08 01          	subl   $0x1,0x8(%ebx)
  release(&icache.lock);
8010184f:	83 c4 10             	add    $0x10,%esp
80101852:	c7 45 08 e0 09 11 80 	movl   $0x801109e0,0x8(%ebp)
}
80101859:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010185c:	5b                   	pop    %ebx
8010185d:	5e                   	pop    %esi
8010185e:	5f                   	pop    %edi
8010185f:	5d                   	pop    %ebp
  release(&icache.lock);
80101860:	e9 db 30 00 00       	jmp    80104940 <release>
80101865:	8d 76 00             	lea    0x0(%esi),%esi
    acquire(&icache.lock);
80101868:	83 ec 0c             	sub    $0xc,%esp
8010186b:	68 e0 09 11 80       	push   $0x801109e0
80101870:	e8 ab 2f 00 00       	call   80104820 <acquire>
    int r = ip->ref;
80101875:	8b 73 08             	mov    0x8(%ebx),%esi
    release(&icache.lock);
80101878:	c7 04 24 e0 09 11 80 	movl   $0x801109e0,(%esp)
8010187f:	e8 bc 30 00 00       	call   80104940 <release>
    if(r == 1){
80101884:	83 c4 10             	add    $0x10,%esp
80101887:	83 fe 01             	cmp    $0x1,%esi
8010188a:	75 aa                	jne    80101836 <iput+0x26>
8010188c:	8d 8b 8c 00 00 00    	lea    0x8c(%ebx),%ecx
80101892:	89 7d e4             	mov    %edi,-0x1c(%ebp)
80101895:	8d 73 5c             	lea    0x5c(%ebx),%esi
80101898:	89 cf                	mov    %ecx,%edi
8010189a:	eb 0b                	jmp    801018a7 <iput+0x97>
8010189c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801018a0:	83 c6 04             	add    $0x4,%esi
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
801018a3:	39 fe                	cmp    %edi,%esi
801018a5:	74 19                	je     801018c0 <iput+0xb0>
    if(ip->addrs[i]){
801018a7:	8b 16                	mov    (%esi),%edx
801018a9:	85 d2                	test   %edx,%edx
801018ab:	74 f3                	je     801018a0 <iput+0x90>
      bfree(ip->dev, ip->addrs[i]);
801018ad:	8b 03                	mov    (%ebx),%eax
801018af:	e8 ac fb ff ff       	call   80101460 <bfree>
      ip->addrs[i] = 0;
801018b4:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
801018ba:	eb e4                	jmp    801018a0 <iput+0x90>
801018bc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    }
  }

  if(ip->addrs[NDIRECT]){
801018c0:	8b 83 8c 00 00 00    	mov    0x8c(%ebx),%eax
801018c6:	8b 7d e4             	mov    -0x1c(%ebp),%edi
801018c9:	85 c0                	test   %eax,%eax
801018cb:	75 33                	jne    80101900 <iput+0xf0>
    bfree(ip->dev, ip->addrs[NDIRECT]);
    ip->addrs[NDIRECT] = 0;
  }

  ip->size = 0;
  iupdate(ip);
801018cd:	83 ec 0c             	sub    $0xc,%esp
  ip->size = 0;
801018d0:	c7 43 58 00 00 00 00 	movl   $0x0,0x58(%ebx)
  iupdate(ip);
801018d7:	53                   	push   %ebx
801018d8:	e8 53 fd ff ff       	call   80101630 <iupdate>
      ip->type = 0;
801018dd:	31 c0                	xor    %eax,%eax
801018df:	66 89 43 50          	mov    %ax,0x50(%ebx)
      iupdate(ip);
801018e3:	89 1c 24             	mov    %ebx,(%esp)
801018e6:	e8 45 fd ff ff       	call   80101630 <iupdate>
      ip->valid = 0;
801018eb:	c7 43 4c 00 00 00 00 	movl   $0x0,0x4c(%ebx)
801018f2:	83 c4 10             	add    $0x10,%esp
801018f5:	e9 3c ff ff ff       	jmp    80101836 <iput+0x26>
801018fa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101900:	83 ec 08             	sub    $0x8,%esp
80101903:	50                   	push   %eax
80101904:	ff 33                	pushl  (%ebx)
80101906:	e8 c5 e7 ff ff       	call   801000d0 <bread>
8010190b:	8d 88 5c 02 00 00    	lea    0x25c(%eax),%ecx
80101911:	89 7d e0             	mov    %edi,-0x20(%ebp)
80101914:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    a = (uint*)bp->data;
80101917:	8d 70 5c             	lea    0x5c(%eax),%esi
8010191a:	83 c4 10             	add    $0x10,%esp
8010191d:	89 cf                	mov    %ecx,%edi
8010191f:	eb 0e                	jmp    8010192f <iput+0x11f>
80101921:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101928:	83 c6 04             	add    $0x4,%esi
    for(j = 0; j < NINDIRECT; j++){
8010192b:	39 fe                	cmp    %edi,%esi
8010192d:	74 0f                	je     8010193e <iput+0x12e>
      if(a[j])
8010192f:	8b 16                	mov    (%esi),%edx
80101931:	85 d2                	test   %edx,%edx
80101933:	74 f3                	je     80101928 <iput+0x118>
        bfree(ip->dev, a[j]);
80101935:	8b 03                	mov    (%ebx),%eax
80101937:	e8 24 fb ff ff       	call   80101460 <bfree>
8010193c:	eb ea                	jmp    80101928 <iput+0x118>
    brelse(bp);
8010193e:	83 ec 0c             	sub    $0xc,%esp
80101941:	ff 75 e4             	pushl  -0x1c(%ebp)
80101944:	8b 7d e0             	mov    -0x20(%ebp),%edi
80101947:	e8 94 e8 ff ff       	call   801001e0 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
8010194c:	8b 93 8c 00 00 00    	mov    0x8c(%ebx),%edx
80101952:	8b 03                	mov    (%ebx),%eax
80101954:	e8 07 fb ff ff       	call   80101460 <bfree>
    ip->addrs[NDIRECT] = 0;
80101959:	c7 83 8c 00 00 00 00 	movl   $0x0,0x8c(%ebx)
80101960:	00 00 00 
80101963:	83 c4 10             	add    $0x10,%esp
80101966:	e9 62 ff ff ff       	jmp    801018cd <iput+0xbd>
8010196b:	90                   	nop
8010196c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101970 <iunlockput>:
{
80101970:	55                   	push   %ebp
80101971:	89 e5                	mov    %esp,%ebp
80101973:	53                   	push   %ebx
80101974:	83 ec 10             	sub    $0x10,%esp
80101977:	8b 5d 08             	mov    0x8(%ebp),%ebx
  iunlock(ip);
8010197a:	53                   	push   %ebx
8010197b:	e8 40 fe ff ff       	call   801017c0 <iunlock>
  iput(ip);
80101980:	89 5d 08             	mov    %ebx,0x8(%ebp)
80101983:	83 c4 10             	add    $0x10,%esp
}
80101986:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80101989:	c9                   	leave  
  iput(ip);
8010198a:	e9 81 fe ff ff       	jmp    80101810 <iput>
8010198f:	90                   	nop

80101990 <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
80101990:	55                   	push   %ebp
80101991:	89 e5                	mov    %esp,%ebp
80101993:	8b 55 08             	mov    0x8(%ebp),%edx
80101996:	8b 45 0c             	mov    0xc(%ebp),%eax
  st->dev = ip->dev;
80101999:	8b 0a                	mov    (%edx),%ecx
8010199b:	89 48 04             	mov    %ecx,0x4(%eax)
  st->ino = ip->inum;
8010199e:	8b 4a 04             	mov    0x4(%edx),%ecx
801019a1:	89 48 08             	mov    %ecx,0x8(%eax)
  st->type = ip->type;
801019a4:	0f b7 4a 50          	movzwl 0x50(%edx),%ecx
801019a8:	66 89 08             	mov    %cx,(%eax)
  st->nlink = ip->nlink;
801019ab:	0f b7 4a 56          	movzwl 0x56(%edx),%ecx
801019af:	66 89 48 0c          	mov    %cx,0xc(%eax)
  st->size = ip->size;
801019b3:	8b 52 58             	mov    0x58(%edx),%edx
801019b6:	89 50 10             	mov    %edx,0x10(%eax)
}
801019b9:	5d                   	pop    %ebp
801019ba:	c3                   	ret    
801019bb:	90                   	nop
801019bc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801019c0 <readi>:
//PAGEBREAK!
// Read data from inode.
// Caller must hold ip->lock.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
801019c0:	55                   	push   %ebp
801019c1:	89 e5                	mov    %esp,%ebp
801019c3:	57                   	push   %edi
801019c4:	56                   	push   %esi
801019c5:	53                   	push   %ebx
801019c6:	83 ec 1c             	sub    $0x1c,%esp
801019c9:	8b 45 08             	mov    0x8(%ebp),%eax
801019cc:	8b 75 0c             	mov    0xc(%ebp),%esi
801019cf:	8b 7d 14             	mov    0x14(%ebp),%edi
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
801019d2:	66 83 78 50 03       	cmpw   $0x3,0x50(%eax)
{
801019d7:	89 75 e0             	mov    %esi,-0x20(%ebp)
801019da:	89 45 d8             	mov    %eax,-0x28(%ebp)
801019dd:	8b 75 10             	mov    0x10(%ebp),%esi
801019e0:	89 7d e4             	mov    %edi,-0x1c(%ebp)
  if(ip->type == T_DEV){
801019e3:	0f 84 a7 00 00 00    	je     80101a90 <readi+0xd0>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
      return -1;
    return devsw[ip->major].read(ip, dst, n);
  }

  if(off > ip->size || off + n < off)
801019e9:	8b 45 d8             	mov    -0x28(%ebp),%eax
801019ec:	8b 40 58             	mov    0x58(%eax),%eax
801019ef:	39 c6                	cmp    %eax,%esi
801019f1:	0f 87 ba 00 00 00    	ja     80101ab1 <readi+0xf1>
801019f7:	8b 7d e4             	mov    -0x1c(%ebp),%edi
801019fa:	89 f9                	mov    %edi,%ecx
801019fc:	01 f1                	add    %esi,%ecx
801019fe:	0f 82 ad 00 00 00    	jb     80101ab1 <readi+0xf1>
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;
80101a04:	89 c2                	mov    %eax,%edx
80101a06:	29 f2                	sub    %esi,%edx
80101a08:	39 c8                	cmp    %ecx,%eax
80101a0a:	0f 43 d7             	cmovae %edi,%edx

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101a0d:	31 ff                	xor    %edi,%edi
80101a0f:	85 d2                	test   %edx,%edx
    n = ip->size - off;
80101a11:	89 55 e4             	mov    %edx,-0x1c(%ebp)
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101a14:	74 6c                	je     80101a82 <readi+0xc2>
80101a16:	8d 76 00             	lea    0x0(%esi),%esi
80101a19:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101a20:	8b 5d d8             	mov    -0x28(%ebp),%ebx
80101a23:	89 f2                	mov    %esi,%edx
80101a25:	c1 ea 09             	shr    $0x9,%edx
80101a28:	89 d8                	mov    %ebx,%eax
80101a2a:	e8 11 f9 ff ff       	call   80101340 <bmap>
80101a2f:	83 ec 08             	sub    $0x8,%esp
80101a32:	50                   	push   %eax
80101a33:	ff 33                	pushl  (%ebx)
80101a35:	e8 96 e6 ff ff       	call   801000d0 <bread>
    m = min(n - tot, BSIZE - off%BSIZE);
80101a3a:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101a3d:	89 c2                	mov    %eax,%edx
    m = min(n - tot, BSIZE - off%BSIZE);
80101a3f:	89 f0                	mov    %esi,%eax
80101a41:	25 ff 01 00 00       	and    $0x1ff,%eax
80101a46:	b9 00 02 00 00       	mov    $0x200,%ecx
80101a4b:	83 c4 0c             	add    $0xc,%esp
80101a4e:	29 c1                	sub    %eax,%ecx
    memmove(dst, bp->data + off%BSIZE, m);
80101a50:	8d 44 02 5c          	lea    0x5c(%edx,%eax,1),%eax
80101a54:	89 55 dc             	mov    %edx,-0x24(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101a57:	29 fb                	sub    %edi,%ebx
80101a59:	39 d9                	cmp    %ebx,%ecx
80101a5b:	0f 46 d9             	cmovbe %ecx,%ebx
    memmove(dst, bp->data + off%BSIZE, m);
80101a5e:	53                   	push   %ebx
80101a5f:	50                   	push   %eax
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101a60:	01 df                	add    %ebx,%edi
    memmove(dst, bp->data + off%BSIZE, m);
80101a62:	ff 75 e0             	pushl  -0x20(%ebp)
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101a65:	01 de                	add    %ebx,%esi
    memmove(dst, bp->data + off%BSIZE, m);
80101a67:	e8 e4 2f 00 00       	call   80104a50 <memmove>
    brelse(bp);
80101a6c:	8b 55 dc             	mov    -0x24(%ebp),%edx
80101a6f:	89 14 24             	mov    %edx,(%esp)
80101a72:	e8 69 e7 ff ff       	call   801001e0 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101a77:	01 5d e0             	add    %ebx,-0x20(%ebp)
80101a7a:	83 c4 10             	add    $0x10,%esp
80101a7d:	39 7d e4             	cmp    %edi,-0x1c(%ebp)
80101a80:	77 9e                	ja     80101a20 <readi+0x60>
  }
  return n;
80101a82:	8b 45 e4             	mov    -0x1c(%ebp),%eax
}
80101a85:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101a88:	5b                   	pop    %ebx
80101a89:	5e                   	pop    %esi
80101a8a:	5f                   	pop    %edi
80101a8b:	5d                   	pop    %ebp
80101a8c:	c3                   	ret    
80101a8d:	8d 76 00             	lea    0x0(%esi),%esi
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101a90:	0f bf 40 52          	movswl 0x52(%eax),%eax
80101a94:	66 83 f8 09          	cmp    $0x9,%ax
80101a98:	77 17                	ja     80101ab1 <readi+0xf1>
80101a9a:	8b 04 c5 60 09 11 80 	mov    -0x7feef6a0(,%eax,8),%eax
80101aa1:	85 c0                	test   %eax,%eax
80101aa3:	74 0c                	je     80101ab1 <readi+0xf1>
    return devsw[ip->major].read(ip, dst, n);
80101aa5:	89 7d 10             	mov    %edi,0x10(%ebp)
}
80101aa8:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101aab:	5b                   	pop    %ebx
80101aac:	5e                   	pop    %esi
80101aad:	5f                   	pop    %edi
80101aae:	5d                   	pop    %ebp
    return devsw[ip->major].read(ip, dst, n);
80101aaf:	ff e0                	jmp    *%eax
      return -1;
80101ab1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101ab6:	eb cd                	jmp    80101a85 <readi+0xc5>
80101ab8:	90                   	nop
80101ab9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80101ac0 <writei>:
// PAGEBREAK!
// Write data to inode.
// Caller must hold ip->lock.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101ac0:	55                   	push   %ebp
80101ac1:	89 e5                	mov    %esp,%ebp
80101ac3:	57                   	push   %edi
80101ac4:	56                   	push   %esi
80101ac5:	53                   	push   %ebx
80101ac6:	83 ec 1c             	sub    $0x1c,%esp
80101ac9:	8b 45 08             	mov    0x8(%ebp),%eax
80101acc:	8b 75 0c             	mov    0xc(%ebp),%esi
80101acf:	8b 7d 14             	mov    0x14(%ebp),%edi
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ad2:	66 83 78 50 03       	cmpw   $0x3,0x50(%eax)
{
80101ad7:	89 75 dc             	mov    %esi,-0x24(%ebp)
80101ada:	89 45 d8             	mov    %eax,-0x28(%ebp)
80101add:	8b 75 10             	mov    0x10(%ebp),%esi
80101ae0:	89 7d e0             	mov    %edi,-0x20(%ebp)
  if(ip->type == T_DEV){
80101ae3:	0f 84 b7 00 00 00    	je     80101ba0 <writei+0xe0>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
      return -1;
    return devsw[ip->major].write(ip, src, n);
  }

  if(off > ip->size || off + n < off)
80101ae9:	8b 45 d8             	mov    -0x28(%ebp),%eax
80101aec:	39 70 58             	cmp    %esi,0x58(%eax)
80101aef:	0f 82 eb 00 00 00    	jb     80101be0 <writei+0x120>
80101af5:	8b 7d e0             	mov    -0x20(%ebp),%edi
80101af8:	31 d2                	xor    %edx,%edx
80101afa:	89 f8                	mov    %edi,%eax
80101afc:	01 f0                	add    %esi,%eax
80101afe:	0f 92 c2             	setb   %dl
    return -1;
  if(off + n > MAXFILE*BSIZE)
80101b01:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101b06:	0f 87 d4 00 00 00    	ja     80101be0 <writei+0x120>
80101b0c:	85 d2                	test   %edx,%edx
80101b0e:	0f 85 cc 00 00 00    	jne    80101be0 <writei+0x120>
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101b14:	85 ff                	test   %edi,%edi
80101b16:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80101b1d:	74 72                	je     80101b91 <writei+0xd1>
80101b1f:	90                   	nop
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101b20:	8b 7d d8             	mov    -0x28(%ebp),%edi
80101b23:	89 f2                	mov    %esi,%edx
80101b25:	c1 ea 09             	shr    $0x9,%edx
80101b28:	89 f8                	mov    %edi,%eax
80101b2a:	e8 11 f8 ff ff       	call   80101340 <bmap>
80101b2f:	83 ec 08             	sub    $0x8,%esp
80101b32:	50                   	push   %eax
80101b33:	ff 37                	pushl  (%edi)
80101b35:	e8 96 e5 ff ff       	call   801000d0 <bread>
    m = min(n - tot, BSIZE - off%BSIZE);
80101b3a:	8b 5d e0             	mov    -0x20(%ebp),%ebx
80101b3d:	2b 5d e4             	sub    -0x1c(%ebp),%ebx
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101b40:	89 c7                	mov    %eax,%edi
    m = min(n - tot, BSIZE - off%BSIZE);
80101b42:	89 f0                	mov    %esi,%eax
80101b44:	b9 00 02 00 00       	mov    $0x200,%ecx
80101b49:	83 c4 0c             	add    $0xc,%esp
80101b4c:	25 ff 01 00 00       	and    $0x1ff,%eax
80101b51:	29 c1                	sub    %eax,%ecx
    memmove(bp->data + off%BSIZE, src, m);
80101b53:	8d 44 07 5c          	lea    0x5c(%edi,%eax,1),%eax
    m = min(n - tot, BSIZE - off%BSIZE);
80101b57:	39 d9                	cmp    %ebx,%ecx
80101b59:	0f 46 d9             	cmovbe %ecx,%ebx
    memmove(bp->data + off%BSIZE, src, m);
80101b5c:	53                   	push   %ebx
80101b5d:	ff 75 dc             	pushl  -0x24(%ebp)
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101b60:	01 de                	add    %ebx,%esi
    memmove(bp->data + off%BSIZE, src, m);
80101b62:	50                   	push   %eax
80101b63:	e8 e8 2e 00 00       	call   80104a50 <memmove>
    log_write(bp);
80101b68:	89 3c 24             	mov    %edi,(%esp)
80101b6b:	e8 f0 15 00 00       	call   80103160 <log_write>
    brelse(bp);
80101b70:	89 3c 24             	mov    %edi,(%esp)
80101b73:	e8 68 e6 ff ff       	call   801001e0 <brelse>
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101b78:	01 5d e4             	add    %ebx,-0x1c(%ebp)
80101b7b:	01 5d dc             	add    %ebx,-0x24(%ebp)
80101b7e:	83 c4 10             	add    $0x10,%esp
80101b81:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101b84:	39 45 e0             	cmp    %eax,-0x20(%ebp)
80101b87:	77 97                	ja     80101b20 <writei+0x60>
  }

  if(n > 0 && off > ip->size){
80101b89:	8b 45 d8             	mov    -0x28(%ebp),%eax
80101b8c:	3b 70 58             	cmp    0x58(%eax),%esi
80101b8f:	77 37                	ja     80101bc8 <writei+0x108>
    ip->size = off;
    iupdate(ip);
  }
  return n;
80101b91:	8b 45 e0             	mov    -0x20(%ebp),%eax
}
80101b94:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101b97:	5b                   	pop    %ebx
80101b98:	5e                   	pop    %esi
80101b99:	5f                   	pop    %edi
80101b9a:	5d                   	pop    %ebp
80101b9b:	c3                   	ret    
80101b9c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101ba0:	0f bf 40 52          	movswl 0x52(%eax),%eax
80101ba4:	66 83 f8 09          	cmp    $0x9,%ax
80101ba8:	77 36                	ja     80101be0 <writei+0x120>
80101baa:	8b 04 c5 64 09 11 80 	mov    -0x7feef69c(,%eax,8),%eax
80101bb1:	85 c0                	test   %eax,%eax
80101bb3:	74 2b                	je     80101be0 <writei+0x120>
    return devsw[ip->major].write(ip, src, n);
80101bb5:	89 7d 10             	mov    %edi,0x10(%ebp)
}
80101bb8:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101bbb:	5b                   	pop    %ebx
80101bbc:	5e                   	pop    %esi
80101bbd:	5f                   	pop    %edi
80101bbe:	5d                   	pop    %ebp
    return devsw[ip->major].write(ip, src, n);
80101bbf:	ff e0                	jmp    *%eax
80101bc1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    ip->size = off;
80101bc8:	8b 45 d8             	mov    -0x28(%ebp),%eax
    iupdate(ip);
80101bcb:	83 ec 0c             	sub    $0xc,%esp
    ip->size = off;
80101bce:	89 70 58             	mov    %esi,0x58(%eax)
    iupdate(ip);
80101bd1:	50                   	push   %eax
80101bd2:	e8 59 fa ff ff       	call   80101630 <iupdate>
80101bd7:	83 c4 10             	add    $0x10,%esp
80101bda:	eb b5                	jmp    80101b91 <writei+0xd1>
80101bdc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      return -1;
80101be0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101be5:	eb ad                	jmp    80101b94 <writei+0xd4>
80101be7:	89 f6                	mov    %esi,%esi
80101be9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80101bf0 <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
80101bf0:	55                   	push   %ebp
80101bf1:	89 e5                	mov    %esp,%ebp
80101bf3:	83 ec 0c             	sub    $0xc,%esp
  return strncmp(s, t, DIRSIZ);
80101bf6:	6a 0e                	push   $0xe
80101bf8:	ff 75 0c             	pushl  0xc(%ebp)
80101bfb:	ff 75 08             	pushl  0x8(%ebp)
80101bfe:	e8 bd 2e 00 00       	call   80104ac0 <strncmp>
}
80101c03:	c9                   	leave  
80101c04:	c3                   	ret    
80101c05:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101c09:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80101c10 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
80101c10:	55                   	push   %ebp
80101c11:	89 e5                	mov    %esp,%ebp
80101c13:	57                   	push   %edi
80101c14:	56                   	push   %esi
80101c15:	53                   	push   %ebx
80101c16:	83 ec 1c             	sub    $0x1c,%esp
80101c19:	8b 5d 08             	mov    0x8(%ebp),%ebx
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
80101c1c:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80101c21:	0f 85 85 00 00 00    	jne    80101cac <dirlookup+0x9c>
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
80101c27:	8b 53 58             	mov    0x58(%ebx),%edx
80101c2a:	31 ff                	xor    %edi,%edi
80101c2c:	8d 75 d8             	lea    -0x28(%ebp),%esi
80101c2f:	85 d2                	test   %edx,%edx
80101c31:	74 3e                	je     80101c71 <dirlookup+0x61>
80101c33:	90                   	nop
80101c34:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80101c38:	6a 10                	push   $0x10
80101c3a:	57                   	push   %edi
80101c3b:	56                   	push   %esi
80101c3c:	53                   	push   %ebx
80101c3d:	e8 7e fd ff ff       	call   801019c0 <readi>
80101c42:	83 c4 10             	add    $0x10,%esp
80101c45:	83 f8 10             	cmp    $0x10,%eax
80101c48:	75 55                	jne    80101c9f <dirlookup+0x8f>
      panic("dirlookup read");
    if(de.inum == 0)
80101c4a:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
80101c4f:	74 18                	je     80101c69 <dirlookup+0x59>
  return strncmp(s, t, DIRSIZ);
80101c51:	8d 45 da             	lea    -0x26(%ebp),%eax
80101c54:	83 ec 04             	sub    $0x4,%esp
80101c57:	6a 0e                	push   $0xe
80101c59:	50                   	push   %eax
80101c5a:	ff 75 0c             	pushl  0xc(%ebp)
80101c5d:	e8 5e 2e 00 00       	call   80104ac0 <strncmp>
      continue;
    if(namecmp(name, de.name) == 0){
80101c62:	83 c4 10             	add    $0x10,%esp
80101c65:	85 c0                	test   %eax,%eax
80101c67:	74 17                	je     80101c80 <dirlookup+0x70>
  for(off = 0; off < dp->size; off += sizeof(de)){
80101c69:	83 c7 10             	add    $0x10,%edi
80101c6c:	3b 7b 58             	cmp    0x58(%ebx),%edi
80101c6f:	72 c7                	jb     80101c38 <dirlookup+0x28>
      return iget(dp->dev, inum);
    }
  }

  return 0;
}
80101c71:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80101c74:	31 c0                	xor    %eax,%eax
}
80101c76:	5b                   	pop    %ebx
80101c77:	5e                   	pop    %esi
80101c78:	5f                   	pop    %edi
80101c79:	5d                   	pop    %ebp
80101c7a:	c3                   	ret    
80101c7b:	90                   	nop
80101c7c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      if(poff)
80101c80:	8b 45 10             	mov    0x10(%ebp),%eax
80101c83:	85 c0                	test   %eax,%eax
80101c85:	74 05                	je     80101c8c <dirlookup+0x7c>
        *poff = off;
80101c87:	8b 45 10             	mov    0x10(%ebp),%eax
80101c8a:	89 38                	mov    %edi,(%eax)
      inum = de.inum;
80101c8c:	0f b7 55 d8          	movzwl -0x28(%ebp),%edx
      return iget(dp->dev, inum);
80101c90:	8b 03                	mov    (%ebx),%eax
80101c92:	e8 d9 f5 ff ff       	call   80101270 <iget>
}
80101c97:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101c9a:	5b                   	pop    %ebx
80101c9b:	5e                   	pop    %esi
80101c9c:	5f                   	pop    %edi
80101c9d:	5d                   	pop    %ebp
80101c9e:	c3                   	ret    
      panic("dirlookup read");
80101c9f:	83 ec 0c             	sub    $0xc,%esp
80101ca2:	68 c4 75 10 80       	push   $0x801075c4
80101ca7:	e8 e4 e6 ff ff       	call   80100390 <panic>
    panic("dirlookup not DIR");
80101cac:	83 ec 0c             	sub    $0xc,%esp
80101caf:	68 b2 75 10 80       	push   $0x801075b2
80101cb4:	e8 d7 e6 ff ff       	call   80100390 <panic>
80101cb9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80101cc0 <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
80101cc0:	55                   	push   %ebp
80101cc1:	89 e5                	mov    %esp,%ebp
80101cc3:	57                   	push   %edi
80101cc4:	56                   	push   %esi
80101cc5:	53                   	push   %ebx
80101cc6:	89 cf                	mov    %ecx,%edi
80101cc8:	89 c3                	mov    %eax,%ebx
80101cca:	83 ec 1c             	sub    $0x1c,%esp
  struct inode *ip, *next;

  if(*path == '/')
80101ccd:	80 38 2f             	cmpb   $0x2f,(%eax)
{
80101cd0:	89 55 e0             	mov    %edx,-0x20(%ebp)
  if(*path == '/')
80101cd3:	0f 84 67 01 00 00    	je     80101e40 <namex+0x180>
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
80101cd9:	e8 22 1f 00 00       	call   80103c00 <myproc>
  acquire(&icache.lock);
80101cde:	83 ec 0c             	sub    $0xc,%esp
    ip = idup(myproc()->cwd);
80101ce1:	8b 70 68             	mov    0x68(%eax),%esi
  acquire(&icache.lock);
80101ce4:	68 e0 09 11 80       	push   $0x801109e0
80101ce9:	e8 32 2b 00 00       	call   80104820 <acquire>
  ip->ref++;
80101cee:	83 46 08 01          	addl   $0x1,0x8(%esi)
  release(&icache.lock);
80101cf2:	c7 04 24 e0 09 11 80 	movl   $0x801109e0,(%esp)
80101cf9:	e8 42 2c 00 00       	call   80104940 <release>
80101cfe:	83 c4 10             	add    $0x10,%esp
80101d01:	eb 08                	jmp    80101d0b <namex+0x4b>
80101d03:	90                   	nop
80101d04:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    path++;
80101d08:	83 c3 01             	add    $0x1,%ebx
  while(*path == '/')
80101d0b:	0f b6 03             	movzbl (%ebx),%eax
80101d0e:	3c 2f                	cmp    $0x2f,%al
80101d10:	74 f6                	je     80101d08 <namex+0x48>
  if(*path == 0)
80101d12:	84 c0                	test   %al,%al
80101d14:	0f 84 ee 00 00 00    	je     80101e08 <namex+0x148>
  while(*path != '/' && *path != 0)
80101d1a:	0f b6 03             	movzbl (%ebx),%eax
80101d1d:	3c 2f                	cmp    $0x2f,%al
80101d1f:	0f 84 b3 00 00 00    	je     80101dd8 <namex+0x118>
80101d25:	84 c0                	test   %al,%al
80101d27:	89 da                	mov    %ebx,%edx
80101d29:	75 09                	jne    80101d34 <namex+0x74>
80101d2b:	e9 a8 00 00 00       	jmp    80101dd8 <namex+0x118>
80101d30:	84 c0                	test   %al,%al
80101d32:	74 0a                	je     80101d3e <namex+0x7e>
    path++;
80101d34:	83 c2 01             	add    $0x1,%edx
  while(*path != '/' && *path != 0)
80101d37:	0f b6 02             	movzbl (%edx),%eax
80101d3a:	3c 2f                	cmp    $0x2f,%al
80101d3c:	75 f2                	jne    80101d30 <namex+0x70>
80101d3e:	89 d1                	mov    %edx,%ecx
80101d40:	29 d9                	sub    %ebx,%ecx
  if(len >= DIRSIZ)
80101d42:	83 f9 0d             	cmp    $0xd,%ecx
80101d45:	0f 8e 91 00 00 00    	jle    80101ddc <namex+0x11c>
    memmove(name, s, DIRSIZ);
80101d4b:	83 ec 04             	sub    $0x4,%esp
80101d4e:	89 55 e4             	mov    %edx,-0x1c(%ebp)
80101d51:	6a 0e                	push   $0xe
80101d53:	53                   	push   %ebx
80101d54:	57                   	push   %edi
80101d55:	e8 f6 2c 00 00       	call   80104a50 <memmove>
    path++;
80101d5a:	8b 55 e4             	mov    -0x1c(%ebp),%edx
    memmove(name, s, DIRSIZ);
80101d5d:	83 c4 10             	add    $0x10,%esp
    path++;
80101d60:	89 d3                	mov    %edx,%ebx
  while(*path == '/')
80101d62:	80 3a 2f             	cmpb   $0x2f,(%edx)
80101d65:	75 11                	jne    80101d78 <namex+0xb8>
80101d67:	89 f6                	mov    %esi,%esi
80101d69:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    path++;
80101d70:	83 c3 01             	add    $0x1,%ebx
  while(*path == '/')
80101d73:	80 3b 2f             	cmpb   $0x2f,(%ebx)
80101d76:	74 f8                	je     80101d70 <namex+0xb0>

  while((path = skipelem(path, name)) != 0){
    ilock(ip);
80101d78:	83 ec 0c             	sub    $0xc,%esp
80101d7b:	56                   	push   %esi
80101d7c:	e8 5f f9 ff ff       	call   801016e0 <ilock>
    if(ip->type != T_DIR){
80101d81:	83 c4 10             	add    $0x10,%esp
80101d84:	66 83 7e 50 01       	cmpw   $0x1,0x50(%esi)
80101d89:	0f 85 91 00 00 00    	jne    80101e20 <namex+0x160>
      iunlockput(ip);
      return 0;
    }
    if(nameiparent && *path == '\0'){
80101d8f:	8b 55 e0             	mov    -0x20(%ebp),%edx
80101d92:	85 d2                	test   %edx,%edx
80101d94:	74 09                	je     80101d9f <namex+0xdf>
80101d96:	80 3b 00             	cmpb   $0x0,(%ebx)
80101d99:	0f 84 b7 00 00 00    	je     80101e56 <namex+0x196>
      // Stop one level early.
      iunlock(ip);
      return ip;
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80101d9f:	83 ec 04             	sub    $0x4,%esp
80101da2:	6a 00                	push   $0x0
80101da4:	57                   	push   %edi
80101da5:	56                   	push   %esi
80101da6:	e8 65 fe ff ff       	call   80101c10 <dirlookup>
80101dab:	83 c4 10             	add    $0x10,%esp
80101dae:	85 c0                	test   %eax,%eax
80101db0:	74 6e                	je     80101e20 <namex+0x160>
  iunlock(ip);
80101db2:	83 ec 0c             	sub    $0xc,%esp
80101db5:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80101db8:	56                   	push   %esi
80101db9:	e8 02 fa ff ff       	call   801017c0 <iunlock>
  iput(ip);
80101dbe:	89 34 24             	mov    %esi,(%esp)
80101dc1:	e8 4a fa ff ff       	call   80101810 <iput>
80101dc6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101dc9:	83 c4 10             	add    $0x10,%esp
80101dcc:	89 c6                	mov    %eax,%esi
80101dce:	e9 38 ff ff ff       	jmp    80101d0b <namex+0x4b>
80101dd3:	90                   	nop
80101dd4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  while(*path != '/' && *path != 0)
80101dd8:	89 da                	mov    %ebx,%edx
80101dda:	31 c9                	xor    %ecx,%ecx
    memmove(name, s, len);
80101ddc:	83 ec 04             	sub    $0x4,%esp
80101ddf:	89 55 dc             	mov    %edx,-0x24(%ebp)
80101de2:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
80101de5:	51                   	push   %ecx
80101de6:	53                   	push   %ebx
80101de7:	57                   	push   %edi
80101de8:	e8 63 2c 00 00       	call   80104a50 <memmove>
    name[len] = 0;
80101ded:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80101df0:	8b 55 dc             	mov    -0x24(%ebp),%edx
80101df3:	83 c4 10             	add    $0x10,%esp
80101df6:	c6 04 0f 00          	movb   $0x0,(%edi,%ecx,1)
80101dfa:	89 d3                	mov    %edx,%ebx
80101dfc:	e9 61 ff ff ff       	jmp    80101d62 <namex+0xa2>
80101e01:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
80101e08:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101e0b:	85 c0                	test   %eax,%eax
80101e0d:	75 5d                	jne    80101e6c <namex+0x1ac>
    iput(ip);
    return 0;
  }
  return ip;
}
80101e0f:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101e12:	89 f0                	mov    %esi,%eax
80101e14:	5b                   	pop    %ebx
80101e15:	5e                   	pop    %esi
80101e16:	5f                   	pop    %edi
80101e17:	5d                   	pop    %ebp
80101e18:	c3                   	ret    
80101e19:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  iunlock(ip);
80101e20:	83 ec 0c             	sub    $0xc,%esp
80101e23:	56                   	push   %esi
80101e24:	e8 97 f9 ff ff       	call   801017c0 <iunlock>
  iput(ip);
80101e29:	89 34 24             	mov    %esi,(%esp)
      return 0;
80101e2c:	31 f6                	xor    %esi,%esi
  iput(ip);
80101e2e:	e8 dd f9 ff ff       	call   80101810 <iput>
      return 0;
80101e33:	83 c4 10             	add    $0x10,%esp
}
80101e36:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101e39:	89 f0                	mov    %esi,%eax
80101e3b:	5b                   	pop    %ebx
80101e3c:	5e                   	pop    %esi
80101e3d:	5f                   	pop    %edi
80101e3e:	5d                   	pop    %ebp
80101e3f:	c3                   	ret    
    ip = iget(ROOTDEV, ROOTINO);
80101e40:	ba 01 00 00 00       	mov    $0x1,%edx
80101e45:	b8 01 00 00 00       	mov    $0x1,%eax
80101e4a:	e8 21 f4 ff ff       	call   80101270 <iget>
80101e4f:	89 c6                	mov    %eax,%esi
80101e51:	e9 b5 fe ff ff       	jmp    80101d0b <namex+0x4b>
      iunlock(ip);
80101e56:	83 ec 0c             	sub    $0xc,%esp
80101e59:	56                   	push   %esi
80101e5a:	e8 61 f9 ff ff       	call   801017c0 <iunlock>
      return ip;
80101e5f:	83 c4 10             	add    $0x10,%esp
}
80101e62:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101e65:	89 f0                	mov    %esi,%eax
80101e67:	5b                   	pop    %ebx
80101e68:	5e                   	pop    %esi
80101e69:	5f                   	pop    %edi
80101e6a:	5d                   	pop    %ebp
80101e6b:	c3                   	ret    
    iput(ip);
80101e6c:	83 ec 0c             	sub    $0xc,%esp
80101e6f:	56                   	push   %esi
    return 0;
80101e70:	31 f6                	xor    %esi,%esi
    iput(ip);
80101e72:	e8 99 f9 ff ff       	call   80101810 <iput>
    return 0;
80101e77:	83 c4 10             	add    $0x10,%esp
80101e7a:	eb 93                	jmp    80101e0f <namex+0x14f>
80101e7c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80101e80 <dirlink>:
{
80101e80:	55                   	push   %ebp
80101e81:	89 e5                	mov    %esp,%ebp
80101e83:	57                   	push   %edi
80101e84:	56                   	push   %esi
80101e85:	53                   	push   %ebx
80101e86:	83 ec 20             	sub    $0x20,%esp
80101e89:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if((ip = dirlookup(dp, name, 0)) != 0){
80101e8c:	6a 00                	push   $0x0
80101e8e:	ff 75 0c             	pushl  0xc(%ebp)
80101e91:	53                   	push   %ebx
80101e92:	e8 79 fd ff ff       	call   80101c10 <dirlookup>
80101e97:	83 c4 10             	add    $0x10,%esp
80101e9a:	85 c0                	test   %eax,%eax
80101e9c:	75 67                	jne    80101f05 <dirlink+0x85>
  for(off = 0; off < dp->size; off += sizeof(de)){
80101e9e:	8b 7b 58             	mov    0x58(%ebx),%edi
80101ea1:	8d 75 d8             	lea    -0x28(%ebp),%esi
80101ea4:	85 ff                	test   %edi,%edi
80101ea6:	74 29                	je     80101ed1 <dirlink+0x51>
80101ea8:	31 ff                	xor    %edi,%edi
80101eaa:	8d 75 d8             	lea    -0x28(%ebp),%esi
80101ead:	eb 09                	jmp    80101eb8 <dirlink+0x38>
80101eaf:	90                   	nop
80101eb0:	83 c7 10             	add    $0x10,%edi
80101eb3:	3b 7b 58             	cmp    0x58(%ebx),%edi
80101eb6:	73 19                	jae    80101ed1 <dirlink+0x51>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80101eb8:	6a 10                	push   $0x10
80101eba:	57                   	push   %edi
80101ebb:	56                   	push   %esi
80101ebc:	53                   	push   %ebx
80101ebd:	e8 fe fa ff ff       	call   801019c0 <readi>
80101ec2:	83 c4 10             	add    $0x10,%esp
80101ec5:	83 f8 10             	cmp    $0x10,%eax
80101ec8:	75 4e                	jne    80101f18 <dirlink+0x98>
    if(de.inum == 0)
80101eca:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
80101ecf:	75 df                	jne    80101eb0 <dirlink+0x30>
  strncpy(de.name, name, DIRSIZ);
80101ed1:	8d 45 da             	lea    -0x26(%ebp),%eax
80101ed4:	83 ec 04             	sub    $0x4,%esp
80101ed7:	6a 0e                	push   $0xe
80101ed9:	ff 75 0c             	pushl  0xc(%ebp)
80101edc:	50                   	push   %eax
80101edd:	e8 3e 2c 00 00       	call   80104b20 <strncpy>
  de.inum = inum;
80101ee2:	8b 45 10             	mov    0x10(%ebp),%eax
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80101ee5:	6a 10                	push   $0x10
80101ee7:	57                   	push   %edi
80101ee8:	56                   	push   %esi
80101ee9:	53                   	push   %ebx
  de.inum = inum;
80101eea:	66 89 45 d8          	mov    %ax,-0x28(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80101eee:	e8 cd fb ff ff       	call   80101ac0 <writei>
80101ef3:	83 c4 20             	add    $0x20,%esp
80101ef6:	83 f8 10             	cmp    $0x10,%eax
80101ef9:	75 2a                	jne    80101f25 <dirlink+0xa5>
  return 0;
80101efb:	31 c0                	xor    %eax,%eax
}
80101efd:	8d 65 f4             	lea    -0xc(%ebp),%esp
80101f00:	5b                   	pop    %ebx
80101f01:	5e                   	pop    %esi
80101f02:	5f                   	pop    %edi
80101f03:	5d                   	pop    %ebp
80101f04:	c3                   	ret    
    iput(ip);
80101f05:	83 ec 0c             	sub    $0xc,%esp
80101f08:	50                   	push   %eax
80101f09:	e8 02 f9 ff ff       	call   80101810 <iput>
    return -1;
80101f0e:	83 c4 10             	add    $0x10,%esp
80101f11:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f16:	eb e5                	jmp    80101efd <dirlink+0x7d>
      panic("dirlink read");
80101f18:	83 ec 0c             	sub    $0xc,%esp
80101f1b:	68 d3 75 10 80       	push   $0x801075d3
80101f20:	e8 6b e4 ff ff       	call   80100390 <panic>
    panic("dirlink");
80101f25:	83 ec 0c             	sub    $0xc,%esp
80101f28:	68 31 7d 10 80       	push   $0x80107d31
80101f2d:	e8 5e e4 ff ff       	call   80100390 <panic>
80101f32:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80101f39:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80101f40 <namei>:

struct inode*
namei(char *path)
{
80101f40:	55                   	push   %ebp
  char name[DIRSIZ];
  return namex(path, 0, name);
80101f41:	31 d2                	xor    %edx,%edx
{
80101f43:	89 e5                	mov    %esp,%ebp
80101f45:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 0, name);
80101f48:	8b 45 08             	mov    0x8(%ebp),%eax
80101f4b:	8d 4d ea             	lea    -0x16(%ebp),%ecx
80101f4e:	e8 6d fd ff ff       	call   80101cc0 <namex>
}
80101f53:	c9                   	leave  
80101f54:	c3                   	ret    
80101f55:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101f59:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80101f60 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
80101f60:	55                   	push   %ebp
  return namex(path, 1, name);
80101f61:	ba 01 00 00 00       	mov    $0x1,%edx
{
80101f66:	89 e5                	mov    %esp,%ebp
  return namex(path, 1, name);
80101f68:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80101f6b:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101f6e:	5d                   	pop    %ebp
  return namex(path, 1, name);
80101f6f:	e9 4c fd ff ff       	jmp    80101cc0 <namex>
80101f74:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80101f7a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80101f80 <itoa>:


#include "fcntl.h"
#define DIGITS 14

char* itoa(int i, char b[]){
80101f80:	55                   	push   %ebp
    char const digit[] = "0123456789";
80101f81:	b8 38 39 00 00       	mov    $0x3938,%eax
char* itoa(int i, char b[]){
80101f86:	89 e5                	mov    %esp,%ebp
80101f88:	57                   	push   %edi
80101f89:	56                   	push   %esi
80101f8a:	53                   	push   %ebx
80101f8b:	83 ec 10             	sub    $0x10,%esp
80101f8e:	8b 4d 08             	mov    0x8(%ebp),%ecx
    char const digit[] = "0123456789";
80101f91:	c7 45 e9 30 31 32 33 	movl   $0x33323130,-0x17(%ebp)
80101f98:	c7 45 ed 34 35 36 37 	movl   $0x37363534,-0x13(%ebp)
80101f9f:	66 89 45 f1          	mov    %ax,-0xf(%ebp)
80101fa3:	c6 45 f3 00          	movb   $0x0,-0xd(%ebp)
80101fa7:	8b 75 0c             	mov    0xc(%ebp),%esi
    char* p = b;
    if(i<0){
80101faa:	85 c9                	test   %ecx,%ecx
80101fac:	79 0a                	jns    80101fb8 <itoa+0x38>
80101fae:	89 f0                	mov    %esi,%eax
80101fb0:	8d 76 01             	lea    0x1(%esi),%esi
        *p++ = '-';
        i *= -1;
80101fb3:	f7 d9                	neg    %ecx
        *p++ = '-';
80101fb5:	c6 00 2d             	movb   $0x2d,(%eax)
    }
    int shifter = i;
80101fb8:	89 cb                	mov    %ecx,%ebx
    do{ //Move to where representation ends
        ++p;
        shifter = shifter/10;
80101fba:	bf 67 66 66 66       	mov    $0x66666667,%edi
80101fbf:	90                   	nop
80101fc0:	89 d8                	mov    %ebx,%eax
80101fc2:	c1 fb 1f             	sar    $0x1f,%ebx
        ++p;
80101fc5:	83 c6 01             	add    $0x1,%esi
        shifter = shifter/10;
80101fc8:	f7 ef                	imul   %edi
80101fca:	c1 fa 02             	sar    $0x2,%edx
    }while(shifter);
80101fcd:	29 da                	sub    %ebx,%edx
80101fcf:	89 d3                	mov    %edx,%ebx
80101fd1:	75 ed                	jne    80101fc0 <itoa+0x40>
    *p = '\0';
80101fd3:	c6 06 00             	movb   $0x0,(%esi)
    do{ //Move back, inserting digits as u go
        *--p = digit[i%10];
80101fd6:	bb 67 66 66 66       	mov    $0x66666667,%ebx
80101fdb:	90                   	nop
80101fdc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80101fe0:	89 c8                	mov    %ecx,%eax
80101fe2:	83 ee 01             	sub    $0x1,%esi
80101fe5:	f7 eb                	imul   %ebx
80101fe7:	89 c8                	mov    %ecx,%eax
80101fe9:	c1 f8 1f             	sar    $0x1f,%eax
80101fec:	c1 fa 02             	sar    $0x2,%edx
80101fef:	29 c2                	sub    %eax,%edx
80101ff1:	8d 04 92             	lea    (%edx,%edx,4),%eax
80101ff4:	01 c0                	add    %eax,%eax
80101ff6:	29 c1                	sub    %eax,%ecx
        i = i/10;
    }while(i);
80101ff8:	85 d2                	test   %edx,%edx
        *--p = digit[i%10];
80101ffa:	0f b6 44 0d e9       	movzbl -0x17(%ebp,%ecx,1),%eax
        i = i/10;
80101fff:	89 d1                	mov    %edx,%ecx
        *--p = digit[i%10];
80102001:	88 06                	mov    %al,(%esi)
    }while(i);
80102003:	75 db                	jne    80101fe0 <itoa+0x60>
    return b;
}
80102005:	8b 45 0c             	mov    0xc(%ebp),%eax
80102008:	83 c4 10             	add    $0x10,%esp
8010200b:	5b                   	pop    %ebx
8010200c:	5e                   	pop    %esi
8010200d:	5f                   	pop    %edi
8010200e:	5d                   	pop    %ebp
8010200f:	c3                   	ret    

80102010 <removeSwapFile>:
//remove swap file of proc p;
int
removeSwapFile(struct proc* p)
{
80102010:	55                   	push   %ebp
80102011:	89 e5                	mov    %esp,%ebp
80102013:	57                   	push   %edi
80102014:	56                   	push   %esi
80102015:	53                   	push   %ebx
  //path of proccess
  char path[DIGITS];
  memmove(path,"/.swap", 6);
80102016:	8d 75 bc             	lea    -0x44(%ebp),%esi
{
80102019:	83 ec 40             	sub    $0x40,%esp
8010201c:	8b 5d 08             	mov    0x8(%ebp),%ebx
  memmove(path,"/.swap", 6);
8010201f:	6a 06                	push   $0x6
80102021:	68 e0 75 10 80       	push   $0x801075e0
80102026:	56                   	push   %esi
80102027:	e8 24 2a 00 00       	call   80104a50 <memmove>
  itoa(p->pid, path+ 6);
8010202c:	58                   	pop    %eax
8010202d:	8d 45 c2             	lea    -0x3e(%ebp),%eax
80102030:	5a                   	pop    %edx
80102031:	50                   	push   %eax
80102032:	ff 73 10             	pushl  0x10(%ebx)
80102035:	e8 46 ff ff ff       	call   80101f80 <itoa>
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ];
  uint off;

  if(0 == p->swapFile)
8010203a:	8b 43 7c             	mov    0x7c(%ebx),%eax
8010203d:	83 c4 10             	add    $0x10,%esp
80102040:	85 c0                	test   %eax,%eax
80102042:	0f 84 88 01 00 00    	je     801021d0 <removeSwapFile+0x1c0>
  {
    return -1;
  }
  fileclose(p->swapFile);
80102048:	83 ec 0c             	sub    $0xc,%esp
  return namex(path, 1, name);
8010204b:	8d 5d ca             	lea    -0x36(%ebp),%ebx
  fileclose(p->swapFile);
8010204e:	50                   	push   %eax
8010204f:	e8 3c ee ff ff       	call   80100e90 <fileclose>

  begin_op();
80102054:	e8 37 0f 00 00       	call   80102f90 <begin_op>
  return namex(path, 1, name);
80102059:	89 f0                	mov    %esi,%eax
8010205b:	89 d9                	mov    %ebx,%ecx
8010205d:	ba 01 00 00 00       	mov    $0x1,%edx
80102062:	e8 59 fc ff ff       	call   80101cc0 <namex>
  if((dp = nameiparent(path, name)) == 0)
80102067:	83 c4 10             	add    $0x10,%esp
8010206a:	85 c0                	test   %eax,%eax
  return namex(path, 1, name);
8010206c:	89 c6                	mov    %eax,%esi
  if((dp = nameiparent(path, name)) == 0)
8010206e:	0f 84 66 01 00 00    	je     801021da <removeSwapFile+0x1ca>
  {
    end_op();
    return -1;
  }

  ilock(dp);
80102074:	83 ec 0c             	sub    $0xc,%esp
80102077:	50                   	push   %eax
80102078:	e8 63 f6 ff ff       	call   801016e0 <ilock>
  return strncmp(s, t, DIRSIZ);
8010207d:	83 c4 0c             	add    $0xc,%esp
80102080:	6a 0e                	push   $0xe
80102082:	68 e8 75 10 80       	push   $0x801075e8
80102087:	53                   	push   %ebx
80102088:	e8 33 2a 00 00       	call   80104ac0 <strncmp>

    // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
8010208d:	83 c4 10             	add    $0x10,%esp
80102090:	85 c0                	test   %eax,%eax
80102092:	0f 84 f8 00 00 00    	je     80102190 <removeSwapFile+0x180>
  return strncmp(s, t, DIRSIZ);
80102098:	83 ec 04             	sub    $0x4,%esp
8010209b:	6a 0e                	push   $0xe
8010209d:	68 e7 75 10 80       	push   $0x801075e7
801020a2:	53                   	push   %ebx
801020a3:	e8 18 2a 00 00       	call   80104ac0 <strncmp>
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
801020a8:	83 c4 10             	add    $0x10,%esp
801020ab:	85 c0                	test   %eax,%eax
801020ad:	0f 84 dd 00 00 00    	je     80102190 <removeSwapFile+0x180>
     goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
801020b3:	8d 45 b8             	lea    -0x48(%ebp),%eax
801020b6:	83 ec 04             	sub    $0x4,%esp
801020b9:	50                   	push   %eax
801020ba:	53                   	push   %ebx
801020bb:	56                   	push   %esi
801020bc:	e8 4f fb ff ff       	call   80101c10 <dirlookup>
801020c1:	83 c4 10             	add    $0x10,%esp
801020c4:	85 c0                	test   %eax,%eax
801020c6:	89 c3                	mov    %eax,%ebx
801020c8:	0f 84 c2 00 00 00    	je     80102190 <removeSwapFile+0x180>
    goto bad;
  ilock(ip);
801020ce:	83 ec 0c             	sub    $0xc,%esp
801020d1:	50                   	push   %eax
801020d2:	e8 09 f6 ff ff       	call   801016e0 <ilock>

  if(ip->nlink < 1)
801020d7:	83 c4 10             	add    $0x10,%esp
801020da:	66 83 7b 56 00       	cmpw   $0x0,0x56(%ebx)
801020df:	0f 8e 11 01 00 00    	jle    801021f6 <removeSwapFile+0x1e6>
    panic("unlink: nlink < 1");
  if(ip->type == T_DIR && !isdirempty(ip)){
801020e5:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
801020ea:	74 74                	je     80102160 <removeSwapFile+0x150>
    iunlockput(ip);
    goto bad;
  }

  memset(&de, 0, sizeof(de));
801020ec:	8d 7d d8             	lea    -0x28(%ebp),%edi
801020ef:	83 ec 04             	sub    $0x4,%esp
801020f2:	6a 10                	push   $0x10
801020f4:	6a 00                	push   $0x0
801020f6:	57                   	push   %edi
801020f7:	e8 a4 28 00 00       	call   801049a0 <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020fc:	6a 10                	push   $0x10
801020fe:	ff 75 b8             	pushl  -0x48(%ebp)
80102101:	57                   	push   %edi
80102102:	56                   	push   %esi
80102103:	e8 b8 f9 ff ff       	call   80101ac0 <writei>
80102108:	83 c4 20             	add    $0x20,%esp
8010210b:	83 f8 10             	cmp    $0x10,%eax
8010210e:	0f 85 d5 00 00 00    	jne    801021e9 <removeSwapFile+0x1d9>
    panic("unlink: writei");
  if(ip->type == T_DIR){
80102114:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80102119:	0f 84 91 00 00 00    	je     801021b0 <removeSwapFile+0x1a0>
  iunlock(ip);
8010211f:	83 ec 0c             	sub    $0xc,%esp
80102122:	56                   	push   %esi
80102123:	e8 98 f6 ff ff       	call   801017c0 <iunlock>
  iput(ip);
80102128:	89 34 24             	mov    %esi,(%esp)
8010212b:	e8 e0 f6 ff ff       	call   80101810 <iput>
    dp->nlink--;
    iupdate(dp);
  }
  iunlockput(dp);

  ip->nlink--;
80102130:	66 83 6b 56 01       	subw   $0x1,0x56(%ebx)
  iupdate(ip);
80102135:	89 1c 24             	mov    %ebx,(%esp)
80102138:	e8 f3 f4 ff ff       	call   80101630 <iupdate>
  iunlock(ip);
8010213d:	89 1c 24             	mov    %ebx,(%esp)
80102140:	e8 7b f6 ff ff       	call   801017c0 <iunlock>
  iput(ip);
80102145:	89 1c 24             	mov    %ebx,(%esp)
80102148:	e8 c3 f6 ff ff       	call   80101810 <iput>
  iunlockput(ip);

  end_op();
8010214d:	e8 ae 0e 00 00       	call   80103000 <end_op>

  return 0;
80102152:	83 c4 10             	add    $0x10,%esp
80102155:	31 c0                	xor    %eax,%eax
  bad:
    iunlockput(dp);
    end_op();
    return -1;

}
80102157:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010215a:	5b                   	pop    %ebx
8010215b:	5e                   	pop    %esi
8010215c:	5f                   	pop    %edi
8010215d:	5d                   	pop    %ebp
8010215e:	c3                   	ret    
8010215f:	90                   	nop
  if(ip->type == T_DIR && !isdirempty(ip)){
80102160:	83 ec 0c             	sub    $0xc,%esp
80102163:	53                   	push   %ebx
80102164:	e8 17 30 00 00       	call   80105180 <isdirempty>
80102169:	83 c4 10             	add    $0x10,%esp
8010216c:	85 c0                	test   %eax,%eax
8010216e:	0f 85 78 ff ff ff    	jne    801020ec <removeSwapFile+0xdc>
  iunlock(ip);
80102174:	83 ec 0c             	sub    $0xc,%esp
80102177:	53                   	push   %ebx
80102178:	e8 43 f6 ff ff       	call   801017c0 <iunlock>
  iput(ip);
8010217d:	89 1c 24             	mov    %ebx,(%esp)
80102180:	e8 8b f6 ff ff       	call   80101810 <iput>
80102185:	83 c4 10             	add    $0x10,%esp
80102188:	90                   	nop
80102189:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  iunlock(ip);
80102190:	83 ec 0c             	sub    $0xc,%esp
80102193:	56                   	push   %esi
80102194:	e8 27 f6 ff ff       	call   801017c0 <iunlock>
  iput(ip);
80102199:	89 34 24             	mov    %esi,(%esp)
8010219c:	e8 6f f6 ff ff       	call   80101810 <iput>
    end_op();
801021a1:	e8 5a 0e 00 00       	call   80103000 <end_op>
    return -1;
801021a6:	83 c4 10             	add    $0x10,%esp
801021a9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801021ae:	eb a7                	jmp    80102157 <removeSwapFile+0x147>
    dp->nlink--;
801021b0:	66 83 6e 56 01       	subw   $0x1,0x56(%esi)
    iupdate(dp);
801021b5:	83 ec 0c             	sub    $0xc,%esp
801021b8:	56                   	push   %esi
801021b9:	e8 72 f4 ff ff       	call   80101630 <iupdate>
801021be:	83 c4 10             	add    $0x10,%esp
801021c1:	e9 59 ff ff ff       	jmp    8010211f <removeSwapFile+0x10f>
801021c6:	8d 76 00             	lea    0x0(%esi),%esi
801021c9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    return -1;
801021d0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801021d5:	e9 7d ff ff ff       	jmp    80102157 <removeSwapFile+0x147>
    end_op();
801021da:	e8 21 0e 00 00       	call   80103000 <end_op>
    return -1;
801021df:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801021e4:	e9 6e ff ff ff       	jmp    80102157 <removeSwapFile+0x147>
    panic("unlink: writei");
801021e9:	83 ec 0c             	sub    $0xc,%esp
801021ec:	68 fc 75 10 80       	push   $0x801075fc
801021f1:	e8 9a e1 ff ff       	call   80100390 <panic>
    panic("unlink: nlink < 1");
801021f6:	83 ec 0c             	sub    $0xc,%esp
801021f9:	68 ea 75 10 80       	push   $0x801075ea
801021fe:	e8 8d e1 ff ff       	call   80100390 <panic>
80102203:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80102209:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80102210 <createSwapFile>:


//return 0 on success
int
createSwapFile(struct proc* p) {
80102210:	55                   	push   %ebp
80102211:	89 e5                	mov    %esp,%ebp
80102213:	56                   	push   %esi
80102214:	53                   	push   %ebx

    char path[DIGITS];
    memmove(path, "/.swap", 6);
80102215:	8d 75 ea             	lea    -0x16(%ebp),%esi
createSwapFile(struct proc* p) {
80102218:	83 ec 14             	sub    $0x14,%esp
8010221b:	8b 5d 08             	mov    0x8(%ebp),%ebx
    memmove(path, "/.swap", 6);
8010221e:	6a 06                	push   $0x6
80102220:	68 e0 75 10 80       	push   $0x801075e0
80102225:	56                   	push   %esi
80102226:	e8 25 28 00 00       	call   80104a50 <memmove>
    itoa(p->pid, path + 6);
8010222b:	58                   	pop    %eax
8010222c:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010222f:	5a                   	pop    %edx
80102230:	50                   	push   %eax
80102231:	ff 73 10             	pushl  0x10(%ebx)
80102234:	e8 47 fd ff ff       	call   80101f80 <itoa>

    begin_op();
80102239:	e8 52 0d 00 00       	call   80102f90 <begin_op>
    struct inode *in = create(path, T_FILE, 0, 0);
8010223e:	6a 00                	push   $0x0
80102240:	6a 00                	push   $0x0
80102242:	6a 02                	push   $0x2
80102244:	56                   	push   %esi
80102245:	e8 46 31 00 00       	call   80105390 <create>
    iunlock(in);
8010224a:	83 c4 14             	add    $0x14,%esp
    struct inode *in = create(path, T_FILE, 0, 0);
8010224d:	89 c6                	mov    %eax,%esi
    iunlock(in);
8010224f:	50                   	push   %eax
80102250:	e8 6b f5 ff ff       	call   801017c0 <iunlock>

    p->swapFile = filealloc();
80102255:	e8 76 eb ff ff       	call   80100dd0 <filealloc>
    if (p->swapFile == 0)
8010225a:	83 c4 10             	add    $0x10,%esp
8010225d:	85 c0                	test   %eax,%eax
    p->swapFile = filealloc();
8010225f:	89 43 7c             	mov    %eax,0x7c(%ebx)
    if (p->swapFile == 0)
80102262:	74 32                	je     80102296 <createSwapFile+0x86>
        panic("no slot for files on /store");

    p->swapFile->ip = in;
80102264:	89 70 10             	mov    %esi,0x10(%eax)
    p->swapFile->type = FD_INODE;
80102267:	8b 43 7c             	mov    0x7c(%ebx),%eax
8010226a:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
    p->swapFile->off = 0;
80102270:	8b 43 7c             	mov    0x7c(%ebx),%eax
80102273:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
    p->swapFile->readable = O_WRONLY;
8010227a:	8b 43 7c             	mov    0x7c(%ebx),%eax
8010227d:	c6 40 08 01          	movb   $0x1,0x8(%eax)
    p->swapFile->writable = O_RDWR;
80102281:	8b 43 7c             	mov    0x7c(%ebx),%eax
80102284:	c6 40 09 02          	movb   $0x2,0x9(%eax)
    end_op();
80102288:	e8 73 0d 00 00       	call   80103000 <end_op>

    return 0;
}
8010228d:	8d 65 f8             	lea    -0x8(%ebp),%esp
80102290:	31 c0                	xor    %eax,%eax
80102292:	5b                   	pop    %ebx
80102293:	5e                   	pop    %esi
80102294:	5d                   	pop    %ebp
80102295:	c3                   	ret    
        panic("no slot for files on /store");
80102296:	83 ec 0c             	sub    $0xc,%esp
80102299:	68 0b 76 10 80       	push   $0x8010760b
8010229e:	e8 ed e0 ff ff       	call   80100390 <panic>
801022a3:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801022a9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801022b0 <writeToSwapFile>:

//return as sys_write (-1 when error)
int
writeToSwapFile(struct proc * p, char* buffer, uint placeOnFile, uint size)
{
801022b0:	55                   	push   %ebp
801022b1:	89 e5                	mov    %esp,%ebp
801022b3:	8b 45 08             	mov    0x8(%ebp),%eax
  p->swapFile->off = placeOnFile;
801022b6:	8b 4d 10             	mov    0x10(%ebp),%ecx
801022b9:	8b 50 7c             	mov    0x7c(%eax),%edx
801022bc:	89 4a 14             	mov    %ecx,0x14(%edx)

  return filewrite(p->swapFile, buffer, size);
801022bf:	8b 55 14             	mov    0x14(%ebp),%edx
801022c2:	89 55 10             	mov    %edx,0x10(%ebp)
801022c5:	8b 40 7c             	mov    0x7c(%eax),%eax
801022c8:	89 45 08             	mov    %eax,0x8(%ebp)

}
801022cb:	5d                   	pop    %ebp
  return filewrite(p->swapFile, buffer, size);
801022cc:	e9 6f ed ff ff       	jmp    80101040 <filewrite>
801022d1:	eb 0d                	jmp    801022e0 <readFromSwapFile>
801022d3:	90                   	nop
801022d4:	90                   	nop
801022d5:	90                   	nop
801022d6:	90                   	nop
801022d7:	90                   	nop
801022d8:	90                   	nop
801022d9:	90                   	nop
801022da:	90                   	nop
801022db:	90                   	nop
801022dc:	90                   	nop
801022dd:	90                   	nop
801022de:	90                   	nop
801022df:	90                   	nop

801022e0 <readFromSwapFile>:

//return as sys_read (-1 when error)
int
readFromSwapFile(struct proc * p, char* buffer, uint placeOnFile, uint size)
{
801022e0:	55                   	push   %ebp
801022e1:	89 e5                	mov    %esp,%ebp
801022e3:	8b 45 08             	mov    0x8(%ebp),%eax
  p->swapFile->off = placeOnFile;
801022e6:	8b 4d 10             	mov    0x10(%ebp),%ecx
801022e9:	8b 50 7c             	mov    0x7c(%eax),%edx
801022ec:	89 4a 14             	mov    %ecx,0x14(%edx)

  return fileread(p->swapFile, buffer,  size);
801022ef:	8b 55 14             	mov    0x14(%ebp),%edx
801022f2:	89 55 10             	mov    %edx,0x10(%ebp)
801022f5:	8b 40 7c             	mov    0x7c(%eax),%eax
801022f8:	89 45 08             	mov    %eax,0x8(%ebp)
}
801022fb:	5d                   	pop    %ebp
  return fileread(p->swapFile, buffer,  size);
801022fc:	e9 af ec ff ff       	jmp    80100fb0 <fileread>
80102301:	66 90                	xchg   %ax,%ax
80102303:	66 90                	xchg   %ax,%ax
80102305:	66 90                	xchg   %ax,%ax
80102307:	66 90                	xchg   %ax,%ax
80102309:	66 90                	xchg   %ax,%ax
8010230b:	66 90                	xchg   %ax,%ax
8010230d:	66 90                	xchg   %ax,%ax
8010230f:	90                   	nop

80102310 <idestart>:
}

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
80102310:	55                   	push   %ebp
80102311:	89 e5                	mov    %esp,%ebp
80102313:	57                   	push   %edi
80102314:	56                   	push   %esi
80102315:	53                   	push   %ebx
80102316:	83 ec 0c             	sub    $0xc,%esp
  if(b == 0)
80102319:	85 c0                	test   %eax,%eax
8010231b:	0f 84 b4 00 00 00    	je     801023d5 <idestart+0xc5>
    panic("idestart");
  if(b->blockno >= FSSIZE)
80102321:	8b 58 08             	mov    0x8(%eax),%ebx
80102324:	89 c6                	mov    %eax,%esi
80102326:	81 fb e7 03 00 00    	cmp    $0x3e7,%ebx
8010232c:	0f 87 96 00 00 00    	ja     801023c8 <idestart+0xb8>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102332:	b9 f7 01 00 00       	mov    $0x1f7,%ecx
80102337:	89 f6                	mov    %esi,%esi
80102339:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
80102340:	89 ca                	mov    %ecx,%edx
80102342:	ec                   	in     (%dx),%al
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
80102343:	83 e0 c0             	and    $0xffffffc0,%eax
80102346:	3c 40                	cmp    $0x40,%al
80102348:	75 f6                	jne    80102340 <idestart+0x30>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010234a:	31 ff                	xor    %edi,%edi
8010234c:	ba f6 03 00 00       	mov    $0x3f6,%edx
80102351:	89 f8                	mov    %edi,%eax
80102353:	ee                   	out    %al,(%dx)
80102354:	b8 01 00 00 00       	mov    $0x1,%eax
80102359:	ba f2 01 00 00       	mov    $0x1f2,%edx
8010235e:	ee                   	out    %al,(%dx)
8010235f:	ba f3 01 00 00       	mov    $0x1f3,%edx
80102364:	89 d8                	mov    %ebx,%eax
80102366:	ee                   	out    %al,(%dx)

  idewait(0);
  outb(0x3f6, 0);  // generate interrupt
  outb(0x1f2, sector_per_block);  // number of sectors
  outb(0x1f3, sector & 0xff);
  outb(0x1f4, (sector >> 8) & 0xff);
80102367:	89 d8                	mov    %ebx,%eax
80102369:	ba f4 01 00 00       	mov    $0x1f4,%edx
8010236e:	c1 f8 08             	sar    $0x8,%eax
80102371:	ee                   	out    %al,(%dx)
80102372:	ba f5 01 00 00       	mov    $0x1f5,%edx
80102377:	89 f8                	mov    %edi,%eax
80102379:	ee                   	out    %al,(%dx)
  outb(0x1f5, (sector >> 16) & 0xff);
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((sector>>24)&0x0f));
8010237a:	0f b6 46 04          	movzbl 0x4(%esi),%eax
8010237e:	ba f6 01 00 00       	mov    $0x1f6,%edx
80102383:	c1 e0 04             	shl    $0x4,%eax
80102386:	83 e0 10             	and    $0x10,%eax
80102389:	83 c8 e0             	or     $0xffffffe0,%eax
8010238c:	ee                   	out    %al,(%dx)
  if(b->flags & B_DIRTY){
8010238d:	f6 06 04             	testb  $0x4,(%esi)
80102390:	75 16                	jne    801023a8 <idestart+0x98>
80102392:	b8 20 00 00 00       	mov    $0x20,%eax
80102397:	89 ca                	mov    %ecx,%edx
80102399:	ee                   	out    %al,(%dx)
    outb(0x1f7, write_cmd);
    outsl(0x1f0, b->data, BSIZE/4);
  } else {
    outb(0x1f7, read_cmd);
  }
}
8010239a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010239d:	5b                   	pop    %ebx
8010239e:	5e                   	pop    %esi
8010239f:	5f                   	pop    %edi
801023a0:	5d                   	pop    %ebp
801023a1:	c3                   	ret    
801023a2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801023a8:	b8 30 00 00 00       	mov    $0x30,%eax
801023ad:	89 ca                	mov    %ecx,%edx
801023af:	ee                   	out    %al,(%dx)
  asm volatile("cld; rep outsl" :
801023b0:	b9 80 00 00 00       	mov    $0x80,%ecx
    outsl(0x1f0, b->data, BSIZE/4);
801023b5:	83 c6 5c             	add    $0x5c,%esi
801023b8:	ba f0 01 00 00       	mov    $0x1f0,%edx
801023bd:	fc                   	cld    
801023be:	f3 6f                	rep outsl %ds:(%esi),(%dx)
}
801023c0:	8d 65 f4             	lea    -0xc(%ebp),%esp
801023c3:	5b                   	pop    %ebx
801023c4:	5e                   	pop    %esi
801023c5:	5f                   	pop    %edi
801023c6:	5d                   	pop    %ebp
801023c7:	c3                   	ret    
    panic("incorrect blockno");
801023c8:	83 ec 0c             	sub    $0xc,%esp
801023cb:	68 84 76 10 80       	push   $0x80107684
801023d0:	e8 bb df ff ff       	call   80100390 <panic>
    panic("idestart");
801023d5:	83 ec 0c             	sub    $0xc,%esp
801023d8:	68 7b 76 10 80       	push   $0x8010767b
801023dd:	e8 ae df ff ff       	call   80100390 <panic>
801023e2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801023e9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801023f0 <ideinit>:
{
801023f0:	55                   	push   %ebp
801023f1:	89 e5                	mov    %esp,%ebp
801023f3:	83 ec 10             	sub    $0x10,%esp
  initlock(&idelock, "ide");
801023f6:	68 96 76 10 80       	push   $0x80107696
801023fb:	68 80 a5 10 80       	push   $0x8010a580
80102400:	e8 2b 23 00 00       	call   80104730 <initlock>
  ioapicenable(IRQ_IDE, ncpu - 1);
80102405:	58                   	pop    %eax
80102406:	a1 00 2d 11 80       	mov    0x80112d00,%eax
8010240b:	5a                   	pop    %edx
8010240c:	83 e8 01             	sub    $0x1,%eax
8010240f:	50                   	push   %eax
80102410:	6a 0e                	push   $0xe
80102412:	e8 a9 02 00 00       	call   801026c0 <ioapicenable>
80102417:	83 c4 10             	add    $0x10,%esp
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
8010241a:	ba f7 01 00 00       	mov    $0x1f7,%edx
8010241f:	90                   	nop
80102420:	ec                   	in     (%dx),%al
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
80102421:	83 e0 c0             	and    $0xffffffc0,%eax
80102424:	3c 40                	cmp    $0x40,%al
80102426:	75 f8                	jne    80102420 <ideinit+0x30>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102428:	b8 f0 ff ff ff       	mov    $0xfffffff0,%eax
8010242d:	ba f6 01 00 00       	mov    $0x1f6,%edx
80102432:	ee                   	out    %al,(%dx)
80102433:	b9 e8 03 00 00       	mov    $0x3e8,%ecx
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102438:	ba f7 01 00 00       	mov    $0x1f7,%edx
8010243d:	eb 06                	jmp    80102445 <ideinit+0x55>
8010243f:	90                   	nop
  for(i=0; i<1000; i++){
80102440:	83 e9 01             	sub    $0x1,%ecx
80102443:	74 0f                	je     80102454 <ideinit+0x64>
80102445:	ec                   	in     (%dx),%al
    if(inb(0x1f7) != 0){
80102446:	84 c0                	test   %al,%al
80102448:	74 f6                	je     80102440 <ideinit+0x50>
      havedisk1 = 1;
8010244a:	c7 05 60 a5 10 80 01 	movl   $0x1,0x8010a560
80102451:	00 00 00 
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102454:	b8 e0 ff ff ff       	mov    $0xffffffe0,%eax
80102459:	ba f6 01 00 00       	mov    $0x1f6,%edx
8010245e:	ee                   	out    %al,(%dx)
}
8010245f:	c9                   	leave  
80102460:	c3                   	ret    
80102461:	eb 0d                	jmp    80102470 <ideintr>
80102463:	90                   	nop
80102464:	90                   	nop
80102465:	90                   	nop
80102466:	90                   	nop
80102467:	90                   	nop
80102468:	90                   	nop
80102469:	90                   	nop
8010246a:	90                   	nop
8010246b:	90                   	nop
8010246c:	90                   	nop
8010246d:	90                   	nop
8010246e:	90                   	nop
8010246f:	90                   	nop

80102470 <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
80102470:	55                   	push   %ebp
80102471:	89 e5                	mov    %esp,%ebp
80102473:	57                   	push   %edi
80102474:	56                   	push   %esi
80102475:	53                   	push   %ebx
80102476:	83 ec 18             	sub    $0x18,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
80102479:	68 80 a5 10 80       	push   $0x8010a580
8010247e:	e8 9d 23 00 00       	call   80104820 <acquire>

  if((b = idequeue) == 0){
80102483:	8b 1d 64 a5 10 80    	mov    0x8010a564,%ebx
80102489:	83 c4 10             	add    $0x10,%esp
8010248c:	85 db                	test   %ebx,%ebx
8010248e:	74 67                	je     801024f7 <ideintr+0x87>
    release(&idelock);
    return;
  }
  idequeue = b->qnext;
80102490:	8b 43 58             	mov    0x58(%ebx),%eax
80102493:	a3 64 a5 10 80       	mov    %eax,0x8010a564

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
80102498:	8b 3b                	mov    (%ebx),%edi
8010249a:	f7 c7 04 00 00 00    	test   $0x4,%edi
801024a0:	75 31                	jne    801024d3 <ideintr+0x63>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801024a2:	ba f7 01 00 00       	mov    $0x1f7,%edx
801024a7:	89 f6                	mov    %esi,%esi
801024a9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
801024b0:	ec                   	in     (%dx),%al
  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
801024b1:	89 c6                	mov    %eax,%esi
801024b3:	83 e6 c0             	and    $0xffffffc0,%esi
801024b6:	89 f1                	mov    %esi,%ecx
801024b8:	80 f9 40             	cmp    $0x40,%cl
801024bb:	75 f3                	jne    801024b0 <ideintr+0x40>
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
801024bd:	a8 21                	test   $0x21,%al
801024bf:	75 12                	jne    801024d3 <ideintr+0x63>
    insl(0x1f0, b->data, BSIZE/4);
801024c1:	8d 7b 5c             	lea    0x5c(%ebx),%edi
  asm volatile("cld; rep insl" :
801024c4:	b9 80 00 00 00       	mov    $0x80,%ecx
801024c9:	ba f0 01 00 00       	mov    $0x1f0,%edx
801024ce:	fc                   	cld    
801024cf:	f3 6d                	rep insl (%dx),%es:(%edi)
801024d1:	8b 3b                	mov    (%ebx),%edi

  // Wake process waiting for this buf.
  b->flags |= B_VALID;
  b->flags &= ~B_DIRTY;
801024d3:	83 e7 fb             	and    $0xfffffffb,%edi
  wakeup(b);
801024d6:	83 ec 0c             	sub    $0xc,%esp
  b->flags &= ~B_DIRTY;
801024d9:	89 f9                	mov    %edi,%ecx
801024db:	83 c9 02             	or     $0x2,%ecx
801024de:	89 0b                	mov    %ecx,(%ebx)
  wakeup(b);
801024e0:	53                   	push   %ebx
801024e1:	e8 6a 1f 00 00       	call   80104450 <wakeup>

  // Start disk on next buf in queue.
  if(idequeue != 0)
801024e6:	a1 64 a5 10 80       	mov    0x8010a564,%eax
801024eb:	83 c4 10             	add    $0x10,%esp
801024ee:	85 c0                	test   %eax,%eax
801024f0:	74 05                	je     801024f7 <ideintr+0x87>
    idestart(idequeue);
801024f2:	e8 19 fe ff ff       	call   80102310 <idestart>
    release(&idelock);
801024f7:	83 ec 0c             	sub    $0xc,%esp
801024fa:	68 80 a5 10 80       	push   $0x8010a580
801024ff:	e8 3c 24 00 00       	call   80104940 <release>

  release(&idelock);
}
80102504:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102507:	5b                   	pop    %ebx
80102508:	5e                   	pop    %esi
80102509:	5f                   	pop    %edi
8010250a:	5d                   	pop    %ebp
8010250b:	c3                   	ret    
8010250c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80102510 <iderw>:
// Sync buf with disk.
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
80102510:	55                   	push   %ebp
80102511:	89 e5                	mov    %esp,%ebp
80102513:	53                   	push   %ebx
80102514:	83 ec 10             	sub    $0x10,%esp
80102517:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct buf **pp;

  if(!holdingsleep(&b->lock))
8010251a:	8d 43 0c             	lea    0xc(%ebx),%eax
8010251d:	50                   	push   %eax
8010251e:	e8 dd 21 00 00       	call   80104700 <holdingsleep>
80102523:	83 c4 10             	add    $0x10,%esp
80102526:	85 c0                	test   %eax,%eax
80102528:	0f 84 c6 00 00 00    	je     801025f4 <iderw+0xe4>
    panic("iderw: buf not locked");
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
8010252e:	8b 03                	mov    (%ebx),%eax
80102530:	83 e0 06             	and    $0x6,%eax
80102533:	83 f8 02             	cmp    $0x2,%eax
80102536:	0f 84 ab 00 00 00    	je     801025e7 <iderw+0xd7>
    panic("iderw: nothing to do");
  if(b->dev != 0 && !havedisk1)
8010253c:	8b 53 04             	mov    0x4(%ebx),%edx
8010253f:	85 d2                	test   %edx,%edx
80102541:	74 0d                	je     80102550 <iderw+0x40>
80102543:	a1 60 a5 10 80       	mov    0x8010a560,%eax
80102548:	85 c0                	test   %eax,%eax
8010254a:	0f 84 b1 00 00 00    	je     80102601 <iderw+0xf1>
    panic("iderw: ide disk 1 not present");

  acquire(&idelock);  //DOC:acquire-lock
80102550:	83 ec 0c             	sub    $0xc,%esp
80102553:	68 80 a5 10 80       	push   $0x8010a580
80102558:	e8 c3 22 00 00       	call   80104820 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
8010255d:	8b 15 64 a5 10 80    	mov    0x8010a564,%edx
80102563:	83 c4 10             	add    $0x10,%esp
  b->qnext = 0;
80102566:	c7 43 58 00 00 00 00 	movl   $0x0,0x58(%ebx)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
8010256d:	85 d2                	test   %edx,%edx
8010256f:	75 09                	jne    8010257a <iderw+0x6a>
80102571:	eb 6d                	jmp    801025e0 <iderw+0xd0>
80102573:	90                   	nop
80102574:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102578:	89 c2                	mov    %eax,%edx
8010257a:	8b 42 58             	mov    0x58(%edx),%eax
8010257d:	85 c0                	test   %eax,%eax
8010257f:	75 f7                	jne    80102578 <iderw+0x68>
80102581:	83 c2 58             	add    $0x58,%edx
    ;
  *pp = b;
80102584:	89 1a                	mov    %ebx,(%edx)

  // Start disk if necessary.
  if(idequeue == b)
80102586:	39 1d 64 a5 10 80    	cmp    %ebx,0x8010a564
8010258c:	74 42                	je     801025d0 <iderw+0xc0>
    idestart(b);

  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
8010258e:	8b 03                	mov    (%ebx),%eax
80102590:	83 e0 06             	and    $0x6,%eax
80102593:	83 f8 02             	cmp    $0x2,%eax
80102596:	74 23                	je     801025bb <iderw+0xab>
80102598:	90                   	nop
80102599:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    sleep(b, &idelock);
801025a0:	83 ec 08             	sub    $0x8,%esp
801025a3:	68 80 a5 10 80       	push   $0x8010a580
801025a8:	53                   	push   %ebx
801025a9:	e8 c2 1c 00 00       	call   80104270 <sleep>
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
801025ae:	8b 03                	mov    (%ebx),%eax
801025b0:	83 c4 10             	add    $0x10,%esp
801025b3:	83 e0 06             	and    $0x6,%eax
801025b6:	83 f8 02             	cmp    $0x2,%eax
801025b9:	75 e5                	jne    801025a0 <iderw+0x90>
  }


  release(&idelock);
801025bb:	c7 45 08 80 a5 10 80 	movl   $0x8010a580,0x8(%ebp)
}
801025c2:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801025c5:	c9                   	leave  
  release(&idelock);
801025c6:	e9 75 23 00 00       	jmp    80104940 <release>
801025cb:	90                   	nop
801025cc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    idestart(b);
801025d0:	89 d8                	mov    %ebx,%eax
801025d2:	e8 39 fd ff ff       	call   80102310 <idestart>
801025d7:	eb b5                	jmp    8010258e <iderw+0x7e>
801025d9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
801025e0:	ba 64 a5 10 80       	mov    $0x8010a564,%edx
801025e5:	eb 9d                	jmp    80102584 <iderw+0x74>
    panic("iderw: nothing to do");
801025e7:	83 ec 0c             	sub    $0xc,%esp
801025ea:	68 b0 76 10 80       	push   $0x801076b0
801025ef:	e8 9c dd ff ff       	call   80100390 <panic>
    panic("iderw: buf not locked");
801025f4:	83 ec 0c             	sub    $0xc,%esp
801025f7:	68 9a 76 10 80       	push   $0x8010769a
801025fc:	e8 8f dd ff ff       	call   80100390 <panic>
    panic("iderw: ide disk 1 not present");
80102601:	83 ec 0c             	sub    $0xc,%esp
80102604:	68 c5 76 10 80       	push   $0x801076c5
80102609:	e8 82 dd ff ff       	call   80100390 <panic>
8010260e:	66 90                	xchg   %ax,%ax

80102610 <ioapicinit>:
  ioapic->data = data;
}

void
ioapicinit(void)
{
80102610:	55                   	push   %ebp
  int i, id, maxintr;

  ioapic = (volatile struct ioapic*)IOAPIC;
80102611:	c7 05 34 26 11 80 00 	movl   $0xfec00000,0x80112634
80102618:	00 c0 fe 
{
8010261b:	89 e5                	mov    %esp,%ebp
8010261d:	56                   	push   %esi
8010261e:	53                   	push   %ebx
  ioapic->reg = reg;
8010261f:	c7 05 00 00 c0 fe 01 	movl   $0x1,0xfec00000
80102626:	00 00 00 
  return ioapic->data;
80102629:	a1 34 26 11 80       	mov    0x80112634,%eax
8010262e:	8b 58 10             	mov    0x10(%eax),%ebx
  ioapic->reg = reg;
80102631:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  return ioapic->data;
80102637:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
  id = ioapicread(REG_ID) >> 24;
  if(id != ioapicid)
8010263d:	0f b6 15 60 27 11 80 	movzbl 0x80112760,%edx
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
80102644:	c1 eb 10             	shr    $0x10,%ebx
  return ioapic->data;
80102647:	8b 41 10             	mov    0x10(%ecx),%eax
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
8010264a:	0f b6 db             	movzbl %bl,%ebx
  id = ioapicread(REG_ID) >> 24;
8010264d:	c1 e8 18             	shr    $0x18,%eax
  if(id != ioapicid)
80102650:	39 c2                	cmp    %eax,%edx
80102652:	74 16                	je     8010266a <ioapicinit+0x5a>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
80102654:	83 ec 0c             	sub    $0xc,%esp
80102657:	68 e4 76 10 80       	push   $0x801076e4
8010265c:	e8 ff df ff ff       	call   80100660 <cprintf>
80102661:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
80102667:	83 c4 10             	add    $0x10,%esp
8010266a:	83 c3 21             	add    $0x21,%ebx
{
8010266d:	ba 10 00 00 00       	mov    $0x10,%edx
80102672:	b8 20 00 00 00       	mov    $0x20,%eax
80102677:	89 f6                	mov    %esi,%esi
80102679:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
  ioapic->reg = reg;
80102680:	89 11                	mov    %edx,(%ecx)
  ioapic->data = data;
80102682:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
80102688:	89 c6                	mov    %eax,%esi
8010268a:	81 ce 00 00 01 00    	or     $0x10000,%esi
80102690:	83 c0 01             	add    $0x1,%eax
  ioapic->data = data;
80102693:	89 71 10             	mov    %esi,0x10(%ecx)
80102696:	8d 72 01             	lea    0x1(%edx),%esi
80102699:	83 c2 02             	add    $0x2,%edx
  for(i = 0; i <= maxintr; i++){
8010269c:	39 d8                	cmp    %ebx,%eax
  ioapic->reg = reg;
8010269e:	89 31                	mov    %esi,(%ecx)
  ioapic->data = data;
801026a0:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
801026a6:	c7 41 10 00 00 00 00 	movl   $0x0,0x10(%ecx)
  for(i = 0; i <= maxintr; i++){
801026ad:	75 d1                	jne    80102680 <ioapicinit+0x70>
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
801026af:	8d 65 f8             	lea    -0x8(%ebp),%esp
801026b2:	5b                   	pop    %ebx
801026b3:	5e                   	pop    %esi
801026b4:	5d                   	pop    %ebp
801026b5:	c3                   	ret    
801026b6:	8d 76 00             	lea    0x0(%esi),%esi
801026b9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801026c0 <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
801026c0:	55                   	push   %ebp
  ioapic->reg = reg;
801026c1:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
{
801026c7:	89 e5                	mov    %esp,%ebp
801026c9:	8b 45 08             	mov    0x8(%ebp),%eax
  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
801026cc:	8d 50 20             	lea    0x20(%eax),%edx
801026cf:	8d 44 00 10          	lea    0x10(%eax,%eax,1),%eax
  ioapic->reg = reg;
801026d3:	89 01                	mov    %eax,(%ecx)
  ioapic->data = data;
801026d5:	8b 0d 34 26 11 80    	mov    0x80112634,%ecx
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
801026db:	83 c0 01             	add    $0x1,%eax
  ioapic->data = data;
801026de:	89 51 10             	mov    %edx,0x10(%ecx)
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
801026e1:	8b 55 0c             	mov    0xc(%ebp),%edx
  ioapic->reg = reg;
801026e4:	89 01                	mov    %eax,(%ecx)
  ioapic->data = data;
801026e6:	a1 34 26 11 80       	mov    0x80112634,%eax
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
801026eb:	c1 e2 18             	shl    $0x18,%edx
  ioapic->data = data;
801026ee:	89 50 10             	mov    %edx,0x10(%eax)
}
801026f1:	5d                   	pop    %ebp
801026f2:	c3                   	ret    
801026f3:	66 90                	xchg   %ax,%ax
801026f5:	66 90                	xchg   %ax,%ax
801026f7:	66 90                	xchg   %ax,%ax
801026f9:	66 90                	xchg   %ax,%ax
801026fb:	66 90                	xchg   %ax,%ax
801026fd:	66 90                	xchg   %ax,%ax
801026ff:	90                   	nop

80102700 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102700:	55                   	push   %ebp
80102701:	89 e5                	mov    %esp,%ebp
80102703:	53                   	push   %ebx
80102704:	83 ec 04             	sub    $0x4,%esp
80102707:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct run *r;

  if((uint)v % PGSIZE || v < end || V2P(v) >= PHYSTOP)
8010270a:	f7 c3 ff 0f 00 00    	test   $0xfff,%ebx
80102710:	75 70                	jne    80102782 <kfree+0x82>
80102712:	81 fb a8 65 11 80    	cmp    $0x801165a8,%ebx
80102718:	72 68                	jb     80102782 <kfree+0x82>
8010271a:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80102720:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102725:	77 5b                	ja     80102782 <kfree+0x82>
    panic("kfree");

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102727:	83 ec 04             	sub    $0x4,%esp
8010272a:	68 00 10 00 00       	push   $0x1000
8010272f:	6a 01                	push   $0x1
80102731:	53                   	push   %ebx
80102732:	e8 69 22 00 00       	call   801049a0 <memset>

  if(kmem.use_lock)
80102737:	8b 15 74 26 11 80    	mov    0x80112674,%edx
8010273d:	83 c4 10             	add    $0x10,%esp
80102740:	85 d2                	test   %edx,%edx
80102742:	75 2c                	jne    80102770 <kfree+0x70>
    acquire(&kmem.lock);
  r = (struct run*)v;
  r->next = kmem.freelist;
80102744:	a1 78 26 11 80       	mov    0x80112678,%eax
80102749:	89 03                	mov    %eax,(%ebx)
  kmem.freelist = r;
  if(kmem.use_lock)
8010274b:	a1 74 26 11 80       	mov    0x80112674,%eax
  kmem.freelist = r;
80102750:	89 1d 78 26 11 80    	mov    %ebx,0x80112678
  if(kmem.use_lock)
80102756:	85 c0                	test   %eax,%eax
80102758:	75 06                	jne    80102760 <kfree+0x60>
    release(&kmem.lock);
}
8010275a:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010275d:	c9                   	leave  
8010275e:	c3                   	ret    
8010275f:	90                   	nop
    release(&kmem.lock);
80102760:	c7 45 08 40 26 11 80 	movl   $0x80112640,0x8(%ebp)
}
80102767:	8b 5d fc             	mov    -0x4(%ebp),%ebx
8010276a:	c9                   	leave  
    release(&kmem.lock);
8010276b:	e9 d0 21 00 00       	jmp    80104940 <release>
    acquire(&kmem.lock);
80102770:	83 ec 0c             	sub    $0xc,%esp
80102773:	68 40 26 11 80       	push   $0x80112640
80102778:	e8 a3 20 00 00       	call   80104820 <acquire>
8010277d:	83 c4 10             	add    $0x10,%esp
80102780:	eb c2                	jmp    80102744 <kfree+0x44>
    panic("kfree");
80102782:	83 ec 0c             	sub    $0xc,%esp
80102785:	68 16 77 10 80       	push   $0x80107716
8010278a:	e8 01 dc ff ff       	call   80100390 <panic>
8010278f:	90                   	nop

80102790 <freerange>:
{
80102790:	55                   	push   %ebp
80102791:	89 e5                	mov    %esp,%ebp
80102793:	56                   	push   %esi
80102794:	53                   	push   %ebx
  p = (char*)PGROUNDUP((uint)vstart);
80102795:	8b 45 08             	mov    0x8(%ebp),%eax
{
80102798:	8b 75 0c             	mov    0xc(%ebp),%esi
  p = (char*)PGROUNDUP((uint)vstart);
8010279b:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
801027a1:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
801027a7:	81 c3 00 10 00 00    	add    $0x1000,%ebx
801027ad:	39 de                	cmp    %ebx,%esi
801027af:	72 23                	jb     801027d4 <freerange+0x44>
801027b1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    kfree(p);
801027b8:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
801027be:	83 ec 0c             	sub    $0xc,%esp
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
801027c1:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
801027c7:	50                   	push   %eax
801027c8:	e8 33 ff ff ff       	call   80102700 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
801027cd:	83 c4 10             	add    $0x10,%esp
801027d0:	39 f3                	cmp    %esi,%ebx
801027d2:	76 e4                	jbe    801027b8 <freerange+0x28>
}
801027d4:	8d 65 f8             	lea    -0x8(%ebp),%esp
801027d7:	5b                   	pop    %ebx
801027d8:	5e                   	pop    %esi
801027d9:	5d                   	pop    %ebp
801027da:	c3                   	ret    
801027db:	90                   	nop
801027dc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

801027e0 <kinit1>:
{
801027e0:	55                   	push   %ebp
801027e1:	89 e5                	mov    %esp,%ebp
801027e3:	56                   	push   %esi
801027e4:	53                   	push   %ebx
801027e5:	8b 75 0c             	mov    0xc(%ebp),%esi
  initlock(&kmem.lock, "kmem");
801027e8:	83 ec 08             	sub    $0x8,%esp
801027eb:	68 1c 77 10 80       	push   $0x8010771c
801027f0:	68 40 26 11 80       	push   $0x80112640
801027f5:	e8 36 1f 00 00       	call   80104730 <initlock>
  p = (char*)PGROUNDUP((uint)vstart);
801027fa:	8b 45 08             	mov    0x8(%ebp),%eax
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
801027fd:	83 c4 10             	add    $0x10,%esp
  kmem.use_lock = 0;
80102800:	c7 05 74 26 11 80 00 	movl   $0x0,0x80112674
80102807:	00 00 00 
  p = (char*)PGROUNDUP((uint)vstart);
8010280a:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
80102810:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102816:	81 c3 00 10 00 00    	add    $0x1000,%ebx
8010281c:	39 de                	cmp    %ebx,%esi
8010281e:	72 1c                	jb     8010283c <kinit1+0x5c>
    kfree(p);
80102820:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
80102826:	83 ec 0c             	sub    $0xc,%esp
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102829:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
8010282f:	50                   	push   %eax
80102830:	e8 cb fe ff ff       	call   80102700 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102835:	83 c4 10             	add    $0x10,%esp
80102838:	39 de                	cmp    %ebx,%esi
8010283a:	73 e4                	jae    80102820 <kinit1+0x40>
}
8010283c:	8d 65 f8             	lea    -0x8(%ebp),%esp
8010283f:	5b                   	pop    %ebx
80102840:	5e                   	pop    %esi
80102841:	5d                   	pop    %ebp
80102842:	c3                   	ret    
80102843:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80102849:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80102850 <kinit2>:
{
80102850:	55                   	push   %ebp
80102851:	89 e5                	mov    %esp,%ebp
80102853:	56                   	push   %esi
80102854:	53                   	push   %ebx
  p = (char*)PGROUNDUP((uint)vstart);
80102855:	8b 45 08             	mov    0x8(%ebp),%eax
{
80102858:	8b 75 0c             	mov    0xc(%ebp),%esi
  p = (char*)PGROUNDUP((uint)vstart);
8010285b:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
80102861:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102867:	81 c3 00 10 00 00    	add    $0x1000,%ebx
8010286d:	39 de                	cmp    %ebx,%esi
8010286f:	72 23                	jb     80102894 <kinit2+0x44>
80102871:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    kfree(p);
80102878:	8d 83 00 f0 ff ff    	lea    -0x1000(%ebx),%eax
8010287e:	83 ec 0c             	sub    $0xc,%esp
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102881:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    kfree(p);
80102887:	50                   	push   %eax
80102888:	e8 73 fe ff ff       	call   80102700 <kfree>
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
8010288d:	83 c4 10             	add    $0x10,%esp
80102890:	39 de                	cmp    %ebx,%esi
80102892:	73 e4                	jae    80102878 <kinit2+0x28>
  kmem.use_lock = 1;
80102894:	c7 05 74 26 11 80 01 	movl   $0x1,0x80112674
8010289b:	00 00 00 
}
8010289e:	8d 65 f8             	lea    -0x8(%ebp),%esp
801028a1:	5b                   	pop    %ebx
801028a2:	5e                   	pop    %esi
801028a3:	5d                   	pop    %ebp
801028a4:	c3                   	ret    
801028a5:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801028a9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801028b0 <kalloc>:
char*
kalloc(void)
{
  struct run *r;

  if(kmem.use_lock)
801028b0:	a1 74 26 11 80       	mov    0x80112674,%eax
801028b5:	85 c0                	test   %eax,%eax
801028b7:	75 1f                	jne    801028d8 <kalloc+0x28>
    acquire(&kmem.lock);
  r = kmem.freelist;
801028b9:	a1 78 26 11 80       	mov    0x80112678,%eax
  if(r)
801028be:	85 c0                	test   %eax,%eax
801028c0:	74 0e                	je     801028d0 <kalloc+0x20>
    kmem.freelist = r->next;
801028c2:	8b 10                	mov    (%eax),%edx
801028c4:	89 15 78 26 11 80    	mov    %edx,0x80112678
801028ca:	c3                   	ret    
801028cb:	90                   	nop
801028cc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  if(kmem.use_lock)
    release(&kmem.lock);
  return (char*)r;
}
801028d0:	f3 c3                	repz ret 
801028d2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
{
801028d8:	55                   	push   %ebp
801028d9:	89 e5                	mov    %esp,%ebp
801028db:	83 ec 24             	sub    $0x24,%esp
    acquire(&kmem.lock);
801028de:	68 40 26 11 80       	push   $0x80112640
801028e3:	e8 38 1f 00 00       	call   80104820 <acquire>
  r = kmem.freelist;
801028e8:	a1 78 26 11 80       	mov    0x80112678,%eax
  if(r)
801028ed:	83 c4 10             	add    $0x10,%esp
801028f0:	8b 15 74 26 11 80    	mov    0x80112674,%edx
801028f6:	85 c0                	test   %eax,%eax
801028f8:	74 08                	je     80102902 <kalloc+0x52>
    kmem.freelist = r->next;
801028fa:	8b 08                	mov    (%eax),%ecx
801028fc:	89 0d 78 26 11 80    	mov    %ecx,0x80112678
  if(kmem.use_lock)
80102902:	85 d2                	test   %edx,%edx
80102904:	74 16                	je     8010291c <kalloc+0x6c>
    release(&kmem.lock);
80102906:	83 ec 0c             	sub    $0xc,%esp
80102909:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010290c:	68 40 26 11 80       	push   $0x80112640
80102911:	e8 2a 20 00 00       	call   80104940 <release>
  return (char*)r;
80102916:	8b 45 f4             	mov    -0xc(%ebp),%eax
    release(&kmem.lock);
80102919:	83 c4 10             	add    $0x10,%esp
}
8010291c:	c9                   	leave  
8010291d:	c3                   	ret    
8010291e:	66 90                	xchg   %ax,%ax

80102920 <kbdgetc>:
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102920:	ba 64 00 00 00       	mov    $0x64,%edx
80102925:	ec                   	in     (%dx),%al
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
  if((st & KBS_DIB) == 0)
80102926:	a8 01                	test   $0x1,%al
80102928:	0f 84 c2 00 00 00    	je     801029f0 <kbdgetc+0xd0>
8010292e:	ba 60 00 00 00       	mov    $0x60,%edx
80102933:	ec                   	in     (%dx),%al
    return -1;
  data = inb(KBDATAP);
80102934:	0f b6 d0             	movzbl %al,%edx
80102937:	8b 0d b4 a5 10 80    	mov    0x8010a5b4,%ecx

  if(data == 0xE0){
8010293d:	81 fa e0 00 00 00    	cmp    $0xe0,%edx
80102943:	0f 84 7f 00 00 00    	je     801029c8 <kbdgetc+0xa8>
{
80102949:	55                   	push   %ebp
8010294a:	89 e5                	mov    %esp,%ebp
8010294c:	53                   	push   %ebx
8010294d:	89 cb                	mov    %ecx,%ebx
8010294f:	83 e3 40             	and    $0x40,%ebx
    shift |= E0ESC;
    return 0;
  } else if(data & 0x80){
80102952:	84 c0                	test   %al,%al
80102954:	78 4a                	js     801029a0 <kbdgetc+0x80>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
    shift &= ~(shiftcode[data] | E0ESC);
    return 0;
  } else if(shift & E0ESC){
80102956:	85 db                	test   %ebx,%ebx
80102958:	74 09                	je     80102963 <kbdgetc+0x43>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
8010295a:	83 c8 80             	or     $0xffffff80,%eax
    shift &= ~E0ESC;
8010295d:	83 e1 bf             	and    $0xffffffbf,%ecx
    data |= 0x80;
80102960:	0f b6 d0             	movzbl %al,%edx
  }

  shift |= shiftcode[data];
80102963:	0f b6 82 60 78 10 80 	movzbl -0x7fef87a0(%edx),%eax
8010296a:	09 c1                	or     %eax,%ecx
  shift ^= togglecode[data];
8010296c:	0f b6 82 60 77 10 80 	movzbl -0x7fef88a0(%edx),%eax
80102973:	31 c1                	xor    %eax,%ecx
  c = charcode[shift & (CTL | SHIFT)][data];
80102975:	89 c8                	mov    %ecx,%eax
  shift ^= togglecode[data];
80102977:	89 0d b4 a5 10 80    	mov    %ecx,0x8010a5b4
  c = charcode[shift & (CTL | SHIFT)][data];
8010297d:	83 e0 03             	and    $0x3,%eax
  if(shift & CAPSLOCK){
80102980:	83 e1 08             	and    $0x8,%ecx
  c = charcode[shift & (CTL | SHIFT)][data];
80102983:	8b 04 85 40 77 10 80 	mov    -0x7fef88c0(,%eax,4),%eax
8010298a:	0f b6 04 10          	movzbl (%eax,%edx,1),%eax
  if(shift & CAPSLOCK){
8010298e:	74 31                	je     801029c1 <kbdgetc+0xa1>
    if('a' <= c && c <= 'z')
80102990:	8d 50 9f             	lea    -0x61(%eax),%edx
80102993:	83 fa 19             	cmp    $0x19,%edx
80102996:	77 40                	ja     801029d8 <kbdgetc+0xb8>
      c += 'A' - 'a';
80102998:	83 e8 20             	sub    $0x20,%eax
    else if('A' <= c && c <= 'Z')
      c += 'a' - 'A';
  }
  return c;
}
8010299b:	5b                   	pop    %ebx
8010299c:	5d                   	pop    %ebp
8010299d:	c3                   	ret    
8010299e:	66 90                	xchg   %ax,%ax
    data = (shift & E0ESC ? data : data & 0x7F);
801029a0:	83 e0 7f             	and    $0x7f,%eax
801029a3:	85 db                	test   %ebx,%ebx
801029a5:	0f 44 d0             	cmove  %eax,%edx
    shift &= ~(shiftcode[data] | E0ESC);
801029a8:	0f b6 82 60 78 10 80 	movzbl -0x7fef87a0(%edx),%eax
801029af:	83 c8 40             	or     $0x40,%eax
801029b2:	0f b6 c0             	movzbl %al,%eax
801029b5:	f7 d0                	not    %eax
801029b7:	21 c1                	and    %eax,%ecx
    return 0;
801029b9:	31 c0                	xor    %eax,%eax
    shift &= ~(shiftcode[data] | E0ESC);
801029bb:	89 0d b4 a5 10 80    	mov    %ecx,0x8010a5b4
}
801029c1:	5b                   	pop    %ebx
801029c2:	5d                   	pop    %ebp
801029c3:	c3                   	ret    
801029c4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    shift |= E0ESC;
801029c8:	83 c9 40             	or     $0x40,%ecx
    return 0;
801029cb:	31 c0                	xor    %eax,%eax
    shift |= E0ESC;
801029cd:	89 0d b4 a5 10 80    	mov    %ecx,0x8010a5b4
    return 0;
801029d3:	c3                   	ret    
801029d4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    else if('A' <= c && c <= 'Z')
801029d8:	8d 48 bf             	lea    -0x41(%eax),%ecx
      c += 'a' - 'A';
801029db:	8d 50 20             	lea    0x20(%eax),%edx
}
801029de:	5b                   	pop    %ebx
      c += 'a' - 'A';
801029df:	83 f9 1a             	cmp    $0x1a,%ecx
801029e2:	0f 42 c2             	cmovb  %edx,%eax
}
801029e5:	5d                   	pop    %ebp
801029e6:	c3                   	ret    
801029e7:	89 f6                	mov    %esi,%esi
801029e9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    return -1;
801029f0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801029f5:	c3                   	ret    
801029f6:	8d 76 00             	lea    0x0(%esi),%esi
801029f9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80102a00 <kbdintr>:

void
kbdintr(void)
{
80102a00:	55                   	push   %ebp
80102a01:	89 e5                	mov    %esp,%ebp
80102a03:	83 ec 14             	sub    $0x14,%esp
  consoleintr(kbdgetc);
80102a06:	68 20 29 10 80       	push   $0x80102920
80102a0b:	e8 00 de ff ff       	call   80100810 <consoleintr>
}
80102a10:	83 c4 10             	add    $0x10,%esp
80102a13:	c9                   	leave  
80102a14:	c3                   	ret    
80102a15:	66 90                	xchg   %ax,%ax
80102a17:	66 90                	xchg   %ax,%ax
80102a19:	66 90                	xchg   %ax,%ax
80102a1b:	66 90                	xchg   %ax,%ax
80102a1d:	66 90                	xchg   %ax,%ax
80102a1f:	90                   	nop

80102a20 <lapicinit>:
}

void
lapicinit(void)
{
  if(!lapic)
80102a20:	a1 7c 26 11 80       	mov    0x8011267c,%eax
{
80102a25:	55                   	push   %ebp
80102a26:	89 e5                	mov    %esp,%ebp
  if(!lapic)
80102a28:	85 c0                	test   %eax,%eax
80102a2a:	0f 84 c8 00 00 00    	je     80102af8 <lapicinit+0xd8>
  lapic[index] = value;
80102a30:	c7 80 f0 00 00 00 3f 	movl   $0x13f,0xf0(%eax)
80102a37:	01 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102a3a:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102a3d:	c7 80 e0 03 00 00 0b 	movl   $0xb,0x3e0(%eax)
80102a44:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102a47:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102a4a:	c7 80 20 03 00 00 20 	movl   $0x20020,0x320(%eax)
80102a51:	00 02 00 
  lapic[ID];  // wait for write to finish, by reading
80102a54:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102a57:	c7 80 80 03 00 00 80 	movl   $0x989680,0x380(%eax)
80102a5e:	96 98 00 
  lapic[ID];  // wait for write to finish, by reading
80102a61:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102a64:	c7 80 50 03 00 00 00 	movl   $0x10000,0x350(%eax)
80102a6b:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
80102a6e:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102a71:	c7 80 60 03 00 00 00 	movl   $0x10000,0x360(%eax)
80102a78:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
80102a7b:	8b 50 20             	mov    0x20(%eax),%edx
  lapicw(LINT0, MASKED);
  lapicw(LINT1, MASKED);

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102a7e:	8b 50 30             	mov    0x30(%eax),%edx
80102a81:	c1 ea 10             	shr    $0x10,%edx
80102a84:	80 fa 03             	cmp    $0x3,%dl
80102a87:	77 77                	ja     80102b00 <lapicinit+0xe0>
  lapic[index] = value;
80102a89:	c7 80 70 03 00 00 33 	movl   $0x33,0x370(%eax)
80102a90:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102a93:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102a96:	c7 80 80 02 00 00 00 	movl   $0x0,0x280(%eax)
80102a9d:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102aa0:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102aa3:	c7 80 80 02 00 00 00 	movl   $0x0,0x280(%eax)
80102aaa:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102aad:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102ab0:	c7 80 b0 00 00 00 00 	movl   $0x0,0xb0(%eax)
80102ab7:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102aba:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102abd:	c7 80 10 03 00 00 00 	movl   $0x0,0x310(%eax)
80102ac4:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102ac7:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102aca:	c7 80 00 03 00 00 00 	movl   $0x88500,0x300(%eax)
80102ad1:	85 08 00 
  lapic[ID];  // wait for write to finish, by reading
80102ad4:	8b 50 20             	mov    0x20(%eax),%edx
80102ad7:	89 f6                	mov    %esi,%esi
80102ad9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
  lapicw(EOI, 0);

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
  lapicw(ICRLO, BCAST | INIT | LEVEL);
  while(lapic[ICRLO] & DELIVS)
80102ae0:	8b 90 00 03 00 00    	mov    0x300(%eax),%edx
80102ae6:	80 e6 10             	and    $0x10,%dh
80102ae9:	75 f5                	jne    80102ae0 <lapicinit+0xc0>
  lapic[index] = value;
80102aeb:	c7 80 80 00 00 00 00 	movl   $0x0,0x80(%eax)
80102af2:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102af5:	8b 40 20             	mov    0x20(%eax),%eax
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
}
80102af8:	5d                   	pop    %ebp
80102af9:	c3                   	ret    
80102afa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  lapic[index] = value;
80102b00:	c7 80 40 03 00 00 00 	movl   $0x10000,0x340(%eax)
80102b07:	00 01 00 
  lapic[ID];  // wait for write to finish, by reading
80102b0a:	8b 50 20             	mov    0x20(%eax),%edx
80102b0d:	e9 77 ff ff ff       	jmp    80102a89 <lapicinit+0x69>
80102b12:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80102b19:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80102b20 <lapicid>:

int
lapicid(void)
{
  if (!lapic)
80102b20:	8b 15 7c 26 11 80    	mov    0x8011267c,%edx
{
80102b26:	55                   	push   %ebp
80102b27:	31 c0                	xor    %eax,%eax
80102b29:	89 e5                	mov    %esp,%ebp
  if (!lapic)
80102b2b:	85 d2                	test   %edx,%edx
80102b2d:	74 06                	je     80102b35 <lapicid+0x15>
    return 0;
  return lapic[ID] >> 24;
80102b2f:	8b 42 20             	mov    0x20(%edx),%eax
80102b32:	c1 e8 18             	shr    $0x18,%eax
}
80102b35:	5d                   	pop    %ebp
80102b36:	c3                   	ret    
80102b37:	89 f6                	mov    %esi,%esi
80102b39:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80102b40 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
  if(lapic)
80102b40:	a1 7c 26 11 80       	mov    0x8011267c,%eax
{
80102b45:	55                   	push   %ebp
80102b46:	89 e5                	mov    %esp,%ebp
  if(lapic)
80102b48:	85 c0                	test   %eax,%eax
80102b4a:	74 0d                	je     80102b59 <lapiceoi+0x19>
  lapic[index] = value;
80102b4c:	c7 80 b0 00 00 00 00 	movl   $0x0,0xb0(%eax)
80102b53:	00 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102b56:	8b 40 20             	mov    0x20(%eax),%eax
    lapicw(EOI, 0);
}
80102b59:	5d                   	pop    %ebp
80102b5a:	c3                   	ret    
80102b5b:	90                   	nop
80102b5c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80102b60 <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80102b60:	55                   	push   %ebp
80102b61:	89 e5                	mov    %esp,%ebp
}
80102b63:	5d                   	pop    %ebp
80102b64:	c3                   	ret    
80102b65:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102b69:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80102b70 <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
80102b70:	55                   	push   %ebp
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102b71:	b8 0f 00 00 00       	mov    $0xf,%eax
80102b76:	ba 70 00 00 00       	mov    $0x70,%edx
80102b7b:	89 e5                	mov    %esp,%ebp
80102b7d:	53                   	push   %ebx
80102b7e:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102b81:	8b 5d 08             	mov    0x8(%ebp),%ebx
80102b84:	ee                   	out    %al,(%dx)
80102b85:	b8 0a 00 00 00       	mov    $0xa,%eax
80102b8a:	ba 71 00 00 00       	mov    $0x71,%edx
80102b8f:	ee                   	out    %al,(%dx)
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(CMOS_PORT, 0xF);  // offset 0xF is shutdown code
  outb(CMOS_PORT+1, 0x0A);
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
  wrv[0] = 0;
80102b90:	31 c0                	xor    %eax,%eax
  wrv[1] = addr >> 4;

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102b92:	c1 e3 18             	shl    $0x18,%ebx
  wrv[0] = 0;
80102b95:	66 a3 67 04 00 80    	mov    %ax,0x80000467
  wrv[1] = addr >> 4;
80102b9b:	89 c8                	mov    %ecx,%eax
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
80102b9d:	c1 e9 0c             	shr    $0xc,%ecx
  wrv[1] = addr >> 4;
80102ba0:	c1 e8 04             	shr    $0x4,%eax
  lapicw(ICRHI, apicid<<24);
80102ba3:	89 da                	mov    %ebx,%edx
    lapicw(ICRLO, STARTUP | (addr>>12));
80102ba5:	80 cd 06             	or     $0x6,%ch
  wrv[1] = addr >> 4;
80102ba8:	66 a3 69 04 00 80    	mov    %ax,0x80000469
  lapic[index] = value;
80102bae:	a1 7c 26 11 80       	mov    0x8011267c,%eax
80102bb3:	89 98 10 03 00 00    	mov    %ebx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102bb9:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102bbc:	c7 80 00 03 00 00 00 	movl   $0xc500,0x300(%eax)
80102bc3:	c5 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102bc6:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102bc9:	c7 80 00 03 00 00 00 	movl   $0x8500,0x300(%eax)
80102bd0:	85 00 00 
  lapic[ID];  // wait for write to finish, by reading
80102bd3:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102bd6:	89 90 10 03 00 00    	mov    %edx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102bdc:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102bdf:	89 88 00 03 00 00    	mov    %ecx,0x300(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102be5:	8b 58 20             	mov    0x20(%eax),%ebx
  lapic[index] = value;
80102be8:	89 90 10 03 00 00    	mov    %edx,0x310(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102bee:	8b 50 20             	mov    0x20(%eax),%edx
  lapic[index] = value;
80102bf1:	89 88 00 03 00 00    	mov    %ecx,0x300(%eax)
  lapic[ID];  // wait for write to finish, by reading
80102bf7:	8b 40 20             	mov    0x20(%eax),%eax
    microdelay(200);
  }
}
80102bfa:	5b                   	pop    %ebx
80102bfb:	5d                   	pop    %ebp
80102bfc:	c3                   	ret    
80102bfd:	8d 76 00             	lea    0x0(%esi),%esi

80102c00 <cmostime>:
  r->year   = cmos_read(YEAR);
}

// qemu seems to use 24-hour GWT and the values are BCD encoded
void cmostime(struct rtcdate *r)
{
80102c00:	55                   	push   %ebp
80102c01:	b8 0b 00 00 00       	mov    $0xb,%eax
80102c06:	ba 70 00 00 00       	mov    $0x70,%edx
80102c0b:	89 e5                	mov    %esp,%ebp
80102c0d:	57                   	push   %edi
80102c0e:	56                   	push   %esi
80102c0f:	53                   	push   %ebx
80102c10:	83 ec 4c             	sub    $0x4c,%esp
80102c13:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102c14:	ba 71 00 00 00       	mov    $0x71,%edx
80102c19:	ec                   	in     (%dx),%al
80102c1a:	83 e0 04             	and    $0x4,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102c1d:	bb 70 00 00 00       	mov    $0x70,%ebx
80102c22:	88 45 b3             	mov    %al,-0x4d(%ebp)
80102c25:	8d 76 00             	lea    0x0(%esi),%esi
80102c28:	31 c0                	xor    %eax,%eax
80102c2a:	89 da                	mov    %ebx,%edx
80102c2c:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102c2d:	b9 71 00 00 00       	mov    $0x71,%ecx
80102c32:	89 ca                	mov    %ecx,%edx
80102c34:	ec                   	in     (%dx),%al
80102c35:	88 45 b7             	mov    %al,-0x49(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102c38:	89 da                	mov    %ebx,%edx
80102c3a:	b8 02 00 00 00       	mov    $0x2,%eax
80102c3f:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102c40:	89 ca                	mov    %ecx,%edx
80102c42:	ec                   	in     (%dx),%al
80102c43:	88 45 b6             	mov    %al,-0x4a(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102c46:	89 da                	mov    %ebx,%edx
80102c48:	b8 04 00 00 00       	mov    $0x4,%eax
80102c4d:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102c4e:	89 ca                	mov    %ecx,%edx
80102c50:	ec                   	in     (%dx),%al
80102c51:	88 45 b5             	mov    %al,-0x4b(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102c54:	89 da                	mov    %ebx,%edx
80102c56:	b8 07 00 00 00       	mov    $0x7,%eax
80102c5b:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102c5c:	89 ca                	mov    %ecx,%edx
80102c5e:	ec                   	in     (%dx),%al
80102c5f:	88 45 b4             	mov    %al,-0x4c(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102c62:	89 da                	mov    %ebx,%edx
80102c64:	b8 08 00 00 00       	mov    $0x8,%eax
80102c69:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102c6a:	89 ca                	mov    %ecx,%edx
80102c6c:	ec                   	in     (%dx),%al
80102c6d:	89 c7                	mov    %eax,%edi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102c6f:	89 da                	mov    %ebx,%edx
80102c71:	b8 09 00 00 00       	mov    $0x9,%eax
80102c76:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102c77:	89 ca                	mov    %ecx,%edx
80102c79:	ec                   	in     (%dx),%al
80102c7a:	89 c6                	mov    %eax,%esi
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102c7c:	89 da                	mov    %ebx,%edx
80102c7e:	b8 0a 00 00 00       	mov    $0xa,%eax
80102c83:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102c84:	89 ca                	mov    %ecx,%edx
80102c86:	ec                   	in     (%dx),%al
  bcd = (sb & (1 << 2)) == 0;

  // make sure CMOS doesn't modify time while we read it
  for(;;) {
    fill_rtcdate(&t1);
    if(cmos_read(CMOS_STATA) & CMOS_UIP)
80102c87:	84 c0                	test   %al,%al
80102c89:	78 9d                	js     80102c28 <cmostime+0x28>
  return inb(CMOS_RETURN);
80102c8b:	0f b6 45 b7          	movzbl -0x49(%ebp),%eax
80102c8f:	89 fa                	mov    %edi,%edx
80102c91:	0f b6 fa             	movzbl %dl,%edi
80102c94:	89 f2                	mov    %esi,%edx
80102c96:	0f b6 f2             	movzbl %dl,%esi
80102c99:	89 7d c8             	mov    %edi,-0x38(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102c9c:	89 da                	mov    %ebx,%edx
80102c9e:	89 75 cc             	mov    %esi,-0x34(%ebp)
80102ca1:	89 45 b8             	mov    %eax,-0x48(%ebp)
80102ca4:	0f b6 45 b6          	movzbl -0x4a(%ebp),%eax
80102ca8:	89 45 bc             	mov    %eax,-0x44(%ebp)
80102cab:	0f b6 45 b5          	movzbl -0x4b(%ebp),%eax
80102caf:	89 45 c0             	mov    %eax,-0x40(%ebp)
80102cb2:	0f b6 45 b4          	movzbl -0x4c(%ebp),%eax
80102cb6:	89 45 c4             	mov    %eax,-0x3c(%ebp)
80102cb9:	31 c0                	xor    %eax,%eax
80102cbb:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102cbc:	89 ca                	mov    %ecx,%edx
80102cbe:	ec                   	in     (%dx),%al
80102cbf:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102cc2:	89 da                	mov    %ebx,%edx
80102cc4:	89 45 d0             	mov    %eax,-0x30(%ebp)
80102cc7:	b8 02 00 00 00       	mov    $0x2,%eax
80102ccc:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102ccd:	89 ca                	mov    %ecx,%edx
80102ccf:	ec                   	in     (%dx),%al
80102cd0:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102cd3:	89 da                	mov    %ebx,%edx
80102cd5:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80102cd8:	b8 04 00 00 00       	mov    $0x4,%eax
80102cdd:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102cde:	89 ca                	mov    %ecx,%edx
80102ce0:	ec                   	in     (%dx),%al
80102ce1:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ce4:	89 da                	mov    %ebx,%edx
80102ce6:	89 45 d8             	mov    %eax,-0x28(%ebp)
80102ce9:	b8 07 00 00 00       	mov    $0x7,%eax
80102cee:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102cef:	89 ca                	mov    %ecx,%edx
80102cf1:	ec                   	in     (%dx),%al
80102cf2:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102cf5:	89 da                	mov    %ebx,%edx
80102cf7:	89 45 dc             	mov    %eax,-0x24(%ebp)
80102cfa:	b8 08 00 00 00       	mov    $0x8,%eax
80102cff:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102d00:	89 ca                	mov    %ecx,%edx
80102d02:	ec                   	in     (%dx),%al
80102d03:	0f b6 c0             	movzbl %al,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102d06:	89 da                	mov    %ebx,%edx
80102d08:	89 45 e0             	mov    %eax,-0x20(%ebp)
80102d0b:	b8 09 00 00 00       	mov    $0x9,%eax
80102d10:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102d11:	89 ca                	mov    %ecx,%edx
80102d13:	ec                   	in     (%dx),%al
80102d14:	0f b6 c0             	movzbl %al,%eax
        continue;
    fill_rtcdate(&t2);
    if(memcmp(&t1, &t2, sizeof(t1)) == 0)
80102d17:	83 ec 04             	sub    $0x4,%esp
  return inb(CMOS_RETURN);
80102d1a:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(memcmp(&t1, &t2, sizeof(t1)) == 0)
80102d1d:	8d 45 d0             	lea    -0x30(%ebp),%eax
80102d20:	6a 18                	push   $0x18
80102d22:	50                   	push   %eax
80102d23:	8d 45 b8             	lea    -0x48(%ebp),%eax
80102d26:	50                   	push   %eax
80102d27:	e8 c4 1c 00 00       	call   801049f0 <memcmp>
80102d2c:	83 c4 10             	add    $0x10,%esp
80102d2f:	85 c0                	test   %eax,%eax
80102d31:	0f 85 f1 fe ff ff    	jne    80102c28 <cmostime+0x28>
      break;
  }

  // convert
  if(bcd) {
80102d37:	80 7d b3 00          	cmpb   $0x0,-0x4d(%ebp)
80102d3b:	75 78                	jne    80102db5 <cmostime+0x1b5>
#define    CONV(x)     (t1.x = ((t1.x >> 4) * 10) + (t1.x & 0xf))
    CONV(second);
80102d3d:	8b 45 b8             	mov    -0x48(%ebp),%eax
80102d40:	89 c2                	mov    %eax,%edx
80102d42:	83 e0 0f             	and    $0xf,%eax
80102d45:	c1 ea 04             	shr    $0x4,%edx
80102d48:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102d4b:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102d4e:	89 45 b8             	mov    %eax,-0x48(%ebp)
    CONV(minute);
80102d51:	8b 45 bc             	mov    -0x44(%ebp),%eax
80102d54:	89 c2                	mov    %eax,%edx
80102d56:	83 e0 0f             	and    $0xf,%eax
80102d59:	c1 ea 04             	shr    $0x4,%edx
80102d5c:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102d5f:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102d62:	89 45 bc             	mov    %eax,-0x44(%ebp)
    CONV(hour  );
80102d65:	8b 45 c0             	mov    -0x40(%ebp),%eax
80102d68:	89 c2                	mov    %eax,%edx
80102d6a:	83 e0 0f             	and    $0xf,%eax
80102d6d:	c1 ea 04             	shr    $0x4,%edx
80102d70:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102d73:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102d76:	89 45 c0             	mov    %eax,-0x40(%ebp)
    CONV(day   );
80102d79:	8b 45 c4             	mov    -0x3c(%ebp),%eax
80102d7c:	89 c2                	mov    %eax,%edx
80102d7e:	83 e0 0f             	and    $0xf,%eax
80102d81:	c1 ea 04             	shr    $0x4,%edx
80102d84:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102d87:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102d8a:	89 45 c4             	mov    %eax,-0x3c(%ebp)
    CONV(month );
80102d8d:	8b 45 c8             	mov    -0x38(%ebp),%eax
80102d90:	89 c2                	mov    %eax,%edx
80102d92:	83 e0 0f             	and    $0xf,%eax
80102d95:	c1 ea 04             	shr    $0x4,%edx
80102d98:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102d9b:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102d9e:	89 45 c8             	mov    %eax,-0x38(%ebp)
    CONV(year  );
80102da1:	8b 45 cc             	mov    -0x34(%ebp),%eax
80102da4:	89 c2                	mov    %eax,%edx
80102da6:	83 e0 0f             	and    $0xf,%eax
80102da9:	c1 ea 04             	shr    $0x4,%edx
80102dac:	8d 14 92             	lea    (%edx,%edx,4),%edx
80102daf:	8d 04 50             	lea    (%eax,%edx,2),%eax
80102db2:	89 45 cc             	mov    %eax,-0x34(%ebp)
#undef     CONV
  }

  *r = t1;
80102db5:	8b 75 08             	mov    0x8(%ebp),%esi
80102db8:	8b 45 b8             	mov    -0x48(%ebp),%eax
80102dbb:	89 06                	mov    %eax,(%esi)
80102dbd:	8b 45 bc             	mov    -0x44(%ebp),%eax
80102dc0:	89 46 04             	mov    %eax,0x4(%esi)
80102dc3:	8b 45 c0             	mov    -0x40(%ebp),%eax
80102dc6:	89 46 08             	mov    %eax,0x8(%esi)
80102dc9:	8b 45 c4             	mov    -0x3c(%ebp),%eax
80102dcc:	89 46 0c             	mov    %eax,0xc(%esi)
80102dcf:	8b 45 c8             	mov    -0x38(%ebp),%eax
80102dd2:	89 46 10             	mov    %eax,0x10(%esi)
80102dd5:	8b 45 cc             	mov    -0x34(%ebp),%eax
80102dd8:	89 46 14             	mov    %eax,0x14(%esi)
  r->year += 2000;
80102ddb:	81 46 14 d0 07 00 00 	addl   $0x7d0,0x14(%esi)
}
80102de2:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102de5:	5b                   	pop    %ebx
80102de6:	5e                   	pop    %esi
80102de7:	5f                   	pop    %edi
80102de8:	5d                   	pop    %ebp
80102de9:	c3                   	ret    
80102dea:	66 90                	xchg   %ax,%ax
80102dec:	66 90                	xchg   %ax,%ax
80102dee:	66 90                	xchg   %ax,%ax

80102df0 <install_trans>:
static void
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80102df0:	8b 0d c8 26 11 80    	mov    0x801126c8,%ecx
80102df6:	85 c9                	test   %ecx,%ecx
80102df8:	0f 8e 8a 00 00 00    	jle    80102e88 <install_trans+0x98>
{
80102dfe:	55                   	push   %ebp
80102dff:	89 e5                	mov    %esp,%ebp
80102e01:	57                   	push   %edi
80102e02:	56                   	push   %esi
80102e03:	53                   	push   %ebx
  for (tail = 0; tail < log.lh.n; tail++) {
80102e04:	31 db                	xor    %ebx,%ebx
{
80102e06:	83 ec 0c             	sub    $0xc,%esp
80102e09:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
80102e10:	a1 b4 26 11 80       	mov    0x801126b4,%eax
80102e15:	83 ec 08             	sub    $0x8,%esp
80102e18:	01 d8                	add    %ebx,%eax
80102e1a:	83 c0 01             	add    $0x1,%eax
80102e1d:	50                   	push   %eax
80102e1e:	ff 35 c4 26 11 80    	pushl  0x801126c4
80102e24:	e8 a7 d2 ff ff       	call   801000d0 <bread>
80102e29:	89 c7                	mov    %eax,%edi
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
80102e2b:	58                   	pop    %eax
80102e2c:	5a                   	pop    %edx
80102e2d:	ff 34 9d cc 26 11 80 	pushl  -0x7feed934(,%ebx,4)
80102e34:	ff 35 c4 26 11 80    	pushl  0x801126c4
  for (tail = 0; tail < log.lh.n; tail++) {
80102e3a:	83 c3 01             	add    $0x1,%ebx
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
80102e3d:	e8 8e d2 ff ff       	call   801000d0 <bread>
80102e42:	89 c6                	mov    %eax,%esi
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
80102e44:	8d 47 5c             	lea    0x5c(%edi),%eax
80102e47:	83 c4 0c             	add    $0xc,%esp
80102e4a:	68 00 02 00 00       	push   $0x200
80102e4f:	50                   	push   %eax
80102e50:	8d 46 5c             	lea    0x5c(%esi),%eax
80102e53:	50                   	push   %eax
80102e54:	e8 f7 1b 00 00       	call   80104a50 <memmove>
    bwrite(dbuf);  // write dst to disk
80102e59:	89 34 24             	mov    %esi,(%esp)
80102e5c:	e8 3f d3 ff ff       	call   801001a0 <bwrite>
    brelse(lbuf);
80102e61:	89 3c 24             	mov    %edi,(%esp)
80102e64:	e8 77 d3 ff ff       	call   801001e0 <brelse>
    brelse(dbuf);
80102e69:	89 34 24             	mov    %esi,(%esp)
80102e6c:	e8 6f d3 ff ff       	call   801001e0 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
80102e71:	83 c4 10             	add    $0x10,%esp
80102e74:	39 1d c8 26 11 80    	cmp    %ebx,0x801126c8
80102e7a:	7f 94                	jg     80102e10 <install_trans+0x20>
  }
}
80102e7c:	8d 65 f4             	lea    -0xc(%ebp),%esp
80102e7f:	5b                   	pop    %ebx
80102e80:	5e                   	pop    %esi
80102e81:	5f                   	pop    %edi
80102e82:	5d                   	pop    %ebp
80102e83:	c3                   	ret    
80102e84:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80102e88:	f3 c3                	repz ret 
80102e8a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80102e90 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80102e90:	55                   	push   %ebp
80102e91:	89 e5                	mov    %esp,%ebp
80102e93:	56                   	push   %esi
80102e94:	53                   	push   %ebx
  struct buf *buf = bread(log.dev, log.start);
80102e95:	83 ec 08             	sub    $0x8,%esp
80102e98:	ff 35 b4 26 11 80    	pushl  0x801126b4
80102e9e:	ff 35 c4 26 11 80    	pushl  0x801126c4
80102ea4:	e8 27 d2 ff ff       	call   801000d0 <bread>
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
80102ea9:	8b 1d c8 26 11 80    	mov    0x801126c8,%ebx
  for (i = 0; i < log.lh.n; i++) {
80102eaf:	83 c4 10             	add    $0x10,%esp
  struct buf *buf = bread(log.dev, log.start);
80102eb2:	89 c6                	mov    %eax,%esi
  for (i = 0; i < log.lh.n; i++) {
80102eb4:	85 db                	test   %ebx,%ebx
  hb->n = log.lh.n;
80102eb6:	89 58 5c             	mov    %ebx,0x5c(%eax)
  for (i = 0; i < log.lh.n; i++) {
80102eb9:	7e 16                	jle    80102ed1 <write_head+0x41>
80102ebb:	c1 e3 02             	shl    $0x2,%ebx
80102ebe:	31 d2                	xor    %edx,%edx
    hb->block[i] = log.lh.block[i];
80102ec0:	8b 8a cc 26 11 80    	mov    -0x7feed934(%edx),%ecx
80102ec6:	89 4c 16 60          	mov    %ecx,0x60(%esi,%edx,1)
80102eca:	83 c2 04             	add    $0x4,%edx
  for (i = 0; i < log.lh.n; i++) {
80102ecd:	39 da                	cmp    %ebx,%edx
80102ecf:	75 ef                	jne    80102ec0 <write_head+0x30>
  }
  bwrite(buf);
80102ed1:	83 ec 0c             	sub    $0xc,%esp
80102ed4:	56                   	push   %esi
80102ed5:	e8 c6 d2 ff ff       	call   801001a0 <bwrite>
  brelse(buf);
80102eda:	89 34 24             	mov    %esi,(%esp)
80102edd:	e8 fe d2 ff ff       	call   801001e0 <brelse>
}
80102ee2:	83 c4 10             	add    $0x10,%esp
80102ee5:	8d 65 f8             	lea    -0x8(%ebp),%esp
80102ee8:	5b                   	pop    %ebx
80102ee9:	5e                   	pop    %esi
80102eea:	5d                   	pop    %ebp
80102eeb:	c3                   	ret    
80102eec:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80102ef0 <initlog>:
{
80102ef0:	55                   	push   %ebp
80102ef1:	89 e5                	mov    %esp,%ebp
80102ef3:	53                   	push   %ebx
80102ef4:	83 ec 2c             	sub    $0x2c,%esp
80102ef7:	8b 5d 08             	mov    0x8(%ebp),%ebx
  initlock(&log.lock, "log");
80102efa:	68 60 79 10 80       	push   $0x80107960
80102eff:	68 80 26 11 80       	push   $0x80112680
80102f04:	e8 27 18 00 00       	call   80104730 <initlock>
  readsb(dev, &sb);
80102f09:	58                   	pop    %eax
80102f0a:	8d 45 dc             	lea    -0x24(%ebp),%eax
80102f0d:	5a                   	pop    %edx
80102f0e:	50                   	push   %eax
80102f0f:	53                   	push   %ebx
80102f10:	e8 0b e5 ff ff       	call   80101420 <readsb>
  log.size = sb.nlog;
80102f15:	8b 55 e8             	mov    -0x18(%ebp),%edx
  log.start = sb.logstart;
80102f18:	8b 45 ec             	mov    -0x14(%ebp),%eax
  struct buf *buf = bread(log.dev, log.start);
80102f1b:	59                   	pop    %ecx
  log.dev = dev;
80102f1c:	89 1d c4 26 11 80    	mov    %ebx,0x801126c4
  log.size = sb.nlog;
80102f22:	89 15 b8 26 11 80    	mov    %edx,0x801126b8
  log.start = sb.logstart;
80102f28:	a3 b4 26 11 80       	mov    %eax,0x801126b4
  struct buf *buf = bread(log.dev, log.start);
80102f2d:	5a                   	pop    %edx
80102f2e:	50                   	push   %eax
80102f2f:	53                   	push   %ebx
80102f30:	e8 9b d1 ff ff       	call   801000d0 <bread>
  log.lh.n = lh->n;
80102f35:	8b 58 5c             	mov    0x5c(%eax),%ebx
  for (i = 0; i < log.lh.n; i++) {
80102f38:	83 c4 10             	add    $0x10,%esp
80102f3b:	85 db                	test   %ebx,%ebx
  log.lh.n = lh->n;
80102f3d:	89 1d c8 26 11 80    	mov    %ebx,0x801126c8
  for (i = 0; i < log.lh.n; i++) {
80102f43:	7e 1c                	jle    80102f61 <initlog+0x71>
80102f45:	c1 e3 02             	shl    $0x2,%ebx
80102f48:	31 d2                	xor    %edx,%edx
80102f4a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    log.lh.block[i] = lh->block[i];
80102f50:	8b 4c 10 60          	mov    0x60(%eax,%edx,1),%ecx
80102f54:	83 c2 04             	add    $0x4,%edx
80102f57:	89 8a c8 26 11 80    	mov    %ecx,-0x7feed938(%edx)
  for (i = 0; i < log.lh.n; i++) {
80102f5d:	39 d3                	cmp    %edx,%ebx
80102f5f:	75 ef                	jne    80102f50 <initlog+0x60>
  brelse(buf);
80102f61:	83 ec 0c             	sub    $0xc,%esp
80102f64:	50                   	push   %eax
80102f65:	e8 76 d2 ff ff       	call   801001e0 <brelse>

static void
recover_from_log(void)
{
  read_head();
  install_trans(); // if committed, copy from log to disk
80102f6a:	e8 81 fe ff ff       	call   80102df0 <install_trans>
  log.lh.n = 0;
80102f6f:	c7 05 c8 26 11 80 00 	movl   $0x0,0x801126c8
80102f76:	00 00 00 
  write_head(); // clear the log
80102f79:	e8 12 ff ff ff       	call   80102e90 <write_head>
}
80102f7e:	83 c4 10             	add    $0x10,%esp
80102f81:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80102f84:	c9                   	leave  
80102f85:	c3                   	ret    
80102f86:	8d 76 00             	lea    0x0(%esi),%esi
80102f89:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80102f90 <begin_op>:
}

// called at the start of each FS system call.
void
begin_op(void)
{
80102f90:	55                   	push   %ebp
80102f91:	89 e5                	mov    %esp,%ebp
80102f93:	83 ec 14             	sub    $0x14,%esp
  acquire(&log.lock);
80102f96:	68 80 26 11 80       	push   $0x80112680
80102f9b:	e8 80 18 00 00       	call   80104820 <acquire>
80102fa0:	83 c4 10             	add    $0x10,%esp
80102fa3:	eb 18                	jmp    80102fbd <begin_op+0x2d>
80102fa5:	8d 76 00             	lea    0x0(%esi),%esi
  while(1){
    if(log.committing){
      sleep(&log, &log.lock);
80102fa8:	83 ec 08             	sub    $0x8,%esp
80102fab:	68 80 26 11 80       	push   $0x80112680
80102fb0:	68 80 26 11 80       	push   $0x80112680
80102fb5:	e8 b6 12 00 00       	call   80104270 <sleep>
80102fba:	83 c4 10             	add    $0x10,%esp
    if(log.committing){
80102fbd:	a1 c0 26 11 80       	mov    0x801126c0,%eax
80102fc2:	85 c0                	test   %eax,%eax
80102fc4:	75 e2                	jne    80102fa8 <begin_op+0x18>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
80102fc6:	a1 bc 26 11 80       	mov    0x801126bc,%eax
80102fcb:	8b 15 c8 26 11 80    	mov    0x801126c8,%edx
80102fd1:	83 c0 01             	add    $0x1,%eax
80102fd4:	8d 0c 80             	lea    (%eax,%eax,4),%ecx
80102fd7:	8d 14 4a             	lea    (%edx,%ecx,2),%edx
80102fda:	83 fa 1e             	cmp    $0x1e,%edx
80102fdd:	7f c9                	jg     80102fa8 <begin_op+0x18>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
    } else {
      log.outstanding += 1;
      release(&log.lock);
80102fdf:	83 ec 0c             	sub    $0xc,%esp
      log.outstanding += 1;
80102fe2:	a3 bc 26 11 80       	mov    %eax,0x801126bc
      release(&log.lock);
80102fe7:	68 80 26 11 80       	push   $0x80112680
80102fec:	e8 4f 19 00 00       	call   80104940 <release>
      break;
    }
  }
}
80102ff1:	83 c4 10             	add    $0x10,%esp
80102ff4:	c9                   	leave  
80102ff5:	c3                   	ret    
80102ff6:	8d 76 00             	lea    0x0(%esi),%esi
80102ff9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80103000 <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
80103000:	55                   	push   %ebp
80103001:	89 e5                	mov    %esp,%ebp
80103003:	57                   	push   %edi
80103004:	56                   	push   %esi
80103005:	53                   	push   %ebx
80103006:	83 ec 18             	sub    $0x18,%esp
  int do_commit = 0;

  acquire(&log.lock);
80103009:	68 80 26 11 80       	push   $0x80112680
8010300e:	e8 0d 18 00 00       	call   80104820 <acquire>
  log.outstanding -= 1;
80103013:	a1 bc 26 11 80       	mov    0x801126bc,%eax
  if(log.committing)
80103018:	8b 35 c0 26 11 80    	mov    0x801126c0,%esi
8010301e:	83 c4 10             	add    $0x10,%esp
  log.outstanding -= 1;
80103021:	8d 58 ff             	lea    -0x1(%eax),%ebx
  if(log.committing)
80103024:	85 f6                	test   %esi,%esi
  log.outstanding -= 1;
80103026:	89 1d bc 26 11 80    	mov    %ebx,0x801126bc
  if(log.committing)
8010302c:	0f 85 1a 01 00 00    	jne    8010314c <end_op+0x14c>
    panic("log.committing");
  if(log.outstanding == 0){
80103032:	85 db                	test   %ebx,%ebx
80103034:	0f 85 ee 00 00 00    	jne    80103128 <end_op+0x128>
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
  }
  release(&log.lock);
8010303a:	83 ec 0c             	sub    $0xc,%esp
    log.committing = 1;
8010303d:	c7 05 c0 26 11 80 01 	movl   $0x1,0x801126c0
80103044:	00 00 00 
  release(&log.lock);
80103047:	68 80 26 11 80       	push   $0x80112680
8010304c:	e8 ef 18 00 00       	call   80104940 <release>
}

static void
commit()
{
  if (log.lh.n > 0) {
80103051:	8b 0d c8 26 11 80    	mov    0x801126c8,%ecx
80103057:	83 c4 10             	add    $0x10,%esp
8010305a:	85 c9                	test   %ecx,%ecx
8010305c:	0f 8e 85 00 00 00    	jle    801030e7 <end_op+0xe7>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
80103062:	a1 b4 26 11 80       	mov    0x801126b4,%eax
80103067:	83 ec 08             	sub    $0x8,%esp
8010306a:	01 d8                	add    %ebx,%eax
8010306c:	83 c0 01             	add    $0x1,%eax
8010306f:	50                   	push   %eax
80103070:	ff 35 c4 26 11 80    	pushl  0x801126c4
80103076:	e8 55 d0 ff ff       	call   801000d0 <bread>
8010307b:	89 c6                	mov    %eax,%esi
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
8010307d:	58                   	pop    %eax
8010307e:	5a                   	pop    %edx
8010307f:	ff 34 9d cc 26 11 80 	pushl  -0x7feed934(,%ebx,4)
80103086:	ff 35 c4 26 11 80    	pushl  0x801126c4
  for (tail = 0; tail < log.lh.n; tail++) {
8010308c:	83 c3 01             	add    $0x1,%ebx
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
8010308f:	e8 3c d0 ff ff       	call   801000d0 <bread>
80103094:	89 c7                	mov    %eax,%edi
    memmove(to->data, from->data, BSIZE);
80103096:	8d 40 5c             	lea    0x5c(%eax),%eax
80103099:	83 c4 0c             	add    $0xc,%esp
8010309c:	68 00 02 00 00       	push   $0x200
801030a1:	50                   	push   %eax
801030a2:	8d 46 5c             	lea    0x5c(%esi),%eax
801030a5:	50                   	push   %eax
801030a6:	e8 a5 19 00 00       	call   80104a50 <memmove>
    bwrite(to);  // write the log
801030ab:	89 34 24             	mov    %esi,(%esp)
801030ae:	e8 ed d0 ff ff       	call   801001a0 <bwrite>
    brelse(from);
801030b3:	89 3c 24             	mov    %edi,(%esp)
801030b6:	e8 25 d1 ff ff       	call   801001e0 <brelse>
    brelse(to);
801030bb:	89 34 24             	mov    %esi,(%esp)
801030be:	e8 1d d1 ff ff       	call   801001e0 <brelse>
  for (tail = 0; tail < log.lh.n; tail++) {
801030c3:	83 c4 10             	add    $0x10,%esp
801030c6:	3b 1d c8 26 11 80    	cmp    0x801126c8,%ebx
801030cc:	7c 94                	jl     80103062 <end_op+0x62>
    write_log();     // Write modified blocks from cache to log
    write_head();    // Write header to disk -- the real commit
801030ce:	e8 bd fd ff ff       	call   80102e90 <write_head>
    install_trans(); // Now install writes to home locations
801030d3:	e8 18 fd ff ff       	call   80102df0 <install_trans>
    log.lh.n = 0;
801030d8:	c7 05 c8 26 11 80 00 	movl   $0x0,0x801126c8
801030df:	00 00 00 
    write_head();    // Erase the transaction from the log
801030e2:	e8 a9 fd ff ff       	call   80102e90 <write_head>
    acquire(&log.lock);
801030e7:	83 ec 0c             	sub    $0xc,%esp
801030ea:	68 80 26 11 80       	push   $0x80112680
801030ef:	e8 2c 17 00 00       	call   80104820 <acquire>
    wakeup(&log);
801030f4:	c7 04 24 80 26 11 80 	movl   $0x80112680,(%esp)
    log.committing = 0;
801030fb:	c7 05 c0 26 11 80 00 	movl   $0x0,0x801126c0
80103102:	00 00 00 
    wakeup(&log);
80103105:	e8 46 13 00 00       	call   80104450 <wakeup>
    release(&log.lock);
8010310a:	c7 04 24 80 26 11 80 	movl   $0x80112680,(%esp)
80103111:	e8 2a 18 00 00       	call   80104940 <release>
80103116:	83 c4 10             	add    $0x10,%esp
}
80103119:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010311c:	5b                   	pop    %ebx
8010311d:	5e                   	pop    %esi
8010311e:	5f                   	pop    %edi
8010311f:	5d                   	pop    %ebp
80103120:	c3                   	ret    
80103121:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    wakeup(&log);
80103128:	83 ec 0c             	sub    $0xc,%esp
8010312b:	68 80 26 11 80       	push   $0x80112680
80103130:	e8 1b 13 00 00       	call   80104450 <wakeup>
  release(&log.lock);
80103135:	c7 04 24 80 26 11 80 	movl   $0x80112680,(%esp)
8010313c:	e8 ff 17 00 00       	call   80104940 <release>
80103141:	83 c4 10             	add    $0x10,%esp
}
80103144:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103147:	5b                   	pop    %ebx
80103148:	5e                   	pop    %esi
80103149:	5f                   	pop    %edi
8010314a:	5d                   	pop    %ebp
8010314b:	c3                   	ret    
    panic("log.committing");
8010314c:	83 ec 0c             	sub    $0xc,%esp
8010314f:	68 64 79 10 80       	push   $0x80107964
80103154:	e8 37 d2 ff ff       	call   80100390 <panic>
80103159:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80103160 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
80103160:	55                   	push   %ebp
80103161:	89 e5                	mov    %esp,%ebp
80103163:	53                   	push   %ebx
80103164:	83 ec 04             	sub    $0x4,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
80103167:	8b 15 c8 26 11 80    	mov    0x801126c8,%edx
{
8010316d:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
80103170:	83 fa 1d             	cmp    $0x1d,%edx
80103173:	0f 8f 9d 00 00 00    	jg     80103216 <log_write+0xb6>
80103179:	a1 b8 26 11 80       	mov    0x801126b8,%eax
8010317e:	83 e8 01             	sub    $0x1,%eax
80103181:	39 c2                	cmp    %eax,%edx
80103183:	0f 8d 8d 00 00 00    	jge    80103216 <log_write+0xb6>
    panic("too big a transaction");
  if (log.outstanding < 1)
80103189:	a1 bc 26 11 80       	mov    0x801126bc,%eax
8010318e:	85 c0                	test   %eax,%eax
80103190:	0f 8e 8d 00 00 00    	jle    80103223 <log_write+0xc3>
    panic("log_write outside of trans");

  acquire(&log.lock);
80103196:	83 ec 0c             	sub    $0xc,%esp
80103199:	68 80 26 11 80       	push   $0x80112680
8010319e:	e8 7d 16 00 00       	call   80104820 <acquire>
  for (i = 0; i < log.lh.n; i++) {
801031a3:	8b 0d c8 26 11 80    	mov    0x801126c8,%ecx
801031a9:	83 c4 10             	add    $0x10,%esp
801031ac:	83 f9 00             	cmp    $0x0,%ecx
801031af:	7e 57                	jle    80103208 <log_write+0xa8>
    if (log.lh.block[i] == b->blockno)   // log absorbtion
801031b1:	8b 53 08             	mov    0x8(%ebx),%edx
  for (i = 0; i < log.lh.n; i++) {
801031b4:	31 c0                	xor    %eax,%eax
    if (log.lh.block[i] == b->blockno)   // log absorbtion
801031b6:	3b 15 cc 26 11 80    	cmp    0x801126cc,%edx
801031bc:	75 0b                	jne    801031c9 <log_write+0x69>
801031be:	eb 38                	jmp    801031f8 <log_write+0x98>
801031c0:	39 14 85 cc 26 11 80 	cmp    %edx,-0x7feed934(,%eax,4)
801031c7:	74 2f                	je     801031f8 <log_write+0x98>
  for (i = 0; i < log.lh.n; i++) {
801031c9:	83 c0 01             	add    $0x1,%eax
801031cc:	39 c1                	cmp    %eax,%ecx
801031ce:	75 f0                	jne    801031c0 <log_write+0x60>
      break;
  }
  log.lh.block[i] = b->blockno;
801031d0:	89 14 85 cc 26 11 80 	mov    %edx,-0x7feed934(,%eax,4)
  if (i == log.lh.n)
    log.lh.n++;
801031d7:	83 c0 01             	add    $0x1,%eax
801031da:	a3 c8 26 11 80       	mov    %eax,0x801126c8
  b->flags |= B_DIRTY; // prevent eviction
801031df:	83 0b 04             	orl    $0x4,(%ebx)
  release(&log.lock);
801031e2:	c7 45 08 80 26 11 80 	movl   $0x80112680,0x8(%ebp)
}
801031e9:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801031ec:	c9                   	leave  
  release(&log.lock);
801031ed:	e9 4e 17 00 00       	jmp    80104940 <release>
801031f2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  log.lh.block[i] = b->blockno;
801031f8:	89 14 85 cc 26 11 80 	mov    %edx,-0x7feed934(,%eax,4)
801031ff:	eb de                	jmp    801031df <log_write+0x7f>
80103201:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103208:	8b 43 08             	mov    0x8(%ebx),%eax
8010320b:	a3 cc 26 11 80       	mov    %eax,0x801126cc
  if (i == log.lh.n)
80103210:	75 cd                	jne    801031df <log_write+0x7f>
80103212:	31 c0                	xor    %eax,%eax
80103214:	eb c1                	jmp    801031d7 <log_write+0x77>
    panic("too big a transaction");
80103216:	83 ec 0c             	sub    $0xc,%esp
80103219:	68 73 79 10 80       	push   $0x80107973
8010321e:	e8 6d d1 ff ff       	call   80100390 <panic>
    panic("log_write outside of trans");
80103223:	83 ec 0c             	sub    $0xc,%esp
80103226:	68 89 79 10 80       	push   $0x80107989
8010322b:	e8 60 d1 ff ff       	call   80100390 <panic>

80103230 <mpmain>:
  mpmain();
}

// Common CPU setup code.
static void
mpmain(void) {
80103230:	55                   	push   %ebp
80103231:	89 e5                	mov    %esp,%ebp
80103233:	53                   	push   %ebx
80103234:	83 ec 04             	sub    $0x4,%esp
    cprintf("cpu%d: starting %d\n", cpuid(), cpuid());
80103237:	e8 a4 09 00 00       	call   80103be0 <cpuid>
8010323c:	89 c3                	mov    %eax,%ebx
8010323e:	e8 9d 09 00 00       	call   80103be0 <cpuid>
80103243:	83 ec 04             	sub    $0x4,%esp
80103246:	53                   	push   %ebx
80103247:	50                   	push   %eax
80103248:	68 a4 79 10 80       	push   $0x801079a4
8010324d:	e8 0e d4 ff ff       	call   80100660 <cprintf>
    idtinit();       // load idt register
80103252:	e8 19 2a 00 00       	call   80105c70 <idtinit>
    xchg(&(mycpu()->started), 1); // tell startothers() we're up
80103257:	e8 04 09 00 00       	call   80103b60 <mycpu>
8010325c:	89 c2                	mov    %eax,%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;

  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
8010325e:	b8 01 00 00 00       	mov    $0x1,%eax
80103263:	f0 87 82 a0 00 00 00 	lock xchg %eax,0xa0(%edx)
    scheduler();     // start running processes
8010326a:	e8 11 0d 00 00       	call   80103f80 <scheduler>
8010326f:	90                   	nop

80103270 <mpenter>:
{
80103270:	55                   	push   %ebp
80103271:	89 e5                	mov    %esp,%ebp
80103273:	83 ec 08             	sub    $0x8,%esp
  switchkvm();
80103276:	e8 e5 3a 00 00       	call   80106d60 <switchkvm>
  seginit();
8010327b:	e8 50 3a 00 00       	call   80106cd0 <seginit>
  lapicinit();
80103280:	e8 9b f7 ff ff       	call   80102a20 <lapicinit>
  mpmain();
80103285:	e8 a6 ff ff ff       	call   80103230 <mpmain>
8010328a:	66 90                	xchg   %ax,%ax
8010328c:	66 90                	xchg   %ax,%ax
8010328e:	66 90                	xchg   %ax,%ax

80103290 <main>:
{
80103290:	8d 4c 24 04          	lea    0x4(%esp),%ecx
80103294:	83 e4 f0             	and    $0xfffffff0,%esp
80103297:	ff 71 fc             	pushl  -0x4(%ecx)
8010329a:	55                   	push   %ebp
8010329b:	89 e5                	mov    %esp,%ebp
8010329d:	53                   	push   %ebx
8010329e:	51                   	push   %ecx
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
8010329f:	83 ec 08             	sub    $0x8,%esp
801032a2:	68 00 00 40 80       	push   $0x80400000
801032a7:	68 a8 65 11 80       	push   $0x801165a8
801032ac:	e8 2f f5 ff ff       	call   801027e0 <kinit1>
  kvmalloc();      // kernel page table
801032b1:	e8 7a 3f 00 00       	call   80107230 <kvmalloc>
  mpinit();        // detect other processors
801032b6:	e8 75 01 00 00       	call   80103430 <mpinit>
  lapicinit();     // interrupt controller
801032bb:	e8 60 f7 ff ff       	call   80102a20 <lapicinit>
  seginit();       // segment descriptors
801032c0:	e8 0b 3a 00 00       	call   80106cd0 <seginit>
  picinit();       // disable pic
801032c5:	e8 46 03 00 00       	call   80103610 <picinit>
  ioapicinit();    // another interrupt controller
801032ca:	e8 41 f3 ff ff       	call   80102610 <ioapicinit>
  consoleinit();   // console hardware
801032cf:	e8 ec d6 ff ff       	call   801009c0 <consoleinit>
  uartinit();      // serial port
801032d4:	e8 c7 2c 00 00       	call   80105fa0 <uartinit>
  pinit();         // process table
801032d9:	e8 62 08 00 00       	call   80103b40 <pinit>
  tvinit();        // trap vectors
801032de:	e8 0d 29 00 00       	call   80105bf0 <tvinit>
  binit();         // buffer cache
801032e3:	e8 58 cd ff ff       	call   80100040 <binit>
  fileinit();      // file table
801032e8:	e8 c3 da ff ff       	call   80100db0 <fileinit>
  ideinit();       // disk 
801032ed:	e8 fe f0 ff ff       	call   801023f0 <ideinit>

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = P2V(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
801032f2:	83 c4 0c             	add    $0xc,%esp
801032f5:	68 8a 00 00 00       	push   $0x8a
801032fa:	68 8c a4 10 80       	push   $0x8010a48c
801032ff:	68 00 70 00 80       	push   $0x80007000
80103304:	e8 47 17 00 00       	call   80104a50 <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
80103309:	69 05 00 2d 11 80 b0 	imul   $0xb0,0x80112d00,%eax
80103310:	00 00 00 
80103313:	83 c4 10             	add    $0x10,%esp
80103316:	05 80 27 11 80       	add    $0x80112780,%eax
8010331b:	3d 80 27 11 80       	cmp    $0x80112780,%eax
80103320:	76 71                	jbe    80103393 <main+0x103>
80103322:	bb 80 27 11 80       	mov    $0x80112780,%ebx
80103327:	89 f6                	mov    %esi,%esi
80103329:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    if(c == mycpu())  // We've started already.
80103330:	e8 2b 08 00 00       	call   80103b60 <mycpu>
80103335:	39 d8                	cmp    %ebx,%eax
80103337:	74 41                	je     8010337a <main+0xea>
      continue;

    // Tell entryother.S what stack to use, where to enter, and what
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
80103339:	e8 72 f5 ff ff       	call   801028b0 <kalloc>
    *(void**)(code-4) = stack + KSTACKSIZE;
8010333e:	05 00 10 00 00       	add    $0x1000,%eax
    *(void**)(code-8) = mpenter;
80103343:	c7 05 f8 6f 00 80 70 	movl   $0x80103270,0x80006ff8
8010334a:	32 10 80 
    *(int**)(code-12) = (void *) V2P(entrypgdir);
8010334d:	c7 05 f4 6f 00 80 00 	movl   $0x109000,0x80006ff4
80103354:	90 10 00 
    *(void**)(code-4) = stack + KSTACKSIZE;
80103357:	a3 fc 6f 00 80       	mov    %eax,0x80006ffc

    lapicstartap(c->apicid, V2P(code));
8010335c:	0f b6 03             	movzbl (%ebx),%eax
8010335f:	83 ec 08             	sub    $0x8,%esp
80103362:	68 00 70 00 00       	push   $0x7000
80103367:	50                   	push   %eax
80103368:	e8 03 f8 ff ff       	call   80102b70 <lapicstartap>
8010336d:	83 c4 10             	add    $0x10,%esp

    // wait for cpu to finish mpmain()
    while(c->started == 0)
80103370:	8b 83 a0 00 00 00    	mov    0xa0(%ebx),%eax
80103376:	85 c0                	test   %eax,%eax
80103378:	74 f6                	je     80103370 <main+0xe0>
  for(c = cpus; c < cpus+ncpu; c++){
8010337a:	69 05 00 2d 11 80 b0 	imul   $0xb0,0x80112d00,%eax
80103381:	00 00 00 
80103384:	81 c3 b0 00 00 00    	add    $0xb0,%ebx
8010338a:	05 80 27 11 80       	add    $0x80112780,%eax
8010338f:	39 c3                	cmp    %eax,%ebx
80103391:	72 9d                	jb     80103330 <main+0xa0>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103393:	83 ec 08             	sub    $0x8,%esp
80103396:	68 00 00 00 8e       	push   $0x8e000000
8010339b:	68 00 00 40 80       	push   $0x80400000
801033a0:	e8 ab f4 ff ff       	call   80102850 <kinit2>
  userinit();      // first user process
801033a5:	e8 86 08 00 00       	call   80103c30 <userinit>
  mpmain();        // finish this processor's setup
801033aa:	e8 81 fe ff ff       	call   80103230 <mpmain>
801033af:	90                   	nop

801033b0 <mpsearch1>:
}

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
801033b0:	55                   	push   %ebp
801033b1:	89 e5                	mov    %esp,%ebp
801033b3:	57                   	push   %edi
801033b4:	56                   	push   %esi
  uchar *e, *p, *addr;

  addr = P2V(a);
801033b5:	8d b0 00 00 00 80    	lea    -0x80000000(%eax),%esi
{
801033bb:	53                   	push   %ebx
  e = addr+len;
801033bc:	8d 1c 16             	lea    (%esi,%edx,1),%ebx
{
801033bf:	83 ec 0c             	sub    $0xc,%esp
  for(p = addr; p < e; p += sizeof(struct mp))
801033c2:	39 de                	cmp    %ebx,%esi
801033c4:	72 10                	jb     801033d6 <mpsearch1+0x26>
801033c6:	eb 50                	jmp    80103418 <mpsearch1+0x68>
801033c8:	90                   	nop
801033c9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801033d0:	39 fb                	cmp    %edi,%ebx
801033d2:	89 fe                	mov    %edi,%esi
801033d4:	76 42                	jbe    80103418 <mpsearch1+0x68>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
801033d6:	83 ec 04             	sub    $0x4,%esp
801033d9:	8d 7e 10             	lea    0x10(%esi),%edi
801033dc:	6a 04                	push   $0x4
801033de:	68 b8 79 10 80       	push   $0x801079b8
801033e3:	56                   	push   %esi
801033e4:	e8 07 16 00 00       	call   801049f0 <memcmp>
801033e9:	83 c4 10             	add    $0x10,%esp
801033ec:	85 c0                	test   %eax,%eax
801033ee:	75 e0                	jne    801033d0 <mpsearch1+0x20>
801033f0:	89 f1                	mov    %esi,%ecx
801033f2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    sum += addr[i];
801033f8:	0f b6 11             	movzbl (%ecx),%edx
801033fb:	83 c1 01             	add    $0x1,%ecx
801033fe:	01 d0                	add    %edx,%eax
  for(i=0; i<len; i++)
80103400:	39 f9                	cmp    %edi,%ecx
80103402:	75 f4                	jne    801033f8 <mpsearch1+0x48>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
80103404:	84 c0                	test   %al,%al
80103406:	75 c8                	jne    801033d0 <mpsearch1+0x20>
      return (struct mp*)p;
  return 0;
}
80103408:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010340b:	89 f0                	mov    %esi,%eax
8010340d:	5b                   	pop    %ebx
8010340e:	5e                   	pop    %esi
8010340f:	5f                   	pop    %edi
80103410:	5d                   	pop    %ebp
80103411:	c3                   	ret    
80103412:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80103418:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
8010341b:	31 f6                	xor    %esi,%esi
}
8010341d:	89 f0                	mov    %esi,%eax
8010341f:	5b                   	pop    %ebx
80103420:	5e                   	pop    %esi
80103421:	5f                   	pop    %edi
80103422:	5d                   	pop    %ebp
80103423:	c3                   	ret    
80103424:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
8010342a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80103430 <mpinit>:
  return conf;
}

void
mpinit(void)
{
80103430:	55                   	push   %ebp
80103431:	89 e5                	mov    %esp,%ebp
80103433:	57                   	push   %edi
80103434:	56                   	push   %esi
80103435:	53                   	push   %ebx
80103436:	83 ec 1c             	sub    $0x1c,%esp
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
80103439:	0f b6 05 0f 04 00 80 	movzbl 0x8000040f,%eax
80103440:	0f b6 15 0e 04 00 80 	movzbl 0x8000040e,%edx
80103447:	c1 e0 08             	shl    $0x8,%eax
8010344a:	09 d0                	or     %edx,%eax
8010344c:	c1 e0 04             	shl    $0x4,%eax
8010344f:	85 c0                	test   %eax,%eax
80103451:	75 1b                	jne    8010346e <mpinit+0x3e>
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
80103453:	0f b6 05 14 04 00 80 	movzbl 0x80000414,%eax
8010345a:	0f b6 15 13 04 00 80 	movzbl 0x80000413,%edx
80103461:	c1 e0 08             	shl    $0x8,%eax
80103464:	09 d0                	or     %edx,%eax
80103466:	c1 e0 0a             	shl    $0xa,%eax
    if((mp = mpsearch1(p-1024, 1024)))
80103469:	2d 00 04 00 00       	sub    $0x400,%eax
    if((mp = mpsearch1(p, 1024)))
8010346e:	ba 00 04 00 00       	mov    $0x400,%edx
80103473:	e8 38 ff ff ff       	call   801033b0 <mpsearch1>
80103478:	85 c0                	test   %eax,%eax
8010347a:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010347d:	0f 84 3d 01 00 00    	je     801035c0 <mpinit+0x190>
  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
80103483:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103486:	8b 58 04             	mov    0x4(%eax),%ebx
80103489:	85 db                	test   %ebx,%ebx
8010348b:	0f 84 4f 01 00 00    	je     801035e0 <mpinit+0x1b0>
  conf = (struct mpconf*) P2V((uint) mp->physaddr);
80103491:	8d b3 00 00 00 80    	lea    -0x80000000(%ebx),%esi
  if(memcmp(conf, "PCMP", 4) != 0)
80103497:	83 ec 04             	sub    $0x4,%esp
8010349a:	6a 04                	push   $0x4
8010349c:	68 d5 79 10 80       	push   $0x801079d5
801034a1:	56                   	push   %esi
801034a2:	e8 49 15 00 00       	call   801049f0 <memcmp>
801034a7:	83 c4 10             	add    $0x10,%esp
801034aa:	85 c0                	test   %eax,%eax
801034ac:	0f 85 2e 01 00 00    	jne    801035e0 <mpinit+0x1b0>
  if(conf->version != 1 && conf->version != 4)
801034b2:	0f b6 83 06 00 00 80 	movzbl -0x7ffffffa(%ebx),%eax
801034b9:	3c 01                	cmp    $0x1,%al
801034bb:	0f 95 c2             	setne  %dl
801034be:	3c 04                	cmp    $0x4,%al
801034c0:	0f 95 c0             	setne  %al
801034c3:	20 c2                	and    %al,%dl
801034c5:	0f 85 15 01 00 00    	jne    801035e0 <mpinit+0x1b0>
  if(sum((uchar*)conf, conf->length) != 0)
801034cb:	0f b7 bb 04 00 00 80 	movzwl -0x7ffffffc(%ebx),%edi
  for(i=0; i<len; i++)
801034d2:	66 85 ff             	test   %di,%di
801034d5:	74 1a                	je     801034f1 <mpinit+0xc1>
801034d7:	89 f0                	mov    %esi,%eax
801034d9:	01 f7                	add    %esi,%edi
  sum = 0;
801034db:	31 d2                	xor    %edx,%edx
801034dd:	8d 76 00             	lea    0x0(%esi),%esi
    sum += addr[i];
801034e0:	0f b6 08             	movzbl (%eax),%ecx
801034e3:	83 c0 01             	add    $0x1,%eax
801034e6:	01 ca                	add    %ecx,%edx
  for(i=0; i<len; i++)
801034e8:	39 c7                	cmp    %eax,%edi
801034ea:	75 f4                	jne    801034e0 <mpinit+0xb0>
801034ec:	84 d2                	test   %dl,%dl
801034ee:	0f 95 c2             	setne  %dl
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  if((conf = mpconfig(&mp)) == 0)
801034f1:	85 f6                	test   %esi,%esi
801034f3:	0f 84 e7 00 00 00    	je     801035e0 <mpinit+0x1b0>
801034f9:	84 d2                	test   %dl,%dl
801034fb:	0f 85 df 00 00 00    	jne    801035e0 <mpinit+0x1b0>
    panic("Expect to run on an SMP");
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
80103501:	8b 83 24 00 00 80    	mov    -0x7fffffdc(%ebx),%eax
80103507:	a3 7c 26 11 80       	mov    %eax,0x8011267c
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
8010350c:	0f b7 93 04 00 00 80 	movzwl -0x7ffffffc(%ebx),%edx
80103513:	8d 83 2c 00 00 80    	lea    -0x7fffffd4(%ebx),%eax
  ismp = 1;
80103519:	bb 01 00 00 00       	mov    $0x1,%ebx
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
8010351e:	01 d6                	add    %edx,%esi
80103520:	39 c6                	cmp    %eax,%esi
80103522:	76 23                	jbe    80103547 <mpinit+0x117>
    switch(*p){
80103524:	0f b6 10             	movzbl (%eax),%edx
80103527:	80 fa 04             	cmp    $0x4,%dl
8010352a:	0f 87 ca 00 00 00    	ja     801035fa <mpinit+0x1ca>
80103530:	ff 24 95 fc 79 10 80 	jmp    *-0x7fef8604(,%edx,4)
80103537:	89 f6                	mov    %esi,%esi
80103539:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
      p += sizeof(struct mpioapic);
      continue;
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
80103540:	83 c0 08             	add    $0x8,%eax
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
80103543:	39 c6                	cmp    %eax,%esi
80103545:	77 dd                	ja     80103524 <mpinit+0xf4>
    default:
      ismp = 0;
      break;
    }
  }
  if(!ismp)
80103547:	85 db                	test   %ebx,%ebx
80103549:	0f 84 9e 00 00 00    	je     801035ed <mpinit+0x1bd>
    panic("Didn't find a suitable machine");

  if(mp->imcrp){
8010354f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103552:	80 78 0c 00          	cmpb   $0x0,0xc(%eax)
80103556:	74 15                	je     8010356d <mpinit+0x13d>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103558:	b8 70 00 00 00       	mov    $0x70,%eax
8010355d:	ba 22 00 00 00       	mov    $0x22,%edx
80103562:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80103563:	ba 23 00 00 00       	mov    $0x23,%edx
80103568:	ec                   	in     (%dx),%al
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
80103569:	83 c8 01             	or     $0x1,%eax
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010356c:	ee                   	out    %al,(%dx)
  }
}
8010356d:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103570:	5b                   	pop    %ebx
80103571:	5e                   	pop    %esi
80103572:	5f                   	pop    %edi
80103573:	5d                   	pop    %ebp
80103574:	c3                   	ret    
80103575:	8d 76 00             	lea    0x0(%esi),%esi
      if(ncpu < NCPU) {
80103578:	8b 0d 00 2d 11 80    	mov    0x80112d00,%ecx
8010357e:	83 f9 07             	cmp    $0x7,%ecx
80103581:	7f 19                	jg     8010359c <mpinit+0x16c>
        cpus[ncpu].apicid = proc->apicid;  // apicid may differ from ncpu
80103583:	0f b6 50 01          	movzbl 0x1(%eax),%edx
80103587:	69 f9 b0 00 00 00    	imul   $0xb0,%ecx,%edi
        ncpu++;
8010358d:	83 c1 01             	add    $0x1,%ecx
80103590:	89 0d 00 2d 11 80    	mov    %ecx,0x80112d00
        cpus[ncpu].apicid = proc->apicid;  // apicid may differ from ncpu
80103596:	88 97 80 27 11 80    	mov    %dl,-0x7feed880(%edi)
      p += sizeof(struct mpproc);
8010359c:	83 c0 14             	add    $0x14,%eax
      continue;
8010359f:	e9 7c ff ff ff       	jmp    80103520 <mpinit+0xf0>
801035a4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      ioapicid = ioapic->apicno;
801035a8:	0f b6 50 01          	movzbl 0x1(%eax),%edx
      p += sizeof(struct mpioapic);
801035ac:	83 c0 08             	add    $0x8,%eax
      ioapicid = ioapic->apicno;
801035af:	88 15 60 27 11 80    	mov    %dl,0x80112760
      continue;
801035b5:	e9 66 ff ff ff       	jmp    80103520 <mpinit+0xf0>
801035ba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  return mpsearch1(0xF0000, 0x10000);
801035c0:	ba 00 00 01 00       	mov    $0x10000,%edx
801035c5:	b8 00 00 0f 00       	mov    $0xf0000,%eax
801035ca:	e8 e1 fd ff ff       	call   801033b0 <mpsearch1>
  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
801035cf:	85 c0                	test   %eax,%eax
  return mpsearch1(0xF0000, 0x10000);
801035d1:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
801035d4:	0f 85 a9 fe ff ff    	jne    80103483 <mpinit+0x53>
801035da:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    panic("Expect to run on an SMP");
801035e0:	83 ec 0c             	sub    $0xc,%esp
801035e3:	68 bd 79 10 80       	push   $0x801079bd
801035e8:	e8 a3 cd ff ff       	call   80100390 <panic>
    panic("Didn't find a suitable machine");
801035ed:	83 ec 0c             	sub    $0xc,%esp
801035f0:	68 dc 79 10 80       	push   $0x801079dc
801035f5:	e8 96 cd ff ff       	call   80100390 <panic>
      ismp = 0;
801035fa:	31 db                	xor    %ebx,%ebx
801035fc:	e9 26 ff ff ff       	jmp    80103527 <mpinit+0xf7>
80103601:	66 90                	xchg   %ax,%ax
80103603:	66 90                	xchg   %ax,%ax
80103605:	66 90                	xchg   %ax,%ax
80103607:	66 90                	xchg   %ax,%ax
80103609:	66 90                	xchg   %ax,%ax
8010360b:	66 90                	xchg   %ax,%ax
8010360d:	66 90                	xchg   %ax,%ax
8010360f:	90                   	nop

80103610 <picinit>:
#define IO_PIC2         0xA0    // Slave (IRQs 8-15)

// Don't use the 8259A interrupt controllers.  Xv6 assumes SMP hardware.
void
picinit(void)
{
80103610:	55                   	push   %ebp
80103611:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103616:	ba 21 00 00 00       	mov    $0x21,%edx
8010361b:	89 e5                	mov    %esp,%ebp
8010361d:	ee                   	out    %al,(%dx)
8010361e:	ba a1 00 00 00       	mov    $0xa1,%edx
80103623:	ee                   	out    %al,(%dx)
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
  outb(IO_PIC2+1, 0xFF);
}
80103624:	5d                   	pop    %ebp
80103625:	c3                   	ret    
80103626:	66 90                	xchg   %ax,%ax
80103628:	66 90                	xchg   %ax,%ax
8010362a:	66 90                	xchg   %ax,%ax
8010362c:	66 90                	xchg   %ax,%ax
8010362e:	66 90                	xchg   %ax,%ax

80103630 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80103630:	55                   	push   %ebp
80103631:	89 e5                	mov    %esp,%ebp
80103633:	57                   	push   %edi
80103634:	56                   	push   %esi
80103635:	53                   	push   %ebx
80103636:	83 ec 0c             	sub    $0xc,%esp
80103639:	8b 5d 08             	mov    0x8(%ebp),%ebx
8010363c:	8b 75 0c             	mov    0xc(%ebp),%esi
  struct pipe *p;

  p = 0;
  *f0 = *f1 = 0;
8010363f:	c7 06 00 00 00 00    	movl   $0x0,(%esi)
80103645:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
8010364b:	e8 80 d7 ff ff       	call   80100dd0 <filealloc>
80103650:	85 c0                	test   %eax,%eax
80103652:	89 03                	mov    %eax,(%ebx)
80103654:	74 22                	je     80103678 <pipealloc+0x48>
80103656:	e8 75 d7 ff ff       	call   80100dd0 <filealloc>
8010365b:	85 c0                	test   %eax,%eax
8010365d:	89 06                	mov    %eax,(%esi)
8010365f:	74 3f                	je     801036a0 <pipealloc+0x70>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103661:	e8 4a f2 ff ff       	call   801028b0 <kalloc>
80103666:	85 c0                	test   %eax,%eax
80103668:	89 c7                	mov    %eax,%edi
8010366a:	75 54                	jne    801036c0 <pipealloc+0x90>

//PAGEBREAK: 20
 bad:
  if(p)
    kfree((char*)p);
  if(*f0)
8010366c:	8b 03                	mov    (%ebx),%eax
8010366e:	85 c0                	test   %eax,%eax
80103670:	75 34                	jne    801036a6 <pipealloc+0x76>
80103672:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    fileclose(*f0);
  if(*f1)
80103678:	8b 06                	mov    (%esi),%eax
8010367a:	85 c0                	test   %eax,%eax
8010367c:	74 0c                	je     8010368a <pipealloc+0x5a>
    fileclose(*f1);
8010367e:	83 ec 0c             	sub    $0xc,%esp
80103681:	50                   	push   %eax
80103682:	e8 09 d8 ff ff       	call   80100e90 <fileclose>
80103687:	83 c4 10             	add    $0x10,%esp
  return -1;
}
8010368a:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return -1;
8010368d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80103692:	5b                   	pop    %ebx
80103693:	5e                   	pop    %esi
80103694:	5f                   	pop    %edi
80103695:	5d                   	pop    %ebp
80103696:	c3                   	ret    
80103697:	89 f6                	mov    %esi,%esi
80103699:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
  if(*f0)
801036a0:	8b 03                	mov    (%ebx),%eax
801036a2:	85 c0                	test   %eax,%eax
801036a4:	74 e4                	je     8010368a <pipealloc+0x5a>
    fileclose(*f0);
801036a6:	83 ec 0c             	sub    $0xc,%esp
801036a9:	50                   	push   %eax
801036aa:	e8 e1 d7 ff ff       	call   80100e90 <fileclose>
  if(*f1)
801036af:	8b 06                	mov    (%esi),%eax
    fileclose(*f0);
801036b1:	83 c4 10             	add    $0x10,%esp
  if(*f1)
801036b4:	85 c0                	test   %eax,%eax
801036b6:	75 c6                	jne    8010367e <pipealloc+0x4e>
801036b8:	eb d0                	jmp    8010368a <pipealloc+0x5a>
801036ba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  initlock(&p->lock, "pipe");
801036c0:	83 ec 08             	sub    $0x8,%esp
  p->readopen = 1;
801036c3:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
801036ca:	00 00 00 
  p->writeopen = 1;
801036cd:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
801036d4:	00 00 00 
  p->nwrite = 0;
801036d7:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
801036de:	00 00 00 
  p->nread = 0;
801036e1:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
801036e8:	00 00 00 
  initlock(&p->lock, "pipe");
801036eb:	68 10 7a 10 80       	push   $0x80107a10
801036f0:	50                   	push   %eax
801036f1:	e8 3a 10 00 00       	call   80104730 <initlock>
  (*f0)->type = FD_PIPE;
801036f6:	8b 03                	mov    (%ebx),%eax
  return 0;
801036f8:	83 c4 10             	add    $0x10,%esp
  (*f0)->type = FD_PIPE;
801036fb:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103701:	8b 03                	mov    (%ebx),%eax
80103703:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103707:	8b 03                	mov    (%ebx),%eax
80103709:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
8010370d:	8b 03                	mov    (%ebx),%eax
8010370f:	89 78 0c             	mov    %edi,0xc(%eax)
  (*f1)->type = FD_PIPE;
80103712:	8b 06                	mov    (%esi),%eax
80103714:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
8010371a:	8b 06                	mov    (%esi),%eax
8010371c:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103720:	8b 06                	mov    (%esi),%eax
80103722:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80103726:	8b 06                	mov    (%esi),%eax
80103728:	89 78 0c             	mov    %edi,0xc(%eax)
}
8010372b:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
8010372e:	31 c0                	xor    %eax,%eax
}
80103730:	5b                   	pop    %ebx
80103731:	5e                   	pop    %esi
80103732:	5f                   	pop    %edi
80103733:	5d                   	pop    %ebp
80103734:	c3                   	ret    
80103735:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80103739:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80103740 <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80103740:	55                   	push   %ebp
80103741:	89 e5                	mov    %esp,%ebp
80103743:	56                   	push   %esi
80103744:	53                   	push   %ebx
80103745:	8b 5d 08             	mov    0x8(%ebp),%ebx
80103748:	8b 75 0c             	mov    0xc(%ebp),%esi
  acquire(&p->lock);
8010374b:	83 ec 0c             	sub    $0xc,%esp
8010374e:	53                   	push   %ebx
8010374f:	e8 cc 10 00 00       	call   80104820 <acquire>
  if(writable){
80103754:	83 c4 10             	add    $0x10,%esp
80103757:	85 f6                	test   %esi,%esi
80103759:	74 45                	je     801037a0 <pipeclose+0x60>
    p->writeopen = 0;
    wakeup(&p->nread);
8010375b:	8d 83 34 02 00 00    	lea    0x234(%ebx),%eax
80103761:	83 ec 0c             	sub    $0xc,%esp
    p->writeopen = 0;
80103764:	c7 83 40 02 00 00 00 	movl   $0x0,0x240(%ebx)
8010376b:	00 00 00 
    wakeup(&p->nread);
8010376e:	50                   	push   %eax
8010376f:	e8 dc 0c 00 00       	call   80104450 <wakeup>
80103774:	83 c4 10             	add    $0x10,%esp
  } else {
    p->readopen = 0;
    wakeup(&p->nwrite);
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103777:	8b 93 3c 02 00 00    	mov    0x23c(%ebx),%edx
8010377d:	85 d2                	test   %edx,%edx
8010377f:	75 0a                	jne    8010378b <pipeclose+0x4b>
80103781:	8b 83 40 02 00 00    	mov    0x240(%ebx),%eax
80103787:	85 c0                	test   %eax,%eax
80103789:	74 35                	je     801037c0 <pipeclose+0x80>
    release(&p->lock);
    kfree((char*)p);
  } else
    release(&p->lock);
8010378b:	89 5d 08             	mov    %ebx,0x8(%ebp)
}
8010378e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103791:	5b                   	pop    %ebx
80103792:	5e                   	pop    %esi
80103793:	5d                   	pop    %ebp
    release(&p->lock);
80103794:	e9 a7 11 00 00       	jmp    80104940 <release>
80103799:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    wakeup(&p->nwrite);
801037a0:	8d 83 38 02 00 00    	lea    0x238(%ebx),%eax
801037a6:	83 ec 0c             	sub    $0xc,%esp
    p->readopen = 0;
801037a9:	c7 83 3c 02 00 00 00 	movl   $0x0,0x23c(%ebx)
801037b0:	00 00 00 
    wakeup(&p->nwrite);
801037b3:	50                   	push   %eax
801037b4:	e8 97 0c 00 00       	call   80104450 <wakeup>
801037b9:	83 c4 10             	add    $0x10,%esp
801037bc:	eb b9                	jmp    80103777 <pipeclose+0x37>
801037be:	66 90                	xchg   %ax,%ax
    release(&p->lock);
801037c0:	83 ec 0c             	sub    $0xc,%esp
801037c3:	53                   	push   %ebx
801037c4:	e8 77 11 00 00       	call   80104940 <release>
    kfree((char*)p);
801037c9:	89 5d 08             	mov    %ebx,0x8(%ebp)
801037cc:	83 c4 10             	add    $0x10,%esp
}
801037cf:	8d 65 f8             	lea    -0x8(%ebp),%esp
801037d2:	5b                   	pop    %ebx
801037d3:	5e                   	pop    %esi
801037d4:	5d                   	pop    %ebp
    kfree((char*)p);
801037d5:	e9 26 ef ff ff       	jmp    80102700 <kfree>
801037da:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801037e0 <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
801037e0:	55                   	push   %ebp
801037e1:	89 e5                	mov    %esp,%ebp
801037e3:	57                   	push   %edi
801037e4:	56                   	push   %esi
801037e5:	53                   	push   %ebx
801037e6:	83 ec 28             	sub    $0x28,%esp
801037e9:	8b 5d 08             	mov    0x8(%ebp),%ebx
  int i;

  acquire(&p->lock);
801037ec:	53                   	push   %ebx
801037ed:	e8 2e 10 00 00       	call   80104820 <acquire>
  for(i = 0; i < n; i++){
801037f2:	8b 45 10             	mov    0x10(%ebp),%eax
801037f5:	83 c4 10             	add    $0x10,%esp
801037f8:	85 c0                	test   %eax,%eax
801037fa:	0f 8e c9 00 00 00    	jle    801038c9 <pipewrite+0xe9>
80103800:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80103803:	8b 83 38 02 00 00    	mov    0x238(%ebx),%eax
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
      if(p->readopen == 0 || myproc()->killed){
        release(&p->lock);
        return -1;
      }
      wakeup(&p->nread);
80103809:	8d bb 34 02 00 00    	lea    0x234(%ebx),%edi
8010380f:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
80103812:	03 4d 10             	add    0x10(%ebp),%ecx
80103815:	89 4d e0             	mov    %ecx,-0x20(%ebp)
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103818:	8b 8b 34 02 00 00    	mov    0x234(%ebx),%ecx
8010381e:	8d 91 00 02 00 00    	lea    0x200(%ecx),%edx
80103824:	39 d0                	cmp    %edx,%eax
80103826:	75 71                	jne    80103899 <pipewrite+0xb9>
      if(p->readopen == 0 || myproc()->killed){
80103828:	8b 83 3c 02 00 00    	mov    0x23c(%ebx),%eax
8010382e:	85 c0                	test   %eax,%eax
80103830:	74 4e                	je     80103880 <pipewrite+0xa0>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103832:	8d b3 38 02 00 00    	lea    0x238(%ebx),%esi
80103838:	eb 3a                	jmp    80103874 <pipewrite+0x94>
8010383a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      wakeup(&p->nread);
80103840:	83 ec 0c             	sub    $0xc,%esp
80103843:	57                   	push   %edi
80103844:	e8 07 0c 00 00       	call   80104450 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103849:	5a                   	pop    %edx
8010384a:	59                   	pop    %ecx
8010384b:	53                   	push   %ebx
8010384c:	56                   	push   %esi
8010384d:	e8 1e 0a 00 00       	call   80104270 <sleep>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103852:	8b 83 34 02 00 00    	mov    0x234(%ebx),%eax
80103858:	8b 93 38 02 00 00    	mov    0x238(%ebx),%edx
8010385e:	83 c4 10             	add    $0x10,%esp
80103861:	05 00 02 00 00       	add    $0x200,%eax
80103866:	39 c2                	cmp    %eax,%edx
80103868:	75 36                	jne    801038a0 <pipewrite+0xc0>
      if(p->readopen == 0 || myproc()->killed){
8010386a:	8b 83 3c 02 00 00    	mov    0x23c(%ebx),%eax
80103870:	85 c0                	test   %eax,%eax
80103872:	74 0c                	je     80103880 <pipewrite+0xa0>
80103874:	e8 87 03 00 00       	call   80103c00 <myproc>
80103879:	8b 40 24             	mov    0x24(%eax),%eax
8010387c:	85 c0                	test   %eax,%eax
8010387e:	74 c0                	je     80103840 <pipewrite+0x60>
        release(&p->lock);
80103880:	83 ec 0c             	sub    $0xc,%esp
80103883:	53                   	push   %ebx
80103884:	e8 b7 10 00 00       	call   80104940 <release>
        return -1;
80103889:	83 c4 10             	add    $0x10,%esp
8010388c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
  release(&p->lock);
  return n;
}
80103891:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103894:	5b                   	pop    %ebx
80103895:	5e                   	pop    %esi
80103896:	5f                   	pop    %edi
80103897:	5d                   	pop    %ebp
80103898:	c3                   	ret    
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103899:	89 c2                	mov    %eax,%edx
8010389b:	90                   	nop
8010389c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
801038a0:	8b 75 e4             	mov    -0x1c(%ebp),%esi
801038a3:	8d 42 01             	lea    0x1(%edx),%eax
801038a6:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
801038ac:	89 83 38 02 00 00    	mov    %eax,0x238(%ebx)
801038b2:	83 c6 01             	add    $0x1,%esi
801038b5:	0f b6 4e ff          	movzbl -0x1(%esi),%ecx
  for(i = 0; i < n; i++){
801038b9:	3b 75 e0             	cmp    -0x20(%ebp),%esi
801038bc:	89 75 e4             	mov    %esi,-0x1c(%ebp)
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
801038bf:	88 4c 13 34          	mov    %cl,0x34(%ebx,%edx,1)
  for(i = 0; i < n; i++){
801038c3:	0f 85 4f ff ff ff    	jne    80103818 <pipewrite+0x38>
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
801038c9:	8d 83 34 02 00 00    	lea    0x234(%ebx),%eax
801038cf:	83 ec 0c             	sub    $0xc,%esp
801038d2:	50                   	push   %eax
801038d3:	e8 78 0b 00 00       	call   80104450 <wakeup>
  release(&p->lock);
801038d8:	89 1c 24             	mov    %ebx,(%esp)
801038db:	e8 60 10 00 00       	call   80104940 <release>
  return n;
801038e0:	83 c4 10             	add    $0x10,%esp
801038e3:	8b 45 10             	mov    0x10(%ebp),%eax
801038e6:	eb a9                	jmp    80103891 <pipewrite+0xb1>
801038e8:	90                   	nop
801038e9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

801038f0 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
801038f0:	55                   	push   %ebp
801038f1:	89 e5                	mov    %esp,%ebp
801038f3:	57                   	push   %edi
801038f4:	56                   	push   %esi
801038f5:	53                   	push   %ebx
801038f6:	83 ec 18             	sub    $0x18,%esp
801038f9:	8b 75 08             	mov    0x8(%ebp),%esi
801038fc:	8b 7d 0c             	mov    0xc(%ebp),%edi
  int i;

  acquire(&p->lock);
801038ff:	56                   	push   %esi
80103900:	e8 1b 0f 00 00       	call   80104820 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103905:	83 c4 10             	add    $0x10,%esp
80103908:	8b 8e 34 02 00 00    	mov    0x234(%esi),%ecx
8010390e:	3b 8e 38 02 00 00    	cmp    0x238(%esi),%ecx
80103914:	75 6a                	jne    80103980 <piperead+0x90>
80103916:	8b 9e 40 02 00 00    	mov    0x240(%esi),%ebx
8010391c:	85 db                	test   %ebx,%ebx
8010391e:	0f 84 c4 00 00 00    	je     801039e8 <piperead+0xf8>
    if(myproc()->killed){
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
80103924:	8d 9e 34 02 00 00    	lea    0x234(%esi),%ebx
8010392a:	eb 2d                	jmp    80103959 <piperead+0x69>
8010392c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80103930:	83 ec 08             	sub    $0x8,%esp
80103933:	56                   	push   %esi
80103934:	53                   	push   %ebx
80103935:	e8 36 09 00 00       	call   80104270 <sleep>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
8010393a:	83 c4 10             	add    $0x10,%esp
8010393d:	8b 8e 34 02 00 00    	mov    0x234(%esi),%ecx
80103943:	3b 8e 38 02 00 00    	cmp    0x238(%esi),%ecx
80103949:	75 35                	jne    80103980 <piperead+0x90>
8010394b:	8b 96 40 02 00 00    	mov    0x240(%esi),%edx
80103951:	85 d2                	test   %edx,%edx
80103953:	0f 84 8f 00 00 00    	je     801039e8 <piperead+0xf8>
    if(myproc()->killed){
80103959:	e8 a2 02 00 00       	call   80103c00 <myproc>
8010395e:	8b 48 24             	mov    0x24(%eax),%ecx
80103961:	85 c9                	test   %ecx,%ecx
80103963:	74 cb                	je     80103930 <piperead+0x40>
      release(&p->lock);
80103965:	83 ec 0c             	sub    $0xc,%esp
      return -1;
80103968:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
      release(&p->lock);
8010396d:	56                   	push   %esi
8010396e:	e8 cd 0f 00 00       	call   80104940 <release>
      return -1;
80103973:	83 c4 10             	add    $0x10,%esp
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
  release(&p->lock);
  return i;
}
80103976:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103979:	89 d8                	mov    %ebx,%eax
8010397b:	5b                   	pop    %ebx
8010397c:	5e                   	pop    %esi
8010397d:	5f                   	pop    %edi
8010397e:	5d                   	pop    %ebp
8010397f:	c3                   	ret    
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103980:	8b 45 10             	mov    0x10(%ebp),%eax
80103983:	85 c0                	test   %eax,%eax
80103985:	7e 61                	jle    801039e8 <piperead+0xf8>
    if(p->nread == p->nwrite)
80103987:	31 db                	xor    %ebx,%ebx
80103989:	eb 13                	jmp    8010399e <piperead+0xae>
8010398b:	90                   	nop
8010398c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80103990:	8b 8e 34 02 00 00    	mov    0x234(%esi),%ecx
80103996:	3b 8e 38 02 00 00    	cmp    0x238(%esi),%ecx
8010399c:	74 1f                	je     801039bd <piperead+0xcd>
    addr[i] = p->data[p->nread++ % PIPESIZE];
8010399e:	8d 41 01             	lea    0x1(%ecx),%eax
801039a1:	81 e1 ff 01 00 00    	and    $0x1ff,%ecx
801039a7:	89 86 34 02 00 00    	mov    %eax,0x234(%esi)
801039ad:	0f b6 44 0e 34       	movzbl 0x34(%esi,%ecx,1),%eax
801039b2:	88 04 1f             	mov    %al,(%edi,%ebx,1)
  for(i = 0; i < n; i++){  //DOC: piperead-copy
801039b5:	83 c3 01             	add    $0x1,%ebx
801039b8:	39 5d 10             	cmp    %ebx,0x10(%ebp)
801039bb:	75 d3                	jne    80103990 <piperead+0xa0>
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
801039bd:	8d 86 38 02 00 00    	lea    0x238(%esi),%eax
801039c3:	83 ec 0c             	sub    $0xc,%esp
801039c6:	50                   	push   %eax
801039c7:	e8 84 0a 00 00       	call   80104450 <wakeup>
  release(&p->lock);
801039cc:	89 34 24             	mov    %esi,(%esp)
801039cf:	e8 6c 0f 00 00       	call   80104940 <release>
  return i;
801039d4:	83 c4 10             	add    $0x10,%esp
}
801039d7:	8d 65 f4             	lea    -0xc(%ebp),%esp
801039da:	89 d8                	mov    %ebx,%eax
801039dc:	5b                   	pop    %ebx
801039dd:	5e                   	pop    %esi
801039de:	5f                   	pop    %edi
801039df:	5d                   	pop    %ebp
801039e0:	c3                   	ret    
801039e1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801039e8:	31 db                	xor    %ebx,%ebx
801039ea:	eb d1                	jmp    801039bd <piperead+0xcd>
801039ec:	66 90                	xchg   %ax,%ax
801039ee:	66 90                	xchg   %ax,%ax

801039f0 <allocproc>:
// Look in the process table for an UNUSED proc.
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc *
allocproc(void) {
801039f0:	55                   	push   %ebp
801039f1:	89 e5                	mov    %esp,%ebp
801039f3:	53                   	push   %ebx
    struct proc *p;
    char *sp;

    acquire(&ptable.lock);

    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801039f4:	bb 54 3d 11 80       	mov    $0x80113d54,%ebx
allocproc(void) {
801039f9:	83 ec 10             	sub    $0x10,%esp
        cprintf("ALLOCPROC-");
801039fc:	68 15 7a 10 80       	push   $0x80107a15
80103a01:	e8 5a cc ff ff       	call   80100660 <cprintf>
    acquire(&ptable.lock);
80103a06:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
80103a0d:	e8 0e 0e 00 00       	call   80104820 <acquire>
80103a12:	83 c4 10             	add    $0x10,%esp
80103a15:	eb 18                	jmp    80103a2f <allocproc+0x3f>
80103a17:	89 f6                	mov    %esi,%esi
80103a19:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80103a20:	83 eb 80             	sub    $0xffffff80,%ebx
80103a23:	81 fb 54 5d 11 80    	cmp    $0x80115d54,%ebx
80103a29:	0f 83 81 00 00 00    	jae    80103ab0 <allocproc+0xc0>
        if (p->state == UNUSED)
80103a2f:	8b 43 0c             	mov    0xc(%ebx),%eax
80103a32:	85 c0                	test   %eax,%eax
80103a34:	75 ea                	jne    80103a20 <allocproc+0x30>
        cprintf(">ALLOCPROC-FAILED!\t");
    return 0;

    found:
    p->state = EMBRYO;
    p->pid = nextpid++;
80103a36:	a1 04 a0 10 80       	mov    0x8010a004,%eax

    release(&ptable.lock);
80103a3b:	83 ec 0c             	sub    $0xc,%esp
    p->state = EMBRYO;
80103a3e:	c7 43 0c 01 00 00 00 	movl   $0x1,0xc(%ebx)
    p->pid = nextpid++;
80103a45:	8d 50 01             	lea    0x1(%eax),%edx
80103a48:	89 43 10             	mov    %eax,0x10(%ebx)
    release(&ptable.lock);
80103a4b:	68 20 3d 11 80       	push   $0x80113d20
    p->pid = nextpid++;
80103a50:	89 15 04 a0 10 80    	mov    %edx,0x8010a004
    release(&ptable.lock);
80103a56:	e8 e5 0e 00 00       	call   80104940 <release>

    // Allocate kernel stack.
    if ((p->kstack = kalloc()) == 0) {
80103a5b:	e8 50 ee ff ff       	call   801028b0 <kalloc>
80103a60:	83 c4 10             	add    $0x10,%esp
80103a63:	85 c0                	test   %eax,%eax
80103a65:	89 43 08             	mov    %eax,0x8(%ebx)
80103a68:	74 6b                	je     80103ad5 <allocproc+0xe5>
        return 0;
    }
    sp = p->kstack + KSTACKSIZE;

    // Leave room for trap frame.
    sp -= sizeof *p->tf;
80103a6a:	8d 90 b4 0f 00 00    	lea    0xfb4(%eax),%edx
    sp -= 4;
    *(uint *) sp = (uint) trapret;

    sp -= sizeof *p->context;
    p->context = (struct context *) sp;
    memset(p->context, 0, sizeof *p->context);
80103a70:	83 ec 04             	sub    $0x4,%esp
    sp -= sizeof *p->context;
80103a73:	05 9c 0f 00 00       	add    $0xf9c,%eax
    sp -= sizeof *p->tf;
80103a78:	89 53 18             	mov    %edx,0x18(%ebx)
    *(uint *) sp = (uint) trapret;
80103a7b:	c7 40 14 e2 5b 10 80 	movl   $0x80105be2,0x14(%eax)
    p->context = (struct context *) sp;
80103a82:	89 43 1c             	mov    %eax,0x1c(%ebx)
    memset(p->context, 0, sizeof *p->context);
80103a85:	6a 14                	push   $0x14
80103a87:	6a 00                	push   $0x0
80103a89:	50                   	push   %eax
80103a8a:	e8 11 0f 00 00       	call   801049a0 <memset>
    p->context->eip = (uint) forkret;
80103a8f:	8b 43 1c             	mov    0x1c(%ebx),%eax
80103a92:	c7 40 10 f0 3a 10 80 	movl   $0x80103af0,0x10(%eax)
    if (DEBUGMODE==1)
        cprintf(">ALLOCPROC-DONE!\t");
80103a99:	c7 04 24 4f 7a 10 80 	movl   $0x80107a4f,(%esp)
80103aa0:	e8 bb cb ff ff       	call   80100660 <cprintf>
    return p;
80103aa5:	83 c4 10             	add    $0x10,%esp
}
80103aa8:	89 d8                	mov    %ebx,%eax
80103aaa:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103aad:	c9                   	leave  
80103aae:	c3                   	ret    
80103aaf:	90                   	nop
    release(&ptable.lock);
80103ab0:	83 ec 0c             	sub    $0xc,%esp
        cprintf(">ALLOCPROC-FAILED!\t");
80103ab3:	31 db                	xor    %ebx,%ebx
    release(&ptable.lock);
80103ab5:	68 20 3d 11 80       	push   $0x80113d20
80103aba:	e8 81 0e 00 00       	call   80104940 <release>
        cprintf(">ALLOCPROC-FAILED!\t");
80103abf:	c7 04 24 20 7a 10 80 	movl   $0x80107a20,(%esp)
80103ac6:	e8 95 cb ff ff       	call   80100660 <cprintf>
}
80103acb:	89 d8                	mov    %ebx,%eax
        cprintf(">ALLOCPROC-FAILED!\t");
80103acd:	83 c4 10             	add    $0x10,%esp
}
80103ad0:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103ad3:	c9                   	leave  
80103ad4:	c3                   	ret    
            cprintf(">ALLOCPROC-FAILED-kalloc!\t");
80103ad5:	83 ec 0c             	sub    $0xc,%esp
        p->state = UNUSED;
80103ad8:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
        return 0;
80103adf:	31 db                	xor    %ebx,%ebx
            cprintf(">ALLOCPROC-FAILED-kalloc!\t");
80103ae1:	68 34 7a 10 80       	push   $0x80107a34
80103ae6:	e8 75 cb ff ff       	call   80100660 <cprintf>
        return 0;
80103aeb:	83 c4 10             	add    $0x10,%esp
80103aee:	eb b8                	jmp    80103aa8 <allocproc+0xb8>

80103af0 <forkret>:
}

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void) {
80103af0:	55                   	push   %ebp
80103af1:	89 e5                	mov    %esp,%ebp
80103af3:	83 ec 14             	sub    $0x14,%esp
    static int first = 1;
    // Still holding ptable.lock from scheduler.
    release(&ptable.lock);
80103af6:	68 20 3d 11 80       	push   $0x80113d20
80103afb:	e8 40 0e 00 00       	call   80104940 <release>

    if (first) {
80103b00:	a1 00 a0 10 80       	mov    0x8010a000,%eax
80103b05:	83 c4 10             	add    $0x10,%esp
80103b08:	85 c0                	test   %eax,%eax
80103b0a:	75 04                	jne    80103b10 <forkret+0x20>
        iinit(ROOTDEV);
        initlog(ROOTDEV);
    }

    // Return to "caller", actually trapret (see allocproc).
}
80103b0c:	c9                   	leave  
80103b0d:	c3                   	ret    
80103b0e:	66 90                	xchg   %ax,%ax
        iinit(ROOTDEV);
80103b10:	83 ec 0c             	sub    $0xc,%esp
        first = 0;
80103b13:	c7 05 00 a0 10 80 00 	movl   $0x0,0x8010a000
80103b1a:	00 00 00 
        iinit(ROOTDEV);
80103b1d:	6a 01                	push   $0x1
80103b1f:	e8 bc d9 ff ff       	call   801014e0 <iinit>
        initlog(ROOTDEV);
80103b24:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80103b2b:	e8 c0 f3 ff ff       	call   80102ef0 <initlog>
80103b30:	83 c4 10             	add    $0x10,%esp
}
80103b33:	c9                   	leave  
80103b34:	c3                   	ret    
80103b35:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80103b39:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80103b40 <pinit>:
pinit(void) {
80103b40:	55                   	push   %ebp
80103b41:	89 e5                	mov    %esp,%ebp
80103b43:	83 ec 10             	sub    $0x10,%esp
    initlock(&ptable.lock, "ptable");
80103b46:	68 61 7a 10 80       	push   $0x80107a61
80103b4b:	68 20 3d 11 80       	push   $0x80113d20
80103b50:	e8 db 0b 00 00       	call   80104730 <initlock>
}
80103b55:	83 c4 10             	add    $0x10,%esp
80103b58:	c9                   	leave  
80103b59:	c3                   	ret    
80103b5a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80103b60 <mycpu>:
mycpu(void) {
80103b60:	55                   	push   %ebp
80103b61:	89 e5                	mov    %esp,%ebp
80103b63:	56                   	push   %esi
80103b64:	53                   	push   %ebx
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80103b65:	9c                   	pushf  
80103b66:	58                   	pop    %eax
    if (readeflags() & FL_IF)
80103b67:	f6 c4 02             	test   $0x2,%ah
80103b6a:	75 5e                	jne    80103bca <mycpu+0x6a>
    apicid = lapicid();
80103b6c:	e8 af ef ff ff       	call   80102b20 <lapicid>
    for (i = 0; i < ncpu; ++i) {
80103b71:	8b 35 00 2d 11 80    	mov    0x80112d00,%esi
80103b77:	85 f6                	test   %esi,%esi
80103b79:	7e 42                	jle    80103bbd <mycpu+0x5d>
        if (cpus[i].apicid == apicid)
80103b7b:	0f b6 15 80 27 11 80 	movzbl 0x80112780,%edx
80103b82:	39 d0                	cmp    %edx,%eax
80103b84:	74 30                	je     80103bb6 <mycpu+0x56>
80103b86:	b9 30 28 11 80       	mov    $0x80112830,%ecx
    for (i = 0; i < ncpu; ++i) {
80103b8b:	31 d2                	xor    %edx,%edx
80103b8d:	8d 76 00             	lea    0x0(%esi),%esi
80103b90:	83 c2 01             	add    $0x1,%edx
80103b93:	39 f2                	cmp    %esi,%edx
80103b95:	74 26                	je     80103bbd <mycpu+0x5d>
        if (cpus[i].apicid == apicid)
80103b97:	0f b6 19             	movzbl (%ecx),%ebx
80103b9a:	81 c1 b0 00 00 00    	add    $0xb0,%ecx
80103ba0:	39 c3                	cmp    %eax,%ebx
80103ba2:	75 ec                	jne    80103b90 <mycpu+0x30>
80103ba4:	69 c2 b0 00 00 00    	imul   $0xb0,%edx,%eax
80103baa:	05 80 27 11 80       	add    $0x80112780,%eax
}
80103baf:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103bb2:	5b                   	pop    %ebx
80103bb3:	5e                   	pop    %esi
80103bb4:	5d                   	pop    %ebp
80103bb5:	c3                   	ret    
        if (cpus[i].apicid == apicid)
80103bb6:	b8 80 27 11 80       	mov    $0x80112780,%eax
            return &cpus[i];
80103bbb:	eb f2                	jmp    80103baf <mycpu+0x4f>
    panic("unknown apicid\n");
80103bbd:	83 ec 0c             	sub    $0xc,%esp
80103bc0:	68 68 7a 10 80       	push   $0x80107a68
80103bc5:	e8 c6 c7 ff ff       	call   80100390 <panic>
        panic("mycpu called with interrupts enabled\n");
80103bca:	83 ec 0c             	sub    $0xc,%esp
80103bcd:	68 04 7c 10 80       	push   $0x80107c04
80103bd2:	e8 b9 c7 ff ff       	call   80100390 <panic>
80103bd7:	89 f6                	mov    %esi,%esi
80103bd9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80103be0 <cpuid>:
cpuid() {
80103be0:	55                   	push   %ebp
80103be1:	89 e5                	mov    %esp,%ebp
80103be3:	83 ec 08             	sub    $0x8,%esp
    return mycpu() - cpus;
80103be6:	e8 75 ff ff ff       	call   80103b60 <mycpu>
80103beb:	2d 80 27 11 80       	sub    $0x80112780,%eax
}
80103bf0:	c9                   	leave  
    return mycpu() - cpus;
80103bf1:	c1 f8 04             	sar    $0x4,%eax
80103bf4:	69 c0 a3 8b 2e ba    	imul   $0xba2e8ba3,%eax,%eax
}
80103bfa:	c3                   	ret    
80103bfb:	90                   	nop
80103bfc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80103c00 <myproc>:
myproc(void) {
80103c00:	55                   	push   %ebp
80103c01:	89 e5                	mov    %esp,%ebp
80103c03:	53                   	push   %ebx
80103c04:	83 ec 04             	sub    $0x4,%esp
    pushcli();
80103c07:	e8 d4 0b 00 00       	call   801047e0 <pushcli>
    c = mycpu();
80103c0c:	e8 4f ff ff ff       	call   80103b60 <mycpu>
    p = c->proc;
80103c11:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
    popcli();
80103c17:	e8 c4 0c 00 00       	call   801048e0 <popcli>
}
80103c1c:	83 c4 04             	add    $0x4,%esp
80103c1f:	89 d8                	mov    %ebx,%eax
80103c21:	5b                   	pop    %ebx
80103c22:	5d                   	pop    %ebp
80103c23:	c3                   	ret    
80103c24:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80103c2a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80103c30 <userinit>:
userinit(void) {
80103c30:	55                   	push   %ebp
80103c31:	89 e5                	mov    %esp,%ebp
80103c33:	53                   	push   %ebx
80103c34:	83 ec 10             	sub    $0x10,%esp
        cprintf("USERINIT-");
80103c37:	68 78 7a 10 80       	push   $0x80107a78
80103c3c:	e8 1f ca ff ff       	call   80100660 <cprintf>
    p = allocproc();
80103c41:	e8 aa fd ff ff       	call   801039f0 <allocproc>
80103c46:	89 c3                	mov    %eax,%ebx
    initproc = p;
80103c48:	a3 b8 a5 10 80       	mov    %eax,0x8010a5b8
    if ((p->pgdir = setupkvm()) == 0)
80103c4d:	e8 5e 35 00 00       	call   801071b0 <setupkvm>
80103c52:	83 c4 10             	add    $0x10,%esp
80103c55:	85 c0                	test   %eax,%eax
80103c57:	89 43 04             	mov    %eax,0x4(%ebx)
80103c5a:	0f 84 c9 00 00 00    	je     80103d29 <userinit+0xf9>
    inituvm(p->pgdir, _binary_initcode_start, (int) _binary_initcode_size);
80103c60:	83 ec 04             	sub    $0x4,%esp
80103c63:	68 2c 00 00 00       	push   $0x2c
80103c68:	68 60 a4 10 80       	push   $0x8010a460
80103c6d:	50                   	push   %eax
80103c6e:	e8 1d 32 00 00       	call   80106e90 <inituvm>
    memset(p->tf, 0, sizeof(*p->tf));
80103c73:	83 c4 0c             	add    $0xc,%esp
    p->sz = PGSIZE;
80103c76:	c7 03 00 10 00 00    	movl   $0x1000,(%ebx)
    memset(p->tf, 0, sizeof(*p->tf));
80103c7c:	6a 4c                	push   $0x4c
80103c7e:	6a 00                	push   $0x0
80103c80:	ff 73 18             	pushl  0x18(%ebx)
80103c83:	e8 18 0d 00 00       	call   801049a0 <memset>
    p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80103c88:	8b 43 18             	mov    0x18(%ebx),%eax
80103c8b:	ba 1b 00 00 00       	mov    $0x1b,%edx
    p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80103c90:	b9 23 00 00 00       	mov    $0x23,%ecx
    safestrcpy(p->name, "initcode", sizeof(p->name));
80103c95:	83 c4 0c             	add    $0xc,%esp
    p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80103c98:	66 89 50 3c          	mov    %dx,0x3c(%eax)
    p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
80103c9c:	8b 43 18             	mov    0x18(%ebx),%eax
80103c9f:	66 89 48 2c          	mov    %cx,0x2c(%eax)
    p->tf->es = p->tf->ds;
80103ca3:	8b 43 18             	mov    0x18(%ebx),%eax
80103ca6:	0f b7 50 2c          	movzwl 0x2c(%eax),%edx
80103caa:	66 89 50 28          	mov    %dx,0x28(%eax)
    p->tf->ss = p->tf->ds;
80103cae:	8b 43 18             	mov    0x18(%ebx),%eax
80103cb1:	0f b7 50 2c          	movzwl 0x2c(%eax),%edx
80103cb5:	66 89 50 48          	mov    %dx,0x48(%eax)
    p->tf->eflags = FL_IF;
80103cb9:	8b 43 18             	mov    0x18(%ebx),%eax
80103cbc:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
    p->tf->esp = PGSIZE;
80103cc3:	8b 43 18             	mov    0x18(%ebx),%eax
80103cc6:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
    p->tf->eip = 0;  // beginning of initcode.S
80103ccd:	8b 43 18             	mov    0x18(%ebx),%eax
80103cd0:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)
    safestrcpy(p->name, "initcode", sizeof(p->name));
80103cd7:	8d 43 6c             	lea    0x6c(%ebx),%eax
80103cda:	6a 10                	push   $0x10
80103cdc:	68 9b 7a 10 80       	push   $0x80107a9b
80103ce1:	50                   	push   %eax
80103ce2:	e8 99 0e 00 00       	call   80104b80 <safestrcpy>
    p->cwd = namei("/");
80103ce7:	c7 04 24 a4 7a 10 80 	movl   $0x80107aa4,(%esp)
80103cee:	e8 4d e2 ff ff       	call   80101f40 <namei>
80103cf3:	89 43 68             	mov    %eax,0x68(%ebx)
    acquire(&ptable.lock);
80103cf6:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
80103cfd:	e8 1e 0b 00 00       	call   80104820 <acquire>
    p->state = RUNNABLE;
80103d02:	c7 43 0c 03 00 00 00 	movl   $0x3,0xc(%ebx)
    release(&ptable.lock);
80103d09:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
80103d10:	e8 2b 0c 00 00       	call   80104940 <release>
        cprintf(">USERINIT-DONE!\t");
80103d15:	c7 04 24 a6 7a 10 80 	movl   $0x80107aa6,(%esp)
80103d1c:	e8 3f c9 ff ff       	call   80100660 <cprintf>
}
80103d21:	83 c4 10             	add    $0x10,%esp
80103d24:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80103d27:	c9                   	leave  
80103d28:	c3                   	ret    
        panic("userinit: out of memory?");
80103d29:	83 ec 0c             	sub    $0xc,%esp
80103d2c:	68 82 7a 10 80       	push   $0x80107a82
80103d31:	e8 5a c6 ff ff       	call   80100390 <panic>
80103d36:	8d 76 00             	lea    0x0(%esi),%esi
80103d39:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80103d40 <growproc>:
growproc(int n) {
80103d40:	55                   	push   %ebp
80103d41:	89 e5                	mov    %esp,%ebp
80103d43:	56                   	push   %esi
80103d44:	53                   	push   %ebx
80103d45:	8b 75 08             	mov    0x8(%ebp),%esi
        cprintf("GROWPROC-");
80103d48:	83 ec 0c             	sub    $0xc,%esp
80103d4b:	68 b7 7a 10 80       	push   $0x80107ab7
80103d50:	e8 0b c9 ff ff       	call   80100660 <cprintf>
    pushcli();
80103d55:	e8 86 0a 00 00       	call   801047e0 <pushcli>
    c = mycpu();
80103d5a:	e8 01 fe ff ff       	call   80103b60 <mycpu>
    p = c->proc;
80103d5f:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
    popcli();
80103d65:	e8 76 0b 00 00       	call   801048e0 <popcli>
    if (n > 0) {
80103d6a:	83 c4 10             	add    $0x10,%esp
80103d6d:	83 fe 00             	cmp    $0x0,%esi
    sz = curproc->sz;
80103d70:	8b 03                	mov    (%ebx),%eax
    if (n > 0) {
80103d72:	7f 2c                	jg     80103da0 <growproc+0x60>
    } else if (n < 0) {
80103d74:	75 5a                	jne    80103dd0 <growproc+0x90>
    switchuvm(curproc);
80103d76:	83 ec 0c             	sub    $0xc,%esp
    curproc->sz = sz;
80103d79:	89 03                	mov    %eax,(%ebx)
    switchuvm(curproc);
80103d7b:	53                   	push   %ebx
80103d7c:	e8 ff 2f 00 00       	call   80106d80 <switchuvm>
        cprintf(">GROWPROC-DONE!\t");
80103d81:	c7 04 24 d4 7a 10 80 	movl   $0x80107ad4,(%esp)
80103d88:	e8 d3 c8 ff ff       	call   80100660 <cprintf>
    return 0;
80103d8d:	83 c4 10             	add    $0x10,%esp
80103d90:	31 c0                	xor    %eax,%eax
}
80103d92:	8d 65 f8             	lea    -0x8(%ebp),%esp
80103d95:	5b                   	pop    %ebx
80103d96:	5e                   	pop    %esi
80103d97:	5d                   	pop    %ebp
80103d98:	c3                   	ret    
80103d99:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
        if ((sz = allocuvm(curproc->pgdir, sz, sz + n)) == 0) {
80103da0:	83 ec 04             	sub    $0x4,%esp
80103da3:	01 c6                	add    %eax,%esi
80103da5:	56                   	push   %esi
80103da6:	50                   	push   %eax
80103da7:	ff 73 04             	pushl  0x4(%ebx)
80103daa:	e8 21 32 00 00       	call   80106fd0 <allocuvm>
80103daf:	83 c4 10             	add    $0x10,%esp
80103db2:	85 c0                	test   %eax,%eax
80103db4:	75 c0                	jne    80103d76 <growproc+0x36>
                cprintf(">GROWPROC-FAILED!\t");
80103db6:	83 ec 0c             	sub    $0xc,%esp
80103db9:	68 c1 7a 10 80       	push   $0x80107ac1
80103dbe:	e8 9d c8 ff ff       	call   80100660 <cprintf>
            return -1;
80103dc3:	83 c4 10             	add    $0x10,%esp
80103dc6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103dcb:	eb c5                	jmp    80103d92 <growproc+0x52>
80103dcd:	8d 76 00             	lea    0x0(%esi),%esi
        if ((sz = deallocuvm(curproc->pgdir, sz, sz + n)) == 0) {
80103dd0:	83 ec 04             	sub    $0x4,%esp
80103dd3:	01 c6                	add    %eax,%esi
80103dd5:	56                   	push   %esi
80103dd6:	50                   	push   %eax
80103dd7:	ff 73 04             	pushl  0x4(%ebx)
80103dda:	e8 21 33 00 00       	call   80107100 <deallocuvm>
80103ddf:	83 c4 10             	add    $0x10,%esp
80103de2:	85 c0                	test   %eax,%eax
80103de4:	75 90                	jne    80103d76 <growproc+0x36>
80103de6:	eb ce                	jmp    80103db6 <growproc+0x76>
80103de8:	90                   	nop
80103de9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80103df0 <fork>:
fork(void) {
80103df0:	55                   	push   %ebp
80103df1:	89 e5                	mov    %esp,%ebp
80103df3:	57                   	push   %edi
80103df4:	56                   	push   %esi
80103df5:	53                   	push   %ebx
80103df6:	83 ec 28             	sub    $0x28,%esp
        cprintf("FORK-");
80103df9:	68 e5 7a 10 80       	push   $0x80107ae5
80103dfe:	e8 5d c8 ff ff       	call   80100660 <cprintf>
    pushcli();
80103e03:	e8 d8 09 00 00       	call   801047e0 <pushcli>
    c = mycpu();
80103e08:	e8 53 fd ff ff       	call   80103b60 <mycpu>
    p = c->proc;
80103e0d:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
    popcli();
80103e13:	e8 c8 0a 00 00       	call   801048e0 <popcli>
    if ((np = allocproc()) == 0) {
80103e18:	e8 d3 fb ff ff       	call   801039f0 <allocproc>
80103e1d:	83 c4 10             	add    $0x10,%esp
80103e20:	85 c0                	test   %eax,%eax
80103e22:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80103e25:	0f 84 fe 00 00 00    	je     80103f29 <fork+0x139>
    if (firstRun)
80103e2b:	8b 0d 08 a0 10 80    	mov    0x8010a008,%ecx
80103e31:	85 c9                	test   %ecx,%ecx
80103e33:	0f 85 df 00 00 00    	jne    80103f18 <fork+0x128>
    createSwapFile(np);
80103e39:	8b 7d e4             	mov    -0x1c(%ebp),%edi
80103e3c:	83 ec 0c             	sub    $0xc,%esp
80103e3f:	57                   	push   %edi
80103e40:	e8 cb e3 ff ff       	call   80102210 <createSwapFile>
    if ((np->pgdir = copyuvm(curproc->pgdir, curproc->sz)) == 0) {
80103e45:	58                   	pop    %eax
80103e46:	5a                   	pop    %edx
80103e47:	ff 33                	pushl  (%ebx)
80103e49:	ff 73 04             	pushl  0x4(%ebx)
80103e4c:	e8 2f 34 00 00       	call   80107280 <copyuvm>
80103e51:	83 c4 10             	add    $0x10,%esp
80103e54:	85 c0                	test   %eax,%eax
80103e56:	89 47 04             	mov    %eax,0x4(%edi)
80103e59:	0f 84 e1 00 00 00    	je     80103f40 <fork+0x150>
    np->sz = curproc->sz;
80103e5f:	8b 03                	mov    (%ebx),%eax
80103e61:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80103e64:	89 01                	mov    %eax,(%ecx)
    np->parent = curproc;
80103e66:	89 59 14             	mov    %ebx,0x14(%ecx)
80103e69:	89 c8                	mov    %ecx,%eax
    *np->tf = *curproc->tf;
80103e6b:	8b 79 18             	mov    0x18(%ecx),%edi
80103e6e:	8b 73 18             	mov    0x18(%ebx),%esi
80103e71:	b9 13 00 00 00       	mov    $0x13,%ecx
80103e76:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
    for (i = 0; i < NOFILE; i++)
80103e78:	31 f6                	xor    %esi,%esi
    np->tf->eax = 0;
80103e7a:	8b 40 18             	mov    0x18(%eax),%eax
80103e7d:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)
80103e84:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        if (curproc->ofile[i])
80103e88:	8b 44 b3 28          	mov    0x28(%ebx,%esi,4),%eax
80103e8c:	85 c0                	test   %eax,%eax
80103e8e:	74 13                	je     80103ea3 <fork+0xb3>
            np->ofile[i] = filedup(curproc->ofile[i]);
80103e90:	83 ec 0c             	sub    $0xc,%esp
80103e93:	50                   	push   %eax
80103e94:	e8 a7 cf ff ff       	call   80100e40 <filedup>
80103e99:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80103e9c:	83 c4 10             	add    $0x10,%esp
80103e9f:	89 44 b2 28          	mov    %eax,0x28(%edx,%esi,4)
    for (i = 0; i < NOFILE; i++)
80103ea3:	83 c6 01             	add    $0x1,%esi
80103ea6:	83 fe 10             	cmp    $0x10,%esi
80103ea9:	75 dd                	jne    80103e88 <fork+0x98>
    np->cwd = idup(curproc->cwd);
80103eab:	83 ec 0c             	sub    $0xc,%esp
80103eae:	ff 73 68             	pushl  0x68(%ebx)
    safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80103eb1:	83 c3 6c             	add    $0x6c,%ebx
    np->cwd = idup(curproc->cwd);
80103eb4:	e8 f7 d7 ff ff       	call   801016b0 <idup>
80103eb9:	8b 7d e4             	mov    -0x1c(%ebp),%edi
    safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80103ebc:	83 c4 0c             	add    $0xc,%esp
    np->cwd = idup(curproc->cwd);
80103ebf:	89 47 68             	mov    %eax,0x68(%edi)
    safestrcpy(np->name, curproc->name, sizeof(curproc->name));
80103ec2:	8d 47 6c             	lea    0x6c(%edi),%eax
80103ec5:	6a 10                	push   $0x10
80103ec7:	53                   	push   %ebx
80103ec8:	50                   	push   %eax
80103ec9:	e8 b2 0c 00 00       	call   80104b80 <safestrcpy>
    pid = np->pid;
80103ece:	8b 5f 10             	mov    0x10(%edi),%ebx
    acquire(&ptable.lock);
80103ed1:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
80103ed8:	e8 43 09 00 00       	call   80104820 <acquire>
    np->state = RUNNABLE;
80103edd:	c7 47 0c 03 00 00 00 	movl   $0x3,0xc(%edi)
    release(&ptable.lock);
80103ee4:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
80103eeb:	e8 50 0a 00 00       	call   80104940 <release>
        cprintf(">FORK-DONE!\t");
80103ef0:	c7 04 24 fa 7a 10 80 	movl   $0x80107afa,(%esp)
    firstRun = 0;
80103ef7:	c7 05 08 a0 10 80 00 	movl   $0x0,0x8010a008
80103efe:	00 00 00 
        cprintf(">FORK-DONE!\t");
80103f01:	e8 5a c7 ff ff       	call   80100660 <cprintf>
    return pid;
80103f06:	83 c4 10             	add    $0x10,%esp
}
80103f09:	8d 65 f4             	lea    -0xc(%ebp),%esp
80103f0c:	89 d8                	mov    %ebx,%eax
80103f0e:	5b                   	pop    %ebx
80103f0f:	5e                   	pop    %esi
80103f10:	5f                   	pop    %edi
80103f11:	5d                   	pop    %ebp
80103f12:	c3                   	ret    
80103f13:	90                   	nop
80103f14:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        createSwapFile(curproc);
80103f18:	83 ec 0c             	sub    $0xc,%esp
80103f1b:	53                   	push   %ebx
80103f1c:	e8 ef e2 ff ff       	call   80102210 <createSwapFile>
80103f21:	83 c4 10             	add    $0x10,%esp
80103f24:	e9 10 ff ff ff       	jmp    80103e39 <fork+0x49>
            cprintf(">FORK-FAILED!\t");
80103f29:	83 ec 0c             	sub    $0xc,%esp
        return -1;
80103f2c:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
            cprintf(">FORK-FAILED!\t");
80103f31:	68 eb 7a 10 80       	push   $0x80107aeb
80103f36:	e8 25 c7 ff ff       	call   80100660 <cprintf>
        return -1;
80103f3b:	83 c4 10             	add    $0x10,%esp
80103f3e:	eb c9                	jmp    80103f09 <fork+0x119>
        kfree(np->kstack);
80103f40:	8b 7d e4             	mov    -0x1c(%ebp),%edi
80103f43:	83 ec 0c             	sub    $0xc,%esp
        return -1;
80103f46:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
        kfree(np->kstack);
80103f4b:	ff 77 08             	pushl  0x8(%edi)
80103f4e:	e8 ad e7 ff ff       	call   80102700 <kfree>
        np->kstack = 0;
80103f53:	c7 47 08 00 00 00 00 	movl   $0x0,0x8(%edi)
        np->state = UNUSED;
80103f5a:	c7 47 0c 00 00 00 00 	movl   $0x0,0xc(%edi)
            cprintf(">FORK-FAILED!\t");
80103f61:	c7 04 24 eb 7a 10 80 	movl   $0x80107aeb,(%esp)
80103f68:	e8 f3 c6 ff ff       	call   80100660 <cprintf>
        return -1;
80103f6d:	83 c4 10             	add    $0x10,%esp
80103f70:	eb 97                	jmp    80103f09 <fork+0x119>
80103f72:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80103f79:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80103f80 <scheduler>:
scheduler(void) {
80103f80:	55                   	push   %ebp
80103f81:	89 e5                	mov    %esp,%ebp
80103f83:	57                   	push   %edi
80103f84:	56                   	push   %esi
80103f85:	53                   	push   %ebx
80103f86:	83 ec 0c             	sub    $0xc,%esp
    struct cpu *c = mycpu();
80103f89:	e8 d2 fb ff ff       	call   80103b60 <mycpu>
80103f8e:	8d 78 04             	lea    0x4(%eax),%edi
80103f91:	89 c6                	mov    %eax,%esi
    c->proc = 0;
80103f93:	c7 80 ac 00 00 00 00 	movl   $0x0,0xac(%eax)
80103f9a:	00 00 00 
80103f9d:	8d 76 00             	lea    0x0(%esi),%esi
  asm volatile("sti");
80103fa0:	fb                   	sti    
        acquire(&ptable.lock);
80103fa1:	83 ec 0c             	sub    $0xc,%esp
        for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80103fa4:	bb 54 3d 11 80       	mov    $0x80113d54,%ebx
        acquire(&ptable.lock);
80103fa9:	68 20 3d 11 80       	push   $0x80113d20
80103fae:	e8 6d 08 00 00       	call   80104820 <acquire>
80103fb3:	83 c4 10             	add    $0x10,%esp
80103fb6:	8d 76 00             	lea    0x0(%esi),%esi
80103fb9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
            if (p->state != RUNNABLE)
80103fc0:	83 7b 0c 03          	cmpl   $0x3,0xc(%ebx)
80103fc4:	75 33                	jne    80103ff9 <scheduler+0x79>
            switchuvm(p);
80103fc6:	83 ec 0c             	sub    $0xc,%esp
            c->proc = p;
80103fc9:	89 9e ac 00 00 00    	mov    %ebx,0xac(%esi)
            switchuvm(p);
80103fcf:	53                   	push   %ebx
80103fd0:	e8 ab 2d 00 00       	call   80106d80 <switchuvm>
            swtch(&(c->scheduler), p->context);
80103fd5:	58                   	pop    %eax
80103fd6:	5a                   	pop    %edx
80103fd7:	ff 73 1c             	pushl  0x1c(%ebx)
80103fda:	57                   	push   %edi
            p->state = RUNNING;
80103fdb:	c7 43 0c 04 00 00 00 	movl   $0x4,0xc(%ebx)
            swtch(&(c->scheduler), p->context);
80103fe2:	e8 f4 0b 00 00       	call   80104bdb <swtch>
            switchkvm();
80103fe7:	e8 74 2d 00 00       	call   80106d60 <switchkvm>
            c->proc = 0;
80103fec:	c7 86 ac 00 00 00 00 	movl   $0x0,0xac(%esi)
80103ff3:	00 00 00 
80103ff6:	83 c4 10             	add    $0x10,%esp
        for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80103ff9:	83 eb 80             	sub    $0xffffff80,%ebx
80103ffc:	81 fb 54 5d 11 80    	cmp    $0x80115d54,%ebx
80104002:	72 bc                	jb     80103fc0 <scheduler+0x40>
        release(&ptable.lock);
80104004:	83 ec 0c             	sub    $0xc,%esp
80104007:	68 20 3d 11 80       	push   $0x80113d20
8010400c:	e8 2f 09 00 00       	call   80104940 <release>
        sti();
80104011:	83 c4 10             	add    $0x10,%esp
80104014:	eb 8a                	jmp    80103fa0 <scheduler+0x20>
80104016:	8d 76 00             	lea    0x0(%esi),%esi
80104019:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104020 <sched>:
sched(void) {
80104020:	55                   	push   %ebp
80104021:	89 e5                	mov    %esp,%ebp
80104023:	56                   	push   %esi
80104024:	53                   	push   %ebx
    pushcli();
80104025:	e8 b6 07 00 00       	call   801047e0 <pushcli>
    c = mycpu();
8010402a:	e8 31 fb ff ff       	call   80103b60 <mycpu>
    p = c->proc;
8010402f:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
    popcli();
80104035:	e8 a6 08 00 00       	call   801048e0 <popcli>
    if (!holding(&ptable.lock))
8010403a:	83 ec 0c             	sub    $0xc,%esp
8010403d:	68 20 3d 11 80       	push   $0x80113d20
80104042:	e8 59 07 00 00       	call   801047a0 <holding>
80104047:	83 c4 10             	add    $0x10,%esp
8010404a:	85 c0                	test   %eax,%eax
8010404c:	74 4f                	je     8010409d <sched+0x7d>
    if (mycpu()->ncli != 1)
8010404e:	e8 0d fb ff ff       	call   80103b60 <mycpu>
80104053:	83 b8 a4 00 00 00 01 	cmpl   $0x1,0xa4(%eax)
8010405a:	75 68                	jne    801040c4 <sched+0xa4>
    if (p->state == RUNNING)
8010405c:	83 7b 0c 04          	cmpl   $0x4,0xc(%ebx)
80104060:	74 55                	je     801040b7 <sched+0x97>
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104062:	9c                   	pushf  
80104063:	58                   	pop    %eax
    if (readeflags() & FL_IF)
80104064:	f6 c4 02             	test   $0x2,%ah
80104067:	75 41                	jne    801040aa <sched+0x8a>
    intena = mycpu()->intena;
80104069:	e8 f2 fa ff ff       	call   80103b60 <mycpu>
    swtch(&p->context, mycpu()->scheduler);
8010406e:	83 c3 1c             	add    $0x1c,%ebx
    intena = mycpu()->intena;
80104071:	8b b0 a8 00 00 00    	mov    0xa8(%eax),%esi
    swtch(&p->context, mycpu()->scheduler);
80104077:	e8 e4 fa ff ff       	call   80103b60 <mycpu>
8010407c:	83 ec 08             	sub    $0x8,%esp
8010407f:	ff 70 04             	pushl  0x4(%eax)
80104082:	53                   	push   %ebx
80104083:	e8 53 0b 00 00       	call   80104bdb <swtch>
    mycpu()->intena = intena;
80104088:	e8 d3 fa ff ff       	call   80103b60 <mycpu>
}
8010408d:	83 c4 10             	add    $0x10,%esp
    mycpu()->intena = intena;
80104090:	89 b0 a8 00 00 00    	mov    %esi,0xa8(%eax)
}
80104096:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104099:	5b                   	pop    %ebx
8010409a:	5e                   	pop    %esi
8010409b:	5d                   	pop    %ebp
8010409c:	c3                   	ret    
        panic("sched ptable.lock");
8010409d:	83 ec 0c             	sub    $0xc,%esp
801040a0:	68 07 7b 10 80       	push   $0x80107b07
801040a5:	e8 e6 c2 ff ff       	call   80100390 <panic>
        panic("sched interruptible");
801040aa:	83 ec 0c             	sub    $0xc,%esp
801040ad:	68 33 7b 10 80       	push   $0x80107b33
801040b2:	e8 d9 c2 ff ff       	call   80100390 <panic>
        panic("sched running");
801040b7:	83 ec 0c             	sub    $0xc,%esp
801040ba:	68 25 7b 10 80       	push   $0x80107b25
801040bf:	e8 cc c2 ff ff       	call   80100390 <panic>
        panic("sched locks");
801040c4:	83 ec 0c             	sub    $0xc,%esp
801040c7:	68 19 7b 10 80       	push   $0x80107b19
801040cc:	e8 bf c2 ff ff       	call   80100390 <panic>
801040d1:	eb 0d                	jmp    801040e0 <exit>
801040d3:	90                   	nop
801040d4:	90                   	nop
801040d5:	90                   	nop
801040d6:	90                   	nop
801040d7:	90                   	nop
801040d8:	90                   	nop
801040d9:	90                   	nop
801040da:	90                   	nop
801040db:	90                   	nop
801040dc:	90                   	nop
801040dd:	90                   	nop
801040de:	90                   	nop
801040df:	90                   	nop

801040e0 <exit>:
exit(void) {
801040e0:	55                   	push   %ebp
801040e1:	89 e5                	mov    %esp,%ebp
801040e3:	57                   	push   %edi
801040e4:	56                   	push   %esi
801040e5:	53                   	push   %ebx
801040e6:	83 ec 18             	sub    $0x18,%esp
        cprintf("EXIT-");
801040e9:	68 47 7b 10 80       	push   $0x80107b47
801040ee:	e8 6d c5 ff ff       	call   80100660 <cprintf>
    pushcli();
801040f3:	e8 e8 06 00 00       	call   801047e0 <pushcli>
    c = mycpu();
801040f8:	e8 63 fa ff ff       	call   80103b60 <mycpu>
    p = c->proc;
801040fd:	8b b0 ac 00 00 00    	mov    0xac(%eax),%esi
    popcli();
80104103:	e8 d8 07 00 00       	call   801048e0 <popcli>
    if (curproc == initproc)
80104108:	83 c4 10             	add    $0x10,%esp
8010410b:	39 35 b8 a5 10 80    	cmp    %esi,0x8010a5b8
80104111:	8d 5e 28             	lea    0x28(%esi),%ebx
80104114:	8d 7e 68             	lea    0x68(%esi),%edi
80104117:	0f 84 f6 00 00 00    	je     80104213 <exit+0x133>
8010411d:	8d 76 00             	lea    0x0(%esi),%esi
        if (curproc->ofile[fd]) {
80104120:	8b 03                	mov    (%ebx),%eax
80104122:	85 c0                	test   %eax,%eax
80104124:	74 12                	je     80104138 <exit+0x58>
            fileclose(curproc->ofile[fd]);
80104126:	83 ec 0c             	sub    $0xc,%esp
80104129:	50                   	push   %eax
8010412a:	e8 61 cd ff ff       	call   80100e90 <fileclose>
            curproc->ofile[fd] = 0;
8010412f:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
80104135:	83 c4 10             	add    $0x10,%esp
80104138:	83 c3 04             	add    $0x4,%ebx
    for (fd = 0; fd < NOFILE; fd++) {
8010413b:	39 fb                	cmp    %edi,%ebx
8010413d:	75 e1                	jne    80104120 <exit+0x40>
    begin_op();
8010413f:	e8 4c ee ff ff       	call   80102f90 <begin_op>
    iput(curproc->cwd);
80104144:	83 ec 0c             	sub    $0xc,%esp
80104147:	ff 76 68             	pushl  0x68(%esi)
8010414a:	e8 c1 d6 ff ff       	call   80101810 <iput>
    end_op();
8010414f:	e8 ac ee ff ff       	call   80103000 <end_op>
    curproc->cwd = 0;
80104154:	c7 46 68 00 00 00 00 	movl   $0x0,0x68(%esi)
    acquire(&ptable.lock);
8010415b:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
80104162:	e8 b9 06 00 00       	call   80104820 <acquire>
    wakeup1(curproc->parent);
80104167:	8b 56 14             	mov    0x14(%esi),%edx
8010416a:	83 c4 10             	add    $0x10,%esp
// The ptable lock must be held.
static void
wakeup1(void *chan) {
    struct proc *p;

    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010416d:	b8 54 3d 11 80       	mov    $0x80113d54,%eax
80104172:	eb 0e                	jmp    80104182 <exit+0xa2>
80104174:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104178:	83 e8 80             	sub    $0xffffff80,%eax
8010417b:	3d 54 5d 11 80       	cmp    $0x80115d54,%eax
80104180:	73 1c                	jae    8010419e <exit+0xbe>
        if (p->state == SLEEPING && p->chan == chan)
80104182:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
80104186:	75 f0                	jne    80104178 <exit+0x98>
80104188:	3b 50 20             	cmp    0x20(%eax),%edx
8010418b:	75 eb                	jne    80104178 <exit+0x98>
            p->state = RUNNABLE;
8010418d:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104194:	83 e8 80             	sub    $0xffffff80,%eax
80104197:	3d 54 5d 11 80       	cmp    $0x80115d54,%eax
8010419c:	72 e4                	jb     80104182 <exit+0xa2>
            p->parent = initproc;
8010419e:	8b 0d b8 a5 10 80    	mov    0x8010a5b8,%ecx
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
801041a4:	ba 54 3d 11 80       	mov    $0x80113d54,%edx
801041a9:	eb 10                	jmp    801041bb <exit+0xdb>
801041ab:	90                   	nop
801041ac:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801041b0:	83 ea 80             	sub    $0xffffff80,%edx
801041b3:	81 fa 54 5d 11 80    	cmp    $0x80115d54,%edx
801041b9:	73 33                	jae    801041ee <exit+0x10e>
        if (p->parent == curproc) {
801041bb:	39 72 14             	cmp    %esi,0x14(%edx)
801041be:	75 f0                	jne    801041b0 <exit+0xd0>
            if (p->state == ZOMBIE)
801041c0:	83 7a 0c 05          	cmpl   $0x5,0xc(%edx)
            p->parent = initproc;
801041c4:	89 4a 14             	mov    %ecx,0x14(%edx)
            if (p->state == ZOMBIE)
801041c7:	75 e7                	jne    801041b0 <exit+0xd0>
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801041c9:	b8 54 3d 11 80       	mov    $0x80113d54,%eax
801041ce:	eb 0a                	jmp    801041da <exit+0xfa>
801041d0:	83 e8 80             	sub    $0xffffff80,%eax
801041d3:	3d 54 5d 11 80       	cmp    $0x80115d54,%eax
801041d8:	73 d6                	jae    801041b0 <exit+0xd0>
        if (p->state == SLEEPING && p->chan == chan)
801041da:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
801041de:	75 f0                	jne    801041d0 <exit+0xf0>
801041e0:	3b 48 20             	cmp    0x20(%eax),%ecx
801041e3:	75 eb                	jne    801041d0 <exit+0xf0>
            p->state = RUNNABLE;
801041e5:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
801041ec:	eb e2                	jmp    801041d0 <exit+0xf0>
        cprintf(">EXIT-DONE!\t");
801041ee:	83 ec 0c             	sub    $0xc,%esp
    curproc->state = ZOMBIE;
801041f1:	c7 46 0c 05 00 00 00 	movl   $0x5,0xc(%esi)
        cprintf(">EXIT-DONE!\t");
801041f8:	68 5a 7b 10 80       	push   $0x80107b5a
801041fd:	e8 5e c4 ff ff       	call   80100660 <cprintf>
    sched();
80104202:	e8 19 fe ff ff       	call   80104020 <sched>
    panic("zombie exit");
80104207:	c7 04 24 67 7b 10 80 	movl   $0x80107b67,(%esp)
8010420e:	e8 7d c1 ff ff       	call   80100390 <panic>
        panic("init exiting");
80104213:	83 ec 0c             	sub    $0xc,%esp
80104216:	68 4d 7b 10 80       	push   $0x80107b4d
8010421b:	e8 70 c1 ff ff       	call   80100390 <panic>

80104220 <yield>:
yield(void) {
80104220:	55                   	push   %ebp
80104221:	89 e5                	mov    %esp,%ebp
80104223:	53                   	push   %ebx
80104224:	83 ec 10             	sub    $0x10,%esp
    acquire(&ptable.lock);  //DOC: yieldlock
80104227:	68 20 3d 11 80       	push   $0x80113d20
8010422c:	e8 ef 05 00 00       	call   80104820 <acquire>
    pushcli();
80104231:	e8 aa 05 00 00       	call   801047e0 <pushcli>
    c = mycpu();
80104236:	e8 25 f9 ff ff       	call   80103b60 <mycpu>
    p = c->proc;
8010423b:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
    popcli();
80104241:	e8 9a 06 00 00       	call   801048e0 <popcli>
    myproc()->state = RUNNABLE;
80104246:	c7 43 0c 03 00 00 00 	movl   $0x3,0xc(%ebx)
    sched();
8010424d:	e8 ce fd ff ff       	call   80104020 <sched>
    release(&ptable.lock);
80104252:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
80104259:	e8 e2 06 00 00       	call   80104940 <release>
}
8010425e:	83 c4 10             	add    $0x10,%esp
80104261:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104264:	c9                   	leave  
80104265:	c3                   	ret    
80104266:	8d 76 00             	lea    0x0(%esi),%esi
80104269:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104270 <sleep>:
sleep(void *chan, struct spinlock *lk) {
80104270:	55                   	push   %ebp
80104271:	89 e5                	mov    %esp,%ebp
80104273:	57                   	push   %edi
80104274:	56                   	push   %esi
80104275:	53                   	push   %ebx
80104276:	83 ec 0c             	sub    $0xc,%esp
80104279:	8b 7d 08             	mov    0x8(%ebp),%edi
8010427c:	8b 75 0c             	mov    0xc(%ebp),%esi
    pushcli();
8010427f:	e8 5c 05 00 00       	call   801047e0 <pushcli>
    c = mycpu();
80104284:	e8 d7 f8 ff ff       	call   80103b60 <mycpu>
    p = c->proc;
80104289:	8b 98 ac 00 00 00    	mov    0xac(%eax),%ebx
    popcli();
8010428f:	e8 4c 06 00 00       	call   801048e0 <popcli>
    if (p == 0)
80104294:	85 db                	test   %ebx,%ebx
80104296:	0f 84 87 00 00 00    	je     80104323 <sleep+0xb3>
    if (lk == 0)
8010429c:	85 f6                	test   %esi,%esi
8010429e:	74 76                	je     80104316 <sleep+0xa6>
    if (lk != &ptable.lock) {  //DOC: sleeplock0
801042a0:	81 fe 20 3d 11 80    	cmp    $0x80113d20,%esi
801042a6:	74 50                	je     801042f8 <sleep+0x88>
        acquire(&ptable.lock);  //DOC: sleeplock1
801042a8:	83 ec 0c             	sub    $0xc,%esp
801042ab:	68 20 3d 11 80       	push   $0x80113d20
801042b0:	e8 6b 05 00 00       	call   80104820 <acquire>
        release(lk);
801042b5:	89 34 24             	mov    %esi,(%esp)
801042b8:	e8 83 06 00 00       	call   80104940 <release>
    p->chan = chan;
801042bd:	89 7b 20             	mov    %edi,0x20(%ebx)
    p->state = SLEEPING;
801042c0:	c7 43 0c 02 00 00 00 	movl   $0x2,0xc(%ebx)
    sched();
801042c7:	e8 54 fd ff ff       	call   80104020 <sched>
    p->chan = 0;
801042cc:	c7 43 20 00 00 00 00 	movl   $0x0,0x20(%ebx)
        release(&ptable.lock);
801042d3:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
801042da:	e8 61 06 00 00       	call   80104940 <release>
        acquire(lk);
801042df:	89 75 08             	mov    %esi,0x8(%ebp)
801042e2:	83 c4 10             	add    $0x10,%esp
}
801042e5:	8d 65 f4             	lea    -0xc(%ebp),%esp
801042e8:	5b                   	pop    %ebx
801042e9:	5e                   	pop    %esi
801042ea:	5f                   	pop    %edi
801042eb:	5d                   	pop    %ebp
        acquire(lk);
801042ec:	e9 2f 05 00 00       	jmp    80104820 <acquire>
801042f1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    p->chan = chan;
801042f8:	89 7b 20             	mov    %edi,0x20(%ebx)
    p->state = SLEEPING;
801042fb:	c7 43 0c 02 00 00 00 	movl   $0x2,0xc(%ebx)
    sched();
80104302:	e8 19 fd ff ff       	call   80104020 <sched>
    p->chan = 0;
80104307:	c7 43 20 00 00 00 00 	movl   $0x0,0x20(%ebx)
}
8010430e:	8d 65 f4             	lea    -0xc(%ebp),%esp
80104311:	5b                   	pop    %ebx
80104312:	5e                   	pop    %esi
80104313:	5f                   	pop    %edi
80104314:	5d                   	pop    %ebp
80104315:	c3                   	ret    
        panic("sleep without lk");
80104316:	83 ec 0c             	sub    $0xc,%esp
80104319:	68 79 7b 10 80       	push   $0x80107b79
8010431e:	e8 6d c0 ff ff       	call   80100390 <panic>
        panic("sleep");
80104323:	83 ec 0c             	sub    $0xc,%esp
80104326:	68 73 7b 10 80       	push   $0x80107b73
8010432b:	e8 60 c0 ff ff       	call   80100390 <panic>

80104330 <wait>:
wait(void) {
80104330:	55                   	push   %ebp
80104331:	89 e5                	mov    %esp,%ebp
80104333:	56                   	push   %esi
80104334:	53                   	push   %ebx
        cprintf("WAIT-");
80104335:	83 ec 0c             	sub    $0xc,%esp
80104338:	68 8a 7b 10 80       	push   $0x80107b8a
8010433d:	e8 1e c3 ff ff       	call   80100660 <cprintf>
    pushcli();
80104342:	e8 99 04 00 00       	call   801047e0 <pushcli>
    c = mycpu();
80104347:	e8 14 f8 ff ff       	call   80103b60 <mycpu>
    p = c->proc;
8010434c:	8b b0 ac 00 00 00    	mov    0xac(%eax),%esi
    popcli();
80104352:	e8 89 05 00 00       	call   801048e0 <popcli>
    acquire(&ptable.lock);
80104357:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
8010435e:	e8 bd 04 00 00       	call   80104820 <acquire>
80104363:	83 c4 10             	add    $0x10,%esp
        havekids = 0;
80104366:	31 c0                	xor    %eax,%eax
        for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104368:	bb 54 3d 11 80       	mov    $0x80113d54,%ebx
8010436d:	eb 0c                	jmp    8010437b <wait+0x4b>
8010436f:	90                   	nop
80104370:	83 eb 80             	sub    $0xffffff80,%ebx
80104373:	81 fb 54 5d 11 80    	cmp    $0x80115d54,%ebx
80104379:	73 1b                	jae    80104396 <wait+0x66>
            if (p->parent != curproc)
8010437b:	39 73 14             	cmp    %esi,0x14(%ebx)
8010437e:	75 f0                	jne    80104370 <wait+0x40>
            if (p->state == ZOMBIE) {
80104380:	83 7b 0c 05          	cmpl   $0x5,0xc(%ebx)
80104384:	74 3a                	je     801043c0 <wait+0x90>
        for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104386:	83 eb 80             	sub    $0xffffff80,%ebx
            havekids = 1;
80104389:	b8 01 00 00 00       	mov    $0x1,%eax
        for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
8010438e:	81 fb 54 5d 11 80    	cmp    $0x80115d54,%ebx
80104394:	72 e5                	jb     8010437b <wait+0x4b>
        if (!havekids || curproc->killed) {
80104396:	85 c0                	test   %eax,%eax
80104398:	0f 84 84 00 00 00    	je     80104422 <wait+0xf2>
8010439e:	8b 46 24             	mov    0x24(%esi),%eax
801043a1:	85 c0                	test   %eax,%eax
801043a3:	75 7d                	jne    80104422 <wait+0xf2>
        sleep(curproc, &ptable.lock);  //DOC: wait-sleep
801043a5:	83 ec 08             	sub    $0x8,%esp
801043a8:	68 20 3d 11 80       	push   $0x80113d20
801043ad:	56                   	push   %esi
801043ae:	e8 bd fe ff ff       	call   80104270 <sleep>
        havekids = 0;
801043b3:	83 c4 10             	add    $0x10,%esp
801043b6:	eb ae                	jmp    80104366 <wait+0x36>
801043b8:	90                   	nop
801043b9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
                kfree(p->kstack);
801043c0:	83 ec 0c             	sub    $0xc,%esp
801043c3:	ff 73 08             	pushl  0x8(%ebx)
                pid = p->pid;
801043c6:	8b 73 10             	mov    0x10(%ebx),%esi
                kfree(p->kstack);
801043c9:	e8 32 e3 ff ff       	call   80102700 <kfree>
                freevm(p->pgdir);
801043ce:	5a                   	pop    %edx
801043cf:	ff 73 04             	pushl  0x4(%ebx)
                p->kstack = 0;
801043d2:	c7 43 08 00 00 00 00 	movl   $0x0,0x8(%ebx)
                freevm(p->pgdir);
801043d9:	e8 52 2d 00 00       	call   80107130 <freevm>
                release(&ptable.lock);
801043de:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
                p->pid = 0;
801043e5:	c7 43 10 00 00 00 00 	movl   $0x0,0x10(%ebx)
                p->parent = 0;
801043ec:	c7 43 14 00 00 00 00 	movl   $0x0,0x14(%ebx)
                p->name[0] = 0;
801043f3:	c6 43 6c 00          	movb   $0x0,0x6c(%ebx)
                p->killed = 0;
801043f7:	c7 43 24 00 00 00 00 	movl   $0x0,0x24(%ebx)
                p->state = UNUSED;
801043fe:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
                release(&ptable.lock);
80104405:	e8 36 05 00 00       	call   80104940 <release>
                    cprintf(">WAIT-DONE!");
8010440a:	c7 04 24 90 7b 10 80 	movl   $0x80107b90,(%esp)
80104411:	e8 4a c2 ff ff       	call   80100660 <cprintf>
                return pid;
80104416:	83 c4 10             	add    $0x10,%esp
}
80104419:	8d 65 f8             	lea    -0x8(%ebp),%esp
8010441c:	89 f0                	mov    %esi,%eax
8010441e:	5b                   	pop    %ebx
8010441f:	5e                   	pop    %esi
80104420:	5d                   	pop    %ebp
80104421:	c3                   	ret    
            release(&ptable.lock);
80104422:	83 ec 0c             	sub    $0xc,%esp
            return -1;
80104425:	be ff ff ff ff       	mov    $0xffffffff,%esi
            release(&ptable.lock);
8010442a:	68 20 3d 11 80       	push   $0x80113d20
8010442f:	e8 0c 05 00 00       	call   80104940 <release>
                cprintf(">WAIT-FAILED!");
80104434:	c7 04 24 9c 7b 10 80 	movl   $0x80107b9c,(%esp)
8010443b:	e8 20 c2 ff ff       	call   80100660 <cprintf>
            return -1;
80104440:	83 c4 10             	add    $0x10,%esp
80104443:	eb d4                	jmp    80104419 <wait+0xe9>
80104445:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104449:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104450 <wakeup>:
}

// Wake up all processes sleeping on chan.
void
wakeup(void *chan) {
80104450:	55                   	push   %ebp
80104451:	89 e5                	mov    %esp,%ebp
80104453:	53                   	push   %ebx
80104454:	83 ec 10             	sub    $0x10,%esp
80104457:	8b 5d 08             	mov    0x8(%ebp),%ebx
    acquire(&ptable.lock);
8010445a:	68 20 3d 11 80       	push   $0x80113d20
8010445f:	e8 bc 03 00 00       	call   80104820 <acquire>
80104464:	83 c4 10             	add    $0x10,%esp
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104467:	b8 54 3d 11 80       	mov    $0x80113d54,%eax
8010446c:	eb 0c                	jmp    8010447a <wakeup+0x2a>
8010446e:	66 90                	xchg   %ax,%ax
80104470:	83 e8 80             	sub    $0xffffff80,%eax
80104473:	3d 54 5d 11 80       	cmp    $0x80115d54,%eax
80104478:	73 1c                	jae    80104496 <wakeup+0x46>
        if (p->state == SLEEPING && p->chan == chan)
8010447a:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
8010447e:	75 f0                	jne    80104470 <wakeup+0x20>
80104480:	3b 58 20             	cmp    0x20(%eax),%ebx
80104483:	75 eb                	jne    80104470 <wakeup+0x20>
            p->state = RUNNABLE;
80104485:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010448c:	83 e8 80             	sub    $0xffffff80,%eax
8010448f:	3d 54 5d 11 80       	cmp    $0x80115d54,%eax
80104494:	72 e4                	jb     8010447a <wakeup+0x2a>
    wakeup1(chan);
    release(&ptable.lock);
80104496:	c7 45 08 20 3d 11 80 	movl   $0x80113d20,0x8(%ebp)
}
8010449d:	8b 5d fc             	mov    -0x4(%ebp),%ebx
801044a0:	c9                   	leave  
    release(&ptable.lock);
801044a1:	e9 9a 04 00 00       	jmp    80104940 <release>
801044a6:	8d 76 00             	lea    0x0(%esi),%esi
801044a9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801044b0 <kill>:

// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid) {
801044b0:	55                   	push   %ebp
801044b1:	89 e5                	mov    %esp,%ebp
801044b3:	53                   	push   %ebx
801044b4:	83 ec 10             	sub    $0x10,%esp
801044b7:	8b 5d 08             	mov    0x8(%ebp),%ebx
    if (DEBUGMODE==1)
        cprintf("KILL-");
801044ba:	68 aa 7b 10 80       	push   $0x80107baa
801044bf:	e8 9c c1 ff ff       	call   80100660 <cprintf>
    struct proc *p;

    acquire(&ptable.lock);
801044c4:	c7 04 24 20 3d 11 80 	movl   $0x80113d20,(%esp)
801044cb:	e8 50 03 00 00       	call   80104820 <acquire>
801044d0:	83 c4 10             	add    $0x10,%esp
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
801044d3:	b8 54 3d 11 80       	mov    $0x80113d54,%eax
801044d8:	eb 10                	jmp    801044ea <kill+0x3a>
801044da:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
801044e0:	83 e8 80             	sub    $0xffffff80,%eax
801044e3:	3d 54 5d 11 80       	cmp    $0x80115d54,%eax
801044e8:	73 46                	jae    80104530 <kill+0x80>
        if (p->pid == pid) {
801044ea:	39 58 10             	cmp    %ebx,0x10(%eax)
801044ed:	75 f1                	jne    801044e0 <kill+0x30>
            p->killed = 1;
            // Wake process from sleep if necessary.
            if (p->state == SLEEPING)
801044ef:	83 78 0c 02          	cmpl   $0x2,0xc(%eax)
            p->killed = 1;
801044f3:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
            if (p->state == SLEEPING)
801044fa:	75 07                	jne    80104503 <kill+0x53>
                p->state = RUNNABLE;
801044fc:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
            release(&ptable.lock);
80104503:	83 ec 0c             	sub    $0xc,%esp
80104506:	68 20 3d 11 80       	push   $0x80113d20
8010450b:	e8 30 04 00 00       	call   80104940 <release>
            if (DEBUGMODE==1)
                cprintf(">KILL-DONE!\t");
80104510:	c7 04 24 b0 7b 10 80 	movl   $0x80107bb0,(%esp)
80104517:	e8 44 c1 ff ff       	call   80100660 <cprintf>
            return 0;
8010451c:	83 c4 10             	add    $0x10,%esp
8010451f:	31 c0                	xor    %eax,%eax
    }
    release(&ptable.lock);
    if (DEBUGMODE==1)
        cprintf(">KILL-FAILED!\t");
    return -1;
}
80104521:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104524:	c9                   	leave  
80104525:	c3                   	ret    
80104526:	8d 76 00             	lea    0x0(%esi),%esi
80104529:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    release(&ptable.lock);
80104530:	83 ec 0c             	sub    $0xc,%esp
80104533:	68 20 3d 11 80       	push   $0x80113d20
80104538:	e8 03 04 00 00       	call   80104940 <release>
        cprintf(">KILL-FAILED!\t");
8010453d:	c7 04 24 bd 7b 10 80 	movl   $0x80107bbd,(%esp)
80104544:	e8 17 c1 ff ff       	call   80100660 <cprintf>
    return -1;
80104549:	83 c4 10             	add    $0x10,%esp
8010454c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104551:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104554:	c9                   	leave  
80104555:	c3                   	ret    
80104556:	8d 76 00             	lea    0x0(%esi),%esi
80104559:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104560 <procdump>:
//PAGEBREAK: 36
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void) {
80104560:	55                   	push   %ebp
80104561:	89 e5                	mov    %esp,%ebp
80104563:	57                   	push   %edi
80104564:	56                   	push   %esi
80104565:	53                   	push   %ebx
80104566:	8d 75 e8             	lea    -0x18(%ebp),%esi
    int i;
    struct proc *p;
    char *state;
    uint pc[10];

    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104569:	bb 54 3d 11 80       	mov    $0x80113d54,%ebx
procdump(void) {
8010456e:	83 ec 3c             	sub    $0x3c,%esp
80104571:	eb 24                	jmp    80104597 <procdump+0x37>
80104573:	90                   	nop
80104574:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
        if (p->state == SLEEPING) {
            getcallerpcs((uint *) p->context->ebp + 2, pc);
            for (i = 0; i < 10 && pc[i] != 0; i++)
                cprintf(" %p", pc[i]);
        }
        cprintf("\n");
80104578:	83 ec 0c             	sub    $0xc,%esp
8010457b:	68 17 7f 10 80       	push   $0x80107f17
80104580:	e8 db c0 ff ff       	call   80100660 <cprintf>
80104585:	83 c4 10             	add    $0x10,%esp
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104588:	83 eb 80             	sub    $0xffffff80,%ebx
8010458b:	81 fb 54 5d 11 80    	cmp    $0x80115d54,%ebx
80104591:	0f 83 81 00 00 00    	jae    80104618 <procdump+0xb8>
        if (p->state == UNUSED)
80104597:	8b 43 0c             	mov    0xc(%ebx),%eax
8010459a:	85 c0                	test   %eax,%eax
8010459c:	74 ea                	je     80104588 <procdump+0x28>
        if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
8010459e:	83 f8 05             	cmp    $0x5,%eax
            state = "???";
801045a1:	ba cc 7b 10 80       	mov    $0x80107bcc,%edx
        if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
801045a6:	77 11                	ja     801045b9 <procdump+0x59>
801045a8:	8b 14 85 2c 7c 10 80 	mov    -0x7fef83d4(,%eax,4),%edx
            state = "???";
801045af:	b8 cc 7b 10 80       	mov    $0x80107bcc,%eax
801045b4:	85 d2                	test   %edx,%edx
801045b6:	0f 44 d0             	cmove  %eax,%edx
        cprintf("%d %s %s", p->pid, state, p->name);
801045b9:	8d 43 6c             	lea    0x6c(%ebx),%eax
801045bc:	50                   	push   %eax
801045bd:	52                   	push   %edx
801045be:	ff 73 10             	pushl  0x10(%ebx)
801045c1:	68 d0 7b 10 80       	push   $0x80107bd0
801045c6:	e8 95 c0 ff ff       	call   80100660 <cprintf>
        if (p->state == SLEEPING) {
801045cb:	83 c4 10             	add    $0x10,%esp
801045ce:	83 7b 0c 02          	cmpl   $0x2,0xc(%ebx)
801045d2:	75 a4                	jne    80104578 <procdump+0x18>
            getcallerpcs((uint *) p->context->ebp + 2, pc);
801045d4:	8d 45 c0             	lea    -0x40(%ebp),%eax
801045d7:	83 ec 08             	sub    $0x8,%esp
801045da:	8d 7d c0             	lea    -0x40(%ebp),%edi
801045dd:	50                   	push   %eax
801045de:	8b 43 1c             	mov    0x1c(%ebx),%eax
801045e1:	8b 40 0c             	mov    0xc(%eax),%eax
801045e4:	83 c0 08             	add    $0x8,%eax
801045e7:	50                   	push   %eax
801045e8:	e8 63 01 00 00       	call   80104750 <getcallerpcs>
801045ed:	83 c4 10             	add    $0x10,%esp
            for (i = 0; i < 10 && pc[i] != 0; i++)
801045f0:	8b 17                	mov    (%edi),%edx
801045f2:	85 d2                	test   %edx,%edx
801045f4:	74 82                	je     80104578 <procdump+0x18>
                cprintf(" %p", pc[i]);
801045f6:	83 ec 08             	sub    $0x8,%esp
801045f9:	83 c7 04             	add    $0x4,%edi
801045fc:	52                   	push   %edx
801045fd:	68 81 74 10 80       	push   $0x80107481
80104602:	e8 59 c0 ff ff       	call   80100660 <cprintf>
            for (i = 0; i < 10 && pc[i] != 0; i++)
80104607:	83 c4 10             	add    $0x10,%esp
8010460a:	39 fe                	cmp    %edi,%esi
8010460c:	75 e2                	jne    801045f0 <procdump+0x90>
8010460e:	e9 65 ff ff ff       	jmp    80104578 <procdump+0x18>
80104613:	90                   	nop
80104614:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    }
}
80104618:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010461b:	5b                   	pop    %ebx
8010461c:	5e                   	pop    %esi
8010461d:	5f                   	pop    %edi
8010461e:	5d                   	pop    %ebp
8010461f:	c3                   	ret    

80104620 <initsleeplock>:
#include "spinlock.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
80104620:	55                   	push   %ebp
80104621:	89 e5                	mov    %esp,%ebp
80104623:	53                   	push   %ebx
80104624:	83 ec 0c             	sub    $0xc,%esp
80104627:	8b 5d 08             	mov    0x8(%ebp),%ebx
  initlock(&lk->lk, "sleep lock");
8010462a:	68 44 7c 10 80       	push   $0x80107c44
8010462f:	8d 43 04             	lea    0x4(%ebx),%eax
80104632:	50                   	push   %eax
80104633:	e8 f8 00 00 00       	call   80104730 <initlock>
  lk->name = name;
80104638:	8b 45 0c             	mov    0xc(%ebp),%eax
  lk->locked = 0;
8010463b:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  lk->pid = 0;
}
80104641:	83 c4 10             	add    $0x10,%esp
  lk->pid = 0;
80104644:	c7 43 3c 00 00 00 00 	movl   $0x0,0x3c(%ebx)
  lk->name = name;
8010464b:	89 43 38             	mov    %eax,0x38(%ebx)
}
8010464e:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104651:	c9                   	leave  
80104652:	c3                   	ret    
80104653:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80104659:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104660 <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
80104660:	55                   	push   %ebp
80104661:	89 e5                	mov    %esp,%ebp
80104663:	56                   	push   %esi
80104664:	53                   	push   %ebx
80104665:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&lk->lk);
80104668:	83 ec 0c             	sub    $0xc,%esp
8010466b:	8d 73 04             	lea    0x4(%ebx),%esi
8010466e:	56                   	push   %esi
8010466f:	e8 ac 01 00 00       	call   80104820 <acquire>
  while (lk->locked) {
80104674:	8b 13                	mov    (%ebx),%edx
80104676:	83 c4 10             	add    $0x10,%esp
80104679:	85 d2                	test   %edx,%edx
8010467b:	74 16                	je     80104693 <acquiresleep+0x33>
8010467d:	8d 76 00             	lea    0x0(%esi),%esi
    sleep(lk, &lk->lk);
80104680:	83 ec 08             	sub    $0x8,%esp
80104683:	56                   	push   %esi
80104684:	53                   	push   %ebx
80104685:	e8 e6 fb ff ff       	call   80104270 <sleep>
  while (lk->locked) {
8010468a:	8b 03                	mov    (%ebx),%eax
8010468c:	83 c4 10             	add    $0x10,%esp
8010468f:	85 c0                	test   %eax,%eax
80104691:	75 ed                	jne    80104680 <acquiresleep+0x20>
  }
  lk->locked = 1;
80104693:	c7 03 01 00 00 00    	movl   $0x1,(%ebx)
  lk->pid = myproc()->pid;
80104699:	e8 62 f5 ff ff       	call   80103c00 <myproc>
8010469e:	8b 40 10             	mov    0x10(%eax),%eax
801046a1:	89 43 3c             	mov    %eax,0x3c(%ebx)
  release(&lk->lk);
801046a4:	89 75 08             	mov    %esi,0x8(%ebp)
}
801046a7:	8d 65 f8             	lea    -0x8(%ebp),%esp
801046aa:	5b                   	pop    %ebx
801046ab:	5e                   	pop    %esi
801046ac:	5d                   	pop    %ebp
  release(&lk->lk);
801046ad:	e9 8e 02 00 00       	jmp    80104940 <release>
801046b2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
801046b9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801046c0 <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
801046c0:	55                   	push   %ebp
801046c1:	89 e5                	mov    %esp,%ebp
801046c3:	56                   	push   %esi
801046c4:	53                   	push   %ebx
801046c5:	8b 5d 08             	mov    0x8(%ebp),%ebx
  acquire(&lk->lk);
801046c8:	83 ec 0c             	sub    $0xc,%esp
801046cb:	8d 73 04             	lea    0x4(%ebx),%esi
801046ce:	56                   	push   %esi
801046cf:	e8 4c 01 00 00       	call   80104820 <acquire>
  lk->locked = 0;
801046d4:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
  lk->pid = 0;
801046da:	c7 43 3c 00 00 00 00 	movl   $0x0,0x3c(%ebx)
  wakeup(lk);
801046e1:	89 1c 24             	mov    %ebx,(%esp)
801046e4:	e8 67 fd ff ff       	call   80104450 <wakeup>
  release(&lk->lk);
801046e9:	89 75 08             	mov    %esi,0x8(%ebp)
801046ec:	83 c4 10             	add    $0x10,%esp
}
801046ef:	8d 65 f8             	lea    -0x8(%ebp),%esp
801046f2:	5b                   	pop    %ebx
801046f3:	5e                   	pop    %esi
801046f4:	5d                   	pop    %ebp
  release(&lk->lk);
801046f5:	e9 46 02 00 00       	jmp    80104940 <release>
801046fa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80104700 <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
80104700:	55                   	push   %ebp
80104701:	89 e5                	mov    %esp,%ebp
80104703:	56                   	push   %esi
80104704:	53                   	push   %ebx
80104705:	8b 75 08             	mov    0x8(%ebp),%esi
  int r;
  
  acquire(&lk->lk);
80104708:	83 ec 0c             	sub    $0xc,%esp
8010470b:	8d 5e 04             	lea    0x4(%esi),%ebx
8010470e:	53                   	push   %ebx
8010470f:	e8 0c 01 00 00       	call   80104820 <acquire>
  r = lk->locked;
80104714:	8b 36                	mov    (%esi),%esi
  release(&lk->lk);
80104716:	89 1c 24             	mov    %ebx,(%esp)
80104719:	e8 22 02 00 00       	call   80104940 <release>
  return r;
}
8010471e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104721:	89 f0                	mov    %esi,%eax
80104723:	5b                   	pop    %ebx
80104724:	5e                   	pop    %esi
80104725:	5d                   	pop    %ebp
80104726:	c3                   	ret    
80104727:	66 90                	xchg   %ax,%ax
80104729:	66 90                	xchg   %ax,%ax
8010472b:	66 90                	xchg   %ax,%ax
8010472d:	66 90                	xchg   %ax,%ax
8010472f:	90                   	nop

80104730 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80104730:	55                   	push   %ebp
80104731:	89 e5                	mov    %esp,%ebp
80104733:	8b 45 08             	mov    0x8(%ebp),%eax
  lk->name = name;
80104736:	8b 55 0c             	mov    0xc(%ebp),%edx
  lk->locked = 0;
80104739:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->name = name;
8010473f:	89 50 04             	mov    %edx,0x4(%eax)
  lk->cpu = 0;
80104742:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80104749:	5d                   	pop    %ebp
8010474a:	c3                   	ret    
8010474b:	90                   	nop
8010474c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80104750 <getcallerpcs>:
}

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80104750:	55                   	push   %ebp
  uint *ebp;
  int i;

  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80104751:	31 d2                	xor    %edx,%edx
{
80104753:	89 e5                	mov    %esp,%ebp
80104755:	53                   	push   %ebx
  ebp = (uint*)v - 2;
80104756:	8b 45 08             	mov    0x8(%ebp),%eax
{
80104759:	8b 4d 0c             	mov    0xc(%ebp),%ecx
  ebp = (uint*)v - 2;
8010475c:	83 e8 08             	sub    $0x8,%eax
8010475f:	90                   	nop
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80104760:	8d 98 00 00 00 80    	lea    -0x80000000(%eax),%ebx
80104766:	81 fb fe ff ff 7f    	cmp    $0x7ffffffe,%ebx
8010476c:	77 1a                	ja     80104788 <getcallerpcs+0x38>
      break;
    pcs[i] = ebp[1];     // saved %eip
8010476e:	8b 58 04             	mov    0x4(%eax),%ebx
80104771:	89 1c 91             	mov    %ebx,(%ecx,%edx,4)
  for(i = 0; i < 10; i++){
80104774:	83 c2 01             	add    $0x1,%edx
    ebp = (uint*)ebp[0]; // saved %ebp
80104777:	8b 00                	mov    (%eax),%eax
  for(i = 0; i < 10; i++){
80104779:	83 fa 0a             	cmp    $0xa,%edx
8010477c:	75 e2                	jne    80104760 <getcallerpcs+0x10>
  }
  for(; i < 10; i++)
    pcs[i] = 0;
}
8010477e:	5b                   	pop    %ebx
8010477f:	5d                   	pop    %ebp
80104780:	c3                   	ret    
80104781:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104788:	8d 04 91             	lea    (%ecx,%edx,4),%eax
8010478b:	83 c1 28             	add    $0x28,%ecx
8010478e:	66 90                	xchg   %ax,%ax
    pcs[i] = 0;
80104790:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
80104796:	83 c0 04             	add    $0x4,%eax
  for(; i < 10; i++)
80104799:	39 c1                	cmp    %eax,%ecx
8010479b:	75 f3                	jne    80104790 <getcallerpcs+0x40>
}
8010479d:	5b                   	pop    %ebx
8010479e:	5d                   	pop    %ebp
8010479f:	c3                   	ret    

801047a0 <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
801047a0:	55                   	push   %ebp
801047a1:	89 e5                	mov    %esp,%ebp
801047a3:	53                   	push   %ebx
801047a4:	83 ec 04             	sub    $0x4,%esp
801047a7:	8b 55 08             	mov    0x8(%ebp),%edx
  return lock->locked && lock->cpu == mycpu();
801047aa:	8b 02                	mov    (%edx),%eax
801047ac:	85 c0                	test   %eax,%eax
801047ae:	75 10                	jne    801047c0 <holding+0x20>
}
801047b0:	83 c4 04             	add    $0x4,%esp
801047b3:	31 c0                	xor    %eax,%eax
801047b5:	5b                   	pop    %ebx
801047b6:	5d                   	pop    %ebp
801047b7:	c3                   	ret    
801047b8:	90                   	nop
801047b9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  return lock->locked && lock->cpu == mycpu();
801047c0:	8b 5a 08             	mov    0x8(%edx),%ebx
801047c3:	e8 98 f3 ff ff       	call   80103b60 <mycpu>
801047c8:	39 c3                	cmp    %eax,%ebx
801047ca:	0f 94 c0             	sete   %al
}
801047cd:	83 c4 04             	add    $0x4,%esp
  return lock->locked && lock->cpu == mycpu();
801047d0:	0f b6 c0             	movzbl %al,%eax
}
801047d3:	5b                   	pop    %ebx
801047d4:	5d                   	pop    %ebp
801047d5:	c3                   	ret    
801047d6:	8d 76 00             	lea    0x0(%esi),%esi
801047d9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801047e0 <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
801047e0:	55                   	push   %ebp
801047e1:	89 e5                	mov    %esp,%ebp
801047e3:	53                   	push   %ebx
801047e4:	83 ec 04             	sub    $0x4,%esp
801047e7:	9c                   	pushf  
801047e8:	5b                   	pop    %ebx
  asm volatile("cli");
801047e9:	fa                   	cli    
  int eflags;

  eflags = readeflags();
  cli();
  if(mycpu()->ncli == 0)
801047ea:	e8 71 f3 ff ff       	call   80103b60 <mycpu>
801047ef:	8b 80 a4 00 00 00    	mov    0xa4(%eax),%eax
801047f5:	85 c0                	test   %eax,%eax
801047f7:	75 11                	jne    8010480a <pushcli+0x2a>
    mycpu()->intena = eflags & FL_IF;
801047f9:	81 e3 00 02 00 00    	and    $0x200,%ebx
801047ff:	e8 5c f3 ff ff       	call   80103b60 <mycpu>
80104804:	89 98 a8 00 00 00    	mov    %ebx,0xa8(%eax)
  mycpu()->ncli += 1;
8010480a:	e8 51 f3 ff ff       	call   80103b60 <mycpu>
8010480f:	83 80 a4 00 00 00 01 	addl   $0x1,0xa4(%eax)
}
80104816:	83 c4 04             	add    $0x4,%esp
80104819:	5b                   	pop    %ebx
8010481a:	5d                   	pop    %ebp
8010481b:	c3                   	ret    
8010481c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80104820 <acquire>:
{
80104820:	55                   	push   %ebp
80104821:	89 e5                	mov    %esp,%ebp
80104823:	56                   	push   %esi
80104824:	53                   	push   %ebx
  pushcli(); // disable interrupts to avoid deadlock.
80104825:	e8 b6 ff ff ff       	call   801047e0 <pushcli>
  if(holding(lk))
8010482a:	8b 5d 08             	mov    0x8(%ebp),%ebx
  return lock->locked && lock->cpu == mycpu();
8010482d:	8b 03                	mov    (%ebx),%eax
8010482f:	85 c0                	test   %eax,%eax
80104831:	0f 85 81 00 00 00    	jne    801048b8 <acquire+0x98>
  asm volatile("lock; xchgl %0, %1" :
80104837:	ba 01 00 00 00       	mov    $0x1,%edx
8010483c:	eb 05                	jmp    80104843 <acquire+0x23>
8010483e:	66 90                	xchg   %ax,%ax
80104840:	8b 5d 08             	mov    0x8(%ebp),%ebx
80104843:	89 d0                	mov    %edx,%eax
80104845:	f0 87 03             	lock xchg %eax,(%ebx)
  while(xchg(&lk->locked, 1) != 0)
80104848:	85 c0                	test   %eax,%eax
8010484a:	75 f4                	jne    80104840 <acquire+0x20>
  __sync_synchronize();
8010484c:	f0 83 0c 24 00       	lock orl $0x0,(%esp)
  lk->cpu = mycpu();
80104851:	8b 5d 08             	mov    0x8(%ebp),%ebx
80104854:	e8 07 f3 ff ff       	call   80103b60 <mycpu>
  for(i = 0; i < 10; i++){
80104859:	31 d2                	xor    %edx,%edx
  getcallerpcs(&lk, lk->pcs);
8010485b:	8d 4b 0c             	lea    0xc(%ebx),%ecx
  lk->cpu = mycpu();
8010485e:	89 43 08             	mov    %eax,0x8(%ebx)
  ebp = (uint*)v - 2;
80104861:	89 e8                	mov    %ebp,%eax
80104863:	90                   	nop
80104864:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80104868:	8d 98 00 00 00 80    	lea    -0x80000000(%eax),%ebx
8010486e:	81 fb fe ff ff 7f    	cmp    $0x7ffffffe,%ebx
80104874:	77 1a                	ja     80104890 <acquire+0x70>
    pcs[i] = ebp[1];     // saved %eip
80104876:	8b 58 04             	mov    0x4(%eax),%ebx
80104879:	89 1c 91             	mov    %ebx,(%ecx,%edx,4)
  for(i = 0; i < 10; i++){
8010487c:	83 c2 01             	add    $0x1,%edx
    ebp = (uint*)ebp[0]; // saved %ebp
8010487f:	8b 00                	mov    (%eax),%eax
  for(i = 0; i < 10; i++){
80104881:	83 fa 0a             	cmp    $0xa,%edx
80104884:	75 e2                	jne    80104868 <acquire+0x48>
}
80104886:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104889:	5b                   	pop    %ebx
8010488a:	5e                   	pop    %esi
8010488b:	5d                   	pop    %ebp
8010488c:	c3                   	ret    
8010488d:	8d 76 00             	lea    0x0(%esi),%esi
80104890:	8d 04 91             	lea    (%ecx,%edx,4),%eax
80104893:	83 c1 28             	add    $0x28,%ecx
80104896:	8d 76 00             	lea    0x0(%esi),%esi
80104899:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    pcs[i] = 0;
801048a0:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
801048a6:	83 c0 04             	add    $0x4,%eax
  for(; i < 10; i++)
801048a9:	39 c8                	cmp    %ecx,%eax
801048ab:	75 f3                	jne    801048a0 <acquire+0x80>
}
801048ad:	8d 65 f8             	lea    -0x8(%ebp),%esp
801048b0:	5b                   	pop    %ebx
801048b1:	5e                   	pop    %esi
801048b2:	5d                   	pop    %ebp
801048b3:	c3                   	ret    
801048b4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  return lock->locked && lock->cpu == mycpu();
801048b8:	8b 73 08             	mov    0x8(%ebx),%esi
801048bb:	e8 a0 f2 ff ff       	call   80103b60 <mycpu>
801048c0:	39 c6                	cmp    %eax,%esi
801048c2:	0f 85 6f ff ff ff    	jne    80104837 <acquire+0x17>
    panic("acquire");
801048c8:	83 ec 0c             	sub    $0xc,%esp
801048cb:	68 4f 7c 10 80       	push   $0x80107c4f
801048d0:	e8 bb ba ff ff       	call   80100390 <panic>
801048d5:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801048d9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801048e0 <popcli>:

void
popcli(void)
{
801048e0:	55                   	push   %ebp
801048e1:	89 e5                	mov    %esp,%ebp
801048e3:	83 ec 08             	sub    $0x8,%esp
  asm volatile("pushfl; popl %0" : "=r" (eflags));
801048e6:	9c                   	pushf  
801048e7:	58                   	pop    %eax
  if(readeflags()&FL_IF)
801048e8:	f6 c4 02             	test   $0x2,%ah
801048eb:	75 35                	jne    80104922 <popcli+0x42>
    panic("popcli - interruptible");
  if(--mycpu()->ncli < 0)
801048ed:	e8 6e f2 ff ff       	call   80103b60 <mycpu>
801048f2:	83 a8 a4 00 00 00 01 	subl   $0x1,0xa4(%eax)
801048f9:	78 34                	js     8010492f <popcli+0x4f>
    panic("popcli");
  if(mycpu()->ncli == 0 && mycpu()->intena)
801048fb:	e8 60 f2 ff ff       	call   80103b60 <mycpu>
80104900:	8b 90 a4 00 00 00    	mov    0xa4(%eax),%edx
80104906:	85 d2                	test   %edx,%edx
80104908:	74 06                	je     80104910 <popcli+0x30>
    sti();
}
8010490a:	c9                   	leave  
8010490b:	c3                   	ret    
8010490c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  if(mycpu()->ncli == 0 && mycpu()->intena)
80104910:	e8 4b f2 ff ff       	call   80103b60 <mycpu>
80104915:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
8010491b:	85 c0                	test   %eax,%eax
8010491d:	74 eb                	je     8010490a <popcli+0x2a>
  asm volatile("sti");
8010491f:	fb                   	sti    
}
80104920:	c9                   	leave  
80104921:	c3                   	ret    
    panic("popcli - interruptible");
80104922:	83 ec 0c             	sub    $0xc,%esp
80104925:	68 57 7c 10 80       	push   $0x80107c57
8010492a:	e8 61 ba ff ff       	call   80100390 <panic>
    panic("popcli");
8010492f:	83 ec 0c             	sub    $0xc,%esp
80104932:	68 6e 7c 10 80       	push   $0x80107c6e
80104937:	e8 54 ba ff ff       	call   80100390 <panic>
8010493c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80104940 <release>:
{
80104940:	55                   	push   %ebp
80104941:	89 e5                	mov    %esp,%ebp
80104943:	56                   	push   %esi
80104944:	53                   	push   %ebx
80104945:	8b 5d 08             	mov    0x8(%ebp),%ebx
  return lock->locked && lock->cpu == mycpu();
80104948:	8b 03                	mov    (%ebx),%eax
8010494a:	85 c0                	test   %eax,%eax
8010494c:	74 0c                	je     8010495a <release+0x1a>
8010494e:	8b 73 08             	mov    0x8(%ebx),%esi
80104951:	e8 0a f2 ff ff       	call   80103b60 <mycpu>
80104956:	39 c6                	cmp    %eax,%esi
80104958:	74 16                	je     80104970 <release+0x30>
    panic("release");
8010495a:	83 ec 0c             	sub    $0xc,%esp
8010495d:	68 75 7c 10 80       	push   $0x80107c75
80104962:	e8 29 ba ff ff       	call   80100390 <panic>
80104967:	89 f6                	mov    %esi,%esi
80104969:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
  lk->pcs[0] = 0;
80104970:	c7 43 0c 00 00 00 00 	movl   $0x0,0xc(%ebx)
  lk->cpu = 0;
80104977:	c7 43 08 00 00 00 00 	movl   $0x0,0x8(%ebx)
  __sync_synchronize();
8010497e:	f0 83 0c 24 00       	lock orl $0x0,(%esp)
  asm volatile("movl $0, %0" : "+m" (lk->locked) : );
80104983:	c7 03 00 00 00 00    	movl   $0x0,(%ebx)
}
80104989:	8d 65 f8             	lea    -0x8(%ebp),%esp
8010498c:	5b                   	pop    %ebx
8010498d:	5e                   	pop    %esi
8010498e:	5d                   	pop    %ebp
  popcli();
8010498f:	e9 4c ff ff ff       	jmp    801048e0 <popcli>
80104994:	66 90                	xchg   %ax,%ax
80104996:	66 90                	xchg   %ax,%ax
80104998:	66 90                	xchg   %ax,%ax
8010499a:	66 90                	xchg   %ax,%ax
8010499c:	66 90                	xchg   %ax,%ax
8010499e:	66 90                	xchg   %ax,%ax

801049a0 <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
801049a0:	55                   	push   %ebp
801049a1:	89 e5                	mov    %esp,%ebp
801049a3:	57                   	push   %edi
801049a4:	53                   	push   %ebx
801049a5:	8b 55 08             	mov    0x8(%ebp),%edx
801049a8:	8b 4d 10             	mov    0x10(%ebp),%ecx
  if ((int)dst%4 == 0 && n%4 == 0){
801049ab:	f6 c2 03             	test   $0x3,%dl
801049ae:	75 05                	jne    801049b5 <memset+0x15>
801049b0:	f6 c1 03             	test   $0x3,%cl
801049b3:	74 13                	je     801049c8 <memset+0x28>
  asm volatile("cld; rep stosb" :
801049b5:	89 d7                	mov    %edx,%edi
801049b7:	8b 45 0c             	mov    0xc(%ebp),%eax
801049ba:	fc                   	cld    
801049bb:	f3 aa                	rep stos %al,%es:(%edi)
    c &= 0xFF;
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
  } else
    stosb(dst, c, n);
  return dst;
}
801049bd:	5b                   	pop    %ebx
801049be:	89 d0                	mov    %edx,%eax
801049c0:	5f                   	pop    %edi
801049c1:	5d                   	pop    %ebp
801049c2:	c3                   	ret    
801049c3:	90                   	nop
801049c4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    c &= 0xFF;
801049c8:	0f b6 7d 0c          	movzbl 0xc(%ebp),%edi
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
801049cc:	c1 e9 02             	shr    $0x2,%ecx
801049cf:	89 f8                	mov    %edi,%eax
801049d1:	89 fb                	mov    %edi,%ebx
801049d3:	c1 e0 18             	shl    $0x18,%eax
801049d6:	c1 e3 10             	shl    $0x10,%ebx
801049d9:	09 d8                	or     %ebx,%eax
801049db:	09 f8                	or     %edi,%eax
801049dd:	c1 e7 08             	shl    $0x8,%edi
801049e0:	09 f8                	or     %edi,%eax
  asm volatile("cld; rep stosl" :
801049e2:	89 d7                	mov    %edx,%edi
801049e4:	fc                   	cld    
801049e5:	f3 ab                	rep stos %eax,%es:(%edi)
}
801049e7:	5b                   	pop    %ebx
801049e8:	89 d0                	mov    %edx,%eax
801049ea:	5f                   	pop    %edi
801049eb:	5d                   	pop    %ebp
801049ec:	c3                   	ret    
801049ed:	8d 76 00             	lea    0x0(%esi),%esi

801049f0 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
801049f0:	55                   	push   %ebp
801049f1:	89 e5                	mov    %esp,%ebp
801049f3:	57                   	push   %edi
801049f4:	56                   	push   %esi
801049f5:	53                   	push   %ebx
801049f6:	8b 5d 10             	mov    0x10(%ebp),%ebx
801049f9:	8b 75 08             	mov    0x8(%ebp),%esi
801049fc:	8b 7d 0c             	mov    0xc(%ebp),%edi
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
801049ff:	85 db                	test   %ebx,%ebx
80104a01:	74 29                	je     80104a2c <memcmp+0x3c>
    if(*s1 != *s2)
80104a03:	0f b6 16             	movzbl (%esi),%edx
80104a06:	0f b6 0f             	movzbl (%edi),%ecx
80104a09:	38 d1                	cmp    %dl,%cl
80104a0b:	75 2b                	jne    80104a38 <memcmp+0x48>
80104a0d:	b8 01 00 00 00       	mov    $0x1,%eax
80104a12:	eb 14                	jmp    80104a28 <memcmp+0x38>
80104a14:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104a18:	0f b6 14 06          	movzbl (%esi,%eax,1),%edx
80104a1c:	83 c0 01             	add    $0x1,%eax
80104a1f:	0f b6 4c 07 ff       	movzbl -0x1(%edi,%eax,1),%ecx
80104a24:	38 ca                	cmp    %cl,%dl
80104a26:	75 10                	jne    80104a38 <memcmp+0x48>
  while(n-- > 0){
80104a28:	39 d8                	cmp    %ebx,%eax
80104a2a:	75 ec                	jne    80104a18 <memcmp+0x28>
      return *s1 - *s2;
    s1++, s2++;
  }

  return 0;
}
80104a2c:	5b                   	pop    %ebx
  return 0;
80104a2d:	31 c0                	xor    %eax,%eax
}
80104a2f:	5e                   	pop    %esi
80104a30:	5f                   	pop    %edi
80104a31:	5d                   	pop    %ebp
80104a32:	c3                   	ret    
80104a33:	90                   	nop
80104a34:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      return *s1 - *s2;
80104a38:	0f b6 c2             	movzbl %dl,%eax
}
80104a3b:	5b                   	pop    %ebx
      return *s1 - *s2;
80104a3c:	29 c8                	sub    %ecx,%eax
}
80104a3e:	5e                   	pop    %esi
80104a3f:	5f                   	pop    %edi
80104a40:	5d                   	pop    %ebp
80104a41:	c3                   	ret    
80104a42:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104a49:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104a50 <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
80104a50:	55                   	push   %ebp
80104a51:	89 e5                	mov    %esp,%ebp
80104a53:	56                   	push   %esi
80104a54:	53                   	push   %ebx
80104a55:	8b 45 08             	mov    0x8(%ebp),%eax
80104a58:	8b 5d 0c             	mov    0xc(%ebp),%ebx
80104a5b:	8b 75 10             	mov    0x10(%ebp),%esi
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
80104a5e:	39 c3                	cmp    %eax,%ebx
80104a60:	73 26                	jae    80104a88 <memmove+0x38>
80104a62:	8d 0c 33             	lea    (%ebx,%esi,1),%ecx
80104a65:	39 c8                	cmp    %ecx,%eax
80104a67:	73 1f                	jae    80104a88 <memmove+0x38>
    s += n;
    d += n;
    while(n-- > 0)
80104a69:	85 f6                	test   %esi,%esi
80104a6b:	8d 56 ff             	lea    -0x1(%esi),%edx
80104a6e:	74 0f                	je     80104a7f <memmove+0x2f>
      *--d = *--s;
80104a70:	0f b6 0c 13          	movzbl (%ebx,%edx,1),%ecx
80104a74:	88 0c 10             	mov    %cl,(%eax,%edx,1)
    while(n-- > 0)
80104a77:	83 ea 01             	sub    $0x1,%edx
80104a7a:	83 fa ff             	cmp    $0xffffffff,%edx
80104a7d:	75 f1                	jne    80104a70 <memmove+0x20>
  } else
    while(n-- > 0)
      *d++ = *s++;

  return dst;
}
80104a7f:	5b                   	pop    %ebx
80104a80:	5e                   	pop    %esi
80104a81:	5d                   	pop    %ebp
80104a82:	c3                   	ret    
80104a83:	90                   	nop
80104a84:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    while(n-- > 0)
80104a88:	31 d2                	xor    %edx,%edx
80104a8a:	85 f6                	test   %esi,%esi
80104a8c:	74 f1                	je     80104a7f <memmove+0x2f>
80104a8e:	66 90                	xchg   %ax,%ax
      *d++ = *s++;
80104a90:	0f b6 0c 13          	movzbl (%ebx,%edx,1),%ecx
80104a94:	88 0c 10             	mov    %cl,(%eax,%edx,1)
80104a97:	83 c2 01             	add    $0x1,%edx
    while(n-- > 0)
80104a9a:	39 d6                	cmp    %edx,%esi
80104a9c:	75 f2                	jne    80104a90 <memmove+0x40>
}
80104a9e:	5b                   	pop    %ebx
80104a9f:	5e                   	pop    %esi
80104aa0:	5d                   	pop    %ebp
80104aa1:	c3                   	ret    
80104aa2:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104aa9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104ab0 <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
80104ab0:	55                   	push   %ebp
80104ab1:	89 e5                	mov    %esp,%ebp
  return memmove(dst, src, n);
}
80104ab3:	5d                   	pop    %ebp
  return memmove(dst, src, n);
80104ab4:	eb 9a                	jmp    80104a50 <memmove>
80104ab6:	8d 76 00             	lea    0x0(%esi),%esi
80104ab9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104ac0 <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
80104ac0:	55                   	push   %ebp
80104ac1:	89 e5                	mov    %esp,%ebp
80104ac3:	57                   	push   %edi
80104ac4:	56                   	push   %esi
80104ac5:	8b 7d 10             	mov    0x10(%ebp),%edi
80104ac8:	53                   	push   %ebx
80104ac9:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104acc:	8b 75 0c             	mov    0xc(%ebp),%esi
  while(n > 0 && *p && *p == *q)
80104acf:	85 ff                	test   %edi,%edi
80104ad1:	74 2f                	je     80104b02 <strncmp+0x42>
80104ad3:	0f b6 01             	movzbl (%ecx),%eax
80104ad6:	0f b6 1e             	movzbl (%esi),%ebx
80104ad9:	84 c0                	test   %al,%al
80104adb:	74 37                	je     80104b14 <strncmp+0x54>
80104add:	38 c3                	cmp    %al,%bl
80104adf:	75 33                	jne    80104b14 <strncmp+0x54>
80104ae1:	01 f7                	add    %esi,%edi
80104ae3:	eb 13                	jmp    80104af8 <strncmp+0x38>
80104ae5:	8d 76 00             	lea    0x0(%esi),%esi
80104ae8:	0f b6 01             	movzbl (%ecx),%eax
80104aeb:	84 c0                	test   %al,%al
80104aed:	74 21                	je     80104b10 <strncmp+0x50>
80104aef:	0f b6 1a             	movzbl (%edx),%ebx
80104af2:	89 d6                	mov    %edx,%esi
80104af4:	38 d8                	cmp    %bl,%al
80104af6:	75 1c                	jne    80104b14 <strncmp+0x54>
    n--, p++, q++;
80104af8:	8d 56 01             	lea    0x1(%esi),%edx
80104afb:	83 c1 01             	add    $0x1,%ecx
  while(n > 0 && *p && *p == *q)
80104afe:	39 fa                	cmp    %edi,%edx
80104b00:	75 e6                	jne    80104ae8 <strncmp+0x28>
  if(n == 0)
    return 0;
  return (uchar)*p - (uchar)*q;
}
80104b02:	5b                   	pop    %ebx
    return 0;
80104b03:	31 c0                	xor    %eax,%eax
}
80104b05:	5e                   	pop    %esi
80104b06:	5f                   	pop    %edi
80104b07:	5d                   	pop    %ebp
80104b08:	c3                   	ret    
80104b09:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80104b10:	0f b6 5e 01          	movzbl 0x1(%esi),%ebx
  return (uchar)*p - (uchar)*q;
80104b14:	29 d8                	sub    %ebx,%eax
}
80104b16:	5b                   	pop    %ebx
80104b17:	5e                   	pop    %esi
80104b18:	5f                   	pop    %edi
80104b19:	5d                   	pop    %ebp
80104b1a:	c3                   	ret    
80104b1b:	90                   	nop
80104b1c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80104b20 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
80104b20:	55                   	push   %ebp
80104b21:	89 e5                	mov    %esp,%ebp
80104b23:	56                   	push   %esi
80104b24:	53                   	push   %ebx
80104b25:	8b 45 08             	mov    0x8(%ebp),%eax
80104b28:	8b 5d 0c             	mov    0xc(%ebp),%ebx
80104b2b:	8b 4d 10             	mov    0x10(%ebp),%ecx
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
80104b2e:	89 c2                	mov    %eax,%edx
80104b30:	eb 19                	jmp    80104b4b <strncpy+0x2b>
80104b32:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80104b38:	83 c3 01             	add    $0x1,%ebx
80104b3b:	0f b6 4b ff          	movzbl -0x1(%ebx),%ecx
80104b3f:	83 c2 01             	add    $0x1,%edx
80104b42:	84 c9                	test   %cl,%cl
80104b44:	88 4a ff             	mov    %cl,-0x1(%edx)
80104b47:	74 09                	je     80104b52 <strncpy+0x32>
80104b49:	89 f1                	mov    %esi,%ecx
80104b4b:	85 c9                	test   %ecx,%ecx
80104b4d:	8d 71 ff             	lea    -0x1(%ecx),%esi
80104b50:	7f e6                	jg     80104b38 <strncpy+0x18>
    ;
  while(n-- > 0)
80104b52:	31 c9                	xor    %ecx,%ecx
80104b54:	85 f6                	test   %esi,%esi
80104b56:	7e 17                	jle    80104b6f <strncpy+0x4f>
80104b58:	90                   	nop
80104b59:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    *s++ = 0;
80104b60:	c6 04 0a 00          	movb   $0x0,(%edx,%ecx,1)
80104b64:	89 f3                	mov    %esi,%ebx
80104b66:	83 c1 01             	add    $0x1,%ecx
80104b69:	29 cb                	sub    %ecx,%ebx
  while(n-- > 0)
80104b6b:	85 db                	test   %ebx,%ebx
80104b6d:	7f f1                	jg     80104b60 <strncpy+0x40>
  return os;
}
80104b6f:	5b                   	pop    %ebx
80104b70:	5e                   	pop    %esi
80104b71:	5d                   	pop    %ebp
80104b72:	c3                   	ret    
80104b73:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80104b79:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104b80 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
80104b80:	55                   	push   %ebp
80104b81:	89 e5                	mov    %esp,%ebp
80104b83:	56                   	push   %esi
80104b84:	53                   	push   %ebx
80104b85:	8b 4d 10             	mov    0x10(%ebp),%ecx
80104b88:	8b 45 08             	mov    0x8(%ebp),%eax
80104b8b:	8b 55 0c             	mov    0xc(%ebp),%edx
  char *os;

  os = s;
  if(n <= 0)
80104b8e:	85 c9                	test   %ecx,%ecx
80104b90:	7e 26                	jle    80104bb8 <safestrcpy+0x38>
80104b92:	8d 74 0a ff          	lea    -0x1(%edx,%ecx,1),%esi
80104b96:	89 c1                	mov    %eax,%ecx
80104b98:	eb 17                	jmp    80104bb1 <safestrcpy+0x31>
80104b9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    return os;
  while(--n > 0 && (*s++ = *t++) != 0)
80104ba0:	83 c2 01             	add    $0x1,%edx
80104ba3:	0f b6 5a ff          	movzbl -0x1(%edx),%ebx
80104ba7:	83 c1 01             	add    $0x1,%ecx
80104baa:	84 db                	test   %bl,%bl
80104bac:	88 59 ff             	mov    %bl,-0x1(%ecx)
80104baf:	74 04                	je     80104bb5 <safestrcpy+0x35>
80104bb1:	39 f2                	cmp    %esi,%edx
80104bb3:	75 eb                	jne    80104ba0 <safestrcpy+0x20>
    ;
  *s = 0;
80104bb5:	c6 01 00             	movb   $0x0,(%ecx)
  return os;
}
80104bb8:	5b                   	pop    %ebx
80104bb9:	5e                   	pop    %esi
80104bba:	5d                   	pop    %ebp
80104bbb:	c3                   	ret    
80104bbc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80104bc0 <strlen>:

int
strlen(const char *s)
{
80104bc0:	55                   	push   %ebp
  int n;

  for(n = 0; s[n]; n++)
80104bc1:	31 c0                	xor    %eax,%eax
{
80104bc3:	89 e5                	mov    %esp,%ebp
80104bc5:	8b 55 08             	mov    0x8(%ebp),%edx
  for(n = 0; s[n]; n++)
80104bc8:	80 3a 00             	cmpb   $0x0,(%edx)
80104bcb:	74 0c                	je     80104bd9 <strlen+0x19>
80104bcd:	8d 76 00             	lea    0x0(%esi),%esi
80104bd0:	83 c0 01             	add    $0x1,%eax
80104bd3:	80 3c 02 00          	cmpb   $0x0,(%edx,%eax,1)
80104bd7:	75 f7                	jne    80104bd0 <strlen+0x10>
    ;
  return n;
}
80104bd9:	5d                   	pop    %ebp
80104bda:	c3                   	ret    

80104bdb <swtch>:
# a struct context, and save its address in *old.
# Switch stacks to new and pop previously-saved registers.

.globl swtch
swtch:
  movl 4(%esp), %eax
80104bdb:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
80104bdf:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-save registers
  pushl %ebp
80104be3:	55                   	push   %ebp
  pushl %ebx
80104be4:	53                   	push   %ebx
  pushl %esi
80104be5:	56                   	push   %esi
  pushl %edi
80104be6:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
80104be7:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
80104be9:	89 d4                	mov    %edx,%esp

  # Load new callee-save registers
  popl %edi
80104beb:	5f                   	pop    %edi
  popl %esi
80104bec:	5e                   	pop    %esi
  popl %ebx
80104bed:	5b                   	pop    %ebx
  popl %ebp
80104bee:	5d                   	pop    %ebp
  ret
80104bef:	c3                   	ret    

80104bf0 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from the current process.
int
fetchint(uint addr, int *ip)
{
80104bf0:	55                   	push   %ebp
80104bf1:	89 e5                	mov    %esp,%ebp
80104bf3:	53                   	push   %ebx
80104bf4:	83 ec 04             	sub    $0x4,%esp
80104bf7:	8b 5d 08             	mov    0x8(%ebp),%ebx
  struct proc *curproc = myproc();
80104bfa:	e8 01 f0 ff ff       	call   80103c00 <myproc>

  if(addr >= curproc->sz || addr+4 > curproc->sz)
80104bff:	8b 00                	mov    (%eax),%eax
80104c01:	39 d8                	cmp    %ebx,%eax
80104c03:	76 1b                	jbe    80104c20 <fetchint+0x30>
80104c05:	8d 53 04             	lea    0x4(%ebx),%edx
80104c08:	39 d0                	cmp    %edx,%eax
80104c0a:	72 14                	jb     80104c20 <fetchint+0x30>
    return -1;
  *ip = *(int*)(addr);
80104c0c:	8b 45 0c             	mov    0xc(%ebp),%eax
80104c0f:	8b 13                	mov    (%ebx),%edx
80104c11:	89 10                	mov    %edx,(%eax)
  return 0;
80104c13:	31 c0                	xor    %eax,%eax
}
80104c15:	83 c4 04             	add    $0x4,%esp
80104c18:	5b                   	pop    %ebx
80104c19:	5d                   	pop    %ebp
80104c1a:	c3                   	ret    
80104c1b:	90                   	nop
80104c1c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80104c20:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104c25:	eb ee                	jmp    80104c15 <fetchint+0x25>
80104c27:	89 f6                	mov    %esi,%esi
80104c29:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104c30 <fetchstr>:
// Fetch the nul-terminated string at addr from the current process.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(uint addr, char **pp)
{
80104c30:	55                   	push   %ebp
80104c31:	89 e5                	mov    %esp,%ebp
80104c33:	53                   	push   %ebx
80104c34:	83 ec 04             	sub    $0x4,%esp
80104c37:	8b 5d 08             	mov    0x8(%ebp),%ebx
  char *s, *ep;
  struct proc *curproc = myproc();
80104c3a:	e8 c1 ef ff ff       	call   80103c00 <myproc>

  if(addr >= curproc->sz)
80104c3f:	39 18                	cmp    %ebx,(%eax)
80104c41:	76 29                	jbe    80104c6c <fetchstr+0x3c>
    return -1;
  *pp = (char*)addr;
80104c43:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80104c46:	89 da                	mov    %ebx,%edx
80104c48:	89 19                	mov    %ebx,(%ecx)
  ep = (char*)curproc->sz;
80104c4a:	8b 00                	mov    (%eax),%eax
  for(s = *pp; s < ep; s++){
80104c4c:	39 c3                	cmp    %eax,%ebx
80104c4e:	73 1c                	jae    80104c6c <fetchstr+0x3c>
    if(*s == 0)
80104c50:	80 3b 00             	cmpb   $0x0,(%ebx)
80104c53:	75 10                	jne    80104c65 <fetchstr+0x35>
80104c55:	eb 39                	jmp    80104c90 <fetchstr+0x60>
80104c57:	89 f6                	mov    %esi,%esi
80104c59:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
80104c60:	80 3a 00             	cmpb   $0x0,(%edx)
80104c63:	74 1b                	je     80104c80 <fetchstr+0x50>
  for(s = *pp; s < ep; s++){
80104c65:	83 c2 01             	add    $0x1,%edx
80104c68:	39 d0                	cmp    %edx,%eax
80104c6a:	77 f4                	ja     80104c60 <fetchstr+0x30>
    return -1;
80104c6c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
      return s - *pp;
  }
  return -1;
}
80104c71:	83 c4 04             	add    $0x4,%esp
80104c74:	5b                   	pop    %ebx
80104c75:	5d                   	pop    %ebp
80104c76:	c3                   	ret    
80104c77:	89 f6                	mov    %esi,%esi
80104c79:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
80104c80:	83 c4 04             	add    $0x4,%esp
80104c83:	89 d0                	mov    %edx,%eax
80104c85:	29 d8                	sub    %ebx,%eax
80104c87:	5b                   	pop    %ebx
80104c88:	5d                   	pop    %ebp
80104c89:	c3                   	ret    
80104c8a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    if(*s == 0)
80104c90:	31 c0                	xor    %eax,%eax
      return s - *pp;
80104c92:	eb dd                	jmp    80104c71 <fetchstr+0x41>
80104c94:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80104c9a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80104ca0 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
80104ca0:	55                   	push   %ebp
80104ca1:	89 e5                	mov    %esp,%ebp
80104ca3:	56                   	push   %esi
80104ca4:	53                   	push   %ebx
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
80104ca5:	e8 56 ef ff ff       	call   80103c00 <myproc>
80104caa:	8b 40 18             	mov    0x18(%eax),%eax
80104cad:	8b 55 08             	mov    0x8(%ebp),%edx
80104cb0:	8b 40 44             	mov    0x44(%eax),%eax
80104cb3:	8d 1c 90             	lea    (%eax,%edx,4),%ebx
  struct proc *curproc = myproc();
80104cb6:	e8 45 ef ff ff       	call   80103c00 <myproc>
  if(addr >= curproc->sz || addr+4 > curproc->sz)
80104cbb:	8b 00                	mov    (%eax),%eax
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
80104cbd:	8d 73 04             	lea    0x4(%ebx),%esi
  if(addr >= curproc->sz || addr+4 > curproc->sz)
80104cc0:	39 c6                	cmp    %eax,%esi
80104cc2:	73 1c                	jae    80104ce0 <argint+0x40>
80104cc4:	8d 53 08             	lea    0x8(%ebx),%edx
80104cc7:	39 d0                	cmp    %edx,%eax
80104cc9:	72 15                	jb     80104ce0 <argint+0x40>
  *ip = *(int*)(addr);
80104ccb:	8b 45 0c             	mov    0xc(%ebp),%eax
80104cce:	8b 53 04             	mov    0x4(%ebx),%edx
80104cd1:	89 10                	mov    %edx,(%eax)
  return 0;
80104cd3:	31 c0                	xor    %eax,%eax
}
80104cd5:	5b                   	pop    %ebx
80104cd6:	5e                   	pop    %esi
80104cd7:	5d                   	pop    %ebp
80104cd8:	c3                   	ret    
80104cd9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80104ce0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
80104ce5:	eb ee                	jmp    80104cd5 <argint+0x35>
80104ce7:	89 f6                	mov    %esi,%esi
80104ce9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104cf0 <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
80104cf0:	55                   	push   %ebp
80104cf1:	89 e5                	mov    %esp,%ebp
80104cf3:	56                   	push   %esi
80104cf4:	53                   	push   %ebx
80104cf5:	83 ec 10             	sub    $0x10,%esp
80104cf8:	8b 5d 10             	mov    0x10(%ebp),%ebx
  int i;
  struct proc *curproc = myproc();
80104cfb:	e8 00 ef ff ff       	call   80103c00 <myproc>
80104d00:	89 c6                	mov    %eax,%esi
 
  if(argint(n, &i) < 0)
80104d02:	8d 45 f4             	lea    -0xc(%ebp),%eax
80104d05:	83 ec 08             	sub    $0x8,%esp
80104d08:	50                   	push   %eax
80104d09:	ff 75 08             	pushl  0x8(%ebp)
80104d0c:	e8 8f ff ff ff       	call   80104ca0 <argint>
    return -1;
  if(size < 0 || (uint)i >= curproc->sz || (uint)i+size > curproc->sz)
80104d11:	83 c4 10             	add    $0x10,%esp
80104d14:	85 c0                	test   %eax,%eax
80104d16:	78 28                	js     80104d40 <argptr+0x50>
80104d18:	85 db                	test   %ebx,%ebx
80104d1a:	78 24                	js     80104d40 <argptr+0x50>
80104d1c:	8b 16                	mov    (%esi),%edx
80104d1e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104d21:	39 c2                	cmp    %eax,%edx
80104d23:	76 1b                	jbe    80104d40 <argptr+0x50>
80104d25:	01 c3                	add    %eax,%ebx
80104d27:	39 da                	cmp    %ebx,%edx
80104d29:	72 15                	jb     80104d40 <argptr+0x50>
    return -1;
  *pp = (char*)i;
80104d2b:	8b 55 0c             	mov    0xc(%ebp),%edx
80104d2e:	89 02                	mov    %eax,(%edx)
  return 0;
80104d30:	31 c0                	xor    %eax,%eax
}
80104d32:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104d35:	5b                   	pop    %ebx
80104d36:	5e                   	pop    %esi
80104d37:	5d                   	pop    %ebp
80104d38:	c3                   	ret    
80104d39:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80104d40:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104d45:	eb eb                	jmp    80104d32 <argptr+0x42>
80104d47:	89 f6                	mov    %esi,%esi
80104d49:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104d50 <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
80104d50:	55                   	push   %ebp
80104d51:	89 e5                	mov    %esp,%ebp
80104d53:	83 ec 20             	sub    $0x20,%esp
  int addr;
  if(argint(n, &addr) < 0)
80104d56:	8d 45 f4             	lea    -0xc(%ebp),%eax
80104d59:	50                   	push   %eax
80104d5a:	ff 75 08             	pushl  0x8(%ebp)
80104d5d:	e8 3e ff ff ff       	call   80104ca0 <argint>
80104d62:	83 c4 10             	add    $0x10,%esp
80104d65:	85 c0                	test   %eax,%eax
80104d67:	78 17                	js     80104d80 <argstr+0x30>
    return -1;
  return fetchstr(addr, pp);
80104d69:	83 ec 08             	sub    $0x8,%esp
80104d6c:	ff 75 0c             	pushl  0xc(%ebp)
80104d6f:	ff 75 f4             	pushl  -0xc(%ebp)
80104d72:	e8 b9 fe ff ff       	call   80104c30 <fetchstr>
80104d77:	83 c4 10             	add    $0x10,%esp
}
80104d7a:	c9                   	leave  
80104d7b:	c3                   	ret    
80104d7c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80104d80:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104d85:	c9                   	leave  
80104d86:	c3                   	ret    
80104d87:	89 f6                	mov    %esi,%esi
80104d89:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104d90 <syscall>:
[SYS_yield]   sys_yield,
};

void
syscall(void)
{
80104d90:	55                   	push   %ebp
80104d91:	89 e5                	mov    %esp,%ebp
80104d93:	53                   	push   %ebx
80104d94:	83 ec 04             	sub    $0x4,%esp
  int num;
  struct proc *curproc = myproc();
80104d97:	e8 64 ee ff ff       	call   80103c00 <myproc>
80104d9c:	89 c3                	mov    %eax,%ebx

  num = curproc->tf->eax;
80104d9e:	8b 40 18             	mov    0x18(%eax),%eax
80104da1:	8b 40 1c             	mov    0x1c(%eax),%eax
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
80104da4:	8d 50 ff             	lea    -0x1(%eax),%edx
80104da7:	83 fa 15             	cmp    $0x15,%edx
80104daa:	77 1c                	ja     80104dc8 <syscall+0x38>
80104dac:	8b 14 85 a0 7c 10 80 	mov    -0x7fef8360(,%eax,4),%edx
80104db3:	85 d2                	test   %edx,%edx
80104db5:	74 11                	je     80104dc8 <syscall+0x38>
    curproc->tf->eax = syscalls[num]();
80104db7:	ff d2                	call   *%edx
80104db9:	8b 53 18             	mov    0x18(%ebx),%edx
80104dbc:	89 42 1c             	mov    %eax,0x1c(%edx)
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            curproc->pid, curproc->name, num);
    curproc->tf->eax = -1;
  }
}
80104dbf:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104dc2:	c9                   	leave  
80104dc3:	c3                   	ret    
80104dc4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    cprintf("%d %s: unknown sys call %d\n",
80104dc8:	50                   	push   %eax
            curproc->pid, curproc->name, num);
80104dc9:	8d 43 6c             	lea    0x6c(%ebx),%eax
    cprintf("%d %s: unknown sys call %d\n",
80104dcc:	50                   	push   %eax
80104dcd:	ff 73 10             	pushl  0x10(%ebx)
80104dd0:	68 7d 7c 10 80       	push   $0x80107c7d
80104dd5:	e8 86 b8 ff ff       	call   80100660 <cprintf>
    curproc->tf->eax = -1;
80104dda:	8b 43 18             	mov    0x18(%ebx),%eax
80104ddd:	83 c4 10             	add    $0x10,%esp
80104de0:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
}
80104de7:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80104dea:	c9                   	leave  
80104deb:	c3                   	ret    
80104dec:	66 90                	xchg   %ax,%ax
80104dee:	66 90                	xchg   %ax,%ax

80104df0 <argfd.constprop.0>:
#include "fcntl.h"

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
80104df0:	55                   	push   %ebp
80104df1:	89 e5                	mov    %esp,%ebp
80104df3:	56                   	push   %esi
80104df4:	53                   	push   %ebx
80104df5:	89 c3                	mov    %eax,%ebx
{
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
80104df7:	8d 45 f4             	lea    -0xc(%ebp),%eax
argfd(int n, int *pfd, struct file **pf)
80104dfa:	89 d6                	mov    %edx,%esi
80104dfc:	83 ec 18             	sub    $0x18,%esp
  if(argint(n, &fd) < 0)
80104dff:	50                   	push   %eax
80104e00:	6a 00                	push   $0x0
80104e02:	e8 99 fe ff ff       	call   80104ca0 <argint>
80104e07:	83 c4 10             	add    $0x10,%esp
80104e0a:	85 c0                	test   %eax,%eax
80104e0c:	78 2a                	js     80104e38 <argfd.constprop.0+0x48>
    return -1;
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
80104e0e:	83 7d f4 0f          	cmpl   $0xf,-0xc(%ebp)
80104e12:	77 24                	ja     80104e38 <argfd.constprop.0+0x48>
80104e14:	e8 e7 ed ff ff       	call   80103c00 <myproc>
80104e19:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104e1c:	8b 44 90 28          	mov    0x28(%eax,%edx,4),%eax
80104e20:	85 c0                	test   %eax,%eax
80104e22:	74 14                	je     80104e38 <argfd.constprop.0+0x48>
    return -1;
  if(pfd)
80104e24:	85 db                	test   %ebx,%ebx
80104e26:	74 02                	je     80104e2a <argfd.constprop.0+0x3a>
    *pfd = fd;
80104e28:	89 13                	mov    %edx,(%ebx)
  if(pf)
    *pf = f;
80104e2a:	89 06                	mov    %eax,(%esi)
  return 0;
80104e2c:	31 c0                	xor    %eax,%eax
}
80104e2e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104e31:	5b                   	pop    %ebx
80104e32:	5e                   	pop    %esi
80104e33:	5d                   	pop    %ebp
80104e34:	c3                   	ret    
80104e35:	8d 76 00             	lea    0x0(%esi),%esi
    return -1;
80104e38:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104e3d:	eb ef                	jmp    80104e2e <argfd.constprop.0+0x3e>
80104e3f:	90                   	nop

80104e40 <sys_dup>:
  return -1;
}

int
sys_dup(void)
{
80104e40:	55                   	push   %ebp
  struct file *f;
  int fd;

  if(argfd(0, 0, &f) < 0)
80104e41:	31 c0                	xor    %eax,%eax
{
80104e43:	89 e5                	mov    %esp,%ebp
80104e45:	56                   	push   %esi
80104e46:	53                   	push   %ebx
  if(argfd(0, 0, &f) < 0)
80104e47:	8d 55 f4             	lea    -0xc(%ebp),%edx
{
80104e4a:	83 ec 10             	sub    $0x10,%esp
  if(argfd(0, 0, &f) < 0)
80104e4d:	e8 9e ff ff ff       	call   80104df0 <argfd.constprop.0>
80104e52:	85 c0                	test   %eax,%eax
80104e54:	78 42                	js     80104e98 <sys_dup+0x58>
    return -1;
  if((fd=fdalloc(f)) < 0)
80104e56:	8b 75 f4             	mov    -0xc(%ebp),%esi
  for(fd = 0; fd < NOFILE; fd++){
80104e59:	31 db                	xor    %ebx,%ebx
  struct proc *curproc = myproc();
80104e5b:	e8 a0 ed ff ff       	call   80103c00 <myproc>
80104e60:	eb 0e                	jmp    80104e70 <sys_dup+0x30>
80104e62:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  for(fd = 0; fd < NOFILE; fd++){
80104e68:	83 c3 01             	add    $0x1,%ebx
80104e6b:	83 fb 10             	cmp    $0x10,%ebx
80104e6e:	74 28                	je     80104e98 <sys_dup+0x58>
    if(curproc->ofile[fd] == 0){
80104e70:	8b 54 98 28          	mov    0x28(%eax,%ebx,4),%edx
80104e74:	85 d2                	test   %edx,%edx
80104e76:	75 f0                	jne    80104e68 <sys_dup+0x28>
      curproc->ofile[fd] = f;
80104e78:	89 74 98 28          	mov    %esi,0x28(%eax,%ebx,4)
    return -1;
  filedup(f);
80104e7c:	83 ec 0c             	sub    $0xc,%esp
80104e7f:	ff 75 f4             	pushl  -0xc(%ebp)
80104e82:	e8 b9 bf ff ff       	call   80100e40 <filedup>
  return fd;
80104e87:	83 c4 10             	add    $0x10,%esp
}
80104e8a:	8d 65 f8             	lea    -0x8(%ebp),%esp
80104e8d:	89 d8                	mov    %ebx,%eax
80104e8f:	5b                   	pop    %ebx
80104e90:	5e                   	pop    %esi
80104e91:	5d                   	pop    %ebp
80104e92:	c3                   	ret    
80104e93:	90                   	nop
80104e94:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
80104e98:	8d 65 f8             	lea    -0x8(%ebp),%esp
    return -1;
80104e9b:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
}
80104ea0:	89 d8                	mov    %ebx,%eax
80104ea2:	5b                   	pop    %ebx
80104ea3:	5e                   	pop    %esi
80104ea4:	5d                   	pop    %ebp
80104ea5:	c3                   	ret    
80104ea6:	8d 76 00             	lea    0x0(%esi),%esi
80104ea9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104eb0 <sys_read>:

int
sys_read(void)
{
80104eb0:	55                   	push   %ebp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80104eb1:	31 c0                	xor    %eax,%eax
{
80104eb3:	89 e5                	mov    %esp,%ebp
80104eb5:	83 ec 18             	sub    $0x18,%esp
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80104eb8:	8d 55 ec             	lea    -0x14(%ebp),%edx
80104ebb:	e8 30 ff ff ff       	call   80104df0 <argfd.constprop.0>
80104ec0:	85 c0                	test   %eax,%eax
80104ec2:	78 4c                	js     80104f10 <sys_read+0x60>
80104ec4:	8d 45 f0             	lea    -0x10(%ebp),%eax
80104ec7:	83 ec 08             	sub    $0x8,%esp
80104eca:	50                   	push   %eax
80104ecb:	6a 02                	push   $0x2
80104ecd:	e8 ce fd ff ff       	call   80104ca0 <argint>
80104ed2:	83 c4 10             	add    $0x10,%esp
80104ed5:	85 c0                	test   %eax,%eax
80104ed7:	78 37                	js     80104f10 <sys_read+0x60>
80104ed9:	8d 45 f4             	lea    -0xc(%ebp),%eax
80104edc:	83 ec 04             	sub    $0x4,%esp
80104edf:	ff 75 f0             	pushl  -0x10(%ebp)
80104ee2:	50                   	push   %eax
80104ee3:	6a 01                	push   $0x1
80104ee5:	e8 06 fe ff ff       	call   80104cf0 <argptr>
80104eea:	83 c4 10             	add    $0x10,%esp
80104eed:	85 c0                	test   %eax,%eax
80104eef:	78 1f                	js     80104f10 <sys_read+0x60>
    return -1;
  return fileread(f, p, n);
80104ef1:	83 ec 04             	sub    $0x4,%esp
80104ef4:	ff 75 f0             	pushl  -0x10(%ebp)
80104ef7:	ff 75 f4             	pushl  -0xc(%ebp)
80104efa:	ff 75 ec             	pushl  -0x14(%ebp)
80104efd:	e8 ae c0 ff ff       	call   80100fb0 <fileread>
80104f02:	83 c4 10             	add    $0x10,%esp
}
80104f05:	c9                   	leave  
80104f06:	c3                   	ret    
80104f07:	89 f6                	mov    %esi,%esi
80104f09:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    return -1;
80104f10:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104f15:	c9                   	leave  
80104f16:	c3                   	ret    
80104f17:	89 f6                	mov    %esi,%esi
80104f19:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104f20 <sys_write>:

int
sys_write(void)
{
80104f20:	55                   	push   %ebp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80104f21:	31 c0                	xor    %eax,%eax
{
80104f23:	89 e5                	mov    %esp,%ebp
80104f25:	83 ec 18             	sub    $0x18,%esp
  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80104f28:	8d 55 ec             	lea    -0x14(%ebp),%edx
80104f2b:	e8 c0 fe ff ff       	call   80104df0 <argfd.constprop.0>
80104f30:	85 c0                	test   %eax,%eax
80104f32:	78 4c                	js     80104f80 <sys_write+0x60>
80104f34:	8d 45 f0             	lea    -0x10(%ebp),%eax
80104f37:	83 ec 08             	sub    $0x8,%esp
80104f3a:	50                   	push   %eax
80104f3b:	6a 02                	push   $0x2
80104f3d:	e8 5e fd ff ff       	call   80104ca0 <argint>
80104f42:	83 c4 10             	add    $0x10,%esp
80104f45:	85 c0                	test   %eax,%eax
80104f47:	78 37                	js     80104f80 <sys_write+0x60>
80104f49:	8d 45 f4             	lea    -0xc(%ebp),%eax
80104f4c:	83 ec 04             	sub    $0x4,%esp
80104f4f:	ff 75 f0             	pushl  -0x10(%ebp)
80104f52:	50                   	push   %eax
80104f53:	6a 01                	push   $0x1
80104f55:	e8 96 fd ff ff       	call   80104cf0 <argptr>
80104f5a:	83 c4 10             	add    $0x10,%esp
80104f5d:	85 c0                	test   %eax,%eax
80104f5f:	78 1f                	js     80104f80 <sys_write+0x60>
    return -1;
  return filewrite(f, p, n);
80104f61:	83 ec 04             	sub    $0x4,%esp
80104f64:	ff 75 f0             	pushl  -0x10(%ebp)
80104f67:	ff 75 f4             	pushl  -0xc(%ebp)
80104f6a:	ff 75 ec             	pushl  -0x14(%ebp)
80104f6d:	e8 ce c0 ff ff       	call   80101040 <filewrite>
80104f72:	83 c4 10             	add    $0x10,%esp
}
80104f75:	c9                   	leave  
80104f76:	c3                   	ret    
80104f77:	89 f6                	mov    %esi,%esi
80104f79:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    return -1;
80104f80:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104f85:	c9                   	leave  
80104f86:	c3                   	ret    
80104f87:	89 f6                	mov    %esi,%esi
80104f89:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104f90 <sys_close>:

int
sys_close(void)
{
80104f90:	55                   	push   %ebp
80104f91:	89 e5                	mov    %esp,%ebp
80104f93:	83 ec 18             	sub    $0x18,%esp
  int fd;
  struct file *f;

  if(argfd(0, &fd, &f) < 0)
80104f96:	8d 55 f4             	lea    -0xc(%ebp),%edx
80104f99:	8d 45 f0             	lea    -0x10(%ebp),%eax
80104f9c:	e8 4f fe ff ff       	call   80104df0 <argfd.constprop.0>
80104fa1:	85 c0                	test   %eax,%eax
80104fa3:	78 2b                	js     80104fd0 <sys_close+0x40>
    return -1;
  myproc()->ofile[fd] = 0;
80104fa5:	e8 56 ec ff ff       	call   80103c00 <myproc>
80104faa:	8b 55 f0             	mov    -0x10(%ebp),%edx
  fileclose(f);
80104fad:	83 ec 0c             	sub    $0xc,%esp
  myproc()->ofile[fd] = 0;
80104fb0:	c7 44 90 28 00 00 00 	movl   $0x0,0x28(%eax,%edx,4)
80104fb7:	00 
  fileclose(f);
80104fb8:	ff 75 f4             	pushl  -0xc(%ebp)
80104fbb:	e8 d0 be ff ff       	call   80100e90 <fileclose>
  return 0;
80104fc0:	83 c4 10             	add    $0x10,%esp
80104fc3:	31 c0                	xor    %eax,%eax
}
80104fc5:	c9                   	leave  
80104fc6:	c3                   	ret    
80104fc7:	89 f6                	mov    %esi,%esi
80104fc9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    return -1;
80104fd0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104fd5:	c9                   	leave  
80104fd6:	c3                   	ret    
80104fd7:	89 f6                	mov    %esi,%esi
80104fd9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80104fe0 <sys_fstat>:

int
sys_fstat(void)
{
80104fe0:	55                   	push   %ebp
  struct file *f;
  struct stat *st;

  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
80104fe1:	31 c0                	xor    %eax,%eax
{
80104fe3:	89 e5                	mov    %esp,%ebp
80104fe5:	83 ec 18             	sub    $0x18,%esp
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
80104fe8:	8d 55 f0             	lea    -0x10(%ebp),%edx
80104feb:	e8 00 fe ff ff       	call   80104df0 <argfd.constprop.0>
80104ff0:	85 c0                	test   %eax,%eax
80104ff2:	78 2c                	js     80105020 <sys_fstat+0x40>
80104ff4:	8d 45 f4             	lea    -0xc(%ebp),%eax
80104ff7:	83 ec 04             	sub    $0x4,%esp
80104ffa:	6a 14                	push   $0x14
80104ffc:	50                   	push   %eax
80104ffd:	6a 01                	push   $0x1
80104fff:	e8 ec fc ff ff       	call   80104cf0 <argptr>
80105004:	83 c4 10             	add    $0x10,%esp
80105007:	85 c0                	test   %eax,%eax
80105009:	78 15                	js     80105020 <sys_fstat+0x40>
    return -1;
  return filestat(f, st);
8010500b:	83 ec 08             	sub    $0x8,%esp
8010500e:	ff 75 f4             	pushl  -0xc(%ebp)
80105011:	ff 75 f0             	pushl  -0x10(%ebp)
80105014:	e8 47 bf ff ff       	call   80100f60 <filestat>
80105019:	83 c4 10             	add    $0x10,%esp
}
8010501c:	c9                   	leave  
8010501d:	c3                   	ret    
8010501e:	66 90                	xchg   %ax,%ax
    return -1;
80105020:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105025:	c9                   	leave  
80105026:	c3                   	ret    
80105027:	89 f6                	mov    %esi,%esi
80105029:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80105030 <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
80105030:	55                   	push   %ebp
80105031:	89 e5                	mov    %esp,%ebp
80105033:	57                   	push   %edi
80105034:	56                   	push   %esi
80105035:	53                   	push   %ebx
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
80105036:	8d 45 d4             	lea    -0x2c(%ebp),%eax
{
80105039:	83 ec 34             	sub    $0x34,%esp
  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
8010503c:	50                   	push   %eax
8010503d:	6a 00                	push   $0x0
8010503f:	e8 0c fd ff ff       	call   80104d50 <argstr>
80105044:	83 c4 10             	add    $0x10,%esp
80105047:	85 c0                	test   %eax,%eax
80105049:	0f 88 fb 00 00 00    	js     8010514a <sys_link+0x11a>
8010504f:	8d 45 d0             	lea    -0x30(%ebp),%eax
80105052:	83 ec 08             	sub    $0x8,%esp
80105055:	50                   	push   %eax
80105056:	6a 01                	push   $0x1
80105058:	e8 f3 fc ff ff       	call   80104d50 <argstr>
8010505d:	83 c4 10             	add    $0x10,%esp
80105060:	85 c0                	test   %eax,%eax
80105062:	0f 88 e2 00 00 00    	js     8010514a <sys_link+0x11a>
    return -1;

  begin_op();
80105068:	e8 23 df ff ff       	call   80102f90 <begin_op>
  if((ip = namei(old)) == 0){
8010506d:	83 ec 0c             	sub    $0xc,%esp
80105070:	ff 75 d4             	pushl  -0x2c(%ebp)
80105073:	e8 c8 ce ff ff       	call   80101f40 <namei>
80105078:	83 c4 10             	add    $0x10,%esp
8010507b:	85 c0                	test   %eax,%eax
8010507d:	89 c3                	mov    %eax,%ebx
8010507f:	0f 84 ea 00 00 00    	je     8010516f <sys_link+0x13f>
    end_op();
    return -1;
  }

  ilock(ip);
80105085:	83 ec 0c             	sub    $0xc,%esp
80105088:	50                   	push   %eax
80105089:	e8 52 c6 ff ff       	call   801016e0 <ilock>
  if(ip->type == T_DIR){
8010508e:	83 c4 10             	add    $0x10,%esp
80105091:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
80105096:	0f 84 bb 00 00 00    	je     80105157 <sys_link+0x127>
    iunlockput(ip);
    end_op();
    return -1;
  }

  ip->nlink++;
8010509c:	66 83 43 56 01       	addw   $0x1,0x56(%ebx)
  iupdate(ip);
801050a1:	83 ec 0c             	sub    $0xc,%esp
  iunlock(ip);

  if((dp = nameiparent(new, name)) == 0)
801050a4:	8d 7d da             	lea    -0x26(%ebp),%edi
  iupdate(ip);
801050a7:	53                   	push   %ebx
801050a8:	e8 83 c5 ff ff       	call   80101630 <iupdate>
  iunlock(ip);
801050ad:	89 1c 24             	mov    %ebx,(%esp)
801050b0:	e8 0b c7 ff ff       	call   801017c0 <iunlock>
  if((dp = nameiparent(new, name)) == 0)
801050b5:	58                   	pop    %eax
801050b6:	5a                   	pop    %edx
801050b7:	57                   	push   %edi
801050b8:	ff 75 d0             	pushl  -0x30(%ebp)
801050bb:	e8 a0 ce ff ff       	call   80101f60 <nameiparent>
801050c0:	83 c4 10             	add    $0x10,%esp
801050c3:	85 c0                	test   %eax,%eax
801050c5:	89 c6                	mov    %eax,%esi
801050c7:	74 5b                	je     80105124 <sys_link+0xf4>
    goto bad;
  ilock(dp);
801050c9:	83 ec 0c             	sub    $0xc,%esp
801050cc:	50                   	push   %eax
801050cd:	e8 0e c6 ff ff       	call   801016e0 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
801050d2:	83 c4 10             	add    $0x10,%esp
801050d5:	8b 03                	mov    (%ebx),%eax
801050d7:	39 06                	cmp    %eax,(%esi)
801050d9:	75 3d                	jne    80105118 <sys_link+0xe8>
801050db:	83 ec 04             	sub    $0x4,%esp
801050de:	ff 73 04             	pushl  0x4(%ebx)
801050e1:	57                   	push   %edi
801050e2:	56                   	push   %esi
801050e3:	e8 98 cd ff ff       	call   80101e80 <dirlink>
801050e8:	83 c4 10             	add    $0x10,%esp
801050eb:	85 c0                	test   %eax,%eax
801050ed:	78 29                	js     80105118 <sys_link+0xe8>
    iunlockput(dp);
    goto bad;
  }
  iunlockput(dp);
801050ef:	83 ec 0c             	sub    $0xc,%esp
801050f2:	56                   	push   %esi
801050f3:	e8 78 c8 ff ff       	call   80101970 <iunlockput>
  iput(ip);
801050f8:	89 1c 24             	mov    %ebx,(%esp)
801050fb:	e8 10 c7 ff ff       	call   80101810 <iput>

  end_op();
80105100:	e8 fb de ff ff       	call   80103000 <end_op>

  return 0;
80105105:	83 c4 10             	add    $0x10,%esp
80105108:	31 c0                	xor    %eax,%eax
  ip->nlink--;
  iupdate(ip);
  iunlockput(ip);
  end_op();
  return -1;
}
8010510a:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010510d:	5b                   	pop    %ebx
8010510e:	5e                   	pop    %esi
8010510f:	5f                   	pop    %edi
80105110:	5d                   	pop    %ebp
80105111:	c3                   	ret    
80105112:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    iunlockput(dp);
80105118:	83 ec 0c             	sub    $0xc,%esp
8010511b:	56                   	push   %esi
8010511c:	e8 4f c8 ff ff       	call   80101970 <iunlockput>
    goto bad;
80105121:	83 c4 10             	add    $0x10,%esp
  ilock(ip);
80105124:	83 ec 0c             	sub    $0xc,%esp
80105127:	53                   	push   %ebx
80105128:	e8 b3 c5 ff ff       	call   801016e0 <ilock>
  ip->nlink--;
8010512d:	66 83 6b 56 01       	subw   $0x1,0x56(%ebx)
  iupdate(ip);
80105132:	89 1c 24             	mov    %ebx,(%esp)
80105135:	e8 f6 c4 ff ff       	call   80101630 <iupdate>
  iunlockput(ip);
8010513a:	89 1c 24             	mov    %ebx,(%esp)
8010513d:	e8 2e c8 ff ff       	call   80101970 <iunlockput>
  end_op();
80105142:	e8 b9 de ff ff       	call   80103000 <end_op>
  return -1;
80105147:	83 c4 10             	add    $0x10,%esp
}
8010514a:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return -1;
8010514d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105152:	5b                   	pop    %ebx
80105153:	5e                   	pop    %esi
80105154:	5f                   	pop    %edi
80105155:	5d                   	pop    %ebp
80105156:	c3                   	ret    
    iunlockput(ip);
80105157:	83 ec 0c             	sub    $0xc,%esp
8010515a:	53                   	push   %ebx
8010515b:	e8 10 c8 ff ff       	call   80101970 <iunlockput>
    end_op();
80105160:	e8 9b de ff ff       	call   80103000 <end_op>
    return -1;
80105165:	83 c4 10             	add    $0x10,%esp
80105168:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010516d:	eb 9b                	jmp    8010510a <sys_link+0xda>
    end_op();
8010516f:	e8 8c de ff ff       	call   80103000 <end_op>
    return -1;
80105174:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105179:	eb 8f                	jmp    8010510a <sys_link+0xda>
8010517b:	90                   	nop
8010517c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105180 <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
int
isdirempty(struct inode *dp)
{
80105180:	55                   	push   %ebp
80105181:	89 e5                	mov    %esp,%ebp
80105183:	57                   	push   %edi
80105184:	56                   	push   %esi
80105185:	53                   	push   %ebx
80105186:	83 ec 1c             	sub    $0x1c,%esp
80105189:	8b 75 08             	mov    0x8(%ebp),%esi
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
8010518c:	83 7e 58 20          	cmpl   $0x20,0x58(%esi)
80105190:	76 3e                	jbe    801051d0 <isdirempty+0x50>
80105192:	bb 20 00 00 00       	mov    $0x20,%ebx
80105197:	8d 7d d8             	lea    -0x28(%ebp),%edi
8010519a:	eb 0c                	jmp    801051a8 <isdirempty+0x28>
8010519c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801051a0:	83 c3 10             	add    $0x10,%ebx
801051a3:	3b 5e 58             	cmp    0x58(%esi),%ebx
801051a6:	73 28                	jae    801051d0 <isdirempty+0x50>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801051a8:	6a 10                	push   $0x10
801051aa:	53                   	push   %ebx
801051ab:	57                   	push   %edi
801051ac:	56                   	push   %esi
801051ad:	e8 0e c8 ff ff       	call   801019c0 <readi>
801051b2:	83 c4 10             	add    $0x10,%esp
801051b5:	83 f8 10             	cmp    $0x10,%eax
801051b8:	75 23                	jne    801051dd <isdirempty+0x5d>
      panic("isdirempty: readi");
    if(de.inum != 0)
801051ba:	66 83 7d d8 00       	cmpw   $0x0,-0x28(%ebp)
801051bf:	74 df                	je     801051a0 <isdirempty+0x20>
      return 0;
  }
  return 1;
}
801051c1:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return 0;
801051c4:	31 c0                	xor    %eax,%eax
}
801051c6:	5b                   	pop    %ebx
801051c7:	5e                   	pop    %esi
801051c8:	5f                   	pop    %edi
801051c9:	5d                   	pop    %ebp
801051ca:	c3                   	ret    
801051cb:	90                   	nop
801051cc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801051d0:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 1;
801051d3:	b8 01 00 00 00       	mov    $0x1,%eax
}
801051d8:	5b                   	pop    %ebx
801051d9:	5e                   	pop    %esi
801051da:	5f                   	pop    %edi
801051db:	5d                   	pop    %ebp
801051dc:	c3                   	ret    
      panic("isdirempty: readi");
801051dd:	83 ec 0c             	sub    $0xc,%esp
801051e0:	68 fc 7c 10 80       	push   $0x80107cfc
801051e5:	e8 a6 b1 ff ff       	call   80100390 <panic>
801051ea:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

801051f0 <sys_unlink>:

//PAGEBREAK!
int
sys_unlink(void)
{
801051f0:	55                   	push   %ebp
801051f1:	89 e5                	mov    %esp,%ebp
801051f3:	57                   	push   %edi
801051f4:	56                   	push   %esi
801051f5:	53                   	push   %ebx
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
801051f6:	8d 45 c0             	lea    -0x40(%ebp),%eax
{
801051f9:	83 ec 44             	sub    $0x44,%esp
  if(argstr(0, &path) < 0)
801051fc:	50                   	push   %eax
801051fd:	6a 00                	push   $0x0
801051ff:	e8 4c fb ff ff       	call   80104d50 <argstr>
80105204:	83 c4 10             	add    $0x10,%esp
80105207:	85 c0                	test   %eax,%eax
80105209:	0f 88 51 01 00 00    	js     80105360 <sys_unlink+0x170>
    return -1;

  begin_op();
  if((dp = nameiparent(path, name)) == 0){
8010520f:	8d 5d ca             	lea    -0x36(%ebp),%ebx
  begin_op();
80105212:	e8 79 dd ff ff       	call   80102f90 <begin_op>
  if((dp = nameiparent(path, name)) == 0){
80105217:	83 ec 08             	sub    $0x8,%esp
8010521a:	53                   	push   %ebx
8010521b:	ff 75 c0             	pushl  -0x40(%ebp)
8010521e:	e8 3d cd ff ff       	call   80101f60 <nameiparent>
80105223:	83 c4 10             	add    $0x10,%esp
80105226:	85 c0                	test   %eax,%eax
80105228:	89 c6                	mov    %eax,%esi
8010522a:	0f 84 37 01 00 00    	je     80105367 <sys_unlink+0x177>
    end_op();
    return -1;
  }

  ilock(dp);
80105230:	83 ec 0c             	sub    $0xc,%esp
80105233:	50                   	push   %eax
80105234:	e8 a7 c4 ff ff       	call   801016e0 <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105239:	58                   	pop    %eax
8010523a:	5a                   	pop    %edx
8010523b:	68 e8 75 10 80       	push   $0x801075e8
80105240:	53                   	push   %ebx
80105241:	e8 aa c9 ff ff       	call   80101bf0 <namecmp>
80105246:	83 c4 10             	add    $0x10,%esp
80105249:	85 c0                	test   %eax,%eax
8010524b:	0f 84 d7 00 00 00    	je     80105328 <sys_unlink+0x138>
80105251:	83 ec 08             	sub    $0x8,%esp
80105254:	68 e7 75 10 80       	push   $0x801075e7
80105259:	53                   	push   %ebx
8010525a:	e8 91 c9 ff ff       	call   80101bf0 <namecmp>
8010525f:	83 c4 10             	add    $0x10,%esp
80105262:	85 c0                	test   %eax,%eax
80105264:	0f 84 be 00 00 00    	je     80105328 <sys_unlink+0x138>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
8010526a:	8d 45 c4             	lea    -0x3c(%ebp),%eax
8010526d:	83 ec 04             	sub    $0x4,%esp
80105270:	50                   	push   %eax
80105271:	53                   	push   %ebx
80105272:	56                   	push   %esi
80105273:	e8 98 c9 ff ff       	call   80101c10 <dirlookup>
80105278:	83 c4 10             	add    $0x10,%esp
8010527b:	85 c0                	test   %eax,%eax
8010527d:	89 c3                	mov    %eax,%ebx
8010527f:	0f 84 a3 00 00 00    	je     80105328 <sys_unlink+0x138>
    goto bad;
  ilock(ip);
80105285:	83 ec 0c             	sub    $0xc,%esp
80105288:	50                   	push   %eax
80105289:	e8 52 c4 ff ff       	call   801016e0 <ilock>

  if(ip->nlink < 1)
8010528e:	83 c4 10             	add    $0x10,%esp
80105291:	66 83 7b 56 00       	cmpw   $0x0,0x56(%ebx)
80105296:	0f 8e e4 00 00 00    	jle    80105380 <sys_unlink+0x190>
    panic("unlink: nlink < 1");
  if(ip->type == T_DIR && !isdirempty(ip)){
8010529c:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
801052a1:	74 65                	je     80105308 <sys_unlink+0x118>
    iunlockput(ip);
    goto bad;
  }

  memset(&de, 0, sizeof(de));
801052a3:	8d 7d d8             	lea    -0x28(%ebp),%edi
801052a6:	83 ec 04             	sub    $0x4,%esp
801052a9:	6a 10                	push   $0x10
801052ab:	6a 00                	push   $0x0
801052ad:	57                   	push   %edi
801052ae:	e8 ed f6 ff ff       	call   801049a0 <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801052b3:	6a 10                	push   $0x10
801052b5:	ff 75 c4             	pushl  -0x3c(%ebp)
801052b8:	57                   	push   %edi
801052b9:	56                   	push   %esi
801052ba:	e8 01 c8 ff ff       	call   80101ac0 <writei>
801052bf:	83 c4 20             	add    $0x20,%esp
801052c2:	83 f8 10             	cmp    $0x10,%eax
801052c5:	0f 85 a8 00 00 00    	jne    80105373 <sys_unlink+0x183>
    panic("unlink: writei");
  if(ip->type == T_DIR){
801052cb:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
801052d0:	74 6e                	je     80105340 <sys_unlink+0x150>
    dp->nlink--;
    iupdate(dp);
  }
  iunlockput(dp);
801052d2:	83 ec 0c             	sub    $0xc,%esp
801052d5:	56                   	push   %esi
801052d6:	e8 95 c6 ff ff       	call   80101970 <iunlockput>

  ip->nlink--;
801052db:	66 83 6b 56 01       	subw   $0x1,0x56(%ebx)
  iupdate(ip);
801052e0:	89 1c 24             	mov    %ebx,(%esp)
801052e3:	e8 48 c3 ff ff       	call   80101630 <iupdate>
  iunlockput(ip);
801052e8:	89 1c 24             	mov    %ebx,(%esp)
801052eb:	e8 80 c6 ff ff       	call   80101970 <iunlockput>

  end_op();
801052f0:	e8 0b dd ff ff       	call   80103000 <end_op>

  return 0;
801052f5:	83 c4 10             	add    $0x10,%esp
801052f8:	31 c0                	xor    %eax,%eax

bad:
  iunlockput(dp);
  end_op();
  return -1;
}
801052fa:	8d 65 f4             	lea    -0xc(%ebp),%esp
801052fd:	5b                   	pop    %ebx
801052fe:	5e                   	pop    %esi
801052ff:	5f                   	pop    %edi
80105300:	5d                   	pop    %ebp
80105301:	c3                   	ret    
80105302:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  if(ip->type == T_DIR && !isdirempty(ip)){
80105308:	83 ec 0c             	sub    $0xc,%esp
8010530b:	53                   	push   %ebx
8010530c:	e8 6f fe ff ff       	call   80105180 <isdirempty>
80105311:	83 c4 10             	add    $0x10,%esp
80105314:	85 c0                	test   %eax,%eax
80105316:	75 8b                	jne    801052a3 <sys_unlink+0xb3>
    iunlockput(ip);
80105318:	83 ec 0c             	sub    $0xc,%esp
8010531b:	53                   	push   %ebx
8010531c:	e8 4f c6 ff ff       	call   80101970 <iunlockput>
    goto bad;
80105321:	83 c4 10             	add    $0x10,%esp
80105324:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  iunlockput(dp);
80105328:	83 ec 0c             	sub    $0xc,%esp
8010532b:	56                   	push   %esi
8010532c:	e8 3f c6 ff ff       	call   80101970 <iunlockput>
  end_op();
80105331:	e8 ca dc ff ff       	call   80103000 <end_op>
  return -1;
80105336:	83 c4 10             	add    $0x10,%esp
80105339:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010533e:	eb ba                	jmp    801052fa <sys_unlink+0x10a>
    dp->nlink--;
80105340:	66 83 6e 56 01       	subw   $0x1,0x56(%esi)
    iupdate(dp);
80105345:	83 ec 0c             	sub    $0xc,%esp
80105348:	56                   	push   %esi
80105349:	e8 e2 c2 ff ff       	call   80101630 <iupdate>
8010534e:	83 c4 10             	add    $0x10,%esp
80105351:	e9 7c ff ff ff       	jmp    801052d2 <sys_unlink+0xe2>
80105356:	8d 76 00             	lea    0x0(%esi),%esi
80105359:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    return -1;
80105360:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105365:	eb 93                	jmp    801052fa <sys_unlink+0x10a>
    end_op();
80105367:	e8 94 dc ff ff       	call   80103000 <end_op>
    return -1;
8010536c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105371:	eb 87                	jmp    801052fa <sys_unlink+0x10a>
    panic("unlink: writei");
80105373:	83 ec 0c             	sub    $0xc,%esp
80105376:	68 fc 75 10 80       	push   $0x801075fc
8010537b:	e8 10 b0 ff ff       	call   80100390 <panic>
    panic("unlink: nlink < 1");
80105380:	83 ec 0c             	sub    $0xc,%esp
80105383:	68 ea 75 10 80       	push   $0x801075ea
80105388:	e8 03 b0 ff ff       	call   80100390 <panic>
8010538d:	8d 76 00             	lea    0x0(%esi),%esi

80105390 <create>:

struct inode*
create(char *path, short type, short major, short minor)
{
80105390:	55                   	push   %ebp
80105391:	89 e5                	mov    %esp,%ebp
80105393:	57                   	push   %edi
80105394:	56                   	push   %esi
80105395:	53                   	push   %ebx
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
80105396:	8d 75 da             	lea    -0x26(%ebp),%esi
{
80105399:	83 ec 44             	sub    $0x44,%esp
8010539c:	8b 45 0c             	mov    0xc(%ebp),%eax
8010539f:	8b 55 10             	mov    0x10(%ebp),%edx
801053a2:	8b 4d 14             	mov    0x14(%ebp),%ecx
  if((dp = nameiparent(path, name)) == 0)
801053a5:	56                   	push   %esi
801053a6:	ff 75 08             	pushl  0x8(%ebp)
{
801053a9:	89 45 c4             	mov    %eax,-0x3c(%ebp)
801053ac:	89 55 c0             	mov    %edx,-0x40(%ebp)
801053af:	89 4d bc             	mov    %ecx,-0x44(%ebp)
  if((dp = nameiparent(path, name)) == 0)
801053b2:	e8 a9 cb ff ff       	call   80101f60 <nameiparent>
801053b7:	83 c4 10             	add    $0x10,%esp
801053ba:	85 c0                	test   %eax,%eax
801053bc:	0f 84 4e 01 00 00    	je     80105510 <create+0x180>
    return 0;
  ilock(dp);
801053c2:	83 ec 0c             	sub    $0xc,%esp
801053c5:	89 c3                	mov    %eax,%ebx
801053c7:	50                   	push   %eax
801053c8:	e8 13 c3 ff ff       	call   801016e0 <ilock>

  if((ip = dirlookup(dp, name, &off)) != 0){
801053cd:	8d 45 d4             	lea    -0x2c(%ebp),%eax
801053d0:	83 c4 0c             	add    $0xc,%esp
801053d3:	50                   	push   %eax
801053d4:	56                   	push   %esi
801053d5:	53                   	push   %ebx
801053d6:	e8 35 c8 ff ff       	call   80101c10 <dirlookup>
801053db:	83 c4 10             	add    $0x10,%esp
801053de:	85 c0                	test   %eax,%eax
801053e0:	89 c7                	mov    %eax,%edi
801053e2:	74 3c                	je     80105420 <create+0x90>
    iunlockput(dp);
801053e4:	83 ec 0c             	sub    $0xc,%esp
801053e7:	53                   	push   %ebx
801053e8:	e8 83 c5 ff ff       	call   80101970 <iunlockput>
    ilock(ip);
801053ed:	89 3c 24             	mov    %edi,(%esp)
801053f0:	e8 eb c2 ff ff       	call   801016e0 <ilock>
    if(type == T_FILE && ip->type == T_FILE)
801053f5:	83 c4 10             	add    $0x10,%esp
801053f8:	66 83 7d c4 02       	cmpw   $0x2,-0x3c(%ebp)
801053fd:	0f 85 9d 00 00 00    	jne    801054a0 <create+0x110>
80105403:	66 83 7f 50 02       	cmpw   $0x2,0x50(%edi)
80105408:	0f 85 92 00 00 00    	jne    801054a0 <create+0x110>
    panic("create: dirlink");

  iunlockput(dp);

  return ip;
}
8010540e:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105411:	89 f8                	mov    %edi,%eax
80105413:	5b                   	pop    %ebx
80105414:	5e                   	pop    %esi
80105415:	5f                   	pop    %edi
80105416:	5d                   	pop    %ebp
80105417:	c3                   	ret    
80105418:	90                   	nop
80105419:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  if((ip = ialloc(dp->dev, type)) == 0)
80105420:	0f bf 45 c4          	movswl -0x3c(%ebp),%eax
80105424:	83 ec 08             	sub    $0x8,%esp
80105427:	50                   	push   %eax
80105428:	ff 33                	pushl  (%ebx)
8010542a:	e8 41 c1 ff ff       	call   80101570 <ialloc>
8010542f:	83 c4 10             	add    $0x10,%esp
80105432:	85 c0                	test   %eax,%eax
80105434:	89 c7                	mov    %eax,%edi
80105436:	0f 84 e8 00 00 00    	je     80105524 <create+0x194>
  ilock(ip);
8010543c:	83 ec 0c             	sub    $0xc,%esp
8010543f:	50                   	push   %eax
80105440:	e8 9b c2 ff ff       	call   801016e0 <ilock>
  ip->major = major;
80105445:	0f b7 45 c0          	movzwl -0x40(%ebp),%eax
80105449:	66 89 47 52          	mov    %ax,0x52(%edi)
  ip->minor = minor;
8010544d:	0f b7 45 bc          	movzwl -0x44(%ebp),%eax
80105451:	66 89 47 54          	mov    %ax,0x54(%edi)
  ip->nlink = 1;
80105455:	b8 01 00 00 00       	mov    $0x1,%eax
8010545a:	66 89 47 56          	mov    %ax,0x56(%edi)
  iupdate(ip);
8010545e:	89 3c 24             	mov    %edi,(%esp)
80105461:	e8 ca c1 ff ff       	call   80101630 <iupdate>
  if(type == T_DIR){  // Create . and .. entries.
80105466:	83 c4 10             	add    $0x10,%esp
80105469:	66 83 7d c4 01       	cmpw   $0x1,-0x3c(%ebp)
8010546e:	74 50                	je     801054c0 <create+0x130>
  if(dirlink(dp, name, ip->inum) < 0)
80105470:	83 ec 04             	sub    $0x4,%esp
80105473:	ff 77 04             	pushl  0x4(%edi)
80105476:	56                   	push   %esi
80105477:	53                   	push   %ebx
80105478:	e8 03 ca ff ff       	call   80101e80 <dirlink>
8010547d:	83 c4 10             	add    $0x10,%esp
80105480:	85 c0                	test   %eax,%eax
80105482:	0f 88 8f 00 00 00    	js     80105517 <create+0x187>
  iunlockput(dp);
80105488:	83 ec 0c             	sub    $0xc,%esp
8010548b:	53                   	push   %ebx
8010548c:	e8 df c4 ff ff       	call   80101970 <iunlockput>
  return ip;
80105491:	83 c4 10             	add    $0x10,%esp
}
80105494:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105497:	89 f8                	mov    %edi,%eax
80105499:	5b                   	pop    %ebx
8010549a:	5e                   	pop    %esi
8010549b:	5f                   	pop    %edi
8010549c:	5d                   	pop    %ebp
8010549d:	c3                   	ret    
8010549e:	66 90                	xchg   %ax,%ax
    iunlockput(ip);
801054a0:	83 ec 0c             	sub    $0xc,%esp
801054a3:	57                   	push   %edi
    return 0;
801054a4:	31 ff                	xor    %edi,%edi
    iunlockput(ip);
801054a6:	e8 c5 c4 ff ff       	call   80101970 <iunlockput>
    return 0;
801054ab:	83 c4 10             	add    $0x10,%esp
}
801054ae:	8d 65 f4             	lea    -0xc(%ebp),%esp
801054b1:	89 f8                	mov    %edi,%eax
801054b3:	5b                   	pop    %ebx
801054b4:	5e                   	pop    %esi
801054b5:	5f                   	pop    %edi
801054b6:	5d                   	pop    %ebp
801054b7:	c3                   	ret    
801054b8:	90                   	nop
801054b9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    dp->nlink++;  // for ".."
801054c0:	66 83 43 56 01       	addw   $0x1,0x56(%ebx)
    iupdate(dp);
801054c5:	83 ec 0c             	sub    $0xc,%esp
801054c8:	53                   	push   %ebx
801054c9:	e8 62 c1 ff ff       	call   80101630 <iupdate>
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
801054ce:	83 c4 0c             	add    $0xc,%esp
801054d1:	ff 77 04             	pushl  0x4(%edi)
801054d4:	68 e8 75 10 80       	push   $0x801075e8
801054d9:	57                   	push   %edi
801054da:	e8 a1 c9 ff ff       	call   80101e80 <dirlink>
801054df:	83 c4 10             	add    $0x10,%esp
801054e2:	85 c0                	test   %eax,%eax
801054e4:	78 1c                	js     80105502 <create+0x172>
801054e6:	83 ec 04             	sub    $0x4,%esp
801054e9:	ff 73 04             	pushl  0x4(%ebx)
801054ec:	68 e7 75 10 80       	push   $0x801075e7
801054f1:	57                   	push   %edi
801054f2:	e8 89 c9 ff ff       	call   80101e80 <dirlink>
801054f7:	83 c4 10             	add    $0x10,%esp
801054fa:	85 c0                	test   %eax,%eax
801054fc:	0f 89 6e ff ff ff    	jns    80105470 <create+0xe0>
      panic("create dots");
80105502:	83 ec 0c             	sub    $0xc,%esp
80105505:	68 1d 7d 10 80       	push   $0x80107d1d
8010550a:	e8 81 ae ff ff       	call   80100390 <panic>
8010550f:	90                   	nop
    return 0;
80105510:	31 ff                	xor    %edi,%edi
80105512:	e9 f7 fe ff ff       	jmp    8010540e <create+0x7e>
    panic("create: dirlink");
80105517:	83 ec 0c             	sub    $0xc,%esp
8010551a:	68 29 7d 10 80       	push   $0x80107d29
8010551f:	e8 6c ae ff ff       	call   80100390 <panic>
    panic("create: ialloc");
80105524:	83 ec 0c             	sub    $0xc,%esp
80105527:	68 0e 7d 10 80       	push   $0x80107d0e
8010552c:	e8 5f ae ff ff       	call   80100390 <panic>
80105531:	eb 0d                	jmp    80105540 <sys_open>
80105533:	90                   	nop
80105534:	90                   	nop
80105535:	90                   	nop
80105536:	90                   	nop
80105537:	90                   	nop
80105538:	90                   	nop
80105539:	90                   	nop
8010553a:	90                   	nop
8010553b:	90                   	nop
8010553c:	90                   	nop
8010553d:	90                   	nop
8010553e:	90                   	nop
8010553f:	90                   	nop

80105540 <sys_open>:

int
sys_open(void)
{
80105540:	55                   	push   %ebp
80105541:	89 e5                	mov    %esp,%ebp
80105543:	57                   	push   %edi
80105544:	56                   	push   %esi
80105545:	53                   	push   %ebx
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80105546:	8d 45 e0             	lea    -0x20(%ebp),%eax
{
80105549:	83 ec 24             	sub    $0x24,%esp
  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
8010554c:	50                   	push   %eax
8010554d:	6a 00                	push   $0x0
8010554f:	e8 fc f7 ff ff       	call   80104d50 <argstr>
80105554:	83 c4 10             	add    $0x10,%esp
80105557:	85 c0                	test   %eax,%eax
80105559:	0f 88 1d 01 00 00    	js     8010567c <sys_open+0x13c>
8010555f:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105562:	83 ec 08             	sub    $0x8,%esp
80105565:	50                   	push   %eax
80105566:	6a 01                	push   $0x1
80105568:	e8 33 f7 ff ff       	call   80104ca0 <argint>
8010556d:	83 c4 10             	add    $0x10,%esp
80105570:	85 c0                	test   %eax,%eax
80105572:	0f 88 04 01 00 00    	js     8010567c <sys_open+0x13c>
    return -1;

  begin_op();
80105578:	e8 13 da ff ff       	call   80102f90 <begin_op>

  if(omode & O_CREATE){
8010557d:	f6 45 e5 02          	testb  $0x2,-0x1b(%ebp)
80105581:	0f 85 a9 00 00 00    	jne    80105630 <sys_open+0xf0>
    if(ip == 0){
      end_op();
      return -1;
    }
  } else {
    if((ip = namei(path)) == 0){
80105587:	83 ec 0c             	sub    $0xc,%esp
8010558a:	ff 75 e0             	pushl  -0x20(%ebp)
8010558d:	e8 ae c9 ff ff       	call   80101f40 <namei>
80105592:	83 c4 10             	add    $0x10,%esp
80105595:	85 c0                	test   %eax,%eax
80105597:	89 c6                	mov    %eax,%esi
80105599:	0f 84 ac 00 00 00    	je     8010564b <sys_open+0x10b>
      end_op();
      return -1;
    }
    ilock(ip);
8010559f:	83 ec 0c             	sub    $0xc,%esp
801055a2:	50                   	push   %eax
801055a3:	e8 38 c1 ff ff       	call   801016e0 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
801055a8:	83 c4 10             	add    $0x10,%esp
801055ab:	66 83 7e 50 01       	cmpw   $0x1,0x50(%esi)
801055b0:	0f 84 aa 00 00 00    	je     80105660 <sys_open+0x120>
      end_op();
      return -1;
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
801055b6:	e8 15 b8 ff ff       	call   80100dd0 <filealloc>
801055bb:	85 c0                	test   %eax,%eax
801055bd:	89 c7                	mov    %eax,%edi
801055bf:	0f 84 a6 00 00 00    	je     8010566b <sys_open+0x12b>
  struct proc *curproc = myproc();
801055c5:	e8 36 e6 ff ff       	call   80103c00 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
801055ca:	31 db                	xor    %ebx,%ebx
801055cc:	eb 0e                	jmp    801055dc <sys_open+0x9c>
801055ce:	66 90                	xchg   %ax,%ax
801055d0:	83 c3 01             	add    $0x1,%ebx
801055d3:	83 fb 10             	cmp    $0x10,%ebx
801055d6:	0f 84 ac 00 00 00    	je     80105688 <sys_open+0x148>
    if(curproc->ofile[fd] == 0){
801055dc:	8b 54 98 28          	mov    0x28(%eax,%ebx,4),%edx
801055e0:	85 d2                	test   %edx,%edx
801055e2:	75 ec                	jne    801055d0 <sys_open+0x90>
      fileclose(f);
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
801055e4:	83 ec 0c             	sub    $0xc,%esp
      curproc->ofile[fd] = f;
801055e7:	89 7c 98 28          	mov    %edi,0x28(%eax,%ebx,4)
  iunlock(ip);
801055eb:	56                   	push   %esi
801055ec:	e8 cf c1 ff ff       	call   801017c0 <iunlock>
  end_op();
801055f1:	e8 0a da ff ff       	call   80103000 <end_op>

  f->type = FD_INODE;
801055f6:	c7 07 02 00 00 00    	movl   $0x2,(%edi)
  f->ip = ip;
  f->off = 0;
  f->readable = !(omode & O_WRONLY);
801055fc:	8b 55 e4             	mov    -0x1c(%ebp),%edx
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
801055ff:	83 c4 10             	add    $0x10,%esp
  f->ip = ip;
80105602:	89 77 10             	mov    %esi,0x10(%edi)
  f->off = 0;
80105605:	c7 47 14 00 00 00 00 	movl   $0x0,0x14(%edi)
  f->readable = !(omode & O_WRONLY);
8010560c:	89 d0                	mov    %edx,%eax
8010560e:	f7 d0                	not    %eax
80105610:	83 e0 01             	and    $0x1,%eax
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105613:	83 e2 03             	and    $0x3,%edx
  f->readable = !(omode & O_WRONLY);
80105616:	88 47 08             	mov    %al,0x8(%edi)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105619:	0f 95 47 09          	setne  0x9(%edi)
  return fd;
}
8010561d:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105620:	89 d8                	mov    %ebx,%eax
80105622:	5b                   	pop    %ebx
80105623:	5e                   	pop    %esi
80105624:	5f                   	pop    %edi
80105625:	5d                   	pop    %ebp
80105626:	c3                   	ret    
80105627:	89 f6                	mov    %esi,%esi
80105629:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    ip = create(path, T_FILE, 0, 0);
80105630:	6a 00                	push   $0x0
80105632:	6a 00                	push   $0x0
80105634:	6a 02                	push   $0x2
80105636:	ff 75 e0             	pushl  -0x20(%ebp)
80105639:	e8 52 fd ff ff       	call   80105390 <create>
    if(ip == 0){
8010563e:	83 c4 10             	add    $0x10,%esp
80105641:	85 c0                	test   %eax,%eax
    ip = create(path, T_FILE, 0, 0);
80105643:	89 c6                	mov    %eax,%esi
    if(ip == 0){
80105645:	0f 85 6b ff ff ff    	jne    801055b6 <sys_open+0x76>
      end_op();
8010564b:	e8 b0 d9 ff ff       	call   80103000 <end_op>
      return -1;
80105650:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80105655:	eb c6                	jmp    8010561d <sys_open+0xdd>
80105657:	89 f6                	mov    %esi,%esi
80105659:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    if(ip->type == T_DIR && omode != O_RDONLY){
80105660:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80105663:	85 c9                	test   %ecx,%ecx
80105665:	0f 84 4b ff ff ff    	je     801055b6 <sys_open+0x76>
    iunlockput(ip);
8010566b:	83 ec 0c             	sub    $0xc,%esp
8010566e:	56                   	push   %esi
8010566f:	e8 fc c2 ff ff       	call   80101970 <iunlockput>
    end_op();
80105674:	e8 87 d9 ff ff       	call   80103000 <end_op>
    return -1;
80105679:	83 c4 10             	add    $0x10,%esp
8010567c:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80105681:	eb 9a                	jmp    8010561d <sys_open+0xdd>
80105683:	90                   	nop
80105684:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      fileclose(f);
80105688:	83 ec 0c             	sub    $0xc,%esp
8010568b:	57                   	push   %edi
8010568c:	e8 ff b7 ff ff       	call   80100e90 <fileclose>
80105691:	83 c4 10             	add    $0x10,%esp
80105694:	eb d5                	jmp    8010566b <sys_open+0x12b>
80105696:	8d 76 00             	lea    0x0(%esi),%esi
80105699:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801056a0 <sys_mkdir>:

int
sys_mkdir(void)
{
801056a0:	55                   	push   %ebp
801056a1:	89 e5                	mov    %esp,%ebp
801056a3:	83 ec 18             	sub    $0x18,%esp
  char *path;
  struct inode *ip;

  begin_op();
801056a6:	e8 e5 d8 ff ff       	call   80102f90 <begin_op>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
801056ab:	8d 45 f4             	lea    -0xc(%ebp),%eax
801056ae:	83 ec 08             	sub    $0x8,%esp
801056b1:	50                   	push   %eax
801056b2:	6a 00                	push   $0x0
801056b4:	e8 97 f6 ff ff       	call   80104d50 <argstr>
801056b9:	83 c4 10             	add    $0x10,%esp
801056bc:	85 c0                	test   %eax,%eax
801056be:	78 30                	js     801056f0 <sys_mkdir+0x50>
801056c0:	6a 00                	push   $0x0
801056c2:	6a 00                	push   $0x0
801056c4:	6a 01                	push   $0x1
801056c6:	ff 75 f4             	pushl  -0xc(%ebp)
801056c9:	e8 c2 fc ff ff       	call   80105390 <create>
801056ce:	83 c4 10             	add    $0x10,%esp
801056d1:	85 c0                	test   %eax,%eax
801056d3:	74 1b                	je     801056f0 <sys_mkdir+0x50>
    end_op();
    return -1;
  }
  iunlockput(ip);
801056d5:	83 ec 0c             	sub    $0xc,%esp
801056d8:	50                   	push   %eax
801056d9:	e8 92 c2 ff ff       	call   80101970 <iunlockput>
  end_op();
801056de:	e8 1d d9 ff ff       	call   80103000 <end_op>
  return 0;
801056e3:	83 c4 10             	add    $0x10,%esp
801056e6:	31 c0                	xor    %eax,%eax
}
801056e8:	c9                   	leave  
801056e9:	c3                   	ret    
801056ea:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    end_op();
801056f0:	e8 0b d9 ff ff       	call   80103000 <end_op>
    return -1;
801056f5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801056fa:	c9                   	leave  
801056fb:	c3                   	ret    
801056fc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105700 <sys_mknod>:

int
sys_mknod(void)
{
80105700:	55                   	push   %ebp
80105701:	89 e5                	mov    %esp,%ebp
80105703:	83 ec 18             	sub    $0x18,%esp
  struct inode *ip;
  char *path;
  int major, minor;

  begin_op();
80105706:	e8 85 d8 ff ff       	call   80102f90 <begin_op>
  if((argstr(0, &path)) < 0 ||
8010570b:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010570e:	83 ec 08             	sub    $0x8,%esp
80105711:	50                   	push   %eax
80105712:	6a 00                	push   $0x0
80105714:	e8 37 f6 ff ff       	call   80104d50 <argstr>
80105719:	83 c4 10             	add    $0x10,%esp
8010571c:	85 c0                	test   %eax,%eax
8010571e:	78 60                	js     80105780 <sys_mknod+0x80>
     argint(1, &major) < 0 ||
80105720:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105723:	83 ec 08             	sub    $0x8,%esp
80105726:	50                   	push   %eax
80105727:	6a 01                	push   $0x1
80105729:	e8 72 f5 ff ff       	call   80104ca0 <argint>
  if((argstr(0, &path)) < 0 ||
8010572e:	83 c4 10             	add    $0x10,%esp
80105731:	85 c0                	test   %eax,%eax
80105733:	78 4b                	js     80105780 <sys_mknod+0x80>
     argint(2, &minor) < 0 ||
80105735:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105738:	83 ec 08             	sub    $0x8,%esp
8010573b:	50                   	push   %eax
8010573c:	6a 02                	push   $0x2
8010573e:	e8 5d f5 ff ff       	call   80104ca0 <argint>
     argint(1, &major) < 0 ||
80105743:	83 c4 10             	add    $0x10,%esp
80105746:	85 c0                	test   %eax,%eax
80105748:	78 36                	js     80105780 <sys_mknod+0x80>
     (ip = create(path, T_DEV, major, minor)) == 0){
8010574a:	0f bf 45 f4          	movswl -0xc(%ebp),%eax
     argint(2, &minor) < 0 ||
8010574e:	50                   	push   %eax
     (ip = create(path, T_DEV, major, minor)) == 0){
8010574f:	0f bf 45 f0          	movswl -0x10(%ebp),%eax
     argint(2, &minor) < 0 ||
80105753:	50                   	push   %eax
80105754:	6a 03                	push   $0x3
80105756:	ff 75 ec             	pushl  -0x14(%ebp)
80105759:	e8 32 fc ff ff       	call   80105390 <create>
8010575e:	83 c4 10             	add    $0x10,%esp
80105761:	85 c0                	test   %eax,%eax
80105763:	74 1b                	je     80105780 <sys_mknod+0x80>
    end_op();
    return -1;
  }
  iunlockput(ip);
80105765:	83 ec 0c             	sub    $0xc,%esp
80105768:	50                   	push   %eax
80105769:	e8 02 c2 ff ff       	call   80101970 <iunlockput>
  end_op();
8010576e:	e8 8d d8 ff ff       	call   80103000 <end_op>
  return 0;
80105773:	83 c4 10             	add    $0x10,%esp
80105776:	31 c0                	xor    %eax,%eax
}
80105778:	c9                   	leave  
80105779:	c3                   	ret    
8010577a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    end_op();
80105780:	e8 7b d8 ff ff       	call   80103000 <end_op>
    return -1;
80105785:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010578a:	c9                   	leave  
8010578b:	c3                   	ret    
8010578c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105790 <sys_chdir>:

int
sys_chdir(void)
{
80105790:	55                   	push   %ebp
80105791:	89 e5                	mov    %esp,%ebp
80105793:	56                   	push   %esi
80105794:	53                   	push   %ebx
80105795:	83 ec 10             	sub    $0x10,%esp
  char *path;
  struct inode *ip;
  struct proc *curproc = myproc();
80105798:	e8 63 e4 ff ff       	call   80103c00 <myproc>
8010579d:	89 c6                	mov    %eax,%esi
  
  begin_op();
8010579f:	e8 ec d7 ff ff       	call   80102f90 <begin_op>
  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0){
801057a4:	8d 45 f4             	lea    -0xc(%ebp),%eax
801057a7:	83 ec 08             	sub    $0x8,%esp
801057aa:	50                   	push   %eax
801057ab:	6a 00                	push   $0x0
801057ad:	e8 9e f5 ff ff       	call   80104d50 <argstr>
801057b2:	83 c4 10             	add    $0x10,%esp
801057b5:	85 c0                	test   %eax,%eax
801057b7:	78 77                	js     80105830 <sys_chdir+0xa0>
801057b9:	83 ec 0c             	sub    $0xc,%esp
801057bc:	ff 75 f4             	pushl  -0xc(%ebp)
801057bf:	e8 7c c7 ff ff       	call   80101f40 <namei>
801057c4:	83 c4 10             	add    $0x10,%esp
801057c7:	85 c0                	test   %eax,%eax
801057c9:	89 c3                	mov    %eax,%ebx
801057cb:	74 63                	je     80105830 <sys_chdir+0xa0>
    end_op();
    return -1;
  }
  ilock(ip);
801057cd:	83 ec 0c             	sub    $0xc,%esp
801057d0:	50                   	push   %eax
801057d1:	e8 0a bf ff ff       	call   801016e0 <ilock>
  if(ip->type != T_DIR){
801057d6:	83 c4 10             	add    $0x10,%esp
801057d9:	66 83 7b 50 01       	cmpw   $0x1,0x50(%ebx)
801057de:	75 30                	jne    80105810 <sys_chdir+0x80>
    iunlockput(ip);
    end_op();
    return -1;
  }
  iunlock(ip);
801057e0:	83 ec 0c             	sub    $0xc,%esp
801057e3:	53                   	push   %ebx
801057e4:	e8 d7 bf ff ff       	call   801017c0 <iunlock>
  iput(curproc->cwd);
801057e9:	58                   	pop    %eax
801057ea:	ff 76 68             	pushl  0x68(%esi)
801057ed:	e8 1e c0 ff ff       	call   80101810 <iput>
  end_op();
801057f2:	e8 09 d8 ff ff       	call   80103000 <end_op>
  curproc->cwd = ip;
801057f7:	89 5e 68             	mov    %ebx,0x68(%esi)
  return 0;
801057fa:	83 c4 10             	add    $0x10,%esp
801057fd:	31 c0                	xor    %eax,%eax
}
801057ff:	8d 65 f8             	lea    -0x8(%ebp),%esp
80105802:	5b                   	pop    %ebx
80105803:	5e                   	pop    %esi
80105804:	5d                   	pop    %ebp
80105805:	c3                   	ret    
80105806:	8d 76 00             	lea    0x0(%esi),%esi
80105809:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    iunlockput(ip);
80105810:	83 ec 0c             	sub    $0xc,%esp
80105813:	53                   	push   %ebx
80105814:	e8 57 c1 ff ff       	call   80101970 <iunlockput>
    end_op();
80105819:	e8 e2 d7 ff ff       	call   80103000 <end_op>
    return -1;
8010581e:	83 c4 10             	add    $0x10,%esp
80105821:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105826:	eb d7                	jmp    801057ff <sys_chdir+0x6f>
80105828:	90                   	nop
80105829:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    end_op();
80105830:	e8 cb d7 ff ff       	call   80103000 <end_op>
    return -1;
80105835:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010583a:	eb c3                	jmp    801057ff <sys_chdir+0x6f>
8010583c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105840 <sys_exec>:

int
sys_exec(void)
{
80105840:	55                   	push   %ebp
80105841:	89 e5                	mov    %esp,%ebp
80105843:	57                   	push   %edi
80105844:	56                   	push   %esi
80105845:	53                   	push   %ebx
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80105846:	8d 85 5c ff ff ff    	lea    -0xa4(%ebp),%eax
{
8010584c:	81 ec a4 00 00 00    	sub    $0xa4,%esp
  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80105852:	50                   	push   %eax
80105853:	6a 00                	push   $0x0
80105855:	e8 f6 f4 ff ff       	call   80104d50 <argstr>
8010585a:	83 c4 10             	add    $0x10,%esp
8010585d:	85 c0                	test   %eax,%eax
8010585f:	0f 88 87 00 00 00    	js     801058ec <sys_exec+0xac>
80105865:	8d 85 60 ff ff ff    	lea    -0xa0(%ebp),%eax
8010586b:	83 ec 08             	sub    $0x8,%esp
8010586e:	50                   	push   %eax
8010586f:	6a 01                	push   $0x1
80105871:	e8 2a f4 ff ff       	call   80104ca0 <argint>
80105876:	83 c4 10             	add    $0x10,%esp
80105879:	85 c0                	test   %eax,%eax
8010587b:	78 6f                	js     801058ec <sys_exec+0xac>
    return -1;
  }
  memset(argv, 0, sizeof(argv));
8010587d:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
80105883:	83 ec 04             	sub    $0x4,%esp
  for(i=0;; i++){
80105886:	31 db                	xor    %ebx,%ebx
  memset(argv, 0, sizeof(argv));
80105888:	68 80 00 00 00       	push   $0x80
8010588d:	6a 00                	push   $0x0
8010588f:	8d bd 64 ff ff ff    	lea    -0x9c(%ebp),%edi
80105895:	50                   	push   %eax
80105896:	e8 05 f1 ff ff       	call   801049a0 <memset>
8010589b:	83 c4 10             	add    $0x10,%esp
8010589e:	eb 2c                	jmp    801058cc <sys_exec+0x8c>
    if(i >= NELEM(argv))
      return -1;
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
      return -1;
    if(uarg == 0){
801058a0:	8b 85 64 ff ff ff    	mov    -0x9c(%ebp),%eax
801058a6:	85 c0                	test   %eax,%eax
801058a8:	74 56                	je     80105900 <sys_exec+0xc0>
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
801058aa:	8d 8d 68 ff ff ff    	lea    -0x98(%ebp),%ecx
801058b0:	83 ec 08             	sub    $0x8,%esp
801058b3:	8d 14 31             	lea    (%ecx,%esi,1),%edx
801058b6:	52                   	push   %edx
801058b7:	50                   	push   %eax
801058b8:	e8 73 f3 ff ff       	call   80104c30 <fetchstr>
801058bd:	83 c4 10             	add    $0x10,%esp
801058c0:	85 c0                	test   %eax,%eax
801058c2:	78 28                	js     801058ec <sys_exec+0xac>
  for(i=0;; i++){
801058c4:	83 c3 01             	add    $0x1,%ebx
    if(i >= NELEM(argv))
801058c7:	83 fb 20             	cmp    $0x20,%ebx
801058ca:	74 20                	je     801058ec <sys_exec+0xac>
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
801058cc:	8b 85 60 ff ff ff    	mov    -0xa0(%ebp),%eax
801058d2:	8d 34 9d 00 00 00 00 	lea    0x0(,%ebx,4),%esi
801058d9:	83 ec 08             	sub    $0x8,%esp
801058dc:	57                   	push   %edi
801058dd:	01 f0                	add    %esi,%eax
801058df:	50                   	push   %eax
801058e0:	e8 0b f3 ff ff       	call   80104bf0 <fetchint>
801058e5:	83 c4 10             	add    $0x10,%esp
801058e8:	85 c0                	test   %eax,%eax
801058ea:	79 b4                	jns    801058a0 <sys_exec+0x60>
      return -1;
  }
  return exec(path, argv);
}
801058ec:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return -1;
801058ef:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801058f4:	5b                   	pop    %ebx
801058f5:	5e                   	pop    %esi
801058f6:	5f                   	pop    %edi
801058f7:	5d                   	pop    %ebp
801058f8:	c3                   	ret    
801058f9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  return exec(path, argv);
80105900:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
80105906:	83 ec 08             	sub    $0x8,%esp
      argv[i] = 0;
80105909:	c7 84 9d 68 ff ff ff 	movl   $0x0,-0x98(%ebp,%ebx,4)
80105910:	00 00 00 00 
  return exec(path, argv);
80105914:	50                   	push   %eax
80105915:	ff b5 5c ff ff ff    	pushl  -0xa4(%ebp)
8010591b:	e8 f0 b0 ff ff       	call   80100a10 <exec>
80105920:	83 c4 10             	add    $0x10,%esp
}
80105923:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105926:	5b                   	pop    %ebx
80105927:	5e                   	pop    %esi
80105928:	5f                   	pop    %edi
80105929:	5d                   	pop    %ebp
8010592a:	c3                   	ret    
8010592b:	90                   	nop
8010592c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80105930 <sys_pipe>:

int
sys_pipe(void)
{
80105930:	55                   	push   %ebp
80105931:	89 e5                	mov    %esp,%ebp
80105933:	57                   	push   %edi
80105934:	56                   	push   %esi
80105935:	53                   	push   %ebx
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
80105936:	8d 45 dc             	lea    -0x24(%ebp),%eax
{
80105939:	83 ec 20             	sub    $0x20,%esp
  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
8010593c:	6a 08                	push   $0x8
8010593e:	50                   	push   %eax
8010593f:	6a 00                	push   $0x0
80105941:	e8 aa f3 ff ff       	call   80104cf0 <argptr>
80105946:	83 c4 10             	add    $0x10,%esp
80105949:	85 c0                	test   %eax,%eax
8010594b:	0f 88 ae 00 00 00    	js     801059ff <sys_pipe+0xcf>
    return -1;
  if(pipealloc(&rf, &wf) < 0)
80105951:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105954:	83 ec 08             	sub    $0x8,%esp
80105957:	50                   	push   %eax
80105958:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010595b:	50                   	push   %eax
8010595c:	e8 cf dc ff ff       	call   80103630 <pipealloc>
80105961:	83 c4 10             	add    $0x10,%esp
80105964:	85 c0                	test   %eax,%eax
80105966:	0f 88 93 00 00 00    	js     801059ff <sys_pipe+0xcf>
    return -1;
  fd0 = -1;
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
8010596c:	8b 7d e0             	mov    -0x20(%ebp),%edi
  for(fd = 0; fd < NOFILE; fd++){
8010596f:	31 db                	xor    %ebx,%ebx
  struct proc *curproc = myproc();
80105971:	e8 8a e2 ff ff       	call   80103c00 <myproc>
80105976:	eb 10                	jmp    80105988 <sys_pipe+0x58>
80105978:	90                   	nop
80105979:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  for(fd = 0; fd < NOFILE; fd++){
80105980:	83 c3 01             	add    $0x1,%ebx
80105983:	83 fb 10             	cmp    $0x10,%ebx
80105986:	74 60                	je     801059e8 <sys_pipe+0xb8>
    if(curproc->ofile[fd] == 0){
80105988:	8b 74 98 28          	mov    0x28(%eax,%ebx,4),%esi
8010598c:	85 f6                	test   %esi,%esi
8010598e:	75 f0                	jne    80105980 <sys_pipe+0x50>
      curproc->ofile[fd] = f;
80105990:	8d 73 08             	lea    0x8(%ebx),%esi
80105993:	89 7c b0 08          	mov    %edi,0x8(%eax,%esi,4)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80105997:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  struct proc *curproc = myproc();
8010599a:	e8 61 e2 ff ff       	call   80103c00 <myproc>
  for(fd = 0; fd < NOFILE; fd++){
8010599f:	31 d2                	xor    %edx,%edx
801059a1:	eb 0d                	jmp    801059b0 <sys_pipe+0x80>
801059a3:	90                   	nop
801059a4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
801059a8:	83 c2 01             	add    $0x1,%edx
801059ab:	83 fa 10             	cmp    $0x10,%edx
801059ae:	74 28                	je     801059d8 <sys_pipe+0xa8>
    if(curproc->ofile[fd] == 0){
801059b0:	8b 4c 90 28          	mov    0x28(%eax,%edx,4),%ecx
801059b4:	85 c9                	test   %ecx,%ecx
801059b6:	75 f0                	jne    801059a8 <sys_pipe+0x78>
      curproc->ofile[fd] = f;
801059b8:	89 7c 90 28          	mov    %edi,0x28(%eax,%edx,4)
      myproc()->ofile[fd0] = 0;
    fileclose(rf);
    fileclose(wf);
    return -1;
  }
  fd[0] = fd0;
801059bc:	8b 45 dc             	mov    -0x24(%ebp),%eax
801059bf:	89 18                	mov    %ebx,(%eax)
  fd[1] = fd1;
801059c1:	8b 45 dc             	mov    -0x24(%ebp),%eax
801059c4:	89 50 04             	mov    %edx,0x4(%eax)
  return 0;
801059c7:	31 c0                	xor    %eax,%eax
}
801059c9:	8d 65 f4             	lea    -0xc(%ebp),%esp
801059cc:	5b                   	pop    %ebx
801059cd:	5e                   	pop    %esi
801059ce:	5f                   	pop    %edi
801059cf:	5d                   	pop    %ebp
801059d0:	c3                   	ret    
801059d1:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      myproc()->ofile[fd0] = 0;
801059d8:	e8 23 e2 ff ff       	call   80103c00 <myproc>
801059dd:	c7 44 b0 08 00 00 00 	movl   $0x0,0x8(%eax,%esi,4)
801059e4:	00 
801059e5:	8d 76 00             	lea    0x0(%esi),%esi
    fileclose(rf);
801059e8:	83 ec 0c             	sub    $0xc,%esp
801059eb:	ff 75 e0             	pushl  -0x20(%ebp)
801059ee:	e8 9d b4 ff ff       	call   80100e90 <fileclose>
    fileclose(wf);
801059f3:	58                   	pop    %eax
801059f4:	ff 75 e4             	pushl  -0x1c(%ebp)
801059f7:	e8 94 b4 ff ff       	call   80100e90 <fileclose>
    return -1;
801059fc:	83 c4 10             	add    $0x10,%esp
801059ff:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a04:	eb c3                	jmp    801059c9 <sys_pipe+0x99>
80105a06:	66 90                	xchg   %ax,%ax
80105a08:	66 90                	xchg   %ax,%ax
80105a0a:	66 90                	xchg   %ax,%ax
80105a0c:	66 90                	xchg   %ax,%ax
80105a0e:	66 90                	xchg   %ax,%ax

80105a10 <sys_yield>:
#include "mmu.h"
#include "proc.h"


int sys_yield(void)
{
80105a10:	55                   	push   %ebp
80105a11:	89 e5                	mov    %esp,%ebp
80105a13:	83 ec 08             	sub    $0x8,%esp
  yield(); 
80105a16:	e8 05 e8 ff ff       	call   80104220 <yield>
  return 0;
}
80105a1b:	31 c0                	xor    %eax,%eax
80105a1d:	c9                   	leave  
80105a1e:	c3                   	ret    
80105a1f:	90                   	nop

80105a20 <sys_fork>:

int
sys_fork(void)
{
80105a20:	55                   	push   %ebp
80105a21:	89 e5                	mov    %esp,%ebp
  return fork();
}
80105a23:	5d                   	pop    %ebp
  return fork();
80105a24:	e9 c7 e3 ff ff       	jmp    80103df0 <fork>
80105a29:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105a30 <sys_exit>:

int
sys_exit(void)
{
80105a30:	55                   	push   %ebp
80105a31:	89 e5                	mov    %esp,%ebp
80105a33:	83 ec 08             	sub    $0x8,%esp
  exit();
80105a36:	e8 a5 e6 ff ff       	call   801040e0 <exit>
  return 0;  // not reached
}
80105a3b:	31 c0                	xor    %eax,%eax
80105a3d:	c9                   	leave  
80105a3e:	c3                   	ret    
80105a3f:	90                   	nop

80105a40 <sys_wait>:

int
sys_wait(void)
{
80105a40:	55                   	push   %ebp
80105a41:	89 e5                	mov    %esp,%ebp
  return wait();
}
80105a43:	5d                   	pop    %ebp
  return wait();
80105a44:	e9 e7 e8 ff ff       	jmp    80104330 <wait>
80105a49:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105a50 <sys_kill>:

int
sys_kill(void)
{
80105a50:	55                   	push   %ebp
80105a51:	89 e5                	mov    %esp,%ebp
80105a53:	83 ec 20             	sub    $0x20,%esp
  int pid;

  if(argint(0, &pid) < 0)
80105a56:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105a59:	50                   	push   %eax
80105a5a:	6a 00                	push   $0x0
80105a5c:	e8 3f f2 ff ff       	call   80104ca0 <argint>
80105a61:	83 c4 10             	add    $0x10,%esp
80105a64:	85 c0                	test   %eax,%eax
80105a66:	78 18                	js     80105a80 <sys_kill+0x30>
    return -1;
  return kill(pid);
80105a68:	83 ec 0c             	sub    $0xc,%esp
80105a6b:	ff 75 f4             	pushl  -0xc(%ebp)
80105a6e:	e8 3d ea ff ff       	call   801044b0 <kill>
80105a73:	83 c4 10             	add    $0x10,%esp
}
80105a76:	c9                   	leave  
80105a77:	c3                   	ret    
80105a78:	90                   	nop
80105a79:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80105a80:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105a85:	c9                   	leave  
80105a86:	c3                   	ret    
80105a87:	89 f6                	mov    %esi,%esi
80105a89:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80105a90 <sys_getpid>:

int
sys_getpid(void)
{
80105a90:	55                   	push   %ebp
80105a91:	89 e5                	mov    %esp,%ebp
80105a93:	83 ec 08             	sub    $0x8,%esp
  return myproc()->pid;
80105a96:	e8 65 e1 ff ff       	call   80103c00 <myproc>
80105a9b:	8b 40 10             	mov    0x10(%eax),%eax
}
80105a9e:	c9                   	leave  
80105a9f:	c3                   	ret    

80105aa0 <sys_sbrk>:

int
sys_sbrk(void)
{
80105aa0:	55                   	push   %ebp
80105aa1:	89 e5                	mov    %esp,%ebp
80105aa3:	53                   	push   %ebx
  int addr;
  int n;

  if(argint(0, &n) < 0)
80105aa4:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
80105aa7:	83 ec 1c             	sub    $0x1c,%esp
  if(argint(0, &n) < 0)
80105aaa:	50                   	push   %eax
80105aab:	6a 00                	push   $0x0
80105aad:	e8 ee f1 ff ff       	call   80104ca0 <argint>
80105ab2:	83 c4 10             	add    $0x10,%esp
80105ab5:	85 c0                	test   %eax,%eax
80105ab7:	78 27                	js     80105ae0 <sys_sbrk+0x40>
    return -1;
  addr = myproc()->sz;
80105ab9:	e8 42 e1 ff ff       	call   80103c00 <myproc>
  if(growproc(n) < 0)
80105abe:	83 ec 0c             	sub    $0xc,%esp
  addr = myproc()->sz;
80105ac1:	8b 18                	mov    (%eax),%ebx
  if(growproc(n) < 0)
80105ac3:	ff 75 f4             	pushl  -0xc(%ebp)
80105ac6:	e8 75 e2 ff ff       	call   80103d40 <growproc>
80105acb:	83 c4 10             	add    $0x10,%esp
80105ace:	85 c0                	test   %eax,%eax
80105ad0:	78 0e                	js     80105ae0 <sys_sbrk+0x40>
    return -1;
  return addr;
}
80105ad2:	89 d8                	mov    %ebx,%eax
80105ad4:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105ad7:	c9                   	leave  
80105ad8:	c3                   	ret    
80105ad9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80105ae0:	bb ff ff ff ff       	mov    $0xffffffff,%ebx
80105ae5:	eb eb                	jmp    80105ad2 <sys_sbrk+0x32>
80105ae7:	89 f6                	mov    %esi,%esi
80105ae9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80105af0 <sys_sleep>:

int
sys_sleep(void)
{
80105af0:	55                   	push   %ebp
80105af1:	89 e5                	mov    %esp,%ebp
80105af3:	53                   	push   %ebx
  int n;
  uint ticks0;

  if(argint(0, &n) < 0)
80105af4:	8d 45 f4             	lea    -0xc(%ebp),%eax
{
80105af7:	83 ec 1c             	sub    $0x1c,%esp
  if(argint(0, &n) < 0)
80105afa:	50                   	push   %eax
80105afb:	6a 00                	push   $0x0
80105afd:	e8 9e f1 ff ff       	call   80104ca0 <argint>
80105b02:	83 c4 10             	add    $0x10,%esp
80105b05:	85 c0                	test   %eax,%eax
80105b07:	0f 88 8a 00 00 00    	js     80105b97 <sys_sleep+0xa7>
    return -1;
  acquire(&tickslock);
80105b0d:	83 ec 0c             	sub    $0xc,%esp
80105b10:	68 60 5d 11 80       	push   $0x80115d60
80105b15:	e8 06 ed ff ff       	call   80104820 <acquire>
  ticks0 = ticks;
  while(ticks - ticks0 < n){
80105b1a:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105b1d:	83 c4 10             	add    $0x10,%esp
  ticks0 = ticks;
80105b20:	8b 1d a0 65 11 80    	mov    0x801165a0,%ebx
  while(ticks - ticks0 < n){
80105b26:	85 d2                	test   %edx,%edx
80105b28:	75 27                	jne    80105b51 <sys_sleep+0x61>
80105b2a:	eb 54                	jmp    80105b80 <sys_sleep+0x90>
80105b2c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    if(myproc()->killed){
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
80105b30:	83 ec 08             	sub    $0x8,%esp
80105b33:	68 60 5d 11 80       	push   $0x80115d60
80105b38:	68 a0 65 11 80       	push   $0x801165a0
80105b3d:	e8 2e e7 ff ff       	call   80104270 <sleep>
  while(ticks - ticks0 < n){
80105b42:	a1 a0 65 11 80       	mov    0x801165a0,%eax
80105b47:	83 c4 10             	add    $0x10,%esp
80105b4a:	29 d8                	sub    %ebx,%eax
80105b4c:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80105b4f:	73 2f                	jae    80105b80 <sys_sleep+0x90>
    if(myproc()->killed){
80105b51:	e8 aa e0 ff ff       	call   80103c00 <myproc>
80105b56:	8b 40 24             	mov    0x24(%eax),%eax
80105b59:	85 c0                	test   %eax,%eax
80105b5b:	74 d3                	je     80105b30 <sys_sleep+0x40>
      release(&tickslock);
80105b5d:	83 ec 0c             	sub    $0xc,%esp
80105b60:	68 60 5d 11 80       	push   $0x80115d60
80105b65:	e8 d6 ed ff ff       	call   80104940 <release>
      return -1;
80105b6a:	83 c4 10             	add    $0x10,%esp
80105b6d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
  }
  release(&tickslock);
  return 0;
}
80105b72:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105b75:	c9                   	leave  
80105b76:	c3                   	ret    
80105b77:	89 f6                	mov    %esi,%esi
80105b79:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
  release(&tickslock);
80105b80:	83 ec 0c             	sub    $0xc,%esp
80105b83:	68 60 5d 11 80       	push   $0x80115d60
80105b88:	e8 b3 ed ff ff       	call   80104940 <release>
  return 0;
80105b8d:	83 c4 10             	add    $0x10,%esp
80105b90:	31 c0                	xor    %eax,%eax
}
80105b92:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105b95:	c9                   	leave  
80105b96:	c3                   	ret    
    return -1;
80105b97:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105b9c:	eb f4                	jmp    80105b92 <sys_sleep+0xa2>
80105b9e:	66 90                	xchg   %ax,%ax

80105ba0 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
80105ba0:	55                   	push   %ebp
80105ba1:	89 e5                	mov    %esp,%ebp
80105ba3:	53                   	push   %ebx
80105ba4:	83 ec 10             	sub    $0x10,%esp
  uint xticks;

  acquire(&tickslock);
80105ba7:	68 60 5d 11 80       	push   $0x80115d60
80105bac:	e8 6f ec ff ff       	call   80104820 <acquire>
  xticks = ticks;
80105bb1:	8b 1d a0 65 11 80    	mov    0x801165a0,%ebx
  release(&tickslock);
80105bb7:	c7 04 24 60 5d 11 80 	movl   $0x80115d60,(%esp)
80105bbe:	e8 7d ed ff ff       	call   80104940 <release>
  return xticks;
}
80105bc3:	89 d8                	mov    %ebx,%eax
80105bc5:	8b 5d fc             	mov    -0x4(%ebp),%ebx
80105bc8:	c9                   	leave  
80105bc9:	c3                   	ret    

80105bca <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
80105bca:	1e                   	push   %ds
  pushl %es
80105bcb:	06                   	push   %es
  pushl %fs
80105bcc:	0f a0                	push   %fs
  pushl %gs
80105bce:	0f a8                	push   %gs
  pushal
80105bd0:	60                   	pusha  
  
  # Set up data segments.
  movw $(SEG_KDATA<<3), %ax
80105bd1:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
80105bd5:	8e d8                	mov    %eax,%ds
  movw %ax, %es
80105bd7:	8e c0                	mov    %eax,%es

  # Call trap(tf), where tf=%esp
  pushl %esp
80105bd9:	54                   	push   %esp
  call trap
80105bda:	e8 c1 00 00 00       	call   80105ca0 <trap>
  addl $4, %esp
80105bdf:	83 c4 04             	add    $0x4,%esp

80105be2 <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
80105be2:	61                   	popa   
  popl %gs
80105be3:	0f a9                	pop    %gs
  popl %fs
80105be5:	0f a1                	pop    %fs
  popl %es
80105be7:	07                   	pop    %es
  popl %ds
80105be8:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
80105be9:	83 c4 08             	add    $0x8,%esp
  iret
80105bec:	cf                   	iret   
80105bed:	66 90                	xchg   %ax,%ax
80105bef:	90                   	nop

80105bf0 <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
80105bf0:	55                   	push   %ebp
  int i;

  for(i = 0; i < 256; i++)
80105bf1:	31 c0                	xor    %eax,%eax
{
80105bf3:	89 e5                	mov    %esp,%ebp
80105bf5:	83 ec 08             	sub    $0x8,%esp
80105bf8:	90                   	nop
80105bf9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
80105c00:	8b 14 85 0c a0 10 80 	mov    -0x7fef5ff4(,%eax,4),%edx
80105c07:	c7 04 c5 a2 5d 11 80 	movl   $0x8e000008,-0x7feea25e(,%eax,8)
80105c0e:	08 00 00 8e 
80105c12:	66 89 14 c5 a0 5d 11 	mov    %dx,-0x7feea260(,%eax,8)
80105c19:	80 
80105c1a:	c1 ea 10             	shr    $0x10,%edx
80105c1d:	66 89 14 c5 a6 5d 11 	mov    %dx,-0x7feea25a(,%eax,8)
80105c24:	80 
  for(i = 0; i < 256; i++)
80105c25:	83 c0 01             	add    $0x1,%eax
80105c28:	3d 00 01 00 00       	cmp    $0x100,%eax
80105c2d:	75 d1                	jne    80105c00 <tvinit+0x10>
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80105c2f:	a1 0c a1 10 80       	mov    0x8010a10c,%eax

  initlock(&tickslock, "time");
80105c34:	83 ec 08             	sub    $0x8,%esp
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80105c37:	c7 05 a2 5f 11 80 08 	movl   $0xef000008,0x80115fa2
80105c3e:	00 00 ef 
  initlock(&tickslock, "time");
80105c41:	68 39 7d 10 80       	push   $0x80107d39
80105c46:	68 60 5d 11 80       	push   $0x80115d60
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80105c4b:	66 a3 a0 5f 11 80    	mov    %ax,0x80115fa0
80105c51:	c1 e8 10             	shr    $0x10,%eax
80105c54:	66 a3 a6 5f 11 80    	mov    %ax,0x80115fa6
  initlock(&tickslock, "time");
80105c5a:	e8 d1 ea ff ff       	call   80104730 <initlock>
}
80105c5f:	83 c4 10             	add    $0x10,%esp
80105c62:	c9                   	leave  
80105c63:	c3                   	ret    
80105c64:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80105c6a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80105c70 <idtinit>:

void
idtinit(void)
{
80105c70:	55                   	push   %ebp
  pd[0] = size-1;
80105c71:	b8 ff 07 00 00       	mov    $0x7ff,%eax
80105c76:	89 e5                	mov    %esp,%ebp
80105c78:	83 ec 10             	sub    $0x10,%esp
80105c7b:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80105c7f:	b8 a0 5d 11 80       	mov    $0x80115da0,%eax
80105c84:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80105c88:	c1 e8 10             	shr    $0x10,%eax
80105c8b:	66 89 45 fe          	mov    %ax,-0x2(%ebp)
  asm volatile("lidt (%0)" : : "r" (pd));
80105c8f:	8d 45 fa             	lea    -0x6(%ebp),%eax
80105c92:	0f 01 18             	lidtl  (%eax)
  lidt(idt, sizeof(idt));
}
80105c95:	c9                   	leave  
80105c96:	c3                   	ret    
80105c97:	89 f6                	mov    %esi,%esi
80105c99:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80105ca0 <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
80105ca0:	55                   	push   %ebp
80105ca1:	89 e5                	mov    %esp,%ebp
80105ca3:	57                   	push   %edi
80105ca4:	56                   	push   %esi
80105ca5:	53                   	push   %ebx
80105ca6:	83 ec 1c             	sub    $0x1c,%esp
80105ca9:	8b 7d 08             	mov    0x8(%ebp),%edi
  if(tf->trapno == T_SYSCALL){
80105cac:	8b 47 30             	mov    0x30(%edi),%eax
80105caf:	83 f8 40             	cmp    $0x40,%eax
80105cb2:	0f 84 f0 00 00 00    	je     80105da8 <trap+0x108>
    if(myproc()->killed)
      exit();
    return;
  }

  switch(tf->trapno){
80105cb8:	83 e8 20             	sub    $0x20,%eax
80105cbb:	83 f8 1f             	cmp    $0x1f,%eax
80105cbe:	77 10                	ja     80105cd0 <trap+0x30>
80105cc0:	ff 24 85 e0 7d 10 80 	jmp    *-0x7fef8220(,%eax,4)
80105cc7:	89 f6                	mov    %esi,%esi
80105cc9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

    //TODO CASE TRAP 14 PGFLT IF IN SWITCH FILE: BRING FROM THERE, ELSE GO DEFAULT

  //PAGEBREAK: 13
  default:
    if(myproc() == 0 || (tf->cs&3) == 0){
80105cd0:	e8 2b df ff ff       	call   80103c00 <myproc>
80105cd5:	85 c0                	test   %eax,%eax
80105cd7:	8b 5f 38             	mov    0x38(%edi),%ebx
80105cda:	0f 84 14 02 00 00    	je     80105ef4 <trap+0x254>
80105ce0:	f6 47 3c 03          	testb  $0x3,0x3c(%edi)
80105ce4:	0f 84 0a 02 00 00    	je     80105ef4 <trap+0x254>

static inline uint
rcr2(void)
{
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
80105cea:	0f 20 d1             	mov    %cr2,%ecx
80105ced:	89 4d d8             	mov    %ecx,-0x28(%ebp)
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpuid(), tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80105cf0:	e8 eb de ff ff       	call   80103be0 <cpuid>
80105cf5:	89 45 dc             	mov    %eax,-0x24(%ebp)
80105cf8:	8b 47 34             	mov    0x34(%edi),%eax
80105cfb:	8b 77 30             	mov    0x30(%edi),%esi
80105cfe:	89 45 e4             	mov    %eax,-0x1c(%ebp)
            "eip 0x%x addr 0x%x--kill proc\n",
            myproc()->pid, myproc()->name, tf->trapno,
80105d01:	e8 fa de ff ff       	call   80103c00 <myproc>
80105d06:	89 45 e0             	mov    %eax,-0x20(%ebp)
80105d09:	e8 f2 de ff ff       	call   80103c00 <myproc>
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80105d0e:	8b 4d d8             	mov    -0x28(%ebp),%ecx
80105d11:	8b 55 dc             	mov    -0x24(%ebp),%edx
80105d14:	51                   	push   %ecx
80105d15:	53                   	push   %ebx
80105d16:	52                   	push   %edx
            myproc()->pid, myproc()->name, tf->trapno,
80105d17:	8b 55 e0             	mov    -0x20(%ebp),%edx
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80105d1a:	ff 75 e4             	pushl  -0x1c(%ebp)
80105d1d:	56                   	push   %esi
            myproc()->pid, myproc()->name, tf->trapno,
80105d1e:	83 c2 6c             	add    $0x6c,%edx
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80105d21:	52                   	push   %edx
80105d22:	ff 70 10             	pushl  0x10(%eax)
80105d25:	68 9c 7d 10 80       	push   $0x80107d9c
80105d2a:	e8 31 a9 ff ff       	call   80100660 <cprintf>
            tf->err, cpuid(), tf->eip, rcr2());
    myproc()->killed = 1;
80105d2f:	83 c4 20             	add    $0x20,%esp
80105d32:	e8 c9 de ff ff       	call   80103c00 <myproc>
80105d37:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running
  // until it gets to the regular system call return.)
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
80105d3e:	e8 bd de ff ff       	call   80103c00 <myproc>
80105d43:	85 c0                	test   %eax,%eax
80105d45:	74 1d                	je     80105d64 <trap+0xc4>
80105d47:	e8 b4 de ff ff       	call   80103c00 <myproc>
80105d4c:	8b 50 24             	mov    0x24(%eax),%edx
80105d4f:	85 d2                	test   %edx,%edx
80105d51:	74 11                	je     80105d64 <trap+0xc4>
80105d53:	0f b7 47 3c          	movzwl 0x3c(%edi),%eax
80105d57:	83 e0 03             	and    $0x3,%eax
80105d5a:	66 83 f8 03          	cmp    $0x3,%ax
80105d5e:	0f 84 4c 01 00 00    	je     80105eb0 <trap+0x210>
    exit();

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(myproc() && myproc()->state == RUNNING &&
80105d64:	e8 97 de ff ff       	call   80103c00 <myproc>
80105d69:	85 c0                	test   %eax,%eax
80105d6b:	74 0b                	je     80105d78 <trap+0xd8>
80105d6d:	e8 8e de ff ff       	call   80103c00 <myproc>
80105d72:	83 78 0c 04          	cmpl   $0x4,0xc(%eax)
80105d76:	74 68                	je     80105de0 <trap+0x140>
     tf->trapno == T_IRQ0+IRQ_TIMER)
    yield();

  // Check if the process has been killed since we yielded
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
80105d78:	e8 83 de ff ff       	call   80103c00 <myproc>
80105d7d:	85 c0                	test   %eax,%eax
80105d7f:	74 19                	je     80105d9a <trap+0xfa>
80105d81:	e8 7a de ff ff       	call   80103c00 <myproc>
80105d86:	8b 40 24             	mov    0x24(%eax),%eax
80105d89:	85 c0                	test   %eax,%eax
80105d8b:	74 0d                	je     80105d9a <trap+0xfa>
80105d8d:	0f b7 47 3c          	movzwl 0x3c(%edi),%eax
80105d91:	83 e0 03             	and    $0x3,%eax
80105d94:	66 83 f8 03          	cmp    $0x3,%ax
80105d98:	74 37                	je     80105dd1 <trap+0x131>
    exit();
}
80105d9a:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105d9d:	5b                   	pop    %ebx
80105d9e:	5e                   	pop    %esi
80105d9f:	5f                   	pop    %edi
80105da0:	5d                   	pop    %ebp
80105da1:	c3                   	ret    
80105da2:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    if(myproc()->killed)
80105da8:	e8 53 de ff ff       	call   80103c00 <myproc>
80105dad:	8b 58 24             	mov    0x24(%eax),%ebx
80105db0:	85 db                	test   %ebx,%ebx
80105db2:	0f 85 e8 00 00 00    	jne    80105ea0 <trap+0x200>
    myproc()->tf = tf;
80105db8:	e8 43 de ff ff       	call   80103c00 <myproc>
80105dbd:	89 78 18             	mov    %edi,0x18(%eax)
    syscall();
80105dc0:	e8 cb ef ff ff       	call   80104d90 <syscall>
    if(myproc()->killed)
80105dc5:	e8 36 de ff ff       	call   80103c00 <myproc>
80105dca:	8b 48 24             	mov    0x24(%eax),%ecx
80105dcd:	85 c9                	test   %ecx,%ecx
80105dcf:	74 c9                	je     80105d9a <trap+0xfa>
}
80105dd1:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105dd4:	5b                   	pop    %ebx
80105dd5:	5e                   	pop    %esi
80105dd6:	5f                   	pop    %edi
80105dd7:	5d                   	pop    %ebp
      exit();
80105dd8:	e9 03 e3 ff ff       	jmp    801040e0 <exit>
80105ddd:	8d 76 00             	lea    0x0(%esi),%esi
  if(myproc() && myproc()->state == RUNNING &&
80105de0:	83 7f 30 20          	cmpl   $0x20,0x30(%edi)
80105de4:	75 92                	jne    80105d78 <trap+0xd8>
    yield();
80105de6:	e8 35 e4 ff ff       	call   80104220 <yield>
80105deb:	eb 8b                	jmp    80105d78 <trap+0xd8>
80105ded:	8d 76 00             	lea    0x0(%esi),%esi
    if(cpuid() == 0){
80105df0:	e8 eb dd ff ff       	call   80103be0 <cpuid>
80105df5:	85 c0                	test   %eax,%eax
80105df7:	0f 84 c3 00 00 00    	je     80105ec0 <trap+0x220>
    lapiceoi();
80105dfd:	e8 3e cd ff ff       	call   80102b40 <lapiceoi>
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
80105e02:	e8 f9 dd ff ff       	call   80103c00 <myproc>
80105e07:	85 c0                	test   %eax,%eax
80105e09:	0f 85 38 ff ff ff    	jne    80105d47 <trap+0xa7>
80105e0f:	e9 50 ff ff ff       	jmp    80105d64 <trap+0xc4>
80105e14:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    kbdintr();
80105e18:	e8 e3 cb ff ff       	call   80102a00 <kbdintr>
    lapiceoi();
80105e1d:	e8 1e cd ff ff       	call   80102b40 <lapiceoi>
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
80105e22:	e8 d9 dd ff ff       	call   80103c00 <myproc>
80105e27:	85 c0                	test   %eax,%eax
80105e29:	0f 85 18 ff ff ff    	jne    80105d47 <trap+0xa7>
80105e2f:	e9 30 ff ff ff       	jmp    80105d64 <trap+0xc4>
80105e34:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    uartintr();
80105e38:	e8 53 02 00 00       	call   80106090 <uartintr>
    lapiceoi();
80105e3d:	e8 fe cc ff ff       	call   80102b40 <lapiceoi>
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
80105e42:	e8 b9 dd ff ff       	call   80103c00 <myproc>
80105e47:	85 c0                	test   %eax,%eax
80105e49:	0f 85 f8 fe ff ff    	jne    80105d47 <trap+0xa7>
80105e4f:	e9 10 ff ff ff       	jmp    80105d64 <trap+0xc4>
80105e54:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80105e58:	0f b7 5f 3c          	movzwl 0x3c(%edi),%ebx
80105e5c:	8b 77 38             	mov    0x38(%edi),%esi
80105e5f:	e8 7c dd ff ff       	call   80103be0 <cpuid>
80105e64:	56                   	push   %esi
80105e65:	53                   	push   %ebx
80105e66:	50                   	push   %eax
80105e67:	68 44 7d 10 80       	push   $0x80107d44
80105e6c:	e8 ef a7 ff ff       	call   80100660 <cprintf>
    lapiceoi();
80105e71:	e8 ca cc ff ff       	call   80102b40 <lapiceoi>
    break;
80105e76:	83 c4 10             	add    $0x10,%esp
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
80105e79:	e8 82 dd ff ff       	call   80103c00 <myproc>
80105e7e:	85 c0                	test   %eax,%eax
80105e80:	0f 85 c1 fe ff ff    	jne    80105d47 <trap+0xa7>
80105e86:	e9 d9 fe ff ff       	jmp    80105d64 <trap+0xc4>
80105e8b:	90                   	nop
80105e8c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
    ideintr();
80105e90:	e8 db c5 ff ff       	call   80102470 <ideintr>
80105e95:	e9 63 ff ff ff       	jmp    80105dfd <trap+0x15d>
80105e9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      exit();
80105ea0:	e8 3b e2 ff ff       	call   801040e0 <exit>
80105ea5:	e9 0e ff ff ff       	jmp    80105db8 <trap+0x118>
80105eaa:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    exit();
80105eb0:	e8 2b e2 ff ff       	call   801040e0 <exit>
80105eb5:	e9 aa fe ff ff       	jmp    80105d64 <trap+0xc4>
80105eba:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
      acquire(&tickslock);
80105ec0:	83 ec 0c             	sub    $0xc,%esp
80105ec3:	68 60 5d 11 80       	push   $0x80115d60
80105ec8:	e8 53 e9 ff ff       	call   80104820 <acquire>
      wakeup(&ticks);
80105ecd:	c7 04 24 a0 65 11 80 	movl   $0x801165a0,(%esp)
      ticks++;
80105ed4:	83 05 a0 65 11 80 01 	addl   $0x1,0x801165a0
      wakeup(&ticks);
80105edb:	e8 70 e5 ff ff       	call   80104450 <wakeup>
      release(&tickslock);
80105ee0:	c7 04 24 60 5d 11 80 	movl   $0x80115d60,(%esp)
80105ee7:	e8 54 ea ff ff       	call   80104940 <release>
80105eec:	83 c4 10             	add    $0x10,%esp
80105eef:	e9 09 ff ff ff       	jmp    80105dfd <trap+0x15d>
80105ef4:	0f 20 d6             	mov    %cr2,%esi
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80105ef7:	e8 e4 dc ff ff       	call   80103be0 <cpuid>
80105efc:	83 ec 0c             	sub    $0xc,%esp
80105eff:	56                   	push   %esi
80105f00:	53                   	push   %ebx
80105f01:	50                   	push   %eax
80105f02:	ff 77 30             	pushl  0x30(%edi)
80105f05:	68 68 7d 10 80       	push   $0x80107d68
80105f0a:	e8 51 a7 ff ff       	call   80100660 <cprintf>
      panic("trap");
80105f0f:	83 c4 14             	add    $0x14,%esp
80105f12:	68 3e 7d 10 80       	push   $0x80107d3e
80105f17:	e8 74 a4 ff ff       	call   80100390 <panic>
80105f1c:	66 90                	xchg   %ax,%ax
80105f1e:	66 90                	xchg   %ax,%ax

80105f20 <uartgetc>:
}

static int
uartgetc(void)
{
  if(!uart)
80105f20:	a1 bc a5 10 80       	mov    0x8010a5bc,%eax
{
80105f25:	55                   	push   %ebp
80105f26:	89 e5                	mov    %esp,%ebp
  if(!uart)
80105f28:	85 c0                	test   %eax,%eax
80105f2a:	74 1c                	je     80105f48 <uartgetc+0x28>
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80105f2c:	ba fd 03 00 00       	mov    $0x3fd,%edx
80105f31:	ec                   	in     (%dx),%al
    return -1;
  if(!(inb(COM1+5) & 0x01))
80105f32:	a8 01                	test   $0x1,%al
80105f34:	74 12                	je     80105f48 <uartgetc+0x28>
80105f36:	ba f8 03 00 00       	mov    $0x3f8,%edx
80105f3b:	ec                   	in     (%dx),%al
    return -1;
  return inb(COM1+0);
80105f3c:	0f b6 c0             	movzbl %al,%eax
}
80105f3f:	5d                   	pop    %ebp
80105f40:	c3                   	ret    
80105f41:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    return -1;
80105f48:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105f4d:	5d                   	pop    %ebp
80105f4e:	c3                   	ret    
80105f4f:	90                   	nop

80105f50 <uartputc.part.0>:
uartputc(int c)
80105f50:	55                   	push   %ebp
80105f51:	89 e5                	mov    %esp,%ebp
80105f53:	57                   	push   %edi
80105f54:	56                   	push   %esi
80105f55:	53                   	push   %ebx
80105f56:	89 c7                	mov    %eax,%edi
80105f58:	bb 80 00 00 00       	mov    $0x80,%ebx
80105f5d:	be fd 03 00 00       	mov    $0x3fd,%esi
80105f62:	83 ec 0c             	sub    $0xc,%esp
80105f65:	eb 1b                	jmp    80105f82 <uartputc.part.0+0x32>
80105f67:	89 f6                	mov    %esi,%esi
80105f69:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
    microdelay(10);
80105f70:	83 ec 0c             	sub    $0xc,%esp
80105f73:	6a 0a                	push   $0xa
80105f75:	e8 e6 cb ff ff       	call   80102b60 <microdelay>
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80105f7a:	83 c4 10             	add    $0x10,%esp
80105f7d:	83 eb 01             	sub    $0x1,%ebx
80105f80:	74 07                	je     80105f89 <uartputc.part.0+0x39>
80105f82:	89 f2                	mov    %esi,%edx
80105f84:	ec                   	in     (%dx),%al
80105f85:	a8 20                	test   $0x20,%al
80105f87:	74 e7                	je     80105f70 <uartputc.part.0+0x20>
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80105f89:	ba f8 03 00 00       	mov    $0x3f8,%edx
80105f8e:	89 f8                	mov    %edi,%eax
80105f90:	ee                   	out    %al,(%dx)
}
80105f91:	8d 65 f4             	lea    -0xc(%ebp),%esp
80105f94:	5b                   	pop    %ebx
80105f95:	5e                   	pop    %esi
80105f96:	5f                   	pop    %edi
80105f97:	5d                   	pop    %ebp
80105f98:	c3                   	ret    
80105f99:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80105fa0 <uartinit>:
{
80105fa0:	55                   	push   %ebp
80105fa1:	31 c9                	xor    %ecx,%ecx
80105fa3:	89 c8                	mov    %ecx,%eax
80105fa5:	89 e5                	mov    %esp,%ebp
80105fa7:	57                   	push   %edi
80105fa8:	56                   	push   %esi
80105fa9:	53                   	push   %ebx
80105faa:	bb fa 03 00 00       	mov    $0x3fa,%ebx
80105faf:	89 da                	mov    %ebx,%edx
80105fb1:	83 ec 0c             	sub    $0xc,%esp
80105fb4:	ee                   	out    %al,(%dx)
80105fb5:	bf fb 03 00 00       	mov    $0x3fb,%edi
80105fba:	b8 80 ff ff ff       	mov    $0xffffff80,%eax
80105fbf:	89 fa                	mov    %edi,%edx
80105fc1:	ee                   	out    %al,(%dx)
80105fc2:	b8 0c 00 00 00       	mov    $0xc,%eax
80105fc7:	ba f8 03 00 00       	mov    $0x3f8,%edx
80105fcc:	ee                   	out    %al,(%dx)
80105fcd:	be f9 03 00 00       	mov    $0x3f9,%esi
80105fd2:	89 c8                	mov    %ecx,%eax
80105fd4:	89 f2                	mov    %esi,%edx
80105fd6:	ee                   	out    %al,(%dx)
80105fd7:	b8 03 00 00 00       	mov    $0x3,%eax
80105fdc:	89 fa                	mov    %edi,%edx
80105fde:	ee                   	out    %al,(%dx)
80105fdf:	ba fc 03 00 00       	mov    $0x3fc,%edx
80105fe4:	89 c8                	mov    %ecx,%eax
80105fe6:	ee                   	out    %al,(%dx)
80105fe7:	b8 01 00 00 00       	mov    $0x1,%eax
80105fec:	89 f2                	mov    %esi,%edx
80105fee:	ee                   	out    %al,(%dx)
  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80105fef:	ba fd 03 00 00       	mov    $0x3fd,%edx
80105ff4:	ec                   	in     (%dx),%al
  if(inb(COM1+5) == 0xFF)
80105ff5:	3c ff                	cmp    $0xff,%al
80105ff7:	74 5a                	je     80106053 <uartinit+0xb3>
  uart = 1;
80105ff9:	c7 05 bc a5 10 80 01 	movl   $0x1,0x8010a5bc
80106000:	00 00 00 
80106003:	89 da                	mov    %ebx,%edx
80106005:	ec                   	in     (%dx),%al
80106006:	ba f8 03 00 00       	mov    $0x3f8,%edx
8010600b:	ec                   	in     (%dx),%al
  ioapicenable(IRQ_COM1, 0);
8010600c:	83 ec 08             	sub    $0x8,%esp
  for(p="xv6...\n"; *p; p++)
8010600f:	bb 60 7e 10 80       	mov    $0x80107e60,%ebx
  ioapicenable(IRQ_COM1, 0);
80106014:	6a 00                	push   $0x0
80106016:	6a 04                	push   $0x4
80106018:	e8 a3 c6 ff ff       	call   801026c0 <ioapicenable>
8010601d:	83 c4 10             	add    $0x10,%esp
  for(p="xv6...\n"; *p; p++)
80106020:	b8 78 00 00 00       	mov    $0x78,%eax
80106025:	eb 13                	jmp    8010603a <uartinit+0x9a>
80106027:	89 f6                	mov    %esi,%esi
80106029:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
80106030:	83 c3 01             	add    $0x1,%ebx
80106033:	0f be 03             	movsbl (%ebx),%eax
80106036:	84 c0                	test   %al,%al
80106038:	74 19                	je     80106053 <uartinit+0xb3>
  if(!uart)
8010603a:	8b 15 bc a5 10 80    	mov    0x8010a5bc,%edx
80106040:	85 d2                	test   %edx,%edx
80106042:	74 ec                	je     80106030 <uartinit+0x90>
  for(p="xv6...\n"; *p; p++)
80106044:	83 c3 01             	add    $0x1,%ebx
80106047:	e8 04 ff ff ff       	call   80105f50 <uartputc.part.0>
8010604c:	0f be 03             	movsbl (%ebx),%eax
8010604f:	84 c0                	test   %al,%al
80106051:	75 e7                	jne    8010603a <uartinit+0x9a>
}
80106053:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106056:	5b                   	pop    %ebx
80106057:	5e                   	pop    %esi
80106058:	5f                   	pop    %edi
80106059:	5d                   	pop    %ebp
8010605a:	c3                   	ret    
8010605b:	90                   	nop
8010605c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80106060 <uartputc>:
  if(!uart)
80106060:	8b 15 bc a5 10 80    	mov    0x8010a5bc,%edx
{
80106066:	55                   	push   %ebp
80106067:	89 e5                	mov    %esp,%ebp
  if(!uart)
80106069:	85 d2                	test   %edx,%edx
{
8010606b:	8b 45 08             	mov    0x8(%ebp),%eax
  if(!uart)
8010606e:	74 10                	je     80106080 <uartputc+0x20>
}
80106070:	5d                   	pop    %ebp
80106071:	e9 da fe ff ff       	jmp    80105f50 <uartputc.part.0>
80106076:	8d 76 00             	lea    0x0(%esi),%esi
80106079:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
80106080:	5d                   	pop    %ebp
80106081:	c3                   	ret    
80106082:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106089:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80106090 <uartintr>:

void
uartintr(void)
{
80106090:	55                   	push   %ebp
80106091:	89 e5                	mov    %esp,%ebp
80106093:	83 ec 14             	sub    $0x14,%esp
  consoleintr(uartgetc);
80106096:	68 20 5f 10 80       	push   $0x80105f20
8010609b:	e8 70 a7 ff ff       	call   80100810 <consoleintr>
}
801060a0:	83 c4 10             	add    $0x10,%esp
801060a3:	c9                   	leave  
801060a4:	c3                   	ret    

801060a5 <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
801060a5:	6a 00                	push   $0x0
  pushl $0
801060a7:	6a 00                	push   $0x0
  jmp alltraps
801060a9:	e9 1c fb ff ff       	jmp    80105bca <alltraps>

801060ae <vector1>:
.globl vector1
vector1:
  pushl $0
801060ae:	6a 00                	push   $0x0
  pushl $1
801060b0:	6a 01                	push   $0x1
  jmp alltraps
801060b2:	e9 13 fb ff ff       	jmp    80105bca <alltraps>

801060b7 <vector2>:
.globl vector2
vector2:
  pushl $0
801060b7:	6a 00                	push   $0x0
  pushl $2
801060b9:	6a 02                	push   $0x2
  jmp alltraps
801060bb:	e9 0a fb ff ff       	jmp    80105bca <alltraps>

801060c0 <vector3>:
.globl vector3
vector3:
  pushl $0
801060c0:	6a 00                	push   $0x0
  pushl $3
801060c2:	6a 03                	push   $0x3
  jmp alltraps
801060c4:	e9 01 fb ff ff       	jmp    80105bca <alltraps>

801060c9 <vector4>:
.globl vector4
vector4:
  pushl $0
801060c9:	6a 00                	push   $0x0
  pushl $4
801060cb:	6a 04                	push   $0x4
  jmp alltraps
801060cd:	e9 f8 fa ff ff       	jmp    80105bca <alltraps>

801060d2 <vector5>:
.globl vector5
vector5:
  pushl $0
801060d2:	6a 00                	push   $0x0
  pushl $5
801060d4:	6a 05                	push   $0x5
  jmp alltraps
801060d6:	e9 ef fa ff ff       	jmp    80105bca <alltraps>

801060db <vector6>:
.globl vector6
vector6:
  pushl $0
801060db:	6a 00                	push   $0x0
  pushl $6
801060dd:	6a 06                	push   $0x6
  jmp alltraps
801060df:	e9 e6 fa ff ff       	jmp    80105bca <alltraps>

801060e4 <vector7>:
.globl vector7
vector7:
  pushl $0
801060e4:	6a 00                	push   $0x0
  pushl $7
801060e6:	6a 07                	push   $0x7
  jmp alltraps
801060e8:	e9 dd fa ff ff       	jmp    80105bca <alltraps>

801060ed <vector8>:
.globl vector8
vector8:
  pushl $8
801060ed:	6a 08                	push   $0x8
  jmp alltraps
801060ef:	e9 d6 fa ff ff       	jmp    80105bca <alltraps>

801060f4 <vector9>:
.globl vector9
vector9:
  pushl $0
801060f4:	6a 00                	push   $0x0
  pushl $9
801060f6:	6a 09                	push   $0x9
  jmp alltraps
801060f8:	e9 cd fa ff ff       	jmp    80105bca <alltraps>

801060fd <vector10>:
.globl vector10
vector10:
  pushl $10
801060fd:	6a 0a                	push   $0xa
  jmp alltraps
801060ff:	e9 c6 fa ff ff       	jmp    80105bca <alltraps>

80106104 <vector11>:
.globl vector11
vector11:
  pushl $11
80106104:	6a 0b                	push   $0xb
  jmp alltraps
80106106:	e9 bf fa ff ff       	jmp    80105bca <alltraps>

8010610b <vector12>:
.globl vector12
vector12:
  pushl $12
8010610b:	6a 0c                	push   $0xc
  jmp alltraps
8010610d:	e9 b8 fa ff ff       	jmp    80105bca <alltraps>

80106112 <vector13>:
.globl vector13
vector13:
  pushl $13
80106112:	6a 0d                	push   $0xd
  jmp alltraps
80106114:	e9 b1 fa ff ff       	jmp    80105bca <alltraps>

80106119 <vector14>:
.globl vector14
vector14:
  pushl $14
80106119:	6a 0e                	push   $0xe
  jmp alltraps
8010611b:	e9 aa fa ff ff       	jmp    80105bca <alltraps>

80106120 <vector15>:
.globl vector15
vector15:
  pushl $0
80106120:	6a 00                	push   $0x0
  pushl $15
80106122:	6a 0f                	push   $0xf
  jmp alltraps
80106124:	e9 a1 fa ff ff       	jmp    80105bca <alltraps>

80106129 <vector16>:
.globl vector16
vector16:
  pushl $0
80106129:	6a 00                	push   $0x0
  pushl $16
8010612b:	6a 10                	push   $0x10
  jmp alltraps
8010612d:	e9 98 fa ff ff       	jmp    80105bca <alltraps>

80106132 <vector17>:
.globl vector17
vector17:
  pushl $17
80106132:	6a 11                	push   $0x11
  jmp alltraps
80106134:	e9 91 fa ff ff       	jmp    80105bca <alltraps>

80106139 <vector18>:
.globl vector18
vector18:
  pushl $0
80106139:	6a 00                	push   $0x0
  pushl $18
8010613b:	6a 12                	push   $0x12
  jmp alltraps
8010613d:	e9 88 fa ff ff       	jmp    80105bca <alltraps>

80106142 <vector19>:
.globl vector19
vector19:
  pushl $0
80106142:	6a 00                	push   $0x0
  pushl $19
80106144:	6a 13                	push   $0x13
  jmp alltraps
80106146:	e9 7f fa ff ff       	jmp    80105bca <alltraps>

8010614b <vector20>:
.globl vector20
vector20:
  pushl $0
8010614b:	6a 00                	push   $0x0
  pushl $20
8010614d:	6a 14                	push   $0x14
  jmp alltraps
8010614f:	e9 76 fa ff ff       	jmp    80105bca <alltraps>

80106154 <vector21>:
.globl vector21
vector21:
  pushl $0
80106154:	6a 00                	push   $0x0
  pushl $21
80106156:	6a 15                	push   $0x15
  jmp alltraps
80106158:	e9 6d fa ff ff       	jmp    80105bca <alltraps>

8010615d <vector22>:
.globl vector22
vector22:
  pushl $0
8010615d:	6a 00                	push   $0x0
  pushl $22
8010615f:	6a 16                	push   $0x16
  jmp alltraps
80106161:	e9 64 fa ff ff       	jmp    80105bca <alltraps>

80106166 <vector23>:
.globl vector23
vector23:
  pushl $0
80106166:	6a 00                	push   $0x0
  pushl $23
80106168:	6a 17                	push   $0x17
  jmp alltraps
8010616a:	e9 5b fa ff ff       	jmp    80105bca <alltraps>

8010616f <vector24>:
.globl vector24
vector24:
  pushl $0
8010616f:	6a 00                	push   $0x0
  pushl $24
80106171:	6a 18                	push   $0x18
  jmp alltraps
80106173:	e9 52 fa ff ff       	jmp    80105bca <alltraps>

80106178 <vector25>:
.globl vector25
vector25:
  pushl $0
80106178:	6a 00                	push   $0x0
  pushl $25
8010617a:	6a 19                	push   $0x19
  jmp alltraps
8010617c:	e9 49 fa ff ff       	jmp    80105bca <alltraps>

80106181 <vector26>:
.globl vector26
vector26:
  pushl $0
80106181:	6a 00                	push   $0x0
  pushl $26
80106183:	6a 1a                	push   $0x1a
  jmp alltraps
80106185:	e9 40 fa ff ff       	jmp    80105bca <alltraps>

8010618a <vector27>:
.globl vector27
vector27:
  pushl $0
8010618a:	6a 00                	push   $0x0
  pushl $27
8010618c:	6a 1b                	push   $0x1b
  jmp alltraps
8010618e:	e9 37 fa ff ff       	jmp    80105bca <alltraps>

80106193 <vector28>:
.globl vector28
vector28:
  pushl $0
80106193:	6a 00                	push   $0x0
  pushl $28
80106195:	6a 1c                	push   $0x1c
  jmp alltraps
80106197:	e9 2e fa ff ff       	jmp    80105bca <alltraps>

8010619c <vector29>:
.globl vector29
vector29:
  pushl $0
8010619c:	6a 00                	push   $0x0
  pushl $29
8010619e:	6a 1d                	push   $0x1d
  jmp alltraps
801061a0:	e9 25 fa ff ff       	jmp    80105bca <alltraps>

801061a5 <vector30>:
.globl vector30
vector30:
  pushl $0
801061a5:	6a 00                	push   $0x0
  pushl $30
801061a7:	6a 1e                	push   $0x1e
  jmp alltraps
801061a9:	e9 1c fa ff ff       	jmp    80105bca <alltraps>

801061ae <vector31>:
.globl vector31
vector31:
  pushl $0
801061ae:	6a 00                	push   $0x0
  pushl $31
801061b0:	6a 1f                	push   $0x1f
  jmp alltraps
801061b2:	e9 13 fa ff ff       	jmp    80105bca <alltraps>

801061b7 <vector32>:
.globl vector32
vector32:
  pushl $0
801061b7:	6a 00                	push   $0x0
  pushl $32
801061b9:	6a 20                	push   $0x20
  jmp alltraps
801061bb:	e9 0a fa ff ff       	jmp    80105bca <alltraps>

801061c0 <vector33>:
.globl vector33
vector33:
  pushl $0
801061c0:	6a 00                	push   $0x0
  pushl $33
801061c2:	6a 21                	push   $0x21
  jmp alltraps
801061c4:	e9 01 fa ff ff       	jmp    80105bca <alltraps>

801061c9 <vector34>:
.globl vector34
vector34:
  pushl $0
801061c9:	6a 00                	push   $0x0
  pushl $34
801061cb:	6a 22                	push   $0x22
  jmp alltraps
801061cd:	e9 f8 f9 ff ff       	jmp    80105bca <alltraps>

801061d2 <vector35>:
.globl vector35
vector35:
  pushl $0
801061d2:	6a 00                	push   $0x0
  pushl $35
801061d4:	6a 23                	push   $0x23
  jmp alltraps
801061d6:	e9 ef f9 ff ff       	jmp    80105bca <alltraps>

801061db <vector36>:
.globl vector36
vector36:
  pushl $0
801061db:	6a 00                	push   $0x0
  pushl $36
801061dd:	6a 24                	push   $0x24
  jmp alltraps
801061df:	e9 e6 f9 ff ff       	jmp    80105bca <alltraps>

801061e4 <vector37>:
.globl vector37
vector37:
  pushl $0
801061e4:	6a 00                	push   $0x0
  pushl $37
801061e6:	6a 25                	push   $0x25
  jmp alltraps
801061e8:	e9 dd f9 ff ff       	jmp    80105bca <alltraps>

801061ed <vector38>:
.globl vector38
vector38:
  pushl $0
801061ed:	6a 00                	push   $0x0
  pushl $38
801061ef:	6a 26                	push   $0x26
  jmp alltraps
801061f1:	e9 d4 f9 ff ff       	jmp    80105bca <alltraps>

801061f6 <vector39>:
.globl vector39
vector39:
  pushl $0
801061f6:	6a 00                	push   $0x0
  pushl $39
801061f8:	6a 27                	push   $0x27
  jmp alltraps
801061fa:	e9 cb f9 ff ff       	jmp    80105bca <alltraps>

801061ff <vector40>:
.globl vector40
vector40:
  pushl $0
801061ff:	6a 00                	push   $0x0
  pushl $40
80106201:	6a 28                	push   $0x28
  jmp alltraps
80106203:	e9 c2 f9 ff ff       	jmp    80105bca <alltraps>

80106208 <vector41>:
.globl vector41
vector41:
  pushl $0
80106208:	6a 00                	push   $0x0
  pushl $41
8010620a:	6a 29                	push   $0x29
  jmp alltraps
8010620c:	e9 b9 f9 ff ff       	jmp    80105bca <alltraps>

80106211 <vector42>:
.globl vector42
vector42:
  pushl $0
80106211:	6a 00                	push   $0x0
  pushl $42
80106213:	6a 2a                	push   $0x2a
  jmp alltraps
80106215:	e9 b0 f9 ff ff       	jmp    80105bca <alltraps>

8010621a <vector43>:
.globl vector43
vector43:
  pushl $0
8010621a:	6a 00                	push   $0x0
  pushl $43
8010621c:	6a 2b                	push   $0x2b
  jmp alltraps
8010621e:	e9 a7 f9 ff ff       	jmp    80105bca <alltraps>

80106223 <vector44>:
.globl vector44
vector44:
  pushl $0
80106223:	6a 00                	push   $0x0
  pushl $44
80106225:	6a 2c                	push   $0x2c
  jmp alltraps
80106227:	e9 9e f9 ff ff       	jmp    80105bca <alltraps>

8010622c <vector45>:
.globl vector45
vector45:
  pushl $0
8010622c:	6a 00                	push   $0x0
  pushl $45
8010622e:	6a 2d                	push   $0x2d
  jmp alltraps
80106230:	e9 95 f9 ff ff       	jmp    80105bca <alltraps>

80106235 <vector46>:
.globl vector46
vector46:
  pushl $0
80106235:	6a 00                	push   $0x0
  pushl $46
80106237:	6a 2e                	push   $0x2e
  jmp alltraps
80106239:	e9 8c f9 ff ff       	jmp    80105bca <alltraps>

8010623e <vector47>:
.globl vector47
vector47:
  pushl $0
8010623e:	6a 00                	push   $0x0
  pushl $47
80106240:	6a 2f                	push   $0x2f
  jmp alltraps
80106242:	e9 83 f9 ff ff       	jmp    80105bca <alltraps>

80106247 <vector48>:
.globl vector48
vector48:
  pushl $0
80106247:	6a 00                	push   $0x0
  pushl $48
80106249:	6a 30                	push   $0x30
  jmp alltraps
8010624b:	e9 7a f9 ff ff       	jmp    80105bca <alltraps>

80106250 <vector49>:
.globl vector49
vector49:
  pushl $0
80106250:	6a 00                	push   $0x0
  pushl $49
80106252:	6a 31                	push   $0x31
  jmp alltraps
80106254:	e9 71 f9 ff ff       	jmp    80105bca <alltraps>

80106259 <vector50>:
.globl vector50
vector50:
  pushl $0
80106259:	6a 00                	push   $0x0
  pushl $50
8010625b:	6a 32                	push   $0x32
  jmp alltraps
8010625d:	e9 68 f9 ff ff       	jmp    80105bca <alltraps>

80106262 <vector51>:
.globl vector51
vector51:
  pushl $0
80106262:	6a 00                	push   $0x0
  pushl $51
80106264:	6a 33                	push   $0x33
  jmp alltraps
80106266:	e9 5f f9 ff ff       	jmp    80105bca <alltraps>

8010626b <vector52>:
.globl vector52
vector52:
  pushl $0
8010626b:	6a 00                	push   $0x0
  pushl $52
8010626d:	6a 34                	push   $0x34
  jmp alltraps
8010626f:	e9 56 f9 ff ff       	jmp    80105bca <alltraps>

80106274 <vector53>:
.globl vector53
vector53:
  pushl $0
80106274:	6a 00                	push   $0x0
  pushl $53
80106276:	6a 35                	push   $0x35
  jmp alltraps
80106278:	e9 4d f9 ff ff       	jmp    80105bca <alltraps>

8010627d <vector54>:
.globl vector54
vector54:
  pushl $0
8010627d:	6a 00                	push   $0x0
  pushl $54
8010627f:	6a 36                	push   $0x36
  jmp alltraps
80106281:	e9 44 f9 ff ff       	jmp    80105bca <alltraps>

80106286 <vector55>:
.globl vector55
vector55:
  pushl $0
80106286:	6a 00                	push   $0x0
  pushl $55
80106288:	6a 37                	push   $0x37
  jmp alltraps
8010628a:	e9 3b f9 ff ff       	jmp    80105bca <alltraps>

8010628f <vector56>:
.globl vector56
vector56:
  pushl $0
8010628f:	6a 00                	push   $0x0
  pushl $56
80106291:	6a 38                	push   $0x38
  jmp alltraps
80106293:	e9 32 f9 ff ff       	jmp    80105bca <alltraps>

80106298 <vector57>:
.globl vector57
vector57:
  pushl $0
80106298:	6a 00                	push   $0x0
  pushl $57
8010629a:	6a 39                	push   $0x39
  jmp alltraps
8010629c:	e9 29 f9 ff ff       	jmp    80105bca <alltraps>

801062a1 <vector58>:
.globl vector58
vector58:
  pushl $0
801062a1:	6a 00                	push   $0x0
  pushl $58
801062a3:	6a 3a                	push   $0x3a
  jmp alltraps
801062a5:	e9 20 f9 ff ff       	jmp    80105bca <alltraps>

801062aa <vector59>:
.globl vector59
vector59:
  pushl $0
801062aa:	6a 00                	push   $0x0
  pushl $59
801062ac:	6a 3b                	push   $0x3b
  jmp alltraps
801062ae:	e9 17 f9 ff ff       	jmp    80105bca <alltraps>

801062b3 <vector60>:
.globl vector60
vector60:
  pushl $0
801062b3:	6a 00                	push   $0x0
  pushl $60
801062b5:	6a 3c                	push   $0x3c
  jmp alltraps
801062b7:	e9 0e f9 ff ff       	jmp    80105bca <alltraps>

801062bc <vector61>:
.globl vector61
vector61:
  pushl $0
801062bc:	6a 00                	push   $0x0
  pushl $61
801062be:	6a 3d                	push   $0x3d
  jmp alltraps
801062c0:	e9 05 f9 ff ff       	jmp    80105bca <alltraps>

801062c5 <vector62>:
.globl vector62
vector62:
  pushl $0
801062c5:	6a 00                	push   $0x0
  pushl $62
801062c7:	6a 3e                	push   $0x3e
  jmp alltraps
801062c9:	e9 fc f8 ff ff       	jmp    80105bca <alltraps>

801062ce <vector63>:
.globl vector63
vector63:
  pushl $0
801062ce:	6a 00                	push   $0x0
  pushl $63
801062d0:	6a 3f                	push   $0x3f
  jmp alltraps
801062d2:	e9 f3 f8 ff ff       	jmp    80105bca <alltraps>

801062d7 <vector64>:
.globl vector64
vector64:
  pushl $0
801062d7:	6a 00                	push   $0x0
  pushl $64
801062d9:	6a 40                	push   $0x40
  jmp alltraps
801062db:	e9 ea f8 ff ff       	jmp    80105bca <alltraps>

801062e0 <vector65>:
.globl vector65
vector65:
  pushl $0
801062e0:	6a 00                	push   $0x0
  pushl $65
801062e2:	6a 41                	push   $0x41
  jmp alltraps
801062e4:	e9 e1 f8 ff ff       	jmp    80105bca <alltraps>

801062e9 <vector66>:
.globl vector66
vector66:
  pushl $0
801062e9:	6a 00                	push   $0x0
  pushl $66
801062eb:	6a 42                	push   $0x42
  jmp alltraps
801062ed:	e9 d8 f8 ff ff       	jmp    80105bca <alltraps>

801062f2 <vector67>:
.globl vector67
vector67:
  pushl $0
801062f2:	6a 00                	push   $0x0
  pushl $67
801062f4:	6a 43                	push   $0x43
  jmp alltraps
801062f6:	e9 cf f8 ff ff       	jmp    80105bca <alltraps>

801062fb <vector68>:
.globl vector68
vector68:
  pushl $0
801062fb:	6a 00                	push   $0x0
  pushl $68
801062fd:	6a 44                	push   $0x44
  jmp alltraps
801062ff:	e9 c6 f8 ff ff       	jmp    80105bca <alltraps>

80106304 <vector69>:
.globl vector69
vector69:
  pushl $0
80106304:	6a 00                	push   $0x0
  pushl $69
80106306:	6a 45                	push   $0x45
  jmp alltraps
80106308:	e9 bd f8 ff ff       	jmp    80105bca <alltraps>

8010630d <vector70>:
.globl vector70
vector70:
  pushl $0
8010630d:	6a 00                	push   $0x0
  pushl $70
8010630f:	6a 46                	push   $0x46
  jmp alltraps
80106311:	e9 b4 f8 ff ff       	jmp    80105bca <alltraps>

80106316 <vector71>:
.globl vector71
vector71:
  pushl $0
80106316:	6a 00                	push   $0x0
  pushl $71
80106318:	6a 47                	push   $0x47
  jmp alltraps
8010631a:	e9 ab f8 ff ff       	jmp    80105bca <alltraps>

8010631f <vector72>:
.globl vector72
vector72:
  pushl $0
8010631f:	6a 00                	push   $0x0
  pushl $72
80106321:	6a 48                	push   $0x48
  jmp alltraps
80106323:	e9 a2 f8 ff ff       	jmp    80105bca <alltraps>

80106328 <vector73>:
.globl vector73
vector73:
  pushl $0
80106328:	6a 00                	push   $0x0
  pushl $73
8010632a:	6a 49                	push   $0x49
  jmp alltraps
8010632c:	e9 99 f8 ff ff       	jmp    80105bca <alltraps>

80106331 <vector74>:
.globl vector74
vector74:
  pushl $0
80106331:	6a 00                	push   $0x0
  pushl $74
80106333:	6a 4a                	push   $0x4a
  jmp alltraps
80106335:	e9 90 f8 ff ff       	jmp    80105bca <alltraps>

8010633a <vector75>:
.globl vector75
vector75:
  pushl $0
8010633a:	6a 00                	push   $0x0
  pushl $75
8010633c:	6a 4b                	push   $0x4b
  jmp alltraps
8010633e:	e9 87 f8 ff ff       	jmp    80105bca <alltraps>

80106343 <vector76>:
.globl vector76
vector76:
  pushl $0
80106343:	6a 00                	push   $0x0
  pushl $76
80106345:	6a 4c                	push   $0x4c
  jmp alltraps
80106347:	e9 7e f8 ff ff       	jmp    80105bca <alltraps>

8010634c <vector77>:
.globl vector77
vector77:
  pushl $0
8010634c:	6a 00                	push   $0x0
  pushl $77
8010634e:	6a 4d                	push   $0x4d
  jmp alltraps
80106350:	e9 75 f8 ff ff       	jmp    80105bca <alltraps>

80106355 <vector78>:
.globl vector78
vector78:
  pushl $0
80106355:	6a 00                	push   $0x0
  pushl $78
80106357:	6a 4e                	push   $0x4e
  jmp alltraps
80106359:	e9 6c f8 ff ff       	jmp    80105bca <alltraps>

8010635e <vector79>:
.globl vector79
vector79:
  pushl $0
8010635e:	6a 00                	push   $0x0
  pushl $79
80106360:	6a 4f                	push   $0x4f
  jmp alltraps
80106362:	e9 63 f8 ff ff       	jmp    80105bca <alltraps>

80106367 <vector80>:
.globl vector80
vector80:
  pushl $0
80106367:	6a 00                	push   $0x0
  pushl $80
80106369:	6a 50                	push   $0x50
  jmp alltraps
8010636b:	e9 5a f8 ff ff       	jmp    80105bca <alltraps>

80106370 <vector81>:
.globl vector81
vector81:
  pushl $0
80106370:	6a 00                	push   $0x0
  pushl $81
80106372:	6a 51                	push   $0x51
  jmp alltraps
80106374:	e9 51 f8 ff ff       	jmp    80105bca <alltraps>

80106379 <vector82>:
.globl vector82
vector82:
  pushl $0
80106379:	6a 00                	push   $0x0
  pushl $82
8010637b:	6a 52                	push   $0x52
  jmp alltraps
8010637d:	e9 48 f8 ff ff       	jmp    80105bca <alltraps>

80106382 <vector83>:
.globl vector83
vector83:
  pushl $0
80106382:	6a 00                	push   $0x0
  pushl $83
80106384:	6a 53                	push   $0x53
  jmp alltraps
80106386:	e9 3f f8 ff ff       	jmp    80105bca <alltraps>

8010638b <vector84>:
.globl vector84
vector84:
  pushl $0
8010638b:	6a 00                	push   $0x0
  pushl $84
8010638d:	6a 54                	push   $0x54
  jmp alltraps
8010638f:	e9 36 f8 ff ff       	jmp    80105bca <alltraps>

80106394 <vector85>:
.globl vector85
vector85:
  pushl $0
80106394:	6a 00                	push   $0x0
  pushl $85
80106396:	6a 55                	push   $0x55
  jmp alltraps
80106398:	e9 2d f8 ff ff       	jmp    80105bca <alltraps>

8010639d <vector86>:
.globl vector86
vector86:
  pushl $0
8010639d:	6a 00                	push   $0x0
  pushl $86
8010639f:	6a 56                	push   $0x56
  jmp alltraps
801063a1:	e9 24 f8 ff ff       	jmp    80105bca <alltraps>

801063a6 <vector87>:
.globl vector87
vector87:
  pushl $0
801063a6:	6a 00                	push   $0x0
  pushl $87
801063a8:	6a 57                	push   $0x57
  jmp alltraps
801063aa:	e9 1b f8 ff ff       	jmp    80105bca <alltraps>

801063af <vector88>:
.globl vector88
vector88:
  pushl $0
801063af:	6a 00                	push   $0x0
  pushl $88
801063b1:	6a 58                	push   $0x58
  jmp alltraps
801063b3:	e9 12 f8 ff ff       	jmp    80105bca <alltraps>

801063b8 <vector89>:
.globl vector89
vector89:
  pushl $0
801063b8:	6a 00                	push   $0x0
  pushl $89
801063ba:	6a 59                	push   $0x59
  jmp alltraps
801063bc:	e9 09 f8 ff ff       	jmp    80105bca <alltraps>

801063c1 <vector90>:
.globl vector90
vector90:
  pushl $0
801063c1:	6a 00                	push   $0x0
  pushl $90
801063c3:	6a 5a                	push   $0x5a
  jmp alltraps
801063c5:	e9 00 f8 ff ff       	jmp    80105bca <alltraps>

801063ca <vector91>:
.globl vector91
vector91:
  pushl $0
801063ca:	6a 00                	push   $0x0
  pushl $91
801063cc:	6a 5b                	push   $0x5b
  jmp alltraps
801063ce:	e9 f7 f7 ff ff       	jmp    80105bca <alltraps>

801063d3 <vector92>:
.globl vector92
vector92:
  pushl $0
801063d3:	6a 00                	push   $0x0
  pushl $92
801063d5:	6a 5c                	push   $0x5c
  jmp alltraps
801063d7:	e9 ee f7 ff ff       	jmp    80105bca <alltraps>

801063dc <vector93>:
.globl vector93
vector93:
  pushl $0
801063dc:	6a 00                	push   $0x0
  pushl $93
801063de:	6a 5d                	push   $0x5d
  jmp alltraps
801063e0:	e9 e5 f7 ff ff       	jmp    80105bca <alltraps>

801063e5 <vector94>:
.globl vector94
vector94:
  pushl $0
801063e5:	6a 00                	push   $0x0
  pushl $94
801063e7:	6a 5e                	push   $0x5e
  jmp alltraps
801063e9:	e9 dc f7 ff ff       	jmp    80105bca <alltraps>

801063ee <vector95>:
.globl vector95
vector95:
  pushl $0
801063ee:	6a 00                	push   $0x0
  pushl $95
801063f0:	6a 5f                	push   $0x5f
  jmp alltraps
801063f2:	e9 d3 f7 ff ff       	jmp    80105bca <alltraps>

801063f7 <vector96>:
.globl vector96
vector96:
  pushl $0
801063f7:	6a 00                	push   $0x0
  pushl $96
801063f9:	6a 60                	push   $0x60
  jmp alltraps
801063fb:	e9 ca f7 ff ff       	jmp    80105bca <alltraps>

80106400 <vector97>:
.globl vector97
vector97:
  pushl $0
80106400:	6a 00                	push   $0x0
  pushl $97
80106402:	6a 61                	push   $0x61
  jmp alltraps
80106404:	e9 c1 f7 ff ff       	jmp    80105bca <alltraps>

80106409 <vector98>:
.globl vector98
vector98:
  pushl $0
80106409:	6a 00                	push   $0x0
  pushl $98
8010640b:	6a 62                	push   $0x62
  jmp alltraps
8010640d:	e9 b8 f7 ff ff       	jmp    80105bca <alltraps>

80106412 <vector99>:
.globl vector99
vector99:
  pushl $0
80106412:	6a 00                	push   $0x0
  pushl $99
80106414:	6a 63                	push   $0x63
  jmp alltraps
80106416:	e9 af f7 ff ff       	jmp    80105bca <alltraps>

8010641b <vector100>:
.globl vector100
vector100:
  pushl $0
8010641b:	6a 00                	push   $0x0
  pushl $100
8010641d:	6a 64                	push   $0x64
  jmp alltraps
8010641f:	e9 a6 f7 ff ff       	jmp    80105bca <alltraps>

80106424 <vector101>:
.globl vector101
vector101:
  pushl $0
80106424:	6a 00                	push   $0x0
  pushl $101
80106426:	6a 65                	push   $0x65
  jmp alltraps
80106428:	e9 9d f7 ff ff       	jmp    80105bca <alltraps>

8010642d <vector102>:
.globl vector102
vector102:
  pushl $0
8010642d:	6a 00                	push   $0x0
  pushl $102
8010642f:	6a 66                	push   $0x66
  jmp alltraps
80106431:	e9 94 f7 ff ff       	jmp    80105bca <alltraps>

80106436 <vector103>:
.globl vector103
vector103:
  pushl $0
80106436:	6a 00                	push   $0x0
  pushl $103
80106438:	6a 67                	push   $0x67
  jmp alltraps
8010643a:	e9 8b f7 ff ff       	jmp    80105bca <alltraps>

8010643f <vector104>:
.globl vector104
vector104:
  pushl $0
8010643f:	6a 00                	push   $0x0
  pushl $104
80106441:	6a 68                	push   $0x68
  jmp alltraps
80106443:	e9 82 f7 ff ff       	jmp    80105bca <alltraps>

80106448 <vector105>:
.globl vector105
vector105:
  pushl $0
80106448:	6a 00                	push   $0x0
  pushl $105
8010644a:	6a 69                	push   $0x69
  jmp alltraps
8010644c:	e9 79 f7 ff ff       	jmp    80105bca <alltraps>

80106451 <vector106>:
.globl vector106
vector106:
  pushl $0
80106451:	6a 00                	push   $0x0
  pushl $106
80106453:	6a 6a                	push   $0x6a
  jmp alltraps
80106455:	e9 70 f7 ff ff       	jmp    80105bca <alltraps>

8010645a <vector107>:
.globl vector107
vector107:
  pushl $0
8010645a:	6a 00                	push   $0x0
  pushl $107
8010645c:	6a 6b                	push   $0x6b
  jmp alltraps
8010645e:	e9 67 f7 ff ff       	jmp    80105bca <alltraps>

80106463 <vector108>:
.globl vector108
vector108:
  pushl $0
80106463:	6a 00                	push   $0x0
  pushl $108
80106465:	6a 6c                	push   $0x6c
  jmp alltraps
80106467:	e9 5e f7 ff ff       	jmp    80105bca <alltraps>

8010646c <vector109>:
.globl vector109
vector109:
  pushl $0
8010646c:	6a 00                	push   $0x0
  pushl $109
8010646e:	6a 6d                	push   $0x6d
  jmp alltraps
80106470:	e9 55 f7 ff ff       	jmp    80105bca <alltraps>

80106475 <vector110>:
.globl vector110
vector110:
  pushl $0
80106475:	6a 00                	push   $0x0
  pushl $110
80106477:	6a 6e                	push   $0x6e
  jmp alltraps
80106479:	e9 4c f7 ff ff       	jmp    80105bca <alltraps>

8010647e <vector111>:
.globl vector111
vector111:
  pushl $0
8010647e:	6a 00                	push   $0x0
  pushl $111
80106480:	6a 6f                	push   $0x6f
  jmp alltraps
80106482:	e9 43 f7 ff ff       	jmp    80105bca <alltraps>

80106487 <vector112>:
.globl vector112
vector112:
  pushl $0
80106487:	6a 00                	push   $0x0
  pushl $112
80106489:	6a 70                	push   $0x70
  jmp alltraps
8010648b:	e9 3a f7 ff ff       	jmp    80105bca <alltraps>

80106490 <vector113>:
.globl vector113
vector113:
  pushl $0
80106490:	6a 00                	push   $0x0
  pushl $113
80106492:	6a 71                	push   $0x71
  jmp alltraps
80106494:	e9 31 f7 ff ff       	jmp    80105bca <alltraps>

80106499 <vector114>:
.globl vector114
vector114:
  pushl $0
80106499:	6a 00                	push   $0x0
  pushl $114
8010649b:	6a 72                	push   $0x72
  jmp alltraps
8010649d:	e9 28 f7 ff ff       	jmp    80105bca <alltraps>

801064a2 <vector115>:
.globl vector115
vector115:
  pushl $0
801064a2:	6a 00                	push   $0x0
  pushl $115
801064a4:	6a 73                	push   $0x73
  jmp alltraps
801064a6:	e9 1f f7 ff ff       	jmp    80105bca <alltraps>

801064ab <vector116>:
.globl vector116
vector116:
  pushl $0
801064ab:	6a 00                	push   $0x0
  pushl $116
801064ad:	6a 74                	push   $0x74
  jmp alltraps
801064af:	e9 16 f7 ff ff       	jmp    80105bca <alltraps>

801064b4 <vector117>:
.globl vector117
vector117:
  pushl $0
801064b4:	6a 00                	push   $0x0
  pushl $117
801064b6:	6a 75                	push   $0x75
  jmp alltraps
801064b8:	e9 0d f7 ff ff       	jmp    80105bca <alltraps>

801064bd <vector118>:
.globl vector118
vector118:
  pushl $0
801064bd:	6a 00                	push   $0x0
  pushl $118
801064bf:	6a 76                	push   $0x76
  jmp alltraps
801064c1:	e9 04 f7 ff ff       	jmp    80105bca <alltraps>

801064c6 <vector119>:
.globl vector119
vector119:
  pushl $0
801064c6:	6a 00                	push   $0x0
  pushl $119
801064c8:	6a 77                	push   $0x77
  jmp alltraps
801064ca:	e9 fb f6 ff ff       	jmp    80105bca <alltraps>

801064cf <vector120>:
.globl vector120
vector120:
  pushl $0
801064cf:	6a 00                	push   $0x0
  pushl $120
801064d1:	6a 78                	push   $0x78
  jmp alltraps
801064d3:	e9 f2 f6 ff ff       	jmp    80105bca <alltraps>

801064d8 <vector121>:
.globl vector121
vector121:
  pushl $0
801064d8:	6a 00                	push   $0x0
  pushl $121
801064da:	6a 79                	push   $0x79
  jmp alltraps
801064dc:	e9 e9 f6 ff ff       	jmp    80105bca <alltraps>

801064e1 <vector122>:
.globl vector122
vector122:
  pushl $0
801064e1:	6a 00                	push   $0x0
  pushl $122
801064e3:	6a 7a                	push   $0x7a
  jmp alltraps
801064e5:	e9 e0 f6 ff ff       	jmp    80105bca <alltraps>

801064ea <vector123>:
.globl vector123
vector123:
  pushl $0
801064ea:	6a 00                	push   $0x0
  pushl $123
801064ec:	6a 7b                	push   $0x7b
  jmp alltraps
801064ee:	e9 d7 f6 ff ff       	jmp    80105bca <alltraps>

801064f3 <vector124>:
.globl vector124
vector124:
  pushl $0
801064f3:	6a 00                	push   $0x0
  pushl $124
801064f5:	6a 7c                	push   $0x7c
  jmp alltraps
801064f7:	e9 ce f6 ff ff       	jmp    80105bca <alltraps>

801064fc <vector125>:
.globl vector125
vector125:
  pushl $0
801064fc:	6a 00                	push   $0x0
  pushl $125
801064fe:	6a 7d                	push   $0x7d
  jmp alltraps
80106500:	e9 c5 f6 ff ff       	jmp    80105bca <alltraps>

80106505 <vector126>:
.globl vector126
vector126:
  pushl $0
80106505:	6a 00                	push   $0x0
  pushl $126
80106507:	6a 7e                	push   $0x7e
  jmp alltraps
80106509:	e9 bc f6 ff ff       	jmp    80105bca <alltraps>

8010650e <vector127>:
.globl vector127
vector127:
  pushl $0
8010650e:	6a 00                	push   $0x0
  pushl $127
80106510:	6a 7f                	push   $0x7f
  jmp alltraps
80106512:	e9 b3 f6 ff ff       	jmp    80105bca <alltraps>

80106517 <vector128>:
.globl vector128
vector128:
  pushl $0
80106517:	6a 00                	push   $0x0
  pushl $128
80106519:	68 80 00 00 00       	push   $0x80
  jmp alltraps
8010651e:	e9 a7 f6 ff ff       	jmp    80105bca <alltraps>

80106523 <vector129>:
.globl vector129
vector129:
  pushl $0
80106523:	6a 00                	push   $0x0
  pushl $129
80106525:	68 81 00 00 00       	push   $0x81
  jmp alltraps
8010652a:	e9 9b f6 ff ff       	jmp    80105bca <alltraps>

8010652f <vector130>:
.globl vector130
vector130:
  pushl $0
8010652f:	6a 00                	push   $0x0
  pushl $130
80106531:	68 82 00 00 00       	push   $0x82
  jmp alltraps
80106536:	e9 8f f6 ff ff       	jmp    80105bca <alltraps>

8010653b <vector131>:
.globl vector131
vector131:
  pushl $0
8010653b:	6a 00                	push   $0x0
  pushl $131
8010653d:	68 83 00 00 00       	push   $0x83
  jmp alltraps
80106542:	e9 83 f6 ff ff       	jmp    80105bca <alltraps>

80106547 <vector132>:
.globl vector132
vector132:
  pushl $0
80106547:	6a 00                	push   $0x0
  pushl $132
80106549:	68 84 00 00 00       	push   $0x84
  jmp alltraps
8010654e:	e9 77 f6 ff ff       	jmp    80105bca <alltraps>

80106553 <vector133>:
.globl vector133
vector133:
  pushl $0
80106553:	6a 00                	push   $0x0
  pushl $133
80106555:	68 85 00 00 00       	push   $0x85
  jmp alltraps
8010655a:	e9 6b f6 ff ff       	jmp    80105bca <alltraps>

8010655f <vector134>:
.globl vector134
vector134:
  pushl $0
8010655f:	6a 00                	push   $0x0
  pushl $134
80106561:	68 86 00 00 00       	push   $0x86
  jmp alltraps
80106566:	e9 5f f6 ff ff       	jmp    80105bca <alltraps>

8010656b <vector135>:
.globl vector135
vector135:
  pushl $0
8010656b:	6a 00                	push   $0x0
  pushl $135
8010656d:	68 87 00 00 00       	push   $0x87
  jmp alltraps
80106572:	e9 53 f6 ff ff       	jmp    80105bca <alltraps>

80106577 <vector136>:
.globl vector136
vector136:
  pushl $0
80106577:	6a 00                	push   $0x0
  pushl $136
80106579:	68 88 00 00 00       	push   $0x88
  jmp alltraps
8010657e:	e9 47 f6 ff ff       	jmp    80105bca <alltraps>

80106583 <vector137>:
.globl vector137
vector137:
  pushl $0
80106583:	6a 00                	push   $0x0
  pushl $137
80106585:	68 89 00 00 00       	push   $0x89
  jmp alltraps
8010658a:	e9 3b f6 ff ff       	jmp    80105bca <alltraps>

8010658f <vector138>:
.globl vector138
vector138:
  pushl $0
8010658f:	6a 00                	push   $0x0
  pushl $138
80106591:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
80106596:	e9 2f f6 ff ff       	jmp    80105bca <alltraps>

8010659b <vector139>:
.globl vector139
vector139:
  pushl $0
8010659b:	6a 00                	push   $0x0
  pushl $139
8010659d:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
801065a2:	e9 23 f6 ff ff       	jmp    80105bca <alltraps>

801065a7 <vector140>:
.globl vector140
vector140:
  pushl $0
801065a7:	6a 00                	push   $0x0
  pushl $140
801065a9:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
801065ae:	e9 17 f6 ff ff       	jmp    80105bca <alltraps>

801065b3 <vector141>:
.globl vector141
vector141:
  pushl $0
801065b3:	6a 00                	push   $0x0
  pushl $141
801065b5:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
801065ba:	e9 0b f6 ff ff       	jmp    80105bca <alltraps>

801065bf <vector142>:
.globl vector142
vector142:
  pushl $0
801065bf:	6a 00                	push   $0x0
  pushl $142
801065c1:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
801065c6:	e9 ff f5 ff ff       	jmp    80105bca <alltraps>

801065cb <vector143>:
.globl vector143
vector143:
  pushl $0
801065cb:	6a 00                	push   $0x0
  pushl $143
801065cd:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
801065d2:	e9 f3 f5 ff ff       	jmp    80105bca <alltraps>

801065d7 <vector144>:
.globl vector144
vector144:
  pushl $0
801065d7:	6a 00                	push   $0x0
  pushl $144
801065d9:	68 90 00 00 00       	push   $0x90
  jmp alltraps
801065de:	e9 e7 f5 ff ff       	jmp    80105bca <alltraps>

801065e3 <vector145>:
.globl vector145
vector145:
  pushl $0
801065e3:	6a 00                	push   $0x0
  pushl $145
801065e5:	68 91 00 00 00       	push   $0x91
  jmp alltraps
801065ea:	e9 db f5 ff ff       	jmp    80105bca <alltraps>

801065ef <vector146>:
.globl vector146
vector146:
  pushl $0
801065ef:	6a 00                	push   $0x0
  pushl $146
801065f1:	68 92 00 00 00       	push   $0x92
  jmp alltraps
801065f6:	e9 cf f5 ff ff       	jmp    80105bca <alltraps>

801065fb <vector147>:
.globl vector147
vector147:
  pushl $0
801065fb:	6a 00                	push   $0x0
  pushl $147
801065fd:	68 93 00 00 00       	push   $0x93
  jmp alltraps
80106602:	e9 c3 f5 ff ff       	jmp    80105bca <alltraps>

80106607 <vector148>:
.globl vector148
vector148:
  pushl $0
80106607:	6a 00                	push   $0x0
  pushl $148
80106609:	68 94 00 00 00       	push   $0x94
  jmp alltraps
8010660e:	e9 b7 f5 ff ff       	jmp    80105bca <alltraps>

80106613 <vector149>:
.globl vector149
vector149:
  pushl $0
80106613:	6a 00                	push   $0x0
  pushl $149
80106615:	68 95 00 00 00       	push   $0x95
  jmp alltraps
8010661a:	e9 ab f5 ff ff       	jmp    80105bca <alltraps>

8010661f <vector150>:
.globl vector150
vector150:
  pushl $0
8010661f:	6a 00                	push   $0x0
  pushl $150
80106621:	68 96 00 00 00       	push   $0x96
  jmp alltraps
80106626:	e9 9f f5 ff ff       	jmp    80105bca <alltraps>

8010662b <vector151>:
.globl vector151
vector151:
  pushl $0
8010662b:	6a 00                	push   $0x0
  pushl $151
8010662d:	68 97 00 00 00       	push   $0x97
  jmp alltraps
80106632:	e9 93 f5 ff ff       	jmp    80105bca <alltraps>

80106637 <vector152>:
.globl vector152
vector152:
  pushl $0
80106637:	6a 00                	push   $0x0
  pushl $152
80106639:	68 98 00 00 00       	push   $0x98
  jmp alltraps
8010663e:	e9 87 f5 ff ff       	jmp    80105bca <alltraps>

80106643 <vector153>:
.globl vector153
vector153:
  pushl $0
80106643:	6a 00                	push   $0x0
  pushl $153
80106645:	68 99 00 00 00       	push   $0x99
  jmp alltraps
8010664a:	e9 7b f5 ff ff       	jmp    80105bca <alltraps>

8010664f <vector154>:
.globl vector154
vector154:
  pushl $0
8010664f:	6a 00                	push   $0x0
  pushl $154
80106651:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
80106656:	e9 6f f5 ff ff       	jmp    80105bca <alltraps>

8010665b <vector155>:
.globl vector155
vector155:
  pushl $0
8010665b:	6a 00                	push   $0x0
  pushl $155
8010665d:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
80106662:	e9 63 f5 ff ff       	jmp    80105bca <alltraps>

80106667 <vector156>:
.globl vector156
vector156:
  pushl $0
80106667:	6a 00                	push   $0x0
  pushl $156
80106669:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
8010666e:	e9 57 f5 ff ff       	jmp    80105bca <alltraps>

80106673 <vector157>:
.globl vector157
vector157:
  pushl $0
80106673:	6a 00                	push   $0x0
  pushl $157
80106675:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
8010667a:	e9 4b f5 ff ff       	jmp    80105bca <alltraps>

8010667f <vector158>:
.globl vector158
vector158:
  pushl $0
8010667f:	6a 00                	push   $0x0
  pushl $158
80106681:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
80106686:	e9 3f f5 ff ff       	jmp    80105bca <alltraps>

8010668b <vector159>:
.globl vector159
vector159:
  pushl $0
8010668b:	6a 00                	push   $0x0
  pushl $159
8010668d:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
80106692:	e9 33 f5 ff ff       	jmp    80105bca <alltraps>

80106697 <vector160>:
.globl vector160
vector160:
  pushl $0
80106697:	6a 00                	push   $0x0
  pushl $160
80106699:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
8010669e:	e9 27 f5 ff ff       	jmp    80105bca <alltraps>

801066a3 <vector161>:
.globl vector161
vector161:
  pushl $0
801066a3:	6a 00                	push   $0x0
  pushl $161
801066a5:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
801066aa:	e9 1b f5 ff ff       	jmp    80105bca <alltraps>

801066af <vector162>:
.globl vector162
vector162:
  pushl $0
801066af:	6a 00                	push   $0x0
  pushl $162
801066b1:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
801066b6:	e9 0f f5 ff ff       	jmp    80105bca <alltraps>

801066bb <vector163>:
.globl vector163
vector163:
  pushl $0
801066bb:	6a 00                	push   $0x0
  pushl $163
801066bd:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
801066c2:	e9 03 f5 ff ff       	jmp    80105bca <alltraps>

801066c7 <vector164>:
.globl vector164
vector164:
  pushl $0
801066c7:	6a 00                	push   $0x0
  pushl $164
801066c9:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
801066ce:	e9 f7 f4 ff ff       	jmp    80105bca <alltraps>

801066d3 <vector165>:
.globl vector165
vector165:
  pushl $0
801066d3:	6a 00                	push   $0x0
  pushl $165
801066d5:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
801066da:	e9 eb f4 ff ff       	jmp    80105bca <alltraps>

801066df <vector166>:
.globl vector166
vector166:
  pushl $0
801066df:	6a 00                	push   $0x0
  pushl $166
801066e1:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
801066e6:	e9 df f4 ff ff       	jmp    80105bca <alltraps>

801066eb <vector167>:
.globl vector167
vector167:
  pushl $0
801066eb:	6a 00                	push   $0x0
  pushl $167
801066ed:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
801066f2:	e9 d3 f4 ff ff       	jmp    80105bca <alltraps>

801066f7 <vector168>:
.globl vector168
vector168:
  pushl $0
801066f7:	6a 00                	push   $0x0
  pushl $168
801066f9:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
801066fe:	e9 c7 f4 ff ff       	jmp    80105bca <alltraps>

80106703 <vector169>:
.globl vector169
vector169:
  pushl $0
80106703:	6a 00                	push   $0x0
  pushl $169
80106705:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
8010670a:	e9 bb f4 ff ff       	jmp    80105bca <alltraps>

8010670f <vector170>:
.globl vector170
vector170:
  pushl $0
8010670f:	6a 00                	push   $0x0
  pushl $170
80106711:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
80106716:	e9 af f4 ff ff       	jmp    80105bca <alltraps>

8010671b <vector171>:
.globl vector171
vector171:
  pushl $0
8010671b:	6a 00                	push   $0x0
  pushl $171
8010671d:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
80106722:	e9 a3 f4 ff ff       	jmp    80105bca <alltraps>

80106727 <vector172>:
.globl vector172
vector172:
  pushl $0
80106727:	6a 00                	push   $0x0
  pushl $172
80106729:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
8010672e:	e9 97 f4 ff ff       	jmp    80105bca <alltraps>

80106733 <vector173>:
.globl vector173
vector173:
  pushl $0
80106733:	6a 00                	push   $0x0
  pushl $173
80106735:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
8010673a:	e9 8b f4 ff ff       	jmp    80105bca <alltraps>

8010673f <vector174>:
.globl vector174
vector174:
  pushl $0
8010673f:	6a 00                	push   $0x0
  pushl $174
80106741:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
80106746:	e9 7f f4 ff ff       	jmp    80105bca <alltraps>

8010674b <vector175>:
.globl vector175
vector175:
  pushl $0
8010674b:	6a 00                	push   $0x0
  pushl $175
8010674d:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
80106752:	e9 73 f4 ff ff       	jmp    80105bca <alltraps>

80106757 <vector176>:
.globl vector176
vector176:
  pushl $0
80106757:	6a 00                	push   $0x0
  pushl $176
80106759:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
8010675e:	e9 67 f4 ff ff       	jmp    80105bca <alltraps>

80106763 <vector177>:
.globl vector177
vector177:
  pushl $0
80106763:	6a 00                	push   $0x0
  pushl $177
80106765:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
8010676a:	e9 5b f4 ff ff       	jmp    80105bca <alltraps>

8010676f <vector178>:
.globl vector178
vector178:
  pushl $0
8010676f:	6a 00                	push   $0x0
  pushl $178
80106771:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
80106776:	e9 4f f4 ff ff       	jmp    80105bca <alltraps>

8010677b <vector179>:
.globl vector179
vector179:
  pushl $0
8010677b:	6a 00                	push   $0x0
  pushl $179
8010677d:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
80106782:	e9 43 f4 ff ff       	jmp    80105bca <alltraps>

80106787 <vector180>:
.globl vector180
vector180:
  pushl $0
80106787:	6a 00                	push   $0x0
  pushl $180
80106789:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
8010678e:	e9 37 f4 ff ff       	jmp    80105bca <alltraps>

80106793 <vector181>:
.globl vector181
vector181:
  pushl $0
80106793:	6a 00                	push   $0x0
  pushl $181
80106795:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
8010679a:	e9 2b f4 ff ff       	jmp    80105bca <alltraps>

8010679f <vector182>:
.globl vector182
vector182:
  pushl $0
8010679f:	6a 00                	push   $0x0
  pushl $182
801067a1:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
801067a6:	e9 1f f4 ff ff       	jmp    80105bca <alltraps>

801067ab <vector183>:
.globl vector183
vector183:
  pushl $0
801067ab:	6a 00                	push   $0x0
  pushl $183
801067ad:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
801067b2:	e9 13 f4 ff ff       	jmp    80105bca <alltraps>

801067b7 <vector184>:
.globl vector184
vector184:
  pushl $0
801067b7:	6a 00                	push   $0x0
  pushl $184
801067b9:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
801067be:	e9 07 f4 ff ff       	jmp    80105bca <alltraps>

801067c3 <vector185>:
.globl vector185
vector185:
  pushl $0
801067c3:	6a 00                	push   $0x0
  pushl $185
801067c5:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
801067ca:	e9 fb f3 ff ff       	jmp    80105bca <alltraps>

801067cf <vector186>:
.globl vector186
vector186:
  pushl $0
801067cf:	6a 00                	push   $0x0
  pushl $186
801067d1:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
801067d6:	e9 ef f3 ff ff       	jmp    80105bca <alltraps>

801067db <vector187>:
.globl vector187
vector187:
  pushl $0
801067db:	6a 00                	push   $0x0
  pushl $187
801067dd:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
801067e2:	e9 e3 f3 ff ff       	jmp    80105bca <alltraps>

801067e7 <vector188>:
.globl vector188
vector188:
  pushl $0
801067e7:	6a 00                	push   $0x0
  pushl $188
801067e9:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
801067ee:	e9 d7 f3 ff ff       	jmp    80105bca <alltraps>

801067f3 <vector189>:
.globl vector189
vector189:
  pushl $0
801067f3:	6a 00                	push   $0x0
  pushl $189
801067f5:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
801067fa:	e9 cb f3 ff ff       	jmp    80105bca <alltraps>

801067ff <vector190>:
.globl vector190
vector190:
  pushl $0
801067ff:	6a 00                	push   $0x0
  pushl $190
80106801:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
80106806:	e9 bf f3 ff ff       	jmp    80105bca <alltraps>

8010680b <vector191>:
.globl vector191
vector191:
  pushl $0
8010680b:	6a 00                	push   $0x0
  pushl $191
8010680d:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
80106812:	e9 b3 f3 ff ff       	jmp    80105bca <alltraps>

80106817 <vector192>:
.globl vector192
vector192:
  pushl $0
80106817:	6a 00                	push   $0x0
  pushl $192
80106819:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
8010681e:	e9 a7 f3 ff ff       	jmp    80105bca <alltraps>

80106823 <vector193>:
.globl vector193
vector193:
  pushl $0
80106823:	6a 00                	push   $0x0
  pushl $193
80106825:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
8010682a:	e9 9b f3 ff ff       	jmp    80105bca <alltraps>

8010682f <vector194>:
.globl vector194
vector194:
  pushl $0
8010682f:	6a 00                	push   $0x0
  pushl $194
80106831:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
80106836:	e9 8f f3 ff ff       	jmp    80105bca <alltraps>

8010683b <vector195>:
.globl vector195
vector195:
  pushl $0
8010683b:	6a 00                	push   $0x0
  pushl $195
8010683d:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
80106842:	e9 83 f3 ff ff       	jmp    80105bca <alltraps>

80106847 <vector196>:
.globl vector196
vector196:
  pushl $0
80106847:	6a 00                	push   $0x0
  pushl $196
80106849:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
8010684e:	e9 77 f3 ff ff       	jmp    80105bca <alltraps>

80106853 <vector197>:
.globl vector197
vector197:
  pushl $0
80106853:	6a 00                	push   $0x0
  pushl $197
80106855:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
8010685a:	e9 6b f3 ff ff       	jmp    80105bca <alltraps>

8010685f <vector198>:
.globl vector198
vector198:
  pushl $0
8010685f:	6a 00                	push   $0x0
  pushl $198
80106861:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
80106866:	e9 5f f3 ff ff       	jmp    80105bca <alltraps>

8010686b <vector199>:
.globl vector199
vector199:
  pushl $0
8010686b:	6a 00                	push   $0x0
  pushl $199
8010686d:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
80106872:	e9 53 f3 ff ff       	jmp    80105bca <alltraps>

80106877 <vector200>:
.globl vector200
vector200:
  pushl $0
80106877:	6a 00                	push   $0x0
  pushl $200
80106879:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
8010687e:	e9 47 f3 ff ff       	jmp    80105bca <alltraps>

80106883 <vector201>:
.globl vector201
vector201:
  pushl $0
80106883:	6a 00                	push   $0x0
  pushl $201
80106885:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
8010688a:	e9 3b f3 ff ff       	jmp    80105bca <alltraps>

8010688f <vector202>:
.globl vector202
vector202:
  pushl $0
8010688f:	6a 00                	push   $0x0
  pushl $202
80106891:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
80106896:	e9 2f f3 ff ff       	jmp    80105bca <alltraps>

8010689b <vector203>:
.globl vector203
vector203:
  pushl $0
8010689b:	6a 00                	push   $0x0
  pushl $203
8010689d:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
801068a2:	e9 23 f3 ff ff       	jmp    80105bca <alltraps>

801068a7 <vector204>:
.globl vector204
vector204:
  pushl $0
801068a7:	6a 00                	push   $0x0
  pushl $204
801068a9:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
801068ae:	e9 17 f3 ff ff       	jmp    80105bca <alltraps>

801068b3 <vector205>:
.globl vector205
vector205:
  pushl $0
801068b3:	6a 00                	push   $0x0
  pushl $205
801068b5:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
801068ba:	e9 0b f3 ff ff       	jmp    80105bca <alltraps>

801068bf <vector206>:
.globl vector206
vector206:
  pushl $0
801068bf:	6a 00                	push   $0x0
  pushl $206
801068c1:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
801068c6:	e9 ff f2 ff ff       	jmp    80105bca <alltraps>

801068cb <vector207>:
.globl vector207
vector207:
  pushl $0
801068cb:	6a 00                	push   $0x0
  pushl $207
801068cd:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
801068d2:	e9 f3 f2 ff ff       	jmp    80105bca <alltraps>

801068d7 <vector208>:
.globl vector208
vector208:
  pushl $0
801068d7:	6a 00                	push   $0x0
  pushl $208
801068d9:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
801068de:	e9 e7 f2 ff ff       	jmp    80105bca <alltraps>

801068e3 <vector209>:
.globl vector209
vector209:
  pushl $0
801068e3:	6a 00                	push   $0x0
  pushl $209
801068e5:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
801068ea:	e9 db f2 ff ff       	jmp    80105bca <alltraps>

801068ef <vector210>:
.globl vector210
vector210:
  pushl $0
801068ef:	6a 00                	push   $0x0
  pushl $210
801068f1:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
801068f6:	e9 cf f2 ff ff       	jmp    80105bca <alltraps>

801068fb <vector211>:
.globl vector211
vector211:
  pushl $0
801068fb:	6a 00                	push   $0x0
  pushl $211
801068fd:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
80106902:	e9 c3 f2 ff ff       	jmp    80105bca <alltraps>

80106907 <vector212>:
.globl vector212
vector212:
  pushl $0
80106907:	6a 00                	push   $0x0
  pushl $212
80106909:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
8010690e:	e9 b7 f2 ff ff       	jmp    80105bca <alltraps>

80106913 <vector213>:
.globl vector213
vector213:
  pushl $0
80106913:	6a 00                	push   $0x0
  pushl $213
80106915:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
8010691a:	e9 ab f2 ff ff       	jmp    80105bca <alltraps>

8010691f <vector214>:
.globl vector214
vector214:
  pushl $0
8010691f:	6a 00                	push   $0x0
  pushl $214
80106921:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
80106926:	e9 9f f2 ff ff       	jmp    80105bca <alltraps>

8010692b <vector215>:
.globl vector215
vector215:
  pushl $0
8010692b:	6a 00                	push   $0x0
  pushl $215
8010692d:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
80106932:	e9 93 f2 ff ff       	jmp    80105bca <alltraps>

80106937 <vector216>:
.globl vector216
vector216:
  pushl $0
80106937:	6a 00                	push   $0x0
  pushl $216
80106939:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
8010693e:	e9 87 f2 ff ff       	jmp    80105bca <alltraps>

80106943 <vector217>:
.globl vector217
vector217:
  pushl $0
80106943:	6a 00                	push   $0x0
  pushl $217
80106945:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
8010694a:	e9 7b f2 ff ff       	jmp    80105bca <alltraps>

8010694f <vector218>:
.globl vector218
vector218:
  pushl $0
8010694f:	6a 00                	push   $0x0
  pushl $218
80106951:	68 da 00 00 00       	push   $0xda
  jmp alltraps
80106956:	e9 6f f2 ff ff       	jmp    80105bca <alltraps>

8010695b <vector219>:
.globl vector219
vector219:
  pushl $0
8010695b:	6a 00                	push   $0x0
  pushl $219
8010695d:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
80106962:	e9 63 f2 ff ff       	jmp    80105bca <alltraps>

80106967 <vector220>:
.globl vector220
vector220:
  pushl $0
80106967:	6a 00                	push   $0x0
  pushl $220
80106969:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
8010696e:	e9 57 f2 ff ff       	jmp    80105bca <alltraps>

80106973 <vector221>:
.globl vector221
vector221:
  pushl $0
80106973:	6a 00                	push   $0x0
  pushl $221
80106975:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
8010697a:	e9 4b f2 ff ff       	jmp    80105bca <alltraps>

8010697f <vector222>:
.globl vector222
vector222:
  pushl $0
8010697f:	6a 00                	push   $0x0
  pushl $222
80106981:	68 de 00 00 00       	push   $0xde
  jmp alltraps
80106986:	e9 3f f2 ff ff       	jmp    80105bca <alltraps>

8010698b <vector223>:
.globl vector223
vector223:
  pushl $0
8010698b:	6a 00                	push   $0x0
  pushl $223
8010698d:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
80106992:	e9 33 f2 ff ff       	jmp    80105bca <alltraps>

80106997 <vector224>:
.globl vector224
vector224:
  pushl $0
80106997:	6a 00                	push   $0x0
  pushl $224
80106999:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
8010699e:	e9 27 f2 ff ff       	jmp    80105bca <alltraps>

801069a3 <vector225>:
.globl vector225
vector225:
  pushl $0
801069a3:	6a 00                	push   $0x0
  pushl $225
801069a5:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
801069aa:	e9 1b f2 ff ff       	jmp    80105bca <alltraps>

801069af <vector226>:
.globl vector226
vector226:
  pushl $0
801069af:	6a 00                	push   $0x0
  pushl $226
801069b1:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
801069b6:	e9 0f f2 ff ff       	jmp    80105bca <alltraps>

801069bb <vector227>:
.globl vector227
vector227:
  pushl $0
801069bb:	6a 00                	push   $0x0
  pushl $227
801069bd:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
801069c2:	e9 03 f2 ff ff       	jmp    80105bca <alltraps>

801069c7 <vector228>:
.globl vector228
vector228:
  pushl $0
801069c7:	6a 00                	push   $0x0
  pushl $228
801069c9:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
801069ce:	e9 f7 f1 ff ff       	jmp    80105bca <alltraps>

801069d3 <vector229>:
.globl vector229
vector229:
  pushl $0
801069d3:	6a 00                	push   $0x0
  pushl $229
801069d5:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
801069da:	e9 eb f1 ff ff       	jmp    80105bca <alltraps>

801069df <vector230>:
.globl vector230
vector230:
  pushl $0
801069df:	6a 00                	push   $0x0
  pushl $230
801069e1:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
801069e6:	e9 df f1 ff ff       	jmp    80105bca <alltraps>

801069eb <vector231>:
.globl vector231
vector231:
  pushl $0
801069eb:	6a 00                	push   $0x0
  pushl $231
801069ed:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
801069f2:	e9 d3 f1 ff ff       	jmp    80105bca <alltraps>

801069f7 <vector232>:
.globl vector232
vector232:
  pushl $0
801069f7:	6a 00                	push   $0x0
  pushl $232
801069f9:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
801069fe:	e9 c7 f1 ff ff       	jmp    80105bca <alltraps>

80106a03 <vector233>:
.globl vector233
vector233:
  pushl $0
80106a03:	6a 00                	push   $0x0
  pushl $233
80106a05:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
80106a0a:	e9 bb f1 ff ff       	jmp    80105bca <alltraps>

80106a0f <vector234>:
.globl vector234
vector234:
  pushl $0
80106a0f:	6a 00                	push   $0x0
  pushl $234
80106a11:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
80106a16:	e9 af f1 ff ff       	jmp    80105bca <alltraps>

80106a1b <vector235>:
.globl vector235
vector235:
  pushl $0
80106a1b:	6a 00                	push   $0x0
  pushl $235
80106a1d:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
80106a22:	e9 a3 f1 ff ff       	jmp    80105bca <alltraps>

80106a27 <vector236>:
.globl vector236
vector236:
  pushl $0
80106a27:	6a 00                	push   $0x0
  pushl $236
80106a29:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
80106a2e:	e9 97 f1 ff ff       	jmp    80105bca <alltraps>

80106a33 <vector237>:
.globl vector237
vector237:
  pushl $0
80106a33:	6a 00                	push   $0x0
  pushl $237
80106a35:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
80106a3a:	e9 8b f1 ff ff       	jmp    80105bca <alltraps>

80106a3f <vector238>:
.globl vector238
vector238:
  pushl $0
80106a3f:	6a 00                	push   $0x0
  pushl $238
80106a41:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
80106a46:	e9 7f f1 ff ff       	jmp    80105bca <alltraps>

80106a4b <vector239>:
.globl vector239
vector239:
  pushl $0
80106a4b:	6a 00                	push   $0x0
  pushl $239
80106a4d:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
80106a52:	e9 73 f1 ff ff       	jmp    80105bca <alltraps>

80106a57 <vector240>:
.globl vector240
vector240:
  pushl $0
80106a57:	6a 00                	push   $0x0
  pushl $240
80106a59:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
80106a5e:	e9 67 f1 ff ff       	jmp    80105bca <alltraps>

80106a63 <vector241>:
.globl vector241
vector241:
  pushl $0
80106a63:	6a 00                	push   $0x0
  pushl $241
80106a65:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
80106a6a:	e9 5b f1 ff ff       	jmp    80105bca <alltraps>

80106a6f <vector242>:
.globl vector242
vector242:
  pushl $0
80106a6f:	6a 00                	push   $0x0
  pushl $242
80106a71:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
80106a76:	e9 4f f1 ff ff       	jmp    80105bca <alltraps>

80106a7b <vector243>:
.globl vector243
vector243:
  pushl $0
80106a7b:	6a 00                	push   $0x0
  pushl $243
80106a7d:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
80106a82:	e9 43 f1 ff ff       	jmp    80105bca <alltraps>

80106a87 <vector244>:
.globl vector244
vector244:
  pushl $0
80106a87:	6a 00                	push   $0x0
  pushl $244
80106a89:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
80106a8e:	e9 37 f1 ff ff       	jmp    80105bca <alltraps>

80106a93 <vector245>:
.globl vector245
vector245:
  pushl $0
80106a93:	6a 00                	push   $0x0
  pushl $245
80106a95:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
80106a9a:	e9 2b f1 ff ff       	jmp    80105bca <alltraps>

80106a9f <vector246>:
.globl vector246
vector246:
  pushl $0
80106a9f:	6a 00                	push   $0x0
  pushl $246
80106aa1:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
80106aa6:	e9 1f f1 ff ff       	jmp    80105bca <alltraps>

80106aab <vector247>:
.globl vector247
vector247:
  pushl $0
80106aab:	6a 00                	push   $0x0
  pushl $247
80106aad:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
80106ab2:	e9 13 f1 ff ff       	jmp    80105bca <alltraps>

80106ab7 <vector248>:
.globl vector248
vector248:
  pushl $0
80106ab7:	6a 00                	push   $0x0
  pushl $248
80106ab9:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
80106abe:	e9 07 f1 ff ff       	jmp    80105bca <alltraps>

80106ac3 <vector249>:
.globl vector249
vector249:
  pushl $0
80106ac3:	6a 00                	push   $0x0
  pushl $249
80106ac5:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
80106aca:	e9 fb f0 ff ff       	jmp    80105bca <alltraps>

80106acf <vector250>:
.globl vector250
vector250:
  pushl $0
80106acf:	6a 00                	push   $0x0
  pushl $250
80106ad1:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
80106ad6:	e9 ef f0 ff ff       	jmp    80105bca <alltraps>

80106adb <vector251>:
.globl vector251
vector251:
  pushl $0
80106adb:	6a 00                	push   $0x0
  pushl $251
80106add:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
80106ae2:	e9 e3 f0 ff ff       	jmp    80105bca <alltraps>

80106ae7 <vector252>:
.globl vector252
vector252:
  pushl $0
80106ae7:	6a 00                	push   $0x0
  pushl $252
80106ae9:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
80106aee:	e9 d7 f0 ff ff       	jmp    80105bca <alltraps>

80106af3 <vector253>:
.globl vector253
vector253:
  pushl $0
80106af3:	6a 00                	push   $0x0
  pushl $253
80106af5:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
80106afa:	e9 cb f0 ff ff       	jmp    80105bca <alltraps>

80106aff <vector254>:
.globl vector254
vector254:
  pushl $0
80106aff:	6a 00                	push   $0x0
  pushl $254
80106b01:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
80106b06:	e9 bf f0 ff ff       	jmp    80105bca <alltraps>

80106b0b <vector255>:
.globl vector255
vector255:
  pushl $0
80106b0b:	6a 00                	push   $0x0
  pushl $255
80106b0d:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
80106b12:	e9 b3 f0 ff ff       	jmp    80105bca <alltraps>
80106b17:	66 90                	xchg   %ax,%ax
80106b19:	66 90                	xchg   %ax,%ax
80106b1b:	66 90                	xchg   %ax,%ax
80106b1d:	66 90                	xchg   %ax,%ax
80106b1f:	90                   	nop

80106b20 <walkpgdir>:

// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
static pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc) {
80106b20:	55                   	push   %ebp
80106b21:	89 e5                	mov    %esp,%ebp
80106b23:	57                   	push   %edi
80106b24:	56                   	push   %esi
80106b25:	53                   	push   %ebx
    pde_t *pde;
    pte_t *pgtab;

    pde = &pgdir[PDX(va)];
80106b26:	89 d3                	mov    %edx,%ebx
walkpgdir(pde_t *pgdir, const void *va, int alloc) {
80106b28:	89 d7                	mov    %edx,%edi
    pde = &pgdir[PDX(va)];
80106b2a:	c1 eb 16             	shr    $0x16,%ebx
80106b2d:	8d 34 98             	lea    (%eax,%ebx,4),%esi
walkpgdir(pde_t *pgdir, const void *va, int alloc) {
80106b30:	83 ec 0c             	sub    $0xc,%esp
    if (*pde & PTE_P) {
80106b33:	8b 06                	mov    (%esi),%eax
80106b35:	a8 01                	test   $0x1,%al
80106b37:	74 27                	je     80106b60 <walkpgdir+0x40>
        pgtab = (pte_t *) P2V(PTE_ADDR(*pde));
80106b39:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80106b3e:	8d 98 00 00 00 80    	lea    -0x80000000(%eax),%ebx
        // The permissions here are overly generous, but they can
        // be further restricted by the permissions in the page table
        // entries, if necessary.
        *pde = V2P(pgtab) | PTE_P | PTE_W | PTE_U;
    }
    return &pgtab[PTX(va)];
80106b44:	c1 ef 0a             	shr    $0xa,%edi
}
80106b47:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return &pgtab[PTX(va)];
80106b4a:	89 fa                	mov    %edi,%edx
80106b4c:	81 e2 fc 0f 00 00    	and    $0xffc,%edx
80106b52:	8d 04 13             	lea    (%ebx,%edx,1),%eax
}
80106b55:	5b                   	pop    %ebx
80106b56:	5e                   	pop    %esi
80106b57:	5f                   	pop    %edi
80106b58:	5d                   	pop    %ebp
80106b59:	c3                   	ret    
80106b5a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
        if (!alloc)
80106b60:	85 c9                	test   %ecx,%ecx
80106b62:	74 2c                	je     80106b90 <walkpgdir+0x70>
        if ((pgtab = (pte_t *) kalloc()) == 0)
80106b64:	e8 47 bd ff ff       	call   801028b0 <kalloc>
80106b69:	85 c0                	test   %eax,%eax
80106b6b:	89 c3                	mov    %eax,%ebx
80106b6d:	74 21                	je     80106b90 <walkpgdir+0x70>
        memset(pgtab, 0, PGSIZE);
80106b6f:	83 ec 04             	sub    $0x4,%esp
80106b72:	68 00 10 00 00       	push   $0x1000
80106b77:	6a 00                	push   $0x0
80106b79:	50                   	push   %eax
80106b7a:	e8 21 de ff ff       	call   801049a0 <memset>
        *pde = V2P(pgtab) | PTE_P | PTE_W | PTE_U;
80106b7f:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80106b85:	83 c4 10             	add    $0x10,%esp
80106b88:	83 c8 07             	or     $0x7,%eax
80106b8b:	89 06                	mov    %eax,(%esi)
80106b8d:	eb b5                	jmp    80106b44 <walkpgdir+0x24>
80106b8f:	90                   	nop
}
80106b90:	8d 65 f4             	lea    -0xc(%ebp),%esp
            return 0;
80106b93:	31 c0                	xor    %eax,%eax
}
80106b95:	5b                   	pop    %ebx
80106b96:	5e                   	pop    %esi
80106b97:	5f                   	pop    %edi
80106b98:	5d                   	pop    %ebp
80106b99:	c3                   	ret    
80106b9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80106ba0 <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
80106ba0:	55                   	push   %ebp
80106ba1:	89 e5                	mov    %esp,%ebp
80106ba3:	57                   	push   %edi
80106ba4:	56                   	push   %esi
80106ba5:	53                   	push   %ebx
  char *a, *last;
  pte_t *pte;

  a = (char*)PGROUNDDOWN((uint)va);
80106ba6:	89 d3                	mov    %edx,%ebx
80106ba8:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
{
80106bae:	83 ec 1c             	sub    $0x1c,%esp
80106bb1:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
80106bb4:	8d 44 0a ff          	lea    -0x1(%edx,%ecx,1),%eax
80106bb8:	8b 7d 08             	mov    0x8(%ebp),%edi
80106bbb:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80106bc0:	89 45 e0             	mov    %eax,-0x20(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
      return -1;
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
80106bc3:	8b 45 0c             	mov    0xc(%ebp),%eax
80106bc6:	29 df                	sub    %ebx,%edi
80106bc8:	83 c8 01             	or     $0x1,%eax
80106bcb:	89 45 dc             	mov    %eax,-0x24(%ebp)
80106bce:	eb 15                	jmp    80106be5 <mappages+0x45>
    if(*pte & PTE_P)
80106bd0:	f6 00 01             	testb  $0x1,(%eax)
80106bd3:	75 45                	jne    80106c1a <mappages+0x7a>
    *pte = pa | perm | PTE_P;
80106bd5:	0b 75 dc             	or     -0x24(%ebp),%esi
    if(a == last)
80106bd8:	3b 5d e0             	cmp    -0x20(%ebp),%ebx
    *pte = pa | perm | PTE_P;
80106bdb:	89 30                	mov    %esi,(%eax)
    if(a == last)
80106bdd:	74 31                	je     80106c10 <mappages+0x70>
      break;
    a += PGSIZE;
80106bdf:	81 c3 00 10 00 00    	add    $0x1000,%ebx
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
80106be5:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106be8:	b9 01 00 00 00       	mov    $0x1,%ecx
80106bed:	89 da                	mov    %ebx,%edx
80106bef:	8d 34 3b             	lea    (%ebx,%edi,1),%esi
80106bf2:	e8 29 ff ff ff       	call   80106b20 <walkpgdir>
80106bf7:	85 c0                	test   %eax,%eax
80106bf9:	75 d5                	jne    80106bd0 <mappages+0x30>
    pa += PGSIZE;
  }
  return 0;
}
80106bfb:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
80106bfe:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106c03:	5b                   	pop    %ebx
80106c04:	5e                   	pop    %esi
80106c05:	5f                   	pop    %edi
80106c06:	5d                   	pop    %ebp
80106c07:	c3                   	ret    
80106c08:	90                   	nop
80106c09:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106c10:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80106c13:	31 c0                	xor    %eax,%eax
}
80106c15:	5b                   	pop    %ebx
80106c16:	5e                   	pop    %esi
80106c17:	5f                   	pop    %edi
80106c18:	5d                   	pop    %ebp
80106c19:	c3                   	ret    
      panic("remap");
80106c1a:	83 ec 0c             	sub    $0xc,%esp
80106c1d:	68 68 7e 10 80       	push   $0x80107e68
80106c22:	e8 69 97 ff ff       	call   80100390 <panic>
80106c27:	89 f6                	mov    %esi,%esi
80106c29:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80106c30 <deallocuvm.part.0>:
// Deallocate user pages to bring the process size from oldsz to
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
80106c30:	55                   	push   %ebp
80106c31:	89 e5                	mov    %esp,%ebp
80106c33:	57                   	push   %edi
80106c34:	56                   	push   %esi
80106c35:	53                   	push   %ebx
  uint a, pa;

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
80106c36:	8d 99 ff 0f 00 00    	lea    0xfff(%ecx),%ebx
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
80106c3c:	89 c7                	mov    %eax,%edi
  a = PGROUNDUP(newsz);
80106c3e:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
80106c44:	83 ec 1c             	sub    $0x1c,%esp
80106c47:	89 4d e0             	mov    %ecx,-0x20(%ebp)
  for(; a  < oldsz; a += PGSIZE){
80106c4a:	39 d3                	cmp    %edx,%ebx
80106c4c:	73 66                	jae    80106cb4 <deallocuvm.part.0+0x84>
80106c4e:	89 d6                	mov    %edx,%esi
80106c50:	eb 3d                	jmp    80106c8f <deallocuvm.part.0+0x5f>
80106c52:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    pte = walkpgdir(pgdir, (char*)a, 0);
    if(!pte)
      a = PGADDR(PDX(a) + 1, 0, 0) - PGSIZE;
    else if((*pte & PTE_P) != 0){
80106c58:	8b 10                	mov    (%eax),%edx
80106c5a:	f6 c2 01             	test   $0x1,%dl
80106c5d:	74 26                	je     80106c85 <deallocuvm.part.0+0x55>
      pa = PTE_ADDR(*pte);
      if(pa == 0)
80106c5f:	81 e2 00 f0 ff ff    	and    $0xfffff000,%edx
80106c65:	74 58                	je     80106cbf <deallocuvm.part.0+0x8f>
        panic("kfree");
      char *v = P2V(pa);
      kfree(v);
80106c67:	83 ec 0c             	sub    $0xc,%esp
      char *v = P2V(pa);
80106c6a:	81 c2 00 00 00 80    	add    $0x80000000,%edx
80106c70:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      kfree(v);
80106c73:	52                   	push   %edx
80106c74:	e8 87 ba ff ff       	call   80102700 <kfree>
      *pte = 0;
80106c79:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106c7c:	83 c4 10             	add    $0x10,%esp
80106c7f:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  for(; a  < oldsz; a += PGSIZE){
80106c85:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80106c8b:	39 f3                	cmp    %esi,%ebx
80106c8d:	73 25                	jae    80106cb4 <deallocuvm.part.0+0x84>
    pte = walkpgdir(pgdir, (char*)a, 0);
80106c8f:	31 c9                	xor    %ecx,%ecx
80106c91:	89 da                	mov    %ebx,%edx
80106c93:	89 f8                	mov    %edi,%eax
80106c95:	e8 86 fe ff ff       	call   80106b20 <walkpgdir>
    if(!pte)
80106c9a:	85 c0                	test   %eax,%eax
80106c9c:	75 ba                	jne    80106c58 <deallocuvm.part.0+0x28>
      a = PGADDR(PDX(a) + 1, 0, 0) - PGSIZE;
80106c9e:	81 e3 00 00 c0 ff    	and    $0xffc00000,%ebx
80106ca4:	81 c3 00 f0 3f 00    	add    $0x3ff000,%ebx
  for(; a  < oldsz; a += PGSIZE){
80106caa:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80106cb0:	39 f3                	cmp    %esi,%ebx
80106cb2:	72 db                	jb     80106c8f <deallocuvm.part.0+0x5f>
    }
  }
  return newsz;
}
80106cb4:	8b 45 e0             	mov    -0x20(%ebp),%eax
80106cb7:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106cba:	5b                   	pop    %ebx
80106cbb:	5e                   	pop    %esi
80106cbc:	5f                   	pop    %edi
80106cbd:	5d                   	pop    %ebp
80106cbe:	c3                   	ret    
        panic("kfree");
80106cbf:	83 ec 0c             	sub    $0xc,%esp
80106cc2:	68 16 77 10 80       	push   $0x80107716
80106cc7:	e8 c4 96 ff ff       	call   80100390 <panic>
80106ccc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi

80106cd0 <seginit>:
{
80106cd0:	55                   	push   %ebp
80106cd1:	89 e5                	mov    %esp,%ebp
80106cd3:	83 ec 18             	sub    $0x18,%esp
  c = &cpus[cpuid()];
80106cd6:	e8 05 cf ff ff       	call   80103be0 <cpuid>
80106cdb:	69 c0 b0 00 00 00    	imul   $0xb0,%eax,%eax
  pd[0] = size-1;
80106ce1:	ba 2f 00 00 00       	mov    $0x2f,%edx
80106ce6:	66 89 55 f2          	mov    %dx,-0xe(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
80106cea:	c7 80 f8 27 11 80 ff 	movl   $0xffff,-0x7feed808(%eax)
80106cf1:	ff 00 00 
80106cf4:	c7 80 fc 27 11 80 00 	movl   $0xcf9a00,-0x7feed804(%eax)
80106cfb:	9a cf 00 
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
80106cfe:	c7 80 00 28 11 80 ff 	movl   $0xffff,-0x7feed800(%eax)
80106d05:	ff 00 00 
80106d08:	c7 80 04 28 11 80 00 	movl   $0xcf9200,-0x7feed7fc(%eax)
80106d0f:	92 cf 00 
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
80106d12:	c7 80 08 28 11 80 ff 	movl   $0xffff,-0x7feed7f8(%eax)
80106d19:	ff 00 00 
80106d1c:	c7 80 0c 28 11 80 00 	movl   $0xcffa00,-0x7feed7f4(%eax)
80106d23:	fa cf 00 
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
80106d26:	c7 80 10 28 11 80 ff 	movl   $0xffff,-0x7feed7f0(%eax)
80106d2d:	ff 00 00 
80106d30:	c7 80 14 28 11 80 00 	movl   $0xcff200,-0x7feed7ec(%eax)
80106d37:	f2 cf 00 
  lgdt(c->gdt, sizeof(c->gdt));
80106d3a:	05 f0 27 11 80       	add    $0x801127f0,%eax
  pd[1] = (uint)p;
80106d3f:	66 89 45 f4          	mov    %ax,-0xc(%ebp)
  pd[2] = (uint)p >> 16;
80106d43:	c1 e8 10             	shr    $0x10,%eax
80106d46:	66 89 45 f6          	mov    %ax,-0xa(%ebp)
  asm volatile("lgdt (%0)" : : "r" (pd));
80106d4a:	8d 45 f2             	lea    -0xe(%ebp),%eax
80106d4d:	0f 01 10             	lgdtl  (%eax)
}
80106d50:	c9                   	leave  
80106d51:	c3                   	ret    
80106d52:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106d59:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80106d60 <switchkvm>:
  lcr3(V2P(kpgdir));   // switch to the kernel page table
80106d60:	a1 a4 65 11 80       	mov    0x801165a4,%eax
{
80106d65:	55                   	push   %ebp
80106d66:	89 e5                	mov    %esp,%ebp
  lcr3(V2P(kpgdir));   // switch to the kernel page table
80106d68:	05 00 00 00 80       	add    $0x80000000,%eax
}

static inline void
lcr3(uint val)
{
  asm volatile("movl %0,%%cr3" : : "r" (val));
80106d6d:	0f 22 d8             	mov    %eax,%cr3
}
80106d70:	5d                   	pop    %ebp
80106d71:	c3                   	ret    
80106d72:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106d79:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80106d80 <switchuvm>:
{
80106d80:	55                   	push   %ebp
80106d81:	89 e5                	mov    %esp,%ebp
80106d83:	57                   	push   %edi
80106d84:	56                   	push   %esi
80106d85:	53                   	push   %ebx
80106d86:	83 ec 1c             	sub    $0x1c,%esp
80106d89:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if(p == 0)
80106d8c:	85 db                	test   %ebx,%ebx
80106d8e:	0f 84 cb 00 00 00    	je     80106e5f <switchuvm+0xdf>
  if(p->kstack == 0)
80106d94:	8b 43 08             	mov    0x8(%ebx),%eax
80106d97:	85 c0                	test   %eax,%eax
80106d99:	0f 84 da 00 00 00    	je     80106e79 <switchuvm+0xf9>
  if(p->pgdir == 0)
80106d9f:	8b 43 04             	mov    0x4(%ebx),%eax
80106da2:	85 c0                	test   %eax,%eax
80106da4:	0f 84 c2 00 00 00    	je     80106e6c <switchuvm+0xec>
  pushcli();
80106daa:	e8 31 da ff ff       	call   801047e0 <pushcli>
  mycpu()->gdt[SEG_TSS] = SEG16(STS_T32A, &mycpu()->ts,
80106daf:	e8 ac cd ff ff       	call   80103b60 <mycpu>
80106db4:	89 c6                	mov    %eax,%esi
80106db6:	e8 a5 cd ff ff       	call   80103b60 <mycpu>
80106dbb:	89 c7                	mov    %eax,%edi
80106dbd:	e8 9e cd ff ff       	call   80103b60 <mycpu>
80106dc2:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80106dc5:	83 c7 08             	add    $0x8,%edi
80106dc8:	e8 93 cd ff ff       	call   80103b60 <mycpu>
80106dcd:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80106dd0:	83 c0 08             	add    $0x8,%eax
80106dd3:	ba 67 00 00 00       	mov    $0x67,%edx
80106dd8:	c1 e8 18             	shr    $0x18,%eax
80106ddb:	66 89 96 98 00 00 00 	mov    %dx,0x98(%esi)
80106de2:	66 89 be 9a 00 00 00 	mov    %di,0x9a(%esi)
80106de9:	88 86 9f 00 00 00    	mov    %al,0x9f(%esi)
  mycpu()->ts.iomb = (ushort) 0xFFFF;
80106def:	bf ff ff ff ff       	mov    $0xffffffff,%edi
  mycpu()->gdt[SEG_TSS] = SEG16(STS_T32A, &mycpu()->ts,
80106df4:	83 c1 08             	add    $0x8,%ecx
80106df7:	c1 e9 10             	shr    $0x10,%ecx
80106dfa:	88 8e 9c 00 00 00    	mov    %cl,0x9c(%esi)
80106e00:	b9 99 40 00 00       	mov    $0x4099,%ecx
80106e05:	66 89 8e 9d 00 00 00 	mov    %cx,0x9d(%esi)
  mycpu()->ts.ss0 = SEG_KDATA << 3;
80106e0c:	be 10 00 00 00       	mov    $0x10,%esi
  mycpu()->gdt[SEG_TSS].s = 0;
80106e11:	e8 4a cd ff ff       	call   80103b60 <mycpu>
80106e16:	80 a0 9d 00 00 00 ef 	andb   $0xef,0x9d(%eax)
  mycpu()->ts.ss0 = SEG_KDATA << 3;
80106e1d:	e8 3e cd ff ff       	call   80103b60 <mycpu>
80106e22:	66 89 70 10          	mov    %si,0x10(%eax)
  mycpu()->ts.esp0 = (uint)p->kstack + KSTACKSIZE;
80106e26:	8b 73 08             	mov    0x8(%ebx),%esi
80106e29:	e8 32 cd ff ff       	call   80103b60 <mycpu>
80106e2e:	81 c6 00 10 00 00    	add    $0x1000,%esi
80106e34:	89 70 0c             	mov    %esi,0xc(%eax)
  mycpu()->ts.iomb = (ushort) 0xFFFF;
80106e37:	e8 24 cd ff ff       	call   80103b60 <mycpu>
80106e3c:	66 89 78 6e          	mov    %di,0x6e(%eax)
  asm volatile("ltr %0" : : "r" (sel));
80106e40:	b8 28 00 00 00       	mov    $0x28,%eax
80106e45:	0f 00 d8             	ltr    %ax
  lcr3(V2P(p->pgdir));  // switch to process's address space
80106e48:	8b 43 04             	mov    0x4(%ebx),%eax
80106e4b:	05 00 00 00 80       	add    $0x80000000,%eax
  asm volatile("movl %0,%%cr3" : : "r" (val));
80106e50:	0f 22 d8             	mov    %eax,%cr3
}
80106e53:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106e56:	5b                   	pop    %ebx
80106e57:	5e                   	pop    %esi
80106e58:	5f                   	pop    %edi
80106e59:	5d                   	pop    %ebp
  popcli();
80106e5a:	e9 81 da ff ff       	jmp    801048e0 <popcli>
    panic("switchuvm: no process");
80106e5f:	83 ec 0c             	sub    $0xc,%esp
80106e62:	68 6e 7e 10 80       	push   $0x80107e6e
80106e67:	e8 24 95 ff ff       	call   80100390 <panic>
    panic("switchuvm: no pgdir");
80106e6c:	83 ec 0c             	sub    $0xc,%esp
80106e6f:	68 99 7e 10 80       	push   $0x80107e99
80106e74:	e8 17 95 ff ff       	call   80100390 <panic>
    panic("switchuvm: no kstack");
80106e79:	83 ec 0c             	sub    $0xc,%esp
80106e7c:	68 84 7e 10 80       	push   $0x80107e84
80106e81:	e8 0a 95 ff ff       	call   80100390 <panic>
80106e86:	8d 76 00             	lea    0x0(%esi),%esi
80106e89:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80106e90 <inituvm>:
{
80106e90:	55                   	push   %ebp
80106e91:	89 e5                	mov    %esp,%ebp
80106e93:	57                   	push   %edi
80106e94:	56                   	push   %esi
80106e95:	53                   	push   %ebx
80106e96:	83 ec 1c             	sub    $0x1c,%esp
80106e99:	8b 75 10             	mov    0x10(%ebp),%esi
80106e9c:	8b 45 08             	mov    0x8(%ebp),%eax
80106e9f:	8b 7d 0c             	mov    0xc(%ebp),%edi
  if(sz >= PGSIZE)
80106ea2:	81 fe ff 0f 00 00    	cmp    $0xfff,%esi
{
80106ea8:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  if(sz >= PGSIZE)
80106eab:	77 49                	ja     80106ef6 <inituvm+0x66>
  mem = kalloc();
80106ead:	e8 fe b9 ff ff       	call   801028b0 <kalloc>
  memset(mem, 0, PGSIZE);
80106eb2:	83 ec 04             	sub    $0x4,%esp
  mem = kalloc();
80106eb5:	89 c3                	mov    %eax,%ebx
  memset(mem, 0, PGSIZE);
80106eb7:	68 00 10 00 00       	push   $0x1000
80106ebc:	6a 00                	push   $0x0
80106ebe:	50                   	push   %eax
80106ebf:	e8 dc da ff ff       	call   801049a0 <memset>
  mappages(pgdir, 0, PGSIZE, V2P(mem), PTE_W|PTE_U);
80106ec4:	58                   	pop    %eax
80106ec5:	8d 83 00 00 00 80    	lea    -0x80000000(%ebx),%eax
80106ecb:	b9 00 10 00 00       	mov    $0x1000,%ecx
80106ed0:	5a                   	pop    %edx
80106ed1:	6a 06                	push   $0x6
80106ed3:	50                   	push   %eax
80106ed4:	31 d2                	xor    %edx,%edx
80106ed6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106ed9:	e8 c2 fc ff ff       	call   80106ba0 <mappages>
  memmove(mem, init, sz);
80106ede:	89 75 10             	mov    %esi,0x10(%ebp)
80106ee1:	89 7d 0c             	mov    %edi,0xc(%ebp)
80106ee4:	83 c4 10             	add    $0x10,%esp
80106ee7:	89 5d 08             	mov    %ebx,0x8(%ebp)
}
80106eea:	8d 65 f4             	lea    -0xc(%ebp),%esp
80106eed:	5b                   	pop    %ebx
80106eee:	5e                   	pop    %esi
80106eef:	5f                   	pop    %edi
80106ef0:	5d                   	pop    %ebp
  memmove(mem, init, sz);
80106ef1:	e9 5a db ff ff       	jmp    80104a50 <memmove>
    panic("inituvm: more than a page");
80106ef6:	83 ec 0c             	sub    $0xc,%esp
80106ef9:	68 ad 7e 10 80       	push   $0x80107ead
80106efe:	e8 8d 94 ff ff       	call   80100390 <panic>
80106f03:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80106f09:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80106f10 <loaduvm>:
{
80106f10:	55                   	push   %ebp
80106f11:	89 e5                	mov    %esp,%ebp
80106f13:	57                   	push   %edi
80106f14:	56                   	push   %esi
80106f15:	53                   	push   %ebx
80106f16:	83 ec 0c             	sub    $0xc,%esp
  if((uint) addr % PGSIZE != 0)
80106f19:	f7 45 0c ff 0f 00 00 	testl  $0xfff,0xc(%ebp)
80106f20:	0f 85 91 00 00 00    	jne    80106fb7 <loaduvm+0xa7>
  for(i = 0; i < sz; i += PGSIZE){
80106f26:	8b 75 18             	mov    0x18(%ebp),%esi
80106f29:	31 db                	xor    %ebx,%ebx
80106f2b:	85 f6                	test   %esi,%esi
80106f2d:	75 1a                	jne    80106f49 <loaduvm+0x39>
80106f2f:	eb 6f                	jmp    80106fa0 <loaduvm+0x90>
80106f31:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80106f38:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80106f3e:	81 ee 00 10 00 00    	sub    $0x1000,%esi
80106f44:	39 5d 18             	cmp    %ebx,0x18(%ebp)
80106f47:	76 57                	jbe    80106fa0 <loaduvm+0x90>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
80106f49:	8b 55 0c             	mov    0xc(%ebp),%edx
80106f4c:	8b 45 08             	mov    0x8(%ebp),%eax
80106f4f:	31 c9                	xor    %ecx,%ecx
80106f51:	01 da                	add    %ebx,%edx
80106f53:	e8 c8 fb ff ff       	call   80106b20 <walkpgdir>
80106f58:	85 c0                	test   %eax,%eax
80106f5a:	74 4e                	je     80106faa <loaduvm+0x9a>
    pa = PTE_ADDR(*pte);
80106f5c:	8b 00                	mov    (%eax),%eax
    if(readi(ip, P2V(pa), offset+i, n) != n)
80106f5e:	8b 4d 14             	mov    0x14(%ebp),%ecx
    if(sz - i < PGSIZE)
80106f61:	bf 00 10 00 00       	mov    $0x1000,%edi
    pa = PTE_ADDR(*pte);
80106f66:	25 00 f0 ff ff       	and    $0xfffff000,%eax
    if(sz - i < PGSIZE)
80106f6b:	81 fe ff 0f 00 00    	cmp    $0xfff,%esi
80106f71:	0f 46 fe             	cmovbe %esi,%edi
    if(readi(ip, P2V(pa), offset+i, n) != n)
80106f74:	01 d9                	add    %ebx,%ecx
80106f76:	05 00 00 00 80       	add    $0x80000000,%eax
80106f7b:	57                   	push   %edi
80106f7c:	51                   	push   %ecx
80106f7d:	50                   	push   %eax
80106f7e:	ff 75 10             	pushl  0x10(%ebp)
80106f81:	e8 3a aa ff ff       	call   801019c0 <readi>
80106f86:	83 c4 10             	add    $0x10,%esp
80106f89:	39 f8                	cmp    %edi,%eax
80106f8b:	74 ab                	je     80106f38 <loaduvm+0x28>
}
80106f8d:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
80106f90:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80106f95:	5b                   	pop    %ebx
80106f96:	5e                   	pop    %esi
80106f97:	5f                   	pop    %edi
80106f98:	5d                   	pop    %ebp
80106f99:	c3                   	ret    
80106f9a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80106fa0:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
80106fa3:	31 c0                	xor    %eax,%eax
}
80106fa5:	5b                   	pop    %ebx
80106fa6:	5e                   	pop    %esi
80106fa7:	5f                   	pop    %edi
80106fa8:	5d                   	pop    %ebp
80106fa9:	c3                   	ret    
      panic("loaduvm: address should exist");
80106faa:	83 ec 0c             	sub    $0xc,%esp
80106fad:	68 c7 7e 10 80       	push   $0x80107ec7
80106fb2:	e8 d9 93 ff ff       	call   80100390 <panic>
    panic("loaduvm: addr must be page aligned");
80106fb7:	83 ec 0c             	sub    $0xc,%esp
80106fba:	68 68 7f 10 80       	push   $0x80107f68
80106fbf:	e8 cc 93 ff ff       	call   80100390 <panic>
80106fc4:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
80106fca:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80106fd0 <allocuvm>:
{
80106fd0:	55                   	push   %ebp
80106fd1:	89 e5                	mov    %esp,%ebp
80106fd3:	57                   	push   %edi
80106fd4:	56                   	push   %esi
80106fd5:	53                   	push   %ebx
80106fd6:	83 ec 1c             	sub    $0x1c,%esp
  if(newsz >= KERNBASE)
80106fd9:	8b 7d 10             	mov    0x10(%ebp),%edi
80106fdc:	85 ff                	test   %edi,%edi
80106fde:	0f 88 8e 00 00 00    	js     80107072 <allocuvm+0xa2>
  if(newsz < oldsz)
80106fe4:	3b 7d 0c             	cmp    0xc(%ebp),%edi
80106fe7:	0f 82 93 00 00 00    	jb     80107080 <allocuvm+0xb0>
  a = PGROUNDUP(oldsz);
80106fed:	8b 45 0c             	mov    0xc(%ebp),%eax
80106ff0:	8d 98 ff 0f 00 00    	lea    0xfff(%eax),%ebx
80106ff6:	81 e3 00 f0 ff ff    	and    $0xfffff000,%ebx
  for(; a < newsz; a += PGSIZE){
80106ffc:	39 5d 10             	cmp    %ebx,0x10(%ebp)
80106fff:	0f 86 7e 00 00 00    	jbe    80107083 <allocuvm+0xb3>
80107005:	89 7d e4             	mov    %edi,-0x1c(%ebp)
80107008:	8b 7d 08             	mov    0x8(%ebp),%edi
8010700b:	eb 42                	jmp    8010704f <allocuvm+0x7f>
8010700d:	8d 76 00             	lea    0x0(%esi),%esi
    memset(mem, 0, PGSIZE);
80107010:	83 ec 04             	sub    $0x4,%esp
80107013:	68 00 10 00 00       	push   $0x1000
80107018:	6a 00                	push   $0x0
8010701a:	50                   	push   %eax
8010701b:	e8 80 d9 ff ff       	call   801049a0 <memset>
    if(mappages(pgdir, (char*)a, PGSIZE, V2P(mem), PTE_W|PTE_U) < 0){
80107020:	58                   	pop    %eax
80107021:	8d 86 00 00 00 80    	lea    -0x80000000(%esi),%eax
80107027:	b9 00 10 00 00       	mov    $0x1000,%ecx
8010702c:	5a                   	pop    %edx
8010702d:	6a 06                	push   $0x6
8010702f:	50                   	push   %eax
80107030:	89 da                	mov    %ebx,%edx
80107032:	89 f8                	mov    %edi,%eax
80107034:	e8 67 fb ff ff       	call   80106ba0 <mappages>
80107039:	83 c4 10             	add    $0x10,%esp
8010703c:	85 c0                	test   %eax,%eax
8010703e:	78 50                	js     80107090 <allocuvm+0xc0>
  for(; a < newsz; a += PGSIZE){
80107040:	81 c3 00 10 00 00    	add    $0x1000,%ebx
80107046:	39 5d 10             	cmp    %ebx,0x10(%ebp)
80107049:	0f 86 81 00 00 00    	jbe    801070d0 <allocuvm+0x100>
    mem = kalloc();
8010704f:	e8 5c b8 ff ff       	call   801028b0 <kalloc>
    if(mem == 0){
80107054:	85 c0                	test   %eax,%eax
    mem = kalloc();
80107056:	89 c6                	mov    %eax,%esi
    if(mem == 0){
80107058:	75 b6                	jne    80107010 <allocuvm+0x40>
      cprintf("allocuvm out of memory\n");
8010705a:	83 ec 0c             	sub    $0xc,%esp
8010705d:	68 e5 7e 10 80       	push   $0x80107ee5
80107062:	e8 f9 95 ff ff       	call   80100660 <cprintf>
  if(newsz >= oldsz)
80107067:	83 c4 10             	add    $0x10,%esp
8010706a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010706d:	39 45 10             	cmp    %eax,0x10(%ebp)
80107070:	77 6e                	ja     801070e0 <allocuvm+0x110>
}
80107072:	8d 65 f4             	lea    -0xc(%ebp),%esp
    return 0;
80107075:	31 ff                	xor    %edi,%edi
}
80107077:	89 f8                	mov    %edi,%eax
80107079:	5b                   	pop    %ebx
8010707a:	5e                   	pop    %esi
8010707b:	5f                   	pop    %edi
8010707c:	5d                   	pop    %ebp
8010707d:	c3                   	ret    
8010707e:	66 90                	xchg   %ax,%ax
    return oldsz;
80107080:	8b 7d 0c             	mov    0xc(%ebp),%edi
}
80107083:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107086:	89 f8                	mov    %edi,%eax
80107088:	5b                   	pop    %ebx
80107089:	5e                   	pop    %esi
8010708a:	5f                   	pop    %edi
8010708b:	5d                   	pop    %ebp
8010708c:	c3                   	ret    
8010708d:	8d 76 00             	lea    0x0(%esi),%esi
      cprintf("allocuvm out of memory (2)\n");
80107090:	83 ec 0c             	sub    $0xc,%esp
80107093:	68 fd 7e 10 80       	push   $0x80107efd
80107098:	e8 c3 95 ff ff       	call   80100660 <cprintf>
  if(newsz >= oldsz)
8010709d:	83 c4 10             	add    $0x10,%esp
801070a0:	8b 45 0c             	mov    0xc(%ebp),%eax
801070a3:	39 45 10             	cmp    %eax,0x10(%ebp)
801070a6:	76 0d                	jbe    801070b5 <allocuvm+0xe5>
801070a8:	89 c1                	mov    %eax,%ecx
801070aa:	8b 55 10             	mov    0x10(%ebp),%edx
801070ad:	8b 45 08             	mov    0x8(%ebp),%eax
801070b0:	e8 7b fb ff ff       	call   80106c30 <deallocuvm.part.0>
      kfree(mem);
801070b5:	83 ec 0c             	sub    $0xc,%esp
      return 0;
801070b8:	31 ff                	xor    %edi,%edi
      kfree(mem);
801070ba:	56                   	push   %esi
801070bb:	e8 40 b6 ff ff       	call   80102700 <kfree>
      return 0;
801070c0:	83 c4 10             	add    $0x10,%esp
}
801070c3:	8d 65 f4             	lea    -0xc(%ebp),%esp
801070c6:	89 f8                	mov    %edi,%eax
801070c8:	5b                   	pop    %ebx
801070c9:	5e                   	pop    %esi
801070ca:	5f                   	pop    %edi
801070cb:	5d                   	pop    %ebp
801070cc:	c3                   	ret    
801070cd:	8d 76 00             	lea    0x0(%esi),%esi
801070d0:	8b 7d e4             	mov    -0x1c(%ebp),%edi
801070d3:	8d 65 f4             	lea    -0xc(%ebp),%esp
801070d6:	5b                   	pop    %ebx
801070d7:	89 f8                	mov    %edi,%eax
801070d9:	5e                   	pop    %esi
801070da:	5f                   	pop    %edi
801070db:	5d                   	pop    %ebp
801070dc:	c3                   	ret    
801070dd:	8d 76 00             	lea    0x0(%esi),%esi
801070e0:	89 c1                	mov    %eax,%ecx
801070e2:	8b 55 10             	mov    0x10(%ebp),%edx
801070e5:	8b 45 08             	mov    0x8(%ebp),%eax
      return 0;
801070e8:	31 ff                	xor    %edi,%edi
801070ea:	e8 41 fb ff ff       	call   80106c30 <deallocuvm.part.0>
801070ef:	eb 92                	jmp    80107083 <allocuvm+0xb3>
801070f1:	eb 0d                	jmp    80107100 <deallocuvm>
801070f3:	90                   	nop
801070f4:	90                   	nop
801070f5:	90                   	nop
801070f6:	90                   	nop
801070f7:	90                   	nop
801070f8:	90                   	nop
801070f9:	90                   	nop
801070fa:	90                   	nop
801070fb:	90                   	nop
801070fc:	90                   	nop
801070fd:	90                   	nop
801070fe:	90                   	nop
801070ff:	90                   	nop

80107100 <deallocuvm>:
{
80107100:	55                   	push   %ebp
80107101:	89 e5                	mov    %esp,%ebp
80107103:	8b 55 0c             	mov    0xc(%ebp),%edx
80107106:	8b 4d 10             	mov    0x10(%ebp),%ecx
80107109:	8b 45 08             	mov    0x8(%ebp),%eax
  if(newsz >= oldsz)
8010710c:	39 d1                	cmp    %edx,%ecx
8010710e:	73 10                	jae    80107120 <deallocuvm+0x20>
}
80107110:	5d                   	pop    %ebp
80107111:	e9 1a fb ff ff       	jmp    80106c30 <deallocuvm.part.0>
80107116:	8d 76 00             	lea    0x0(%esi),%esi
80107119:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi
80107120:	89 d0                	mov    %edx,%eax
80107122:	5d                   	pop    %ebp
80107123:	c3                   	ret    
80107124:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
8010712a:	8d bf 00 00 00 00    	lea    0x0(%edi),%edi

80107130 <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
80107130:	55                   	push   %ebp
80107131:	89 e5                	mov    %esp,%ebp
80107133:	57                   	push   %edi
80107134:	56                   	push   %esi
80107135:	53                   	push   %ebx
80107136:	83 ec 0c             	sub    $0xc,%esp
80107139:	8b 75 08             	mov    0x8(%ebp),%esi
  uint i;

  if(pgdir == 0)
8010713c:	85 f6                	test   %esi,%esi
8010713e:	74 59                	je     80107199 <freevm+0x69>
80107140:	31 c9                	xor    %ecx,%ecx
80107142:	ba 00 00 00 80       	mov    $0x80000000,%edx
80107147:	89 f0                	mov    %esi,%eax
80107149:	e8 e2 fa ff ff       	call   80106c30 <deallocuvm.part.0>
8010714e:	89 f3                	mov    %esi,%ebx
80107150:	8d be 00 10 00 00    	lea    0x1000(%esi),%edi
80107156:	eb 0f                	jmp    80107167 <freevm+0x37>
80107158:	90                   	nop
80107159:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107160:	83 c3 04             	add    $0x4,%ebx
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
80107163:	39 fb                	cmp    %edi,%ebx
80107165:	74 23                	je     8010718a <freevm+0x5a>
    if(pgdir[i] & PTE_P){
80107167:	8b 03                	mov    (%ebx),%eax
80107169:	a8 01                	test   $0x1,%al
8010716b:	74 f3                	je     80107160 <freevm+0x30>
      char * v = P2V(PTE_ADDR(pgdir[i]));
8010716d:	25 00 f0 ff ff       	and    $0xfffff000,%eax
      kfree(v);
80107172:	83 ec 0c             	sub    $0xc,%esp
80107175:	83 c3 04             	add    $0x4,%ebx
      char * v = P2V(PTE_ADDR(pgdir[i]));
80107178:	05 00 00 00 80       	add    $0x80000000,%eax
      kfree(v);
8010717d:	50                   	push   %eax
8010717e:	e8 7d b5 ff ff       	call   80102700 <kfree>
80107183:	83 c4 10             	add    $0x10,%esp
  for(i = 0; i < NPDENTRIES; i++){
80107186:	39 fb                	cmp    %edi,%ebx
80107188:	75 dd                	jne    80107167 <freevm+0x37>
    }
  }
  kfree((char*)pgdir);
8010718a:	89 75 08             	mov    %esi,0x8(%ebp)
}
8010718d:	8d 65 f4             	lea    -0xc(%ebp),%esp
80107190:	5b                   	pop    %ebx
80107191:	5e                   	pop    %esi
80107192:	5f                   	pop    %edi
80107193:	5d                   	pop    %ebp
  kfree((char*)pgdir);
80107194:	e9 67 b5 ff ff       	jmp    80102700 <kfree>
    panic("freevm: no pgdir");
80107199:	83 ec 0c             	sub    $0xc,%esp
8010719c:	68 19 7f 10 80       	push   $0x80107f19
801071a1:	e8 ea 91 ff ff       	call   80100390 <panic>
801071a6:	8d 76 00             	lea    0x0(%esi),%esi
801071a9:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

801071b0 <setupkvm>:
{
801071b0:	55                   	push   %ebp
801071b1:	89 e5                	mov    %esp,%ebp
801071b3:	56                   	push   %esi
801071b4:	53                   	push   %ebx
  if((pgdir = (pde_t*)kalloc()) == 0)
801071b5:	e8 f6 b6 ff ff       	call   801028b0 <kalloc>
801071ba:	85 c0                	test   %eax,%eax
801071bc:	89 c6                	mov    %eax,%esi
801071be:	74 42                	je     80107202 <setupkvm+0x52>
  memset(pgdir, 0, PGSIZE);
801071c0:	83 ec 04             	sub    $0x4,%esp
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
801071c3:	bb 20 a4 10 80       	mov    $0x8010a420,%ebx
  memset(pgdir, 0, PGSIZE);
801071c8:	68 00 10 00 00       	push   $0x1000
801071cd:	6a 00                	push   $0x0
801071cf:	50                   	push   %eax
801071d0:	e8 cb d7 ff ff       	call   801049a0 <memset>
801071d5:	83 c4 10             	add    $0x10,%esp
                (uint)k->phys_start, k->perm) < 0) {
801071d8:	8b 43 04             	mov    0x4(%ebx),%eax
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start,
801071db:	8b 4b 08             	mov    0x8(%ebx),%ecx
801071de:	83 ec 08             	sub    $0x8,%esp
801071e1:	8b 13                	mov    (%ebx),%edx
801071e3:	ff 73 0c             	pushl  0xc(%ebx)
801071e6:	50                   	push   %eax
801071e7:	29 c1                	sub    %eax,%ecx
801071e9:	89 f0                	mov    %esi,%eax
801071eb:	e8 b0 f9 ff ff       	call   80106ba0 <mappages>
801071f0:	83 c4 10             	add    $0x10,%esp
801071f3:	85 c0                	test   %eax,%eax
801071f5:	78 19                	js     80107210 <setupkvm+0x60>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
801071f7:	83 c3 10             	add    $0x10,%ebx
801071fa:	81 fb 60 a4 10 80    	cmp    $0x8010a460,%ebx
80107200:	75 d6                	jne    801071d8 <setupkvm+0x28>
}
80107202:	8d 65 f8             	lea    -0x8(%ebp),%esp
80107205:	89 f0                	mov    %esi,%eax
80107207:	5b                   	pop    %ebx
80107208:	5e                   	pop    %esi
80107209:	5d                   	pop    %ebp
8010720a:	c3                   	ret    
8010720b:	90                   	nop
8010720c:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
      freevm(pgdir);
80107210:	83 ec 0c             	sub    $0xc,%esp
80107213:	56                   	push   %esi
      return 0;
80107214:	31 f6                	xor    %esi,%esi
      freevm(pgdir);
80107216:	e8 15 ff ff ff       	call   80107130 <freevm>
      return 0;
8010721b:	83 c4 10             	add    $0x10,%esp
}
8010721e:	8d 65 f8             	lea    -0x8(%ebp),%esp
80107221:	89 f0                	mov    %esi,%eax
80107223:	5b                   	pop    %ebx
80107224:	5e                   	pop    %esi
80107225:	5d                   	pop    %ebp
80107226:	c3                   	ret    
80107227:	89 f6                	mov    %esi,%esi
80107229:	8d bc 27 00 00 00 00 	lea    0x0(%edi,%eiz,1),%edi

80107230 <kvmalloc>:
{
80107230:	55                   	push   %ebp
80107231:	89 e5                	mov    %esp,%ebp
80107233:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
80107236:	e8 75 ff ff ff       	call   801071b0 <setupkvm>
8010723b:	a3 a4 65 11 80       	mov    %eax,0x801165a4
  lcr3(V2P(kpgdir));   // switch to the kernel page table
80107240:	05 00 00 00 80       	add    $0x80000000,%eax
80107245:	0f 22 d8             	mov    %eax,%cr3
}
80107248:	c9                   	leave  
80107249:	c3                   	ret    
8010724a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi

80107250 <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
80107250:	55                   	push   %ebp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80107251:	31 c9                	xor    %ecx,%ecx
{
80107253:	89 e5                	mov    %esp,%ebp
80107255:	83 ec 08             	sub    $0x8,%esp
  pte = walkpgdir(pgdir, uva, 0);
80107258:	8b 55 0c             	mov    0xc(%ebp),%edx
8010725b:	8b 45 08             	mov    0x8(%ebp),%eax
8010725e:	e8 bd f8 ff ff       	call   80106b20 <walkpgdir>
  if(pte == 0)
80107263:	85 c0                	test   %eax,%eax
80107265:	74 05                	je     8010726c <clearpteu+0x1c>
    panic("clearpteu");
  *pte &= ~PTE_U;
80107267:	83 20 fb             	andl   $0xfffffffb,(%eax)
}
8010726a:	c9                   	leave  
8010726b:	c3                   	ret    
    panic("clearpteu");
8010726c:	83 ec 0c             	sub    $0xc,%esp
8010726f:	68 2a 7f 10 80       	push   $0x80107f2a
80107274:	e8 17 91 ff ff       	call   80100390 <panic>
80107279:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi

80107280 <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
80107280:	55                   	push   %ebp
80107281:	89 e5                	mov    %esp,%ebp
80107283:	57                   	push   %edi
80107284:	56                   	push   %esi
80107285:	53                   	push   %ebx
80107286:	83 ec 1c             	sub    $0x1c,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
80107289:	e8 22 ff ff ff       	call   801071b0 <setupkvm>
8010728e:	85 c0                	test   %eax,%eax
80107290:	89 45 e0             	mov    %eax,-0x20(%ebp)
80107293:	0f 84 a0 00 00 00    	je     80107339 <copyuvm+0xb9>
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
80107299:	8b 4d 0c             	mov    0xc(%ebp),%ecx
8010729c:	85 c9                	test   %ecx,%ecx
8010729e:	0f 84 95 00 00 00    	je     80107339 <copyuvm+0xb9>
801072a4:	31 f6                	xor    %esi,%esi
801072a6:	eb 4e                	jmp    801072f6 <copyuvm+0x76>
801072a8:	90                   	nop
801072a9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
    flags = PTE_FLAGS(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
    memmove(mem, (char*)P2V(pa), PGSIZE);
801072b0:	83 ec 04             	sub    $0x4,%esp
801072b3:	81 c7 00 00 00 80    	add    $0x80000000,%edi
801072b9:	89 45 e4             	mov    %eax,-0x1c(%ebp)
801072bc:	68 00 10 00 00       	push   $0x1000
801072c1:	57                   	push   %edi
801072c2:	50                   	push   %eax
801072c3:	e8 88 d7 ff ff       	call   80104a50 <memmove>
    if(mappages(d, (void*)i, PGSIZE, V2P(mem), flags) < 0)
801072c8:	58                   	pop    %eax
801072c9:	5a                   	pop    %edx
801072ca:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801072cd:	8b 45 e0             	mov    -0x20(%ebp),%eax
801072d0:	b9 00 10 00 00       	mov    $0x1000,%ecx
801072d5:	53                   	push   %ebx
801072d6:	81 c2 00 00 00 80    	add    $0x80000000,%edx
801072dc:	52                   	push   %edx
801072dd:	89 f2                	mov    %esi,%edx
801072df:	e8 bc f8 ff ff       	call   80106ba0 <mappages>
801072e4:	83 c4 10             	add    $0x10,%esp
801072e7:	85 c0                	test   %eax,%eax
801072e9:	78 39                	js     80107324 <copyuvm+0xa4>
  for(i = 0; i < sz; i += PGSIZE){
801072eb:	81 c6 00 10 00 00    	add    $0x1000,%esi
801072f1:	39 75 0c             	cmp    %esi,0xc(%ebp)
801072f4:	76 43                	jbe    80107339 <copyuvm+0xb9>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
801072f6:	8b 45 08             	mov    0x8(%ebp),%eax
801072f9:	31 c9                	xor    %ecx,%ecx
801072fb:	89 f2                	mov    %esi,%edx
801072fd:	e8 1e f8 ff ff       	call   80106b20 <walkpgdir>
80107302:	85 c0                	test   %eax,%eax
80107304:	74 3e                	je     80107344 <copyuvm+0xc4>
    if(!(*pte & PTE_P))
80107306:	8b 18                	mov    (%eax),%ebx
80107308:	f6 c3 01             	test   $0x1,%bl
8010730b:	74 44                	je     80107351 <copyuvm+0xd1>
    pa = PTE_ADDR(*pte);
8010730d:	89 df                	mov    %ebx,%edi
    flags = PTE_FLAGS(*pte);
8010730f:	81 e3 ff 0f 00 00    	and    $0xfff,%ebx
    pa = PTE_ADDR(*pte);
80107315:	81 e7 00 f0 ff ff    	and    $0xfffff000,%edi
    if((mem = kalloc()) == 0)
8010731b:	e8 90 b5 ff ff       	call   801028b0 <kalloc>
80107320:	85 c0                	test   %eax,%eax
80107322:	75 8c                	jne    801072b0 <copyuvm+0x30>
      goto bad;
  }
  return d;

bad:
  freevm(d);
80107324:	83 ec 0c             	sub    $0xc,%esp
80107327:	ff 75 e0             	pushl  -0x20(%ebp)
8010732a:	e8 01 fe ff ff       	call   80107130 <freevm>
  return 0;
8010732f:	83 c4 10             	add    $0x10,%esp
80107332:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
}
80107339:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010733c:	8d 65 f4             	lea    -0xc(%ebp),%esp
8010733f:	5b                   	pop    %ebx
80107340:	5e                   	pop    %esi
80107341:	5f                   	pop    %edi
80107342:	5d                   	pop    %ebp
80107343:	c3                   	ret    
      panic("copyuvm: pte should exist");
80107344:	83 ec 0c             	sub    $0xc,%esp
80107347:	68 34 7f 10 80       	push   $0x80107f34
8010734c:	e8 3f 90 ff ff       	call   80100390 <panic>
      panic("copyuvm: page not present");
80107351:	83 ec 0c             	sub    $0xc,%esp
80107354:	68 4e 7f 10 80       	push   $0x80107f4e
80107359:	e8 32 90 ff ff       	call   80100390 <panic>
8010735e:	66 90                	xchg   %ax,%ax

80107360 <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
80107360:	55                   	push   %ebp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80107361:	31 c9                	xor    %ecx,%ecx
{
80107363:	89 e5                	mov    %esp,%ebp
80107365:	83 ec 08             	sub    $0x8,%esp
  pte = walkpgdir(pgdir, uva, 0);
80107368:	8b 55 0c             	mov    0xc(%ebp),%edx
8010736b:	8b 45 08             	mov    0x8(%ebp),%eax
8010736e:	e8 ad f7 ff ff       	call   80106b20 <walkpgdir>
  if((*pte & PTE_P) == 0)
80107373:	8b 00                	mov    (%eax),%eax
    return 0;
  if((*pte & PTE_U) == 0)
    return 0;
  return (char*)P2V(PTE_ADDR(*pte));
}
80107375:	c9                   	leave  
  if((*pte & PTE_U) == 0)
80107376:	89 c2                	mov    %eax,%edx
  return (char*)P2V(PTE_ADDR(*pte));
80107378:	25 00 f0 ff ff       	and    $0xfffff000,%eax
  if((*pte & PTE_U) == 0)
8010737d:	83 e2 05             	and    $0x5,%edx
  return (char*)P2V(PTE_ADDR(*pte));
80107380:	05 00 00 00 80       	add    $0x80000000,%eax
80107385:	83 fa 05             	cmp    $0x5,%edx
80107388:	ba 00 00 00 00       	mov    $0x0,%edx
8010738d:	0f 45 c2             	cmovne %edx,%eax
}
80107390:	c3                   	ret    
80107391:	eb 0d                	jmp    801073a0 <copyout>
80107393:	90                   	nop
80107394:	90                   	nop
80107395:	90                   	nop
80107396:	90                   	nop
80107397:	90                   	nop
80107398:	90                   	nop
80107399:	90                   	nop
8010739a:	90                   	nop
8010739b:	90                   	nop
8010739c:	90                   	nop
8010739d:	90                   	nop
8010739e:	90                   	nop
8010739f:	90                   	nop

801073a0 <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
801073a0:	55                   	push   %ebp
801073a1:	89 e5                	mov    %esp,%ebp
801073a3:	57                   	push   %edi
801073a4:	56                   	push   %esi
801073a5:	53                   	push   %ebx
801073a6:	83 ec 1c             	sub    $0x1c,%esp
801073a9:	8b 5d 14             	mov    0x14(%ebp),%ebx
801073ac:	8b 55 0c             	mov    0xc(%ebp),%edx
801073af:	8b 7d 10             	mov    0x10(%ebp),%edi
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
801073b2:	85 db                	test   %ebx,%ebx
801073b4:	75 40                	jne    801073f6 <copyout+0x56>
801073b6:	eb 70                	jmp    80107428 <copyout+0x88>
801073b8:	90                   	nop
801073b9:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
    va0 = (uint)PGROUNDDOWN(va);
    pa0 = uva2ka(pgdir, (char*)va0);
    if(pa0 == 0)
      return -1;
    n = PGSIZE - (va - va0);
801073c0:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801073c3:	89 f1                	mov    %esi,%ecx
801073c5:	29 d1                	sub    %edx,%ecx
801073c7:	81 c1 00 10 00 00    	add    $0x1000,%ecx
801073cd:	39 d9                	cmp    %ebx,%ecx
801073cf:	0f 47 cb             	cmova  %ebx,%ecx
    if(n > len)
      n = len;
    memmove(pa0 + (va - va0), buf, n);
801073d2:	29 f2                	sub    %esi,%edx
801073d4:	83 ec 04             	sub    $0x4,%esp
801073d7:	01 d0                	add    %edx,%eax
801073d9:	51                   	push   %ecx
801073da:	57                   	push   %edi
801073db:	50                   	push   %eax
801073dc:	89 4d e4             	mov    %ecx,-0x1c(%ebp)
801073df:	e8 6c d6 ff ff       	call   80104a50 <memmove>
    len -= n;
    buf += n;
801073e4:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
  while(len > 0){
801073e7:	83 c4 10             	add    $0x10,%esp
    va = va0 + PGSIZE;
801073ea:	8d 96 00 10 00 00    	lea    0x1000(%esi),%edx
    buf += n;
801073f0:	01 cf                	add    %ecx,%edi
  while(len > 0){
801073f2:	29 cb                	sub    %ecx,%ebx
801073f4:	74 32                	je     80107428 <copyout+0x88>
    va0 = (uint)PGROUNDDOWN(va);
801073f6:	89 d6                	mov    %edx,%esi
    pa0 = uva2ka(pgdir, (char*)va0);
801073f8:	83 ec 08             	sub    $0x8,%esp
    va0 = (uint)PGROUNDDOWN(va);
801073fb:	89 55 e4             	mov    %edx,-0x1c(%ebp)
801073fe:	81 e6 00 f0 ff ff    	and    $0xfffff000,%esi
    pa0 = uva2ka(pgdir, (char*)va0);
80107404:	56                   	push   %esi
80107405:	ff 75 08             	pushl  0x8(%ebp)
80107408:	e8 53 ff ff ff       	call   80107360 <uva2ka>
    if(pa0 == 0)
8010740d:	83 c4 10             	add    $0x10,%esp
80107410:	85 c0                	test   %eax,%eax
80107412:	75 ac                	jne    801073c0 <copyout+0x20>
  }
  return 0;
}
80107414:	8d 65 f4             	lea    -0xc(%ebp),%esp
      return -1;
80107417:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010741c:	5b                   	pop    %ebx
8010741d:	5e                   	pop    %esi
8010741e:	5f                   	pop    %edi
8010741f:	5d                   	pop    %ebp
80107420:	c3                   	ret    
80107421:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
80107428:	8d 65 f4             	lea    -0xc(%ebp),%esp
  return 0;
8010742b:	31 c0                	xor    %eax,%eax
}
8010742d:	5b                   	pop    %ebx
8010742e:	5e                   	pop    %esi
8010742f:	5f                   	pop    %edi
80107430:	5d                   	pop    %ebp
80107431:	c3                   	ret    
